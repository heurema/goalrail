#!/usr/bin/env python3
"""Deterministic PR intake gate for Goalrail pull_request_target workflows.
...
NEEDS_INTENT_COMMENT = """<!-- goalrail-pr-intake-gate -->

## PR Intake Gate: linked intent needed

Thanks for the contribution. Before ordinary code review, this external non-trivial PR needs a linked intent source.

Please link one of:

1. GitHub Issue or Discussion.
2. Goalrail decision reference such as `D-0000` or `docs/ops/DECISIONS.md#D-0000`.
3. `docs/adr/...` ADR.
4. `docs/research/...` research note.
5. `.goalrail/work/goals/...` goal.
6. `.goalrail/work/reports/...` report.
...
HIGH_RISK_COMMENT = """<!-- goalrail-pr-intake-gate -->

## PR Intake Gate: maintainer review needed

Thanks for the contribution. This external PR touches high-risk Goalrail surfaces, so it needs explicit maintainer attention before ordinary code review proceeds.

High-risk areas include workflows, contributor governance, product canon, ops docs, ADRs, research, Goalrail work overlays, apps, scripts, tooling, dependency files, deployments, migrations, auth, security, crypto, and runtime behavior.

A maintainer can bypass this gate by adding `maintainer/override-intake`.
"""

PASS_COMMENT = """<!-- goalrail-pr-intake-gate -->
...
def api_request(method: str, path: str, token: str, body: Any | None = None, allow_404: bool = False) -> Any:
    ...
    request.add_header("User-Agent", "goalrail-pr-intake-gate")
...
def build_missing_context_comment(missing: list[str], no_code_only: bool) -> str:
    heading = "no-code alternative needed" if no_code_only else "more Goalrail context needed"
    lines = [
        "<!-- goalrail-pr-intake-gate -->",
        "",
        f"## PR Intake Gate: {heading}",
        "",
        "Thanks for the contribution. External non-trivial PRs need enough Goalrail context before ordinary code review.",
        "",
...
def determine_verdict(
    ...
    marker = str(config.get("bot_comment", {}).get("marker") or "<!-- goalrail-pr-intake-gate -->")
    details = {
    ...
    if high_risk_paths:
        return (
            Verdict(
                name="high-risk",
                reason="External PR touches configured high-risk Goalrail paths.",
    ...
    if missing_sections:
        return (
            Verdict(
                ...
                reason="External non-trivial PR is missing required Goalrail context sections.",
    ...
    if not linked:
        return (
            Verdict(
                ...
                reason="External non-trivial PR does not include linked Issue, Discussion, decision, ADR, research, or Goalrail work artifact.",
