...
- **The other end of every seam the change creates.** Follow each contract the
  ...
  diff never touched.
- **Unknowns recorded as measurements.** A field that says `false`, `0`, or an
  empty string when the truth is "not measured" states something nobody
...
- **Scoping claims in the change itself.** A comment or help text saying "only X
  supports this" or "this cannot be done here" is the author's assertion, not
  evidence about the world. Check it against the tool or the code it describes
  before accepting a gap as deliberate — a false scoping claim makes a real
  defect look documented, and a reviewer that trusts it scores the defect as
  non-material.
