...
Guidance for agents working under `web/` in the main `heurema/goalrail`
repository.

## Scope

`web/` owns the server-served web UI plus native app shells that load that UI:

- Vite/React web UI in `web/`.
- Electron desktop shell in `web/electron/`.
- iOS SwiftUI/WKWebView shell in `web/ios/`.
...
- Do not edit `web/electron/package.json` `version`, iOS
...
Build outputs land under `web/electron/dist/`; do not commit them unless the
