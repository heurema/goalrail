// Tests for the pure helpers in src/goalrail_cli.js, run with `node --test`
...
// connected to server X?" and "which goalrail binary do we run?".
...
describe("normalizeServerUrl", () => {
  it("strips trailing slashes and trims", () => {
    ...
    assert.equal(normalizeServerUrl("https://x.com/some/path"), "https://x.com/some/path");
  });
...
describe("candidatePaths", () => {
  it("probes the goalrail binary in each well-known location", () => {
    const paths = candidatePaths();
    assert.ok(paths.some((p) => p.endsWith("/.local/bin/goalrail")));
    assert.ok(paths.includes("/opt/homebrew/bin/goalrail"));
    assert.ok(paths.includes("/usr/local/bin/goalrail"));
  });
});

describe("resolveCliPath", () => {
  it("prefers a usable configured path", () => {
    const got = resolveCliPath("/custom/goalrail", {
      isExecutableFile: (p) => p === "/custom/goalrail",
      whichGoalrail: () => "/usr/bin/goalrail",
      candidatePaths: () => ["/home/me/.local/bin/goalrail"],
    });
    assert.deepEqual(got, { path: "/custom/goalrail", source: "configured" });
  });

  it("falls back to PATH when the configured path is unusable", () => {
    const got = resolveCliPath("/bad/path", {
      isExecutableFile: (p) => p === "/usr/bin/goalrail",
      whichGoalrail: () => "/usr/bin/goalrail",
      candidatePaths: () => ["/home/me/.local/bin/goalrail"],
    });
    assert.deepEqual(got, { path: "/usr/bin/goalrail", source: "path" });
  });

  it("falls back to a candidate when PATH misses (GUI minimal PATH)", () => {
    const got = resolveCliPath(null, {
      isExecutableFile: (p) => p === "/home/me/.local/bin/goalrail",
      whichGoalrail: () => null,
      candidatePaths: () => ["/home/me/.local/bin/goalrail", "/opt/homebrew/bin/goalrail"],
    });
    assert.deepEqual(got, { path: "/home/me/.local/bin/goalrail", source: "candidate" });
  });
...
describe("getHostConnectionFast — probe destination & token handling (S1)", () => {
  afterEach(() => {
    mock.restoreAll();
    delete process.env.GOALRAIL_REMOTE_AUTH_TOKEN;
  });
  ...
  it("never attaches GOALRAIL_REMOTE_AUTH_TOKEN to a probe", async () => {
    ...
    process.env.GOALRAIL_REMOTE_AUTH_TOKEN = "secret-token";
    const record = {
      pid: process.pid,
      target: "http://localhost:6767",
      mode: "server",
