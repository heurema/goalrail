# Goalrail Desktop (Electron)

A thin [Electron](https://www.electronjs.org) desktop shell around the
existing Goalrail web UI. It shows the **same** UI you get in a browser, but
...
- **A foreground attention cue.** macOS (and Windows) suppress the notification
  ...
  turn-end is noticeable even with Goalrail in front.
...
1. If no server URL is saved yet, it shows the setup page (one input +
   Connect). You enter your Goalrail server URL (default
   `http://localhost:8000`).
...
The shell opens on the bundled setup page. Point it at a running Goalrail
...
Any reachable Goalrail server works. For a quick local target, run the
...
`goalrail` CLI to start a server and register this machine as a **host** (a
...
The shell probes for the `goalrail` binary — `settings.goalrail_path` first,
then `PATH`, then the well-known install locations (`~/.local/bin`,
`~/.cargo/bin`, Homebrew, `/usr/local/bin`). A GUI-launched app inherits a
minimal `PATH`, which is why the install locations are probed directly. The
path is resolved once at startup and cached in-memory for the session.
...
A configured path is saved to `settings.json` (`goalrail_path`) only once it
validates as a runnable `goalrail`; clearing it reverts to auto-detection.
Connecting to a **remote** server never needs the CLI — only "Start locally"
and hosting do.

### Start locally

**"Start a server on this machine"** runs `goalrail server start` (idempotent —
...
dialog** ("Allow _host_ to manage Goalrail on this machine?") with **Don't Allow**
...
`goalrail host --server <url>`. **Allow Once** connects this time and re-prompts
...
bridge — `window.goalrailDesktop` → `getHostStatus` / `getHostIdentity` /
