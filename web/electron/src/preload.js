...
contextBridge.exposeInMainWorld("goalrailDesktop", {
   ...
   */
  getHostIdentity: () => ipcRenderer.invoke("goalrail:host-get-identity"),
  /**
   * Start / stop / restart this machine's host daemon for the window's server.
   * Resolves a `{ ok, error? }` result.
   * @param {"start" | "stop" | "restart"} action
   */
  controlHost: (action) => ipcRenderer.invoke("goalrail:host-control", action),
  /**
  ...
  onHostStatusChanged: (callback) => {
    ...
    ipcRenderer.on("goalrail:host-status-changed", listener);
    return () => ipcRenderer.removeListener("goalrail:host-status-changed", listener);
  },
  /**
   * The local `goalrail` CLI status — `{ installed, path, version, source,
   * installCommand }`. Read-only; lets the in-app Local CLI settings show which
   * binary is in use.
   */
  getCliStatus: () => ipcRenderer.invoke("goalrail:cli-get-status"),
  /**
   ...
   */
  resetCliPath: () => ipcRenderer.invoke("goalrail:cli-reset-path"),
});
...
contextBridge.exposeInMainWorld("goalrailSetup", {
  ...
  /**
   * Whether the `goalrail` CLI is installed/runnable, e.g.
   * `{installed, path, version, source, installCommand}`.
   */
  getCliStatus: () => ipcRenderer.invoke("goalrail:get-cli-status"),
  /**
   * Set an explicit path to the goalrail binary. Resolves the CLI status plus
   * `accepted` (whether that exact path validated and was saved).
   * @param {string} path
   */
  setCliPath: (path) => ipcRenderer.invoke("goalrail:set-cli-path", path),
  /** Native file picker for the goalrail binary; resolves the path or null. */
  browseCliPath: () => ipcRenderer.invoke("goalrail:browse-cli-path"),
  /**
   * Start (or reuse) the local server. Resolves `{ok, url?, error?}`; the
   * caller then connects to `url` via setServerUrl.
   */
  startLocalServer: () => ipcRenderer.invoke("goalrail:start-local-server"),
});
