// Process lifecycle for desktop-managed Goalrail servers and host connections.
//
// This is the only place the desktop spawns long-lived processes. It owns:
//   - hostChildren: the foreground `goalrail host --server <url>` processes this
//     app started. They are torn down when the app quits (the confirmed
//     lifecycle: the desktop owns what it starts).
//   - ownedLocalServer: a local `goalrail server` we started ourselves (and so
...
// (goalrail_cli.js), which is the single source of truth. This module only
...
const cli = require("./goalrail_cli");
...
/** The line `goalrail host` prints once the websocket tunnel is up. */
...
/**
 * Every `goalrail host` child we have spawned and not yet seen exit — including
 ...
 * from `goalrail host`'s own messages (HostConnectError: "Authentication
 * failed", "HTTP 401", login-page redirect, or the `goalrail login` hint). Lets
...
function isAuthError(text) {
  return /authentication failed|http 401|unauthor|login page|goalrail login/i.test(
    String(text || ""),
  );
}

/**
 * True when `goalrail host` refused to start because a daemon already serves
...
/**
 * Spawn `goalrail host --server <url>` and resolve once it reports connected
 ...
 * *adopt* it without spawning a duplicate — `goalrail host` would otherwise
...
async function ensureHostConnected(cliPath, serverUrl) {
  ...
  // two `goalrail host` processes for one target.
...
async function connectHost(cliPath, serverUrl, key) {
  ...
  // tunnel, via the fast disk-read + single HTTP probe — not the slow `goalrail
 ...
 * stored credentials, runs `goalrail login <url>` (browser/OIDC; a
 * no-op when the server needs no auth). Returns ok when already authed, after a
 * successful login, or for a no-auth server; an error (pointing at `goalrail
...
async function ensureServerAuth(cliPath, serverUrl) {
  ...
  return {
    ok: false,
    error: `Sign-in required — run \`goalrail login ${serverUrl}\` in a terminal, then try again.`,
  };
}
...
async function startLocalServer(cliPath) {
  ...
  // `goalrail server status` (a Python cold start). We didn't start it, so no
  // ownership claim.
...
async function shutdown(cliPath) {
  ...
  // during a connect can't leave an orphaned `goalrail host` process.
  const exits = [];
