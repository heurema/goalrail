...
const goalrailCli = require("./goalrail_cli");
 ...
 * the user connected with, so the window keeps both.
 *
 * @param {BrowserWindow} win
...
function broadcastHostStatus() {
  for (const [win, state] of windows) {
    ...
    try {
      win.webContents.send("goalrail:host-status-changed");
    } catch {
...
/**
 * Resolve the `goalrail` CLI binary path from the user's configured override
 * (``settings.goalrail_path``) plus the standard locations, or null when none
...
function resolvedCliPath() {
  const configured = loadSettings().goalrail_path ?? null;
  if (
    cachedCli &&
    cachedCli.configuredPath === configured &&
    cachedCli.path &&
    goalrailCli.isExecutableFile(cachedCli.path)
  ) {
    return cachedCli.path;
  }
  const resolved = goalrailCli.resolveCliPath(configured);
...
async function applyCliPath(configuredPath) {
  ...
  const status = await goalrailCli.getCliStatus(trimmed || null);
  const accepted = status.installed && status.source === "configured";
  if (accepted) {
    const settings = loadSettings();
    settings.goalrail_path = trimmed;
    saveSettings(settings);
  } else if (trimmed === "") {
    const settings = loadSettings();
    delete settings.goalrail_path;
    saveSettings(settings);
  }
...
async function clearCliPath() {
  ...
  delete settings.goalrail_path;
  saveSettings(settings);
  return goalrailCli.getCliStatus(null);
}
...
async function confirmHostEnrollment(win) {
  ...
  // Goalrail's own prompt rather than an anonymous system alert; in a packaged
  ...
  const { response } = await dialog.showMessageBox(win, {
    ...
    title: "Goalrail",
    message: `Allow ${host} to manage Goalrail on this machine?`,
    detail:
...
function registerIpc() {
  ...
  // Setup page → is the `goalrail` CLI installed and runnable? Includes the
  // resolved path, version, and the install one-liner to show when missing.
  ipcMain.handle("goalrail:get-cli-status", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("get-cli-status is only available to the setup page");
    }
    return goalrailCli.getCliStatus(loadSettings().goalrail_path);
  });

  // Setup page → set an explicit path to the `goalrail` binary. Persisted only
  // when that exact path validates as a runnable goalrail (so a typo doesn't
  // silently mask a working PATH lookup). Returns the resulting CLI status plus
  // whether the configured path was accepted.
  ipcMain.handle("goalrail:set-cli-path", async (event, configuredPath) => {
    if (!isSetupPageSender(event)) {
  ...
  // Setup page → native file picker for the goalrail binary. Returns the chosen
  // path (the renderer feeds it back through set-cli-path) or null on cancel.
  ipcMain.handle("goalrail:browse-cli-path", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("browse-cli-path is only available to the setup page");
    }
    const win = BrowserWindow.fromWebContents(event.sender) ?? activeWindow();
    const result = await dialog.showOpenDialog(win ?? undefined, {
      title: "Locate the goalrail binary",
      properties: ["openFile"],
    });
  ...
  ipcMain.handle("goalrail:start-local-server", async (event) => {
    if (!isSetupPageSender(event)) {
      throw new Error("start-local-server is only available to the setup page");
    }
    const cliPath = resolvedCliPath();
    if (!cliPath) {
      return { ok: false, error: "The goalrail CLI was not found. Install it or set its path." };
    }
    return serverManager.startLocalServer(cliPath);
  });

  // SPA → this machine's identity: is the CLI installed, and its host id. Both
  // come from local config (no `goalrail host status` subprocess), so this is
  // instant — it lets the new-session picker tag/connect "this machine" without
  // waiting on the slow runner-status check.
  ipcMain.handle("goalrail:host-get-identity", (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[goalrail] host-get-identity from untrusted sender dropped");
      return null;
    }
    return { cliInstalled: Boolean(resolvedCliPath()), hostId: goalrailCli.localHostId() };
  });
  ...
  ipcMain.handle("goalrail:cli-get-status", async (event) => {
    if (!isPinnedOriginSender(event)) {
      console.warn("[goalrail] cli-get-status from untrusted sender dropped");
      return null;
    }
    return goalrailCli.getCliStatus(loadSettings().goalrail_path);
  });
  ...
  ipcMain.handle("goalrail:cli-reset-path", async (event) => {
    if (!isPinnedOriginSender(event)) {
  ...
  ipcMain.handle("goalrail:host-control", async (event, action) => {
    if (!isPinnedOriginSender(event)) {
