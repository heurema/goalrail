// Discovery and invocation of the local `goalrail` CLI for the desktop shell.
//
// The desktop manages servers by shelling out to the same `goalrail` binary a
...
// test/goalrail_cli.test.js; the functions that actually spawn a binary are
...
/**
 * The Goalrail local runtime data dir — `$GOALRAIL_DATA_DIR` (with `~`
 * expanded) or `~/.goalrail`. Mirrors `_local_data_dir()` in
 * goalrail/host/local_server.py. The local-server pidfile lives here.
 *
 * @returns {string}
 */
function localDataDir() {
  const raw = process.env.GOALRAIL_DATA_DIR;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
  return path.join(os.homedir(), ".goalrail");
}

/**
 * The Goalrail config dir — `$GOALRAIL_CONFIG_HOME` (with `~` expanded) or
 * `~/.goalrail`. config.yaml (machine identity) lives here; it can differ from
 * the data dir under test env overrides, but is the same by default.
 *
 * @returns {string}
 */
function localConfigDir() {
  const raw = process.env.GOALRAIL_CONFIG_HOME;
  if (raw && raw.trim() !== "") {
    const expanded = raw.startsWith("~") ? path.join(os.homedir(), raw.slice(1)) : raw;
    return path.resolve(expanded);
  }
...
/**
 * The shared Goalrail state dir, ALWAYS `~/.goalrail` — it ignores
 * `$GOALRAIL_DATA_DIR`, mirroring `state_dir()` in
 * sdks/ui/goalrail_ui_sdk/terminal/_config.py and `_HOST_PID_PATH` in
 * goalrail/cli.py (both hardcode `Path.home()/".goalrail"`). The auth-token
 * store and the daemon registry live here — NOT under the data dir. Only the
 * local-server pidfile honors `$GOALRAIL_DATA_DIR` (see {@link localDataDir}).
 *
 * @returns {string}
 */
function stateDir() {
...
/**
 * This machine's Goalrail host id (e.g. "host_ab12…"), read from the machine
 * identity in `config.yaml` (`host: host_id:`, written by
 * goalrail/host/identity.py) — instant, no subprocess. Present once generated,
...
/**
 * Local-server status from the pidfile + a pid-liveness check — no `goalrail
 ...
 * goalrail/host/local_server.py. Returns null for a stale pidfile (dead pid, a
...
/**
 * Well-known install locations for the `goalrail` binary, in priority order.
...
function candidatePaths() {
  ...
  return [
    path.join(home, ".local", "bin", "goalrail"),
    path.join(home, ".cargo", "bin", "goalrail"),
    "/opt/homebrew/bin/goalrail",
    "/usr/local/bin/goalrail",
  ];
}
...
/**
 * Resolve `goalrail` on PATH (or the user's login shell PATH). Returns null
 * when not found. On POSIX we go through `command -v` so shell-managed PATHs
 * (uv shims) resolve; on Windows we use `where`.
 *
 * @returns {string | null}
 */
function whichGoalrail() {
  try {
    if (process.platform === "win32") {
      const out = execFileSync("where", ["goalrail"], { encoding: "utf8" });
      return out.trim().split(/\r?\n/)[0] || null;
    }
    const out = execFileSync("/bin/sh", ["-c", "command -v goalrail"], {
      encoding: "utf8",
    });
...
/**
 * Locate the `goalrail` binary. Resolution order: a user-configured path, then
 ...
 *
 * @param {string | null | undefined} configuredPath settings.goalrail_path
 * @param {{
 *   isExecutableFile?: (p: string) => boolean,
 *   whichGoalrail?: () => string | null,
...
function resolveCliPath(configuredPath, deps = {}) {
  ...
  const which = deps.whichGoalrail || whichGoalrail;
...
/**
 * Run an `goalrail` subcommand and resolve with its captured output. Never
 ...
 * from `~/.goalrail/auth_tokens.json` (no subprocess), mirroring
 * goalrail/cli_auth.py: keyed by the trailing-slash-stripped URL, a record is
 * valid if it has a non-expired session token. The CLI's `state_dir()` is
 * hardcoded to `~/.goalrail`.
 *
 * @param {string} serverUrl
 * @returns {boolean}
 */
...
/**
 * Run `goalrail login <serverUrl>` to authenticate the CLI to a server. It's a
 * no-op when the server needs no auth (header mode), opens the system browser
 * for OIDC, and fails fast for password (TTY) modes when run without a
 * terminal. Long timeout to allow the interactive browser flow.
 *
 * @param {string} cliPath
 * @param {string} serverUrl
 * @param {{ timeoutMs?: number }} [opts]
...
async function getCliStatus(configuredPath) {
  ...
  // Must exit cleanly AND identify itself as goalrail — `goalrail --version`
  // prints e.g. "goalrail 0.3.0.dev0 (…)". The exit-code alone isn't enough: an
  ...
  const ok = res.code === 0 && /\bgoalrail/i.test(version);
  return {
    installed: ok,
...
/**
 * `goalrail server status --json`. Returns the parsed payload, or a synthetic
 ...
 * `_daemon_registry_dir()` in goalrail/cli.py (`<state_dir>/daemons`).
 *
 * @returns {string}
 */
 ...
 * Mirrors the validation in `_record_from_json()` (goalrail/cli.py): a usable
...
/**
 * Read every daemon registry record from disk (`~/.goalrail/daemons/*.json`).
 * This is the fast substitute for `goalrail host status --json`: it gives the
...
/**
 * The Goalrail server URL a daemon record talks to, mirroring
 * `_daemon_base_url()` (goalrail/cli.py): a local-mode daemon's URL lives in
 ...
 * non-expired session token stored by `goalrail login` in `auth_tokens.json`,
 * looked up by the exact server URL. Returns null when auth is minted out of
 * band rather than stored by the CLI (no token is stored) or when nothing is
 * stored for this URL.
 *
 * Unlike `_remote_headers()` (goalrail/chat.py), this deliberately does NOT
 * honor `GOALRAIL_REMOTE_AUTH_TOKEN`. That token is destination-independent — it
 ...
 * `~/.goalrail/daemons/`. The desktop never needs it: it's not inherited by a
 ...
 * status path — already the behavior for out-of-band auth.
 *
 * @param {string} serverUrl
 * @returns {string | null}
 */
 ...
 * `_add_daemon_host_status()` (goalrail/cli.py) makes, minus the per-session
 ...
 * in-process (auth supplied only out of band, or via the env var the desktop
 * no longer reads), returns `authMissing` so the caller
...
async function probeHostTunnel(serverUrl, hostId, { timeoutMs = 2000 } = {}) {
  ...
  // remote server with no obtainable token can't be probed in-process (auth
  // minted out of band) → report authMissing so the caller falls back to the
  // optimistic/unverified path, not "offline".
...
/**
 * This machine's connection to `serverUrl`, resolved WITHOUT the slow `goalrail
 ...
 * probed — e.g. out-of-band auth — so process-alive is reported
...
async function getHostConnectionFast(serverUrl, { probe = true, timeoutMs = 2000 } = {}) {
  ...
  if (res.authMissing) {
    // Can't reproduce out-of-band auth in-process → report the live
