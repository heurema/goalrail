...
interface ElectronDesktopApi extends NativeShellApi {
  ...
  /** The local `goalrail` CLI status (installed, resolved path, version, source). */
...
/** Status of the local `goalrail` CLI, from the desktop shell. */
...
export interface HostIdentity {
  /** Whether the `goalrail` CLI was found and is runnable. */
...
/**
 * Fetch the local `goalrail` CLI status from the desktop shell (installed, resolved
