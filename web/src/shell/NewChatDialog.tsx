...
// command sandbox) — see goalrail.stores.conversation_store
...
const CODEX_NATIVE_BYPASS_SANDBOX_LABEL_KEY = "goalrail.codex_native.bypass_sandbox";
...
export function harnessWarningMessageText(
  ...
  if (reason === "binary-missing") {
    return `${agentName} is missing the Codex binary on ${hostName} — run goalrail setup on that machine.`;
  }
  return `${agentName} isn't configured on ${hostName} — run goalrail setup on that machine.`;
}
...
function harnessWarningMessage(
  ...
  if (reason === "binary-missing") {
    return (
      <>
        {agentName} is missing the Codex binary on {hostName} — run <code>goalrail setup</code> on
        that machine.
      </>
    );
  }
  return (
    <>
      {agentName} isn&apos;t configured on {hostName} — run <code>goalrail setup</code> on that
      machine.
    </>
  );
}
 ...
 * row with the reserved key ``goalrail.project``). Mirrors the sidebar kebab's
...
export function NewChatLandingScreen() {
  ...
  // it — so the user doesn't have to run `goalrail host` in a terminal first. The
      ...
      if (effectiveAgentId === PENDING_AGENT_ID && pendingAgent) {
        ...
        const res = await authenticatedFetch("/v1/sessions", {
          ...
          body: JSON.stringify({
            ...
            // Native terminal agents open terminal-first: `goalrail.ui:
            // terminal` tells the UI to render the terminal wrapper, and
            // `goalrail.wrapper` selects which CLI bridge the runner launches.
