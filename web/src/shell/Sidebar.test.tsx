    ...
    const rows = !enabled
      ...
      : (override ??
        conversationsRef.current.filter(
          (c) =>
            (c.labels?.["goalrail.project"] ?? null) === project && (c as any).archived !== true,
        ));
    return {
      data: enabled
        ? {
...
describe("Sidebar session list", () => {
  ...
  it("swaps the card content to the settings section nav on /settings", () => {
    ...
    expect(screen.getByRole("link", { name: /Back to Goalrail/ })).toHaveAttribute("href", "/");
...
describe("Sidebar project sections", () => {
  it("groups sessions by their project label, separate from Chats", () => {
    ...
    mockConversations([
      ...
      conv("conv_filed", "Claude Code", { labels: { "goalrail.project": "Customer X" } }),
    ]);
    renderSidebar();
  ...
  it("fills a folder from its own fetch, independent of the global list window", async () => {
    ...
    projectSessionsMock.current["Customer X"] = [
      conv("conv_far_1", "Claude Code", { labels: { "goalrail.project": "Customer X" } }),
      conv("conv_far_2", "Claude Code", { labels: { "goalrail.project": "Customer X" } }),
    ];
    renderSidebar();
  ...
  it("moves a pinned project session out into the global Pinned section", () => {
    projectsMock.push("Customer X");
    mockConversations([
      conv("conv_plain", "Claude Code", { labels: { "goalrail.project": "Customer X" } }),
      conv("conv_pinned", "Claude Code", { labels: { "goalrail.project": "Customer X" } }),
    ]);
    // Pin one of the filed sessions via localStorage (client-side pins).
    localStorage.setItem("goalrail:pinned-conversation-ids", JSON.stringify(["conv_pinned"]));
    renderSidebar();
  ...
  it("does not render a project section when useProjects returns nothing", () => {
    ...
    mockConversations([
      conv("conv_filed", "Claude Code", { labels: { "goalrail.project": "Ghost" } }),
    ]);
    renderSidebar();
  ...
  it("collapses all project folders at once and reopens the previously-open set", () => {
    ...
    mockConversations([
      conv("conv_a", "Claude Code", { labels: { "goalrail.project": "Alpha" } }),
      conv("conv_b", "Claude Code", { labels: { "goalrail.project": "Beta" } }),
    ]);
    renderSidebar();
  ...
  it("shows the row's session-state badge on a collapsed project", () => {
    ...
    mockConversations([
      conv("conv_awaiting", "Claude Code", {
        labels: { "goalrail.project": "Customer X" },
        pending_elicitations_count: 1,
      }),
    ]);
    renderSidebar();
...
describe("Sidebar default section collapse", () => {
  it("expands Pinned and Chats by default when there is no stored preference", () => {
    localStorage.setItem("goalrail:pinned-conversation-ids", JSON.stringify(["conv_pin"]));
  ...
  it("honors a persisted collapse of Chats across remount", () => {
    localStorage.setItem("goalrail:collapsed-sidebar-sections", JSON.stringify(["Chats"]));
...
describe("Sidebar move-to-project action", () => {
  ...
  it("confirms removal only when it's the project's last session", async () => {
    projectsMock.push("Sprint 42");
    mockConversations([
      conv("conv_filed", "Claude Code", { labels: { "goalrail.project": "Sprint 42" } }),
    ]);
