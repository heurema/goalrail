...
describe("NewChatLandingScreen create flow", () => {
  ...
  it("persists the picked permission mode for claude-native so the next session seeds it", async () => {
    ...
    await waitFor(() =>
      expect(JSON.parse(localStorage.getItem("goalrail:last-mode-by-harness") ?? "{}")).toEqual({
  ...
  it("resets the shared approval mode to default when switching codex-native → opencode-native", async () => {
    ...
    const [, init] = vi.mocked(authenticatedFetch).mock.calls[0] as [string, RequestInit];
    const body = JSON.parse(init.body as string);
    expect(body.labels?.["goalrail.wrapper"]).toBe("opencode-native-ui");
