...
struct GoalrailWebView: UIViewRepresentable {
  let initialURL: URL
  ...
  func makeUIView(context: Context) -> WKWebView {
    ...
    contentController.add(context.coordinator, name: "goalrailNative")
  ...
  static func dismantleUIView(_ uiView: WKWebView, coordinator: Coordinator) {
    uiView.configuration.userContentController.removeScriptMessageHandler(forName: "goalrailNative")
    coordinator.detach()
  }

  private static let nativeBridgeScript = """
    (() => {
      if (window.goalrailNative && window.goalrailNative.kind === "ios") return;
      ...
      defineEmit("__goalrailNativeEmitNotificationActivated", (path) => {
        if (typeof path !== "string" || !path.startsWith("/")) return;
        for (const callback of callbacks) {
          try { callback(path); } catch {}
        }
      });
      defineEmit("__goalrailNativeEmitViewModeChanged", (mode) => {
      ...
      let lastInsets = null;
      defineEmit("__goalrailNativeEmitInsets", (topBar, bottomBar) => {
        const insets = {
      ...
      Object.defineProperty(window, "__goalrailNativeEmitSidebarDrag", {
        configurable: false,
        enumerable: false,
        writable: false,
      ...
      window.goalrailNative = Object.freeze({
        kind: "ios",
        setBadgeCount(count) {
          window.webkit.messageHandlers.goalrailNative.postMessage({
    ...
    UIGestureRecognizerDelegate
  {
    ...
    func load(_ url: URL, in webView: WKWebView) {
      pinnedURL = url
      pinnedOrigin = url.goalrailOrigin
    ...
    func webView(
      ...
      guard type == .microphone,
        origin.goalrailOrigin == pinnedOrigin,
        webView.url?.goalrailOrigin == pinnedOrigin
      else {
    ...
    private func isTrustedBridgeMessage(_ message: WKScriptMessage) -> Bool {
      ...
      guard message.frameInfo.securityOrigin.goalrailOrigin == pinnedOrigin else { return false }
      guard webView?.url?.goalrailOrigin == pinnedOrigin else { return false }
    ...
    private func promptForExternalURL(_ url: URL, scheme: String) {
      let onPinnedServer = pinnedOrigin != nil && webView?.url?.goalrailOrigin == pinnedOrigin
      ...
      let requester = webView?.url?.goalrailOrigin ?? "This page"
    ...
    private func handleLoadFailure(_ webView: WKWebView, error: Error) {
      ...
      guard failedURL.goalrailOrigin == pinnedOrigin else { return }
    ...
    private func topViewController() -> UIViewController? {
      ...
      return root?.goalrailTopViewController
    }
  }
}
...
extension UIViewController {
  fileprivate var goalrailTopViewController: UIViewController {
    if let presentedViewController {
      return presentedViewController.goalrailTopViewController
    }
    if let navigation = self as? UINavigationController,
      let visible = navigation.visibleViewController
    {
      return visible.goalrailTopViewController
    }
    if let tab = self as? UITabBarController,
      let selected = tab.selectedViewController
    {
      return selected.goalrailTopViewController
    }
    return self
  }
}
