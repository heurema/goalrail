...
final class WebViewModel: ObservableObject {
  ...
  func emitNotificationActivation(_ path: String) {
    ...
    let script =
      "window.__goalrailNativeEmitNotificationActivated?.(\(Self.javascriptString(path)));"
  ...
  /// web side folds these into its `--goalrail-inset-*` variables so page
  ...
  func emitInsets(topBar: CGFloat, bottomBar: CGFloat) {
    let script =
      "window.__goalrailNativeEmitInsets?.(\(jsNumber(topBar)), \(jsNumber(bottomBar)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitViewModeChanged(_ mode: WebViewMode) {
    let script =
      "window.__goalrailNativeEmitViewModeChanged?.(\(Self.javascriptString(mode.rawValue)));"
    webView?.evaluateJavaScript(script)
  }
  ...
  func emitSidebarDrag(phase: String, progress: Double) {
    ...
    let script =
      "window.__goalrailNativeEmitSidebarDrag?.(\(Self.javascriptString(phase)), \(clamped));"
    webView?.evaluateJavaScript(script)
  }
