...
from goalrail.entities import NewConversationItem
...
# resolvable (``GOALRAIL_LAKEBASE_INSTANCE`` is set, or a provider was injected
...
# token-refresh mode.
_LAKEBASE_INSTANCE_ENV = "GOALRAIL_LAKEBASE_INSTANCE"
...
def set_lakebase_token_provider(provider: LakebaseTokenProvider | None) -> None:
    ...
    *provider* once per new DBAPI connection, and uses the shorter
    ...
    fall back to the ``GOALRAIL_LAKEBASE_INSTANCE`` env-var path.
...
def _databricks_lakebase_token_provider(instance_name: str) -> str:
    ...
    :param instance_name: The Lakebase database instance name, e.g.
        ``"goalrail-db"`` (the value of ``GOALRAIL_LAKEBASE_INSTANCE``).
...
def _create_engine(db_uri: str) -> Engine:
    ...
    multi-process workloads — REPL + Goalrail server + runner subprocess
    ...
    # ``None`` unless ``GOALRAIL_LAKEBASE_INSTANCE`` is set or a provider was
...
def _build_alembic_config(db_uri: str) -> Config:
    ...
    :func:`goalrail debug db-upgrade` CLI command share the same
...
def _run_migrations(engine: Engine, db_uri: str) -> None:
    ...
    from goalrail.db.db_models import Base
...
def _initialize_or_verify_schema(engine: Engine, db_uri: str) -> None:
    ...
    if current != head:
        _logger.warning(
            "Goalrail database schema is out of date "
        ...
        if migrated != head:
            raise RuntimeError(
                f"Goalrail automatic database migration did not reach head "
...
def extract_search_text(item: NewConversationItem) -> str:
    ...
    """
    from goalrail.entities.conversation import CompactionData
