"""SQLAlchemy table definitions for the goalrail database."""
...
class Base(DeclarativeBase):
    """Shared declarative base for all goalrail tables."""
...
class SqlConversationLabel(Base):
    ...
    :param updated_at: Unix epoch seconds of the last write.
        ...
        unchanged (matches goalrail parity and keeps
...
class SqlHost(Base):
    ...
    via ``goalrail host``. The row is upserted on first connect
    ...
    :param created_at: Unix epoch seconds when the host was first
        registered (first ``goalrail host``).
