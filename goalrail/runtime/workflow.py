...
from goalrail.runtime.compaction import (
    CompactionResult,
    SummaryMetadata,
    _CompactionState,
    compact,
...
from goalrail.runtime.prompt import build_instructions, history_to_input_items
...
# Translates Goalrail spec executor types (underscore) to harness subprocess names
...
def _get_runner_client_for_compaction(
    ...
    Propagates :class:`GoalrailError` when the pinned runner is
    ...
    if conversation_id is None:
        return None
    from goalrail.runtime import get_runner_router
...
# (``goalrail/onboarding/provider_config.py`` ``_HARNESS_FAMILY``);
...
def _provider_auth_command(family: FamilyConfig) -> str:
    ...
    :raises GoalrailError: If the family carries neither a static
...
def configure_agent_harness_with_provider(
    ...
    *,
    ...
    :param harness_type: Canonical harness type, e.g. ``"claude-sdk"``.
    :raises GoalrailError: If an inline-family provider lacks the family
    ...
    if entry.kind == BEDROCK_KIND:
        # Bedrock mode is wired only into the native ``goalrail claude`` launch
        # (:func:`goalrail.claude_native._bedrock_config_for_native_claude`),
    ...
    if entry.kind == CLI_CONFIG_KIND:
        ...
        if harness_type != "codex":
            raise GoalrailError(
                ...
                "harness in ~/.goalrail/config.yaml.",
                code=ErrorCode.INVALID_INPUT,
            )
...
# Maps an goalrail provider family to the bundled catalog provider name
...
def _catalog_default_model(family_name: str) -> str | None:
    ...
    :param family_name: The goalrail family, ``"anthropic"`` or
        ``"openai"``.
...
def _apply_provider_family(
    ...
    if cfg.model_key not in env:
        ...
        raise GoalrailError(
            ...
            "'models: {default: ...}' to that provider family in ~/.goalrail/config.yaml.",
            code=ErrorCode.INVALID_INPUT,
        )
    if harness_type == "codex":
...
def _optional_provider_family(entry: ProviderEntry, family_name: str) -> FamilyConfig | None:
    ...
    The only :class:`GoalrailError` raised at family-access time comes from
...
def _apply_provider_to_pi(env: dict[str, str], entry: ProviderEntry) -> None:
    ...
    :raises GoalrailError: If no configured family's credentials resolve,
    ...
    if isinstance(auth, ProviderAuth):
        ...
        if entry is None:
            raise GoalrailError(
                ...
                "configured under 'providers:' in ~/.goalrail/config.yaml. "
                "Run `goalrail setup --no-internal-beta` to configure one.",
                code=ErrorCode.INVALID_INPUT,
            )
        return entry
...
def _build_claude_sdk_spawn_env(
    ...
    *,
    ...
    vars defined in ``goalrail/inner/claude_sdk_harness.py``.
    Per the v1 spec-config-flow design (see §Step 5b in the
    design doc), per-spawn env overrides are how Goalrail threads
    ...
    # for goalrail-style specs (the inner CLI auto-creates a
    # ``caller_process`` os_env when ``--os`` is set, and the
    # SDK's bundled CLI exposes the natives unconditionally in
    # some configurations); routing through Goalrail mode without a
    # similar default leaves Goalrail mode users staring at a
    ...
    # active and approval requests surface via Goalrail elicitation. Read from
...
def _build_codex_spawn_env(
    ...
    *,
    ...
    defined in ``goalrail/inner/codex_harness.py``. Mirrors
    ...
    :func:`goalrail.inner.executor_factory.create_executor`
    ...
    on the Goalrail server directly (they propagate to the subprocess
...
def _build_pi_spawn_env(
    ...
    *,
    ...
    defined in ``goalrail/inner/pi_harness.py``. Mirrors
...
def _build_qwen_spawn_env(
    ...
    *,
    ...
    defined in ``goalrail/inner/qwen_harness.py``. Mirrors
    :func:`_build_claude_sdk_spawn_env` /
...
def _load_global_auth() -> ApiKeyAuth | DatabricksAuth | None:
    ...
    ``$GOALRAIL_CONFIG_HOME`` for test isolation) and parses the
    ...
    path = (
        ...
        if config_home
        else Path.home() / ".goalrail" / "config.yaml"
    )
    if not path.exists():
        return None
    try:
...
def _build_openai_agents_sdk_spawn_env(spec: AgentSpec) -> dict[str, str]:
    ...
    env vars defined in
    ``goalrail/inner/openai_agents_sdk_harness.py``. Threads
...
def _build_cursor_spawn_env(
    ...
    *,
    ...
    if isinstance(spec.executor.auth, ApiKeyAuth):
        ...
        from goalrail.onboarding.cursor_auth import resolve_cursor_api_key
...
def _build_kimi_spawn_env(
    ...
    *,
    ...
    defined in :mod:`goalrail.inner.kimi_harness`.
    ...
    :param cwd: Runtime working directory for the kimi subprocess — the
        ...
        When unset, the harness wrap falls back to ``GOALRAIL_RUNNER_WORKSPACE``.
        Mirrors :func:`_build_pi_spawn_env`'s ``cwd`` handling.
    :returns: A dict of env-var overrides.
    :raises GoalrailError: If the spec declares ``executor.auth`` —
    ...
    if spec.executor.auth is not None:
        raise GoalrailError(
            ...
            "resulting model id in the agent spec. Goalrail-side provider "
...
def _build_copilot_spawn_env(
    ...
    *,
    ...
    no auth at all, a GitHub token registered once via ``goalrail setup`` (the
...
def _load_initial_history(
    ...
    # the empty synthetic pair — the Goalrail thinks context is ~0 tokens
...
async def compact_conversation_now(
    *,
    ...
    :returns: The :class:`CompactionResult` returned by
        :func:`goalrail.runtime.compaction.compact`.
    """
    ...
    if preserve_recent_window is not None:
        ...
        from goalrail.spec.types import CompactionConfig
    ...
    from goalrail.llms.context_window import get_model_context_window
...
def _maybe_persist_compaction_item(
    ...
    # session while the Goalrail thinks context is near-zero and never
