...
orphan sweep so a previous Goalrail crash doesn't leave runner processes
behind.

This module knows nothing about the harness API — it spawns a
:mod:`goalrail.runtime.harnesses._runner` subprocess per
...
:mod:`goalrail.server.schemas`; everything FastAPI-shaped is in
...
from goalrail.inner._subprocess_lifecycle import close_subprocess_transport
from goalrail.runner.identity import strip_runner_auth_secrets
from goalrail.runtime.harnesses import _HARNESS_MODULES

_logger = logging.getLogger(__name__)

# Per-AP-instance directory holding all per-conversation Unix
# sockets (POSIX) and the AP_PID sentinel file. Each Goalrail instance gets a
# uuid-named subdir so concurrent Goalrail processes (zero-downtime restarts,
# multi-tenant single-machine deployments) don't step on each other.
#
# POSIX pins ``/tmp/goalrail`` deliberately: Unix socket paths have a tight
...
# ``/tmp/goalrail`` there resolves to ``\tmp\goalrail`` on the current drive —
# so use the real temp dir.
if IS_WINDOWS:
    _TMP_PARENT = Path(tempfile.gettempdir()) / "goalrail"
else:
    _TMP_PARENT = Path("/tmp/goalrail")
_TMP_PARENT_ENV_VAR = "GOALRAIL_HARNESS_TMP_PARENT"
...
# depth.
_HARNESS_AUTH_TOKEN_ENV = "GOALRAIL_HARNESS_AUTH_TOKEN"

# Sentinel file the Goalrail instance writes into its subdir on boot. The
# orphan sweep uses it to tell whether a sibling subdir belongs to
# a still-running Goalrail (leave alone) or a crashed one (kill its
...
# enough that a wedged subprocess doesn't block Goalrail shutdown.
...
# or model-thinking gaps. Goalrail doesn't bound harness-side latency —
...
def _default_tmp_parent() -> Path:
    ...
    Operators can set :envvar:`GOALRAIL_HARNESS_TMP_PARENT` when
...
def _socket_path(instance_dir: Path, conversation_id: str) -> Path:
    ...
    :param instance_dir: This Goalrail instance's directory, e.g.
        ``/tmp/goalrail/ap-abc123``.
...
def _resolve_module_path(harness: str) -> str:
    ...
    raise RuntimeError(
        ...
        f"goalrail.runtime.harnesses._HARNESS_MODULES)"
    )
    ...
    while True:
        if process.returncode is not None:
            ...
            raise RuntimeError(
                ...
                f"{process.returncode} during spawn (see Goalrail stderr)"
            )
...
class _SubprocessEntry:
    ...
    :param client: The :class:`httpx.AsyncClient` Goalrail uses to talk
        to this subprocess over its Unix socket.
    :param socket_path: Absolute Unix socket path the runner
        bound, e.g.
        ``Path("/tmp/goalrail/ap-abc/conv-xyz.sock")``.
...
class HarnessProcessManager:
    ...
    Use ``start()`` once at Goalrail boot to create the per-instance
    ...
    terminal state, and ``shutdown()`` at Goalrail shutdown to release
    ...
    :param reaper_interval_s: Seconds between idle-reaper passes.
        Defaults to 60.
    :param tmp_parent: Override for the parent ``/tmp/goalrail``
        ...
        :envvar:`GOALRAIL_HARNESS_TMP_PARENT` to choose a
    ...
    def instance_dir(self) -> Path:
        """
        This Goalrail instance's per-instance directory.

        :returns: Path like ``/tmp/goalrail/ap-<uuid>``.
        """
    ...
    async def forward_cancel(
        ...
        :param conversation_id: The Goalrail conversation id whose
    ...
    async def release(self, conversation_id: str) -> None:
        ...
        conversation ids the Goalrail instance has ever spawned for. If
    ...
    async def shutdown(self) -> None:
        ...
        # rmtree leaves it behind; the next Goalrail boot's orphan
        # sweep handles it.
    ...
    async def _spawn_entry(
        ...
        # with Goalrail logs. The runner doesn't currently prefix lines
        ...
        process = await asyncio.create_subprocess_exec(
            sys.executable,
            "-m",
            "goalrail.runtime.harnesses._runner",
            "--harness",
            harness,
            "--module",
            module_path,
    ...
    async def _sweep_orphans(self) -> None:
        ...
        for child in self._tmp_parent.iterdir():
            ...
            try:
                ...
                continue
            if _pid_alive(pid):
                # Sibling Goalrail is still running — leave it alone.
                continue
            _logger.info(
                "sweeping orphaned Goalrail instance dir %s (pid %d not running)",
                child,
                pid,
            )
