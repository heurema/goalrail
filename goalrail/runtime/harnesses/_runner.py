...
Invoked by :class:`goalrail.runtime.harnesses.process_manager.HarnessProcessManager`
...
- ``--module <python_path>``: importable module that exports
  ...
  :data:`goalrail.runtime.harnesses._HARNESS_MODULES` before
...
- ``--conversation-id <id>``: the conversation this process serves.
  ...
  doc. Goalrail allocates the id; the runner does NOT parse it from the
...
_GRACEFUL_SHUTDOWN_TIMEOUT_S = float(os.environ.get("GOALRAIL_HARNESS_SHUTDOWN_TIMEOUT_S", "5"))
...
_HARD_EXIT_TIMEOUT_S = float(
    os.environ.get(
        "GOALRAIL_HARNESS_HARD_EXIT_TIMEOUT_S",
...
def _load_harness_app(harness: str, module_path: str, conversation_id: str) -> FastAPI:
    ...
    :param module_path: Fully-qualified Python module to import,
        e.g. ``"goalrail.inner.claude_sdk_harness"``. Must
    ...
    # (``GOALRAIL_HARNESS_AUTH_TOKEN``); the scaffold's auth check compares
    ...
    # the gate inert.
    app.state.harness_auth_token = os.environ.get("GOALRAIL_HARNESS_AUTH_TOKEN")
    return app
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    :param argv: Argument list (without the program name),
        ...
        "goalrail.inner.claude_sdk_harness", "--socket",
        "/tmp/goalrail/<id>/conv-abc.sock", "--conversation-id",
    ...
    parser = argparse.ArgumentParser(
        prog="python -m goalrail.runtime.harnesses._runner",
        description=(
...
def main(argv: list[str] | None = None) -> None:
    ...
    # Bind the endpoint Goalrail allocated for this conversation: a Unix socket
