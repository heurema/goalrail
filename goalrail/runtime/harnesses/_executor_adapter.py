...
- Per-turn translation of Goalrail :class:`CreateResponseRequest` →
  inner :class:`Message` list + :class:`ExecutorConfig`.
- Per-turn translation of inner :class:`ExecutorEvent`s →
  typed Goalrail SSE events emitted via :meth:`TurnContext.emit`.
...
from goalrail.inner.tracing import TracingContext, is_tracing_enabled
from goalrail.policies.types import FAIL_CLOSED_PHASES
from goalrail.runtime.harnesses._scaffold import HarnessApp, PolicyVerdictPayload, TurnContext
...
# (e.g. ``mcp__goalrail__sys_terminal_launch``). Tools whose
...
def _finalize_trace_status(response_id: str) -> None:
    ...
    """
    try:
        from goalrail.runtime.telemetry import trace_id_from_response_id
...
def _trace_context_or_null(response_id: str) -> contextlib.AbstractContextManager[None]:
    """Return a response trace context, or no-op when the ID is not traceable."""
    try:
        from goalrail.runtime.telemetry import (
            trace_context_for_response,
            trace_id_from_response_id,
        )

        trace_id_from_response_id(response_id)
        return trace_context_for_response(response_id=response_id)
    except Exception:
...
def _strip_mcp_tool_prefix(name: str) -> str:
    ...
    (e.g. ``mcp__goalrail__sys_terminal_launch``). The bare
    name (``sys_terminal_launch``) is what the Goalrail wire shape
    and persisted conversation items carry — kept in sync with
    :func:`goalrail.runtime.workflow._observed_tool_call_sse_dicts`
    and :func:`_build_observed_tool_items` so the SSE name, the
    store-item name, and the adapter's emitted name all agree.

    Mirrors :func:`goalrail.runtime.workflow._strip_mcp_tool_prefix`
...
class ExecutorAdapter(HarnessApp):
    ...
    def __init__(
        ...
        # already carried — that's what lets the Goalrail REPL dedupe
    ...
    async def run_turn(self, request: CreateResponseRequest, ctx: TurnContext) -> None:
        ...
        can round-trip spec-declared tools through Goalrail via the
        ...
        # Goalrail server (routed through the runner).
        ...
        try:
            ...
            if tctx:
                trace_cm = _trace_context_or_null(ctx.response_id)
            with trace_cm:
                if tctx is not None:
                ...
                async for event in executor.run_turn(
                    ...
                    if ctx.cancelled.is_set():
                        if tctx is not None and agent_span is not None:
                            from goalrail.runtime.telemetry import record_cancellation
                    ...
                    if tctx is not None:
                        if isinstance(event, ToolCallRequest):
                            ...
                            if event.usage is not None:
                                from goalrail.runtime.telemetry import record_llm_usage
    ...
    async def _handle_interrupt_event(self) -> Response:
        ...
        :raises GoalrailError: 404 (from the base handler) when no turn is
            in flight.
        """
    ...
    async def _stable_tool_executor(
        ...
        :param tool_name: Tool name from the LLM's call. Carries
            ...
            ``"mcp__goalrail__sys_terminal_launch"``).
        ...
        # strips the ``mcp__goalrail__`` prefix before
    ...
    async def _stable_elicitation_handler(
        ...
        Called by :class:`goalrail.inner.claude_sdk_executor.ClaudeSDKExecutor`
    ...
    async def _stable_policy_evaluator(
        ...
        :returns: The policy verdict from the Goalrail server.
        """
        ctx = self._current_ctx
        if ctx is None:
            ...
            # the runner's phase-aware default in _evaluate_policy_via_goalrail —
    ...
    def _translate_event(self, event: ExecutorEvent, ctx: TurnContext) -> None:
        """
        Translate one inner :class:`ExecutorEvent` into Goalrail SSE
        ...
        if isinstance(event, TextChunk):
            ...
            # Translate inner reasoning to the Goalrail wire shape so the
            ...
            # (``mcp__goalrail__sys_terminal_launch``) but the
            # Goalrail wire shape and persisted conversation items
            # carry the bare form — kept consistent with
            # ``goalrail/runtime/workflow.py``'s
            ...
            # payload to populate TurnComplete.usage on the Goalrail side.
            if event.usage is not None:
    ...
    def _build_error_detail(self, exception: BaseException) -> Any:
        ...
        (:data:`goalrail.runtime.harnesses._client_executor._RETRYABLE_HARNESS_ERROR_CODES`).
        ...
        1. :class:`goalrail.errors.GoalrailError` (incl.
        ...
        :returns: An :class:`ErrorDetail` whose ``code`` matches
            the Goalrail allowlist for known retryable failures, and is
        ...
        if isinstance(exception, GoalrailError):
...
def _classify_openai_exception(exception: BaseException) -> str | None:
    """
    Map an OpenAI SDK exception onto the Goalrail semantic code allowlist.
    ...
    extending the allowlist would just produce strings Goalrail ignores.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the Goalrail allowlist (e.g.
    ...
    from goalrail.llms.errors import is_context_length_exceeded
...
def _classify_claude_sdk_exception(exception: BaseException) -> str | None:
    """
    Map a :mod:`claude_agent_sdk` exception onto the Goalrail semantic
    code allowlist.
    ...
    :param exception: The exception :meth:`ExecutorAdapter.run_turn`
        caught.
    :returns: A semantic code from the Goalrail allowlist, or ``None``
...
def _classify_httpx_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`httpx` exception onto the Goalrail semantic code allowlist.
    ...
    # no semantic code, and the Goalrail retry classifier treats them
    # as permanent.
...
def _classify_anthropic_exception(exception: BaseException) -> str | None:
    """
    Map an :mod:`anthropic` SDK exception onto the Goalrail semantic
    code allowlist.
    ...
    ``[llm] APIConnectionError`` etc., which the Goalrail retry
...
def classify_inner_exception(exception: BaseException) -> str | None:
    """
    Map any inner-SDK exception onto the Goalrail semantic code allowlist.
...
async def _bridge_one_dispatch(
    ...
    *,
    ...
    # Try to parse the output as JSON — if Goalrail serialized the
...
def _extract_role_keyed_messages(
    ...
    """
    Pull role-keyed message items out of an Goalrail ``input`` list.
    ...
    :param input_value: The Goalrail ``input`` list.
...
def _serialize_tool_result(event: ToolCallComplete) -> str:
    ...
    - A list of typed content blocks (``{"type": "text", "text":
      ...
      the LLM-rendered tool output reaches Goalrail intact. Without
    ...
    - Empty string is a last resort — the inner Executor
      ...
      logged so a regression surfaces in the Goalrail logs.
