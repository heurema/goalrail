...
Never import this module outside of goalrail.runtime.
...
# ``goalrail.runtime``. Conversation-scoped: one entry per
...
# lifespan in ``goalrail/server/app.py``; accessed via
# ``get_harness_process_manager()`` in ``goalrail.runtime``.
...
# ``goalrail.runtime``), the runner routes
...
# in :mod:`goalrail.server.routes.terminal_attach` produces the
...
def init(
    *,
    ...
    # ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §4.2 for the design.
