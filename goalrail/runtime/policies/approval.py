...
:class:`goalrail.server.schemas.ElicitationResult` and
...
async def _await_elicitation(
    *,
    ...
    in :func:`goalrail.runtime.workflow._drive_policy_approval`.
...
def _resolve_elicitation_mode() -> str:
    ...
    Reads ``GOALRAIL_ELICITATION_MODE`` (``"url"`` or ``"form"``).
    Defaults to ``"url"`` when unset — the standalone approval page
    is the default experience.

    :returns: ``"url"`` or ``"form"``.
    """
    raw = os.environ.get("GOALRAIL_ELICITATION_MODE", "url").lower().strip()
...
def build_elicitation_request_event(
    ...
    *,
    ...
    :param elicitation_id: Unique id correlating this request to the
        consumer's :class:`goalrail.server.schemas.ElicitationResult`
