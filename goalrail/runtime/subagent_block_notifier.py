...
``goalrail/runner/app.py`` leaves the orchestrator idle and unaware. This
...
coroutine (kept free of HTTP/server knowledge) wired up by the Goalrail server
...
class SubagentBlockNotifier:
    ...
    One instance is constructed and registered per Goalrail server process.
    Its :meth:`observe` method is registered with
    :func:`goalrail.runtime.pending_elicitations.set_elicitation_observer`;
    ...
    :param loop: The Goalrail server's event loop, captured at registration
