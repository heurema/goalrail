...
class _CompactionState:
    ...
    :param conversation_id: Conversation id, e.g.
        ...
        the runner's credentials rather than the Goalrail server's.
    """
...
async def summarize_history(
    ...
    runner's credentials are used instead of the Goalrail server's. Falls
...
async def _summarize_via_runner_uncached(
    ...
    so the Goalrail server needs no LLM auth for this call.
...
def compaction_to_history_items(
    ...
    :param compaction_item: The compaction item from the store,
        ...
        ``data`` of type :class:`~goalrail.entities.CompactionData`.
    :returns: Two :class:`~goalrail.entities.ConversationItem`
...
async def compact(
    ...
    *messages* copy passed to the LLM is compacted.
    ...
    :param runner_client: Optional ``httpx.AsyncClient`` pointed at
        ...
        credentials are used instead of the Goalrail server's. ``None``
    ...
    if conversation_id:
        ...
        from goalrail.runtime import session_stream as _session_stream
