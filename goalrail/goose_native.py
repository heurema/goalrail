"""Native Goose TUI wrapper for the Goalrail CLI.

``goalrail goose`` launches Block's Goose CLI interactive TUI (``goose session``)
inside a Goalrail-runner-owned tmux terminal and attaches the local TTY — the
goose analog of ``goalrail cursor`` / ``goalrail codex`` / ``goalrail pi``. The
runner spawns the process (see
:func:`goalrail.runner.app._auto_create_goose_terminal`); this module owns the
...
``~/.config/goose/config.yaml`` + keyring); no Goalrail-managed key is required.
...
from goalrail._wrapper_labels import GOOSE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_GOOSE_PATH_ENV = "GOALRAIL_GOOSE_PATH"
...
def run_goose_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Goose requires a resolved Goalrail server URL. The CLI should call "
            "_ensure_backend before run_goose_native."
        )
    with TemporaryDirectory(prefix="goalrail-goose-native-") as tmpdir:
...
def _materialize_goose_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``goalrail goose``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Goose on a Goalrail server via a daemon-spawned runner.
