"""Native Hermes TUI wrapper for the Goalrail CLI.

``goalrail hermes`` launches Nous Research's Hermes Agent interactive TUI (the bare
``hermes`` command) inside a Goalrail-runner-owned tmux terminal and attaches the
local TTY — the Hermes analog of ``goalrail goose`` / ``goalrail cursor``. The
runner spawns the process (see
:func:`goalrail.runner.app._auto_create_hermes_terminal`); this module owns the
...
``~/.hermes/config.yaml``); no Goalrail-managed key is required. Like goose there
...
from goalrail._wrapper_labels import HERMES_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_HERMES_PATH_ENV = "GOALRAIL_HERMES_PATH"
...
def run_hermes_native(
    *,
    ...
    if server is None:
        raise click.ClickException(
            "Hermes requires a resolved Goalrail server URL. The CLI should call "
            "_ensure_backend before run_hermes_native."
        )
    with TemporaryDirectory(prefix="goalrail-hermes-native-") as tmpdir:
...
def _materialize_hermes_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``goalrail hermes``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Hermes on a Goalrail server via a daemon-spawned runner.
