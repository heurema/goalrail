"""Native qwen TUI wrapper for the Goalrail CLI.

``goalrail qwen`` launches Qwen Code's interactive TUI (``qwen``) inside an
Goalrail-runner-owned tmux terminal and attaches the local TTY — the qwen analog
of ``goalrail goose`` / ``goalrail cursor``. The runner spawns the process (see
:func:`goalrail.runner.app._auto_create_qwen_terminal`), pointing qwen at the
...
``/auth`` command persisted under ``~/.qwen``); no Goalrail-managed key is
...
from goalrail._native_resume_hint import echo_native_cold_resume_hint, echo_native_resume_hint
from goalrail._runner_startup import RunnerStartupProgress, runner_startup_progress
from goalrail._wrapper_labels import QWEN_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from goalrail.native_terminal import bind_session_runner as _bind_session_runner
from goalrail.native_terminal import url_component

_DEFAULT_QWEN_COMMAND = "qwen"
_QWEN_PATH_ENV = "GOALRAIL_QWEN_PATH"
...
class PreparedQwenTerminal:
    ...
    :param cold_resumed: ``True`` when resuming an existing Goalrail session whose
...
def run_qwen_native(
    *,
    ...
    """
    Launch the qwen TUI in a Goalrail terminal.

    :param server: Resolved Goalrail server URL.
    ...
    if server is None:
        raise click.ClickException(
            "qwen requires a resolved Goalrail server URL. The CLI should call "
            "_ensure_backend before run_qwen_native."
        )
    with TemporaryDirectory(prefix="goalrail-qwen-native-") as tmpdir:
...
def _materialize_qwen_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``goalrail qwen``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch qwen on a Goalrail server via a daemon-spawned runner.
