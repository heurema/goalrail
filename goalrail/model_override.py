...
# Generous-but-safe upper bound; real ids ("us.anthropic.claude-sonnet-4-6")
# stay well under it.
...
# in sync with ``_HARNESS_MODEL_ENV_KEY`` in ``goalrail/runner/app.py``.
...
def validate_model_override(value: str) -> str:
    ...
    :param value: Raw model id, e.g. ``"anthropic/claude-sonnet-4-6"``.
...
# codex stays single-vendor (GPT-only): codex >= 0.137 dropped the
...
# Vertex AI. So unlike the single-vendor harnesses above, the rule here is
# framed as a *reject-list* of the families it definitively cannot serve
# (Claude / GPT), rather than a strict Gemini allow-list — bare/ambiguous ids
# (e.g. a future ``gemini-pro`` alias the SDK accepts) still pass through to
# the Gemini-native SDK path. Mirrors how the cross-family rejection above
# fails loud at the dispatch gate instead of leaking a
# ``HARNESS_ANTIGRAVITY_MODEL`` the SDK can never route.
...
def model_family_mismatch(harness: str, model: str) -> str | None:
    ...
    GPT ids contain ``"gpt"`` or ``"codex"``. Single-vendor harnesses reject
    ...
    families, but accepts Gemini shapes and bare/ambiguous ids the SDK may
    honor.
    ...
    if canon in _ANTIGRAVITY_FAMILY_HARNESSES and (is_claude or is_gpt):
        return (
            f"harness {canon!r} is Gemini-native and cannot run Claude/GPT "
            f"models; got {model!r}. Use a Gemini id "
...
def canonical_model_spelling(model: str) -> str:
    """
    Return the canonical spelling of *model*.

    Kept as a shared hook for callers that compare model ids, but now returns
    the input unchanged because provider-specific aliases are no longer
    supported.

    :param model: A model id, e.g. ``"claude-haiku-4-5"``.
...
def normalize_model_for_provider(model: str, provider_kind: str | None) -> str:
    """
    Return *model* unchanged for the child's resolved provider.

    Provider aliases are no longer supported, so the dispatch gate preserves
    the caller's model id verbatim.
    ...
    :param provider_kind: The child's resolved provider kind from
        :func:`goalrail.model_catalog.resolve_model_provider`.
    :returns: *model* unchanged.
    """
    _ = provider_kind
    return model
