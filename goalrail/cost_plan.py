...
(:mod:`goalrail.runner.cost_advisor`) and the session label that
...
canonicalizer from :mod:`goalrail.model_override`, so tier ranking and
...
# ``create_session`` in :mod:`goalrail.server.routes.sessions`).
...
class AdvisorVerdict:
    ...
    :param rationale: One-sentence judge explanation, surfaced in the
        ...
        always produces a string (:mod:`goalrail.runner.cost_judge`
