...
:mod:`goalrail.codex_native_app_server`). Codex spawns this module as
...
:mod:`goalrail.native_policy_hook`.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        f"goalrail codex hook: unknown subcommand {raw_argv[:1]!r}",
        file=sys.stderr,
    )
    return 0
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    :func:`goalrail.native_policy_hook.hook_payload_to_evaluation_request`,
    ...
    :mod:`goalrail.codex_native_app_server`, not here, because a
    ...
    try:
        ...
        print(f"goalrail codex evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("goalrail codex evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if eval_request is None:
        # Unrecognized hook event or an mcp__goalrail__* tool (relay-enforced).
        return 0
    ...
    if resp is None:
        return _fail_closed()
    if not resp.content:
        print("goalrail codex evaluate-policy hook: empty Goalrail response", file=sys.stderr)
        return _fail_closed()
    ...
    try:
        ...
        print(
            "goalrail codex evaluate-policy hook: malformed Goalrail response",
            file=sys.stderr,
        )
        return _fail_closed()
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m goalrail.codex_native_hook evaluate-policy")
