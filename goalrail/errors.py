"""Centralized error handling for the goalrail server.

All user-facing errors should be raised as GoalrailError with an
...
New code should prefer GoalrailError for consistency.
"""
...
class ErrorCode:
    ...
    :cvar HARNESS_NOT_CONFIGURED: The session's harness is not
        ...
        running ``goalrail setup`` on the host machine).
    """
...
_CODE_TO_HTTP_STATUS: dict[str, int] = {
    ...
    # can't satisfy it until the user runs `goalrail setup` there —
