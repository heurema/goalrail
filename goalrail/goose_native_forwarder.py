...
The ``goalrail goose`` wrapper launches the real ``goose session`` TUI in a
runner-owned tmux pane, and :mod:`goalrail.goose_native_bridge` injects web-UI
...
:mod:`goalrail.cursor_native_forwarder`. Goose stores sessions in a SQLite
...
runner launches ``goose session --name <goalrail-session-id>``, discovery is a
...
:mod:`goalrail.runner.app`), exactly as for cursor-/claude-native.
"""
...
def _resolve_goose_session_id(db_path: Path, session_name: str) -> str | None:
    ...
    The runner launches ``goose session --name <goalrail-session-id>``; the row
...
async def forward_goose_store_to_session(
    *,
    ...
    ``--name <goalrail-session-id>`` the runner launched with), then polls its
...
async def supervise_goose_forwarder(
    *,
    ...
    Mirrors :func:`goalrail.cursor_native_forwarder.supervise_cursor_forwarder`:
