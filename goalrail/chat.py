"""Implementation of the ``goalrail chat`` command.
...
from goalrail_client import (
    GoalrailError as ClientGoalrailError,
)
...
from goalrail.conversation_browser import open_conversation_link_if_enabled
...
from goalrail.spec.types import AgentSpec, SkillSpec
...
# freshly-launched ``goalrail run`` sessions don't burn a
...
# ``_DEFAULT_AD_HOC_MODEL`` so ``goalrail run examples/hello_world.yaml``
...
# ``goalrail/inner/cli.py:344``.
_GOALRAIL_MODEL_ENV_VAR = "GOALRAIL_MODEL"
...
def _default_cli_model() -> str:
    ...
    Reads ``GOALRAIL_MODEL`` from the environment with
    ...
    Mirrors :func:`goalrail.inner.cli._default_cli_model` so
...
class ChatOverrides:
    """
    CLI overrides from ``goalrail run`` flags.
    ...
    :param harness: ``--harness`` value, e.g. ``"claude-sdk"``.
        ...
        ``executor.harness`` key for single-file goalrail YAMLs and to
...
class LocalServer:
    """
    Handle to a locally-launched goalrail server and its sibling runner.
    ...
    :param log_path: Path to the file that captures the
        ...
        e.g. ``Path("~/.goalrail/logs/server/server-abc123.log")``.
...
def run_chat(
    ...
    *,
    ...
    """
    Main entry point for ``goalrail run`` (and the ``attach`` client).
    ...
    :param ephemeral: When ``True``, place the local server's
        ...
        the persistent ``~/.goalrail`` location. Maps to
    ...
    :param log: When ``True``, write a JSON dump of the active
        conversation to ``~/.goalrail/logs/`` on REPL exit.
        Maps to ``--log`` on the CLI (default-on for the legacy
        path, default-off here so it stays explicit on
        Goalrail mode). See ``goalrail.repl._session_log`` for the
    ...
    if _is_url(target):
        ...
        # claude-native.
        from goalrail.cli import _ensure_backend
...
def run_prompt(
    ...
    *,
    ...
    ``goalrail run ... -p``. It deliberately bypasses the
    ...
    *,
    ...
    if not info.runner_online:
        raise click.ClickException(
            ...
            "(`goalrail run` locally, or reconnect it with `goalrail host`), "
            "then attach again."
        )
...
def _remote_headers(
    ...
    :returns: Headers to pass to httpx / GoalrailClient.
    """
    token = os.environ.get(_REMOTE_AUTH_TOKEN_ENV)
    if token and (token := token.strip()):
        return {"Authorization": f"Bearer {token}"}
    # Check stored OIDC token from `goalrail login`.
    if server_url:
...
def _server_headers(
    *,
    ...
    :param runner_id: Optional runner UUID, e.g.
        ...
        request header.
    :returns: Static headers for ``httpx`` / ``GoalrailClient``.
    """
    del runner_id
    return {}
...
def _chat_with_server(
    ...
    *agent_name* is supplied by an upstream setup step such as
    ...
    :param server_log_path: Path to the local server's
        ...
        ``Path("~/.goalrail/logs/server/server-abc123.log")``. Shown in the
        Ctrl+O debug overview. ``None`` for remote servers.
    :param runner_log_path: Path to the local runner's
        stdout/stderr log file, e.g.
        ``Path("~/.goalrail/logs/runner/runner-abc123.log")``. Shown in the
    ...
    # ``goalrail.wrapper`` label on the conversation and re-dispatch
...
def _run_pi_native_resume_redirect(
    *,
    ...
    """
    Hand a pi-native conversation back to ``goalrail pi``.
...
def _run_kiro_native_resume_redirect(
    *,
    ...
    """Hand a kiro-native conversation back to ``goalrail kiro``."""
    _finish_native_redirect_progress(
        progress=progress,
        conversation_id=conversation_id,
...
def _run_cursor_native_resume_redirect(
    *,
    ...
    """
    Hand a cursor-native conversation back to ``goalrail cursor``.
    ...
    recording each user message twice. Redirecting to ``goalrail cursor``'s
...
def _run_kimi_native_resume_redirect(
    *,
    ...
    """
    Hand a kimi-native conversation back to ``goalrail kimi``.
    ...
    ``goalrail kimi``'s direct tmux attach keeps the TUI the single source of
...
def _wrapper_label_for_conversation(
    *,
    ...
    the response's ``labels.goalrail.wrapper`` field. ``None`` on any
...
def _await_accounts_first_run_setup(
    ...
    *,
    ...
    When ``goalrail run`` (re)spawns the local Goalrail server in accounts mode on
    ...
    the browser auto-opened (e.g. ``GOALRAIL_ACCOUNTS_AUTO_OPEN=0``) — and
...
async def _prepare_chat_session_via_daemon(
    *,
    ...
    :raises click.ClickException: If session create/fork or runner launch
        fails.
    """
    from goalrail_client import GoalrailClient
    ...
    from goalrail.native_terminal import bind_session_runner

    async with GoalrailClient(base_url=base_url, headers=headers, auth=auth) as sdk:
    ...
    async with httpx.AsyncClient(
        ...
        # clear the ``goalrail.stopped`` marker on resumed sessions. Must run
...
def _wait_for_remote_runner(
    ...
    *,
    ...
    :param log_path: Optional path to the captured runner log
        ...
        e.g. ``Path("~/.goalrail/logs/runner/runner-abcd.log")``.
    ...
    :param show_progress: When ``True`` (default), render a rich
        ...
        non-TTY stderr; see :mod:`goalrail._runner_startup`.
    :returns: None.
...
def _bundle_agent(agent_path: Path) -> bytes:
    ...
    :raises GoalrailError: If bundling fails, for example due to
        unresolved environment variables.
    """
    from goalrail.cli import _bundle
...
def _chat_local(
    ...
    *,
    ...
    :param ephemeral: When ``True``, point the local server at a
        ...
        (default) uses the persistent ``~/.goalrail``
        location so prior conversations remain reachable —
        see designs/RUN_GOALRAIL_SESSION_RESUMPTION.md.
    ...
    try:
        ...
        try:
            agent_spec = load_spec(spec_path)
        except (GoalrailError, FileNotFoundError) as exc:
...
def _run_headless_prompt(
    ...
    *,
    ...
    try:
        asyncio.run(_main())
    except ClientGoalrailError as exc:
...
async def _query_sessions_once(
    *,
    ...
    """
    from goalrail_client import SessionsChat
    ...
    # raises ``GoalrailError`` on that ``failed`` status, and the
    ...
    # ClientGoalrailError`` and print the message to stderr + exit non-zero.
    turn_error = await _persisted_turn_error(client, bound.id)
    if turn_error is not None:
        raise ClientGoalrailError(turn_error)
    return None
...
async def _persisted_turn_text(
    ...
    * A transport-level runner disconnect publishes
      ...
      :meth:`SessionsChat.send` raises ``GoalrailError`` on it.
...
def _resolve_resume_target(
    *,
    ...
    ``goalrail/inner/cli.py:3082-3084`` ("No saved sessions
    to continue.").
...
def _assert_resume_conversation_exists(
    *,
    ...
    async def _lookup() -> None:
        async with GoalrailClient(base_url=base_url, headers=headers) as client:
...
def _run_picker(
    *,
    ...
    :raises click.ClickException: When the agent has no prior
        ...
        out of.
    """
    from goalrail.repl._resume_picker import pick_conversation_from_sdk

    async def _lookup() -> str | None:
        async with GoalrailClient(base_url=base_url, headers=headers) as client:
            # Multipart ``goalrail run <yaml>`` uploads now create a
...
async def _resolve_latest_conversation_id_async(
    *,
    ...
    ``GoalrailClient`` connected to a real URL — the path
    used in production by ``_chat_local``.

    :param client: A connected :class:`GoalrailClient`.
...
def _materialize_override_bundle(source: Path, overrides: ChatOverrides) -> Path:
    ...
    goalrail validator rejects that shape, and the legacy
    argparse CLI used to paper over it by injecting
    :data:`_DEFAULT_AD_HOC_MODEL`. This preserves that behavior so
    ``goalrail run examples/hello_world.yaml`` (minimal spec) still
    launches cleanly.
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="goalrail-override-"))
    try:
        if source.is_file():
...
def _cleanup_materialized_override_bundle(materialized: Path) -> None:
    ...
    :param materialized: Path returned by
        ...
        ``Path("/tmp/goalrail-override-abc/agent.yaml")``.
    :returns: None.
    """
...
def _load_yaml_if_single_file(source: Path) -> _YamlMapping | None:
    ...
    Directories (goalrail-style with ``config.yaml``) are handled
...
def _effective_openai_auth_harness(
    ...
    if model is None:
        return None
    from goalrail.llms.routing import infer_harness_from_model
...
def _apply_overrides_to_raw(raw: _YamlMapping, overrides: ChatOverrides) -> None:
    ...
    # ``_DEFAULT_AD_HOC_MODEL`` directly so ``GOALRAIL_MODEL=foo``
    # is honored on the ``goalrail/cli.py`` → ``run_chat`` direct
    # path. Without this, that env var was silently dropped on the
    # Goalrail path invoked through the ``goalrail`` console
    # script (see ``designs/RUN_GOALRAIL_REPL_PARITY.md``).
...
def _apply_harness_override_to_executor(
    ...
    Single-file goalrail YAMLs (``name`` + ``prompt``, no
    ...
    silent no-op, which made ``goalrail run examples/polly
    ...
    :raises click.ClickException: If a ``spec_version`` bundle
        declares a non-goalrail ``executor.type`` — those executors
        have no ``config.harness``, so the override cannot apply.
    """
    canonical = canonicalize_harness(harness) or harness
    # "spec_version" is the format discriminator (see is_goalrail_yaml).
    if "spec_version" not in raw:
        executor_block["harness"] = canonical
        return
    etype = str(executor_block.get("type", GOALRAIL_EXECUTOR_TYPE))
    if etype != GOALRAIL_EXECUTOR_TYPE:
        raise click.ClickException(
            f"--harness only applies to specs with executor.type "
            f"{GOALRAIL_EXECUTOR_TYPE!r}; this spec declares executor.type {etype!r}."
        )
    config = executor_block.get("config")
...
def _validate_agent_spec(agent_path: Path) -> None:
    ...
    ``_start_local_server``). Both ``GoalrailError`` (parse/
...
def _extract_agent_name(agent_path: Path) -> str:
    ...
    Accepts both agent-image directories and standalone goalrail
    YAML files. The heavy lifting lives in
    :func:`goalrail.spec.load`, which dispatches on the source
    ...
    except (GoalrailError, FileNotFoundError):
...
def _canonicalize_local_agent_path(agent_path: Path) -> Path:
    ...
    :param agent_path: Existing local path supplied to ``goalrail run``,
...
def _goalrail_persistent_dir() -> Path:
    """
    Resolve the persistent goalrail data directory.

    Honors ``GOALRAIL_DATA_DIR`` (the data-isolation knob a worktree sets to avoid sharing
    ``~/.goalrail/chat.db``), else lives under the effective runtime data
    home. Created on first access; subsequent calls are idempotent.

    Must resolve identically to
    :func:`goalrail.host.local_server._local_data_dir` — the local server
    writes its DB under that dir while ``goalrail run`` reads the resume DB
    from here, so a divergence would silently lose history. ``GOALRAIL_CONFIG_HOME``
...
def _start_local_server(
    ...
    *,
    ...
    per-run directory under ``~/.goalrail/logs`` so concurrent Goalrail sessions don't
    ...
    ``~/.goalrail/{chat.db,artifacts/}`` by default —
    persistent across runs so ``--continue`` / ``--resume``
    can resume prior conversations
    (designs/RUN_GOALRAIL_SESSION_RESUMPTION.md). Pass
    ...
    :param ephemeral: When ``True``, place the SQLite DB and
        ...
        ``~/.goalrail`` location. Used for ``--no-session``
    ...
    """
    log_dir = _goalrail_log_dir() / "server"
    log_dir.mkdir(parents=True, exist_ok=True)
    ...
    if ephemeral:
        ...
        data_dir = _goalrail_persistent_dir()
    ...
    from goalrail.cli import _start_cli_runner_process
    ...
    # we spawn below (read by server() via GOALRAIL_RUNNER_TUNNEL_TOKEN).
    #
    # Accounts opt-in: when the parent shell selects accounts mode
    # (GOALRAIL_AUTH_PROVIDER=accounts), inject the per-spawn
    # base URL + cookie secret so the spawned server's
    # AccountsConfig.from_env() can satisfy its required-fields check.
    # Mirrors the same logic in cli.py:_ensure_local_goalrail_server; see
    # the comment block there for the full UX explanation. (Two
    # spawn paths exist because `goalrail run --server ""` goes
    # through _ensure_local_goalrail_server while `goalrail run` with
    # an agent spec and no --server flag goes through here.)
    child_env = {
        **os.environ,
        "GOALRAIL_RUNNER_TUNNEL_TOKEN": binding_token,
        # Single-user loopback runtime — see ensure_local_goalrail_server for why
        ...
        "GOALRAIL_LOCAL_SINGLE_USER": "1",
    }
    # Mirror create_auth_provider's resolution so this spawn path agrees
    # with the daemon path (host/local_server.py::ensure_local_goalrail_server):
    # header is the env-unset default; GOALRAIL_AUTH_ENABLED=1 opts into
    # accounts (or oidc when GOALRAIL_OIDC_* is set). In header/oidc mode we
    ...
    if _accounts_mode:
        if "GOALRAIL_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            child_env["GOALRAIL_ACCOUNTS_COOKIE_SECRET"] = secrets.token_hex(32)
...
def _stop_local_server(server: LocalServer) -> None:
    ...
    """
    from goalrail.cli import _stop_cli_runner_process
...
def _spec_used_families(agent_yaml: Path | None) -> list[str]:
    ...
    if agent_yaml.is_dir():
        ...
        return []
    try:
        from goalrail.onboarding.provider_config import PI_SURFACE, harness_family
        from goalrail.spec import parse
...
def _run_repl(
    ...
    *,
    ...
    :param auto_open_conversation: When ``True``, open the
        browser conversation URL when the session id becomes known.
    """
    from goalrail.repl import run_repl
    from goalrail.repl._session_log import DEFAULT_LOG_DIR
    from goalrail.repl._tmux_pane import register_pane

    log_dir = DEFAULT_LOG_DIR if log else None

    # Mark this tmux pane as an goalrail context source and wrap
    ...
    async def _main() -> None:
        ...
        from goalrail.cli_diagnostics import install_asyncio_exception_handler
...
def _run_one_shot(
    *,
    ...
    try:
        ...
        # raises as an GoalrailError. Surface its message as a clean
...
def _load_tool_handler(name: str) -> ToolHandler:
    ...
    """
    try:
        from goalrail.client_tools import get_tool_set
    ...
    if hasattr(tool_set, "_TOOL_FNS"):
        ...
        from goalrail_client.tools import build_tool_handler
