...
class FunctionPolicy(Policy):
    ...
    def reset_turn(self) -> None:
        ...
        Stateful policy callables (notably the legacy spec
        ...
        no-op. Modern goalrail-native callables that need
...
def resolve_function_policy(spec: FunctionPolicySpec) -> FunctionPolicy:
    ...
    # function.py → _goalrail_legacy_shim → policies.types →
...
def _coerce_to_policy_result(raw: Any, *, spec_name: str) -> PolicyResult:
    ...
    - A foreign PolicyResult-shaped object (any object with at
      least ``.action``, e.g. ``goalrail.policies.PolicyResult``
      returned by callables ported from the goalrail examples).
    ...
    if hasattr(raw, "action"):
        ...
        # ``goalrail.policies.PolicyResult`` that happen to
