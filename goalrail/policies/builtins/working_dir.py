...
YAML usage::
    ...
    policies:
      no_dir_switch:
        type: function
        function:
          path: goalrail.policies.builtins.working_dir.block_working_dir_changes
          arguments:
...
from goalrail.policies.builtins._shell import (
    MAX_SHELL_NESTING,
...
# Includes both the Goalrail built-in OS shell and Claude Code / Codex
...
def block_working_dir_changes(
    *,
    ...
    :param shell_tools: Names of the shell tools whose ``command`` argument is
        ...
        (Goalrail built-in + Claude/Codex native). Commands run through
...
POLICY_REGISTRY: list[dict[str, Any]] = [  # type: ignore[explicit-any]
    {
        "handler": "goalrail.policies.builtins.working_dir.block_working_dir_changes",
        "kind": "factory",
