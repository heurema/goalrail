...
# PostToolUse hook contract (see ``goalrail.native_policy_hook``).
# These bypass Goalrail' ``sys_os_*`` MCP tools and execute directly
...
# (see ``goalrail.inner.cursor_policy_hook``). Cursor uses ``Shell``
...
# extension hook (see ``goalrail.inner.pi_executor._gate_native_tool``).
...
# as the Goalrail ``sys_os_*`` tools (``path`` / ``command``), so the
...
# (see ``goalrail.inner.hermes_policy_hook``). Hermes uses its own naming
...
def ask_on_os_tools(event: PolicyEvent) -> PolicyResponse:
    ...
    - **Goalrail built-in OS tools** (``sys_os_read``,
    ...
    if tool in _all_os_tools:
        ...
        if tool in ("sys_os_shell", "Bash", "bash", "Shell", "terminal", "developer__shell"):
            ...
            # Goalrail tools use ``path``; Claude native tools use ``file_path``.
            preview = (
...
# Goalrail runner tools that load skills in non-native (SDK) harnesses.
...
def block_skills(blocked: list[str]) -> PolicyCallable:
    ...
    2. **Native ``Skill`` tool** — Claude Code's built-in ``Skill`` tool,
       ...
       the Goalrail server's ``/policies/evaluate`` endpoint. This is how
    ...
    3. **Slash commands** — ``/skill-name`` commands submitted by the
       user (or UI). The Goalrail server evaluates these at the ``request``
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        if event_type == "tool_call":
            ...
            # Path 1: Goalrail runner tools (load_skill / read_skill_file).
            ...
            # Fired via PreToolUse hook → Goalrail /policies/evaluate.
        ...
        # The Goalrail server converts ``/skill-name args`` into a synthetic
...
def deny_pii_in_llm_request(
    ...
    def evaluate(event: PolicyEvent) -> PolicyResponse:
        ...
        - ``request``: user message text, enforced universally on
          the Goalrail server for every harness (including supervisor,
          native).
...
POLICY_REGISTRY: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "covers Goalrail sys_os_* tools, Claude Code native tools "
    ...
    {
        "handler": "goalrail.policies.builtins.safety.enforce_sandbox",
        "kind": "factory",
    ...
    {
        "handler": "goalrail.policies.builtins.safety.deny_pii_in_llm_request",
        "kind": "factory",
