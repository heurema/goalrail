"""Rich-based REPL for goalrail — built on the UI SDK framework.
...
# ``from goalrail_ui_sdk import FormattedItem``.
from goalrail_ui_sdk.terminal._completer import FileMentionCompleter
from goalrail_ui_sdk.terminal._formatter import FormattedItem
...
SlashCommandHandler = Callable[
    [str, Session, GoalrailClient, TerminalHost, RichBlockFormatter],
    Awaitable[None],
]
...
# (tests/e2e/goalrail/_pexpect_harness.py). /quit discoverability is
...
def _load_startup_theme() -> TerminalTheme:
    ...
    On first launch (no persisted theme in ``~/.goalrail/config.yaml``),
    ...
    from goalrail.repl._theme_picker import startup_theme_picker
...
def _display_cwd() -> str:
    ...
    :returns: The cwd with ``$HOME`` collapsed to ``~`` (e.g.
        ``"~/goalrail"``), or the absolute path when it is not
...
def _header_glyph(kind: str) -> str:
    ...
    keeps its :func:`kind_glyph`. CLI surfaces (``goalrail setup``, the
    ...
    from goalrail.onboarding.provider_config import SUBSCRIPTION_KIND
...
def _render_startup_banner_ansi(
    ...
    *,
    ...
    """
    from goalrail.inner.banner import BannerLine, startup_banner_strings
...
async def _fetch_server_version(client: GoalrailClient) -> str | None:
    ...
    Routed through the REPL's already-connected :class:`GoalrailClient` so
...
def _is_remote_server_url(url: str | None) -> bool:
    ...
    A local ``goalrail run`` spawns its own Goalrail server on
...
class _ApprovalState:
    ...
    def remember_always(self, policy_name: str, phase: str) -> None:
        ...
        cache is NEVER persisted to disk; closing ``goalrail chat``
...
def _build_elicitation_content_from_schema(
    ...
    See :func:`goalrail.tools._elicitation_schema.build_accept_content_from_schema`
    ...
    """
    from goalrail.tools._elicitation_schema import build_accept_content_from_schema
...
def _elicitation_resolve_session_id(sdk_event: object, fallback_session_id: str) -> str:
    ...
    :param sdk_event: The translated
        :class:`goalrail_client._events.ElicitationRequest`; its
...
def _server_event_to_sdk_event(event: object) -> object | None:
    ...
    :mod:`goalrail_client._events`. Returns ``None`` for variants
    ...
    from goalrail_client._types import Response as SDKResponse
...
class _SessionsChatReplAdapter:
    ...
    def __init__(
        ...
        :param client: The :class:`GoalrailClient` used to
        ...
        :param attach_only: When ``True``, run as a pure co-drive client:
            ...
            existing host-bound runner). Used by ``goalrail attach``.
    ...
    async def _runner_recover_watch(self) -> None:
        ...
        :returns: None.
        """
        _dbg = bool(os.environ.get("GOALRAIL_SESSIONS_ADAPTER_DEBUG"))
        while True:
            try:
    ...
    async def _ensure_session(self) -> str:
        ...
        Set ``GOALRAIL_SESSIONS_ADAPTER_DEBUG=1`` to trace
            ...
            if self._session_id is None:
                if self._session_bundle is None:
                    raise RuntimeError(
                        ...
                        "Start the REPL from `goalrail run <agent.yaml>` so the CLI can "
    ...
    def _emit_runner_recovery_error_once(self, exc: Exception) -> None:
        ...
        :param exc: Failure raised while relaunching or rebinding a
            runner, e.g. an SDK ``GoalrailError`` from
    ...
    def _is_terminal_runner_recovery_error(self, exc: Exception) -> bool:
        ...
        :returns: ``True`` for typed server runner-state errors,
            ...
            retrying.
        """
        if not isinstance(exc, GoalrailError):
            return False
        code = exc.code or ""
    ...
    async def _stream_pump(self) -> None:
        ...
        """
        from goalrail.server.schemas import SessionStatusEvent as _StatusEv
    ...
    async def send(
        ...
        *,
        ...
        try:
            ...
            # _on_event.
            from goalrail_client._events import ResponseCompleted
            from goalrail_client._types import Response as SDKResponse
    ...
    async def send_skill_slash_command(
        ...
        The Goalrail server persists a visible ``slash_command`` item and
    ...
    async def _handle_elicitation(
        ...
        from goalrail_client._tool_handler import ElicitationRequestCtx
...
class _TurnProseTracker:
    ...
    (``_flush_relay_text`` in ``goalrail/server/routes/sessions.py``).
...
def _render_failed_status_error(
    ...
    """
    from goalrail_client import ErrorBlock
...
async def run_repl(
    ...
    *,
    ...
    :param client: Connected GoalrailClient.
    ...
    :param resume_conversation_id: When set, the REPL opens
        ...
        ``--resume <id>`` (see designs/RUN_GOALRAIL_SESSION_RESUMPTION.md).
    ...
    :param log_dir: When set, write a JSON dump of the active
        ...
        the CLI ``--log`` flag (and the ``~/.goalrail/logs/``
        default location); see ``goalrail.repl._session_log`` for
        the schema. The dump runs in the SAME ``async with
        GoalrailClient(...)`` scope as the REPL itself, so the
    ...
    :param server_url: Base URL of the Goalrail server the REPL is
    ...
    # behavior (goalrail/inner/cli.py:2979 + :2984): when a user
    ...
    from goalrail.repl._event_tape import EventTape, PipelineCounters, TapeEntry
        ...
        * ``ClientTaskCancelEvent`` -> cancel local tool task
        ...
        if isinstance(event, _StatusEv):
            ...
            if event.status == "running":
                from goalrail_client import BlockContext, ResponseStartBlock
                ...
                from goalrail_client import TextDone
        ...
        from goalrail.server.schemas import ElicitationResolvedEvent
        ...
        if isinstance(sdk_ev, _TD):
            from goalrail_client import TextChunk
        ...
        if isinstance(sdk_ev, _RS):
            from goalrail_client import BlockContext, ReasoningStartBlock
        ...
        if isinstance(sdk_ev, _RD | _RSD):
            from goalrail_client import BlockContext
            from goalrail_client import ReasoningChunk as _RC
        ...
        from goalrail_client._events import ResponseCompleted as _Completed
        ...
        from goalrail_client._events import ErrorEvent as _ErrorEvent

        if isinstance(sdk_ev, _ErrorEvent):
            from goalrail_client import BlockContext, ErrorBlock
        ...
        from goalrail_client._events import ResponseFailed as _Failed
    ...
    def _maybe_log_tape_entry(entry: TapeEntry) -> None:
        ...
        if _event_log_fh is not None:
            from goalrail.repl._event_tape import log_entry_jsonl
    ...
    async def on_input(text: str, attachments: list[PendingAttachment] | None = None) -> None:
        ...
        try:
            ...
            # was the exact user-reported bug where ``goalrail chat`` would
    ...
    # usage totals) is goalrail-specific. The SDK's
    ...
    # ``goalrail/inner/cli.py:1791-1797``.
    from goalrail_ui_sdk import OverlayAction
    ...
    if debug_events and _event_tape is not None:
        from goalrail.repl._event_tape import build_tape_detail, build_tape_targets
        ...
        async def _tape_targets() -> list[OverlayTarget]:
            ...
            """
            from goalrail.repl._event_tape import _OverlayTargetLike
    ...
    async with host:
        ...
        if debug_events:
            ...
            from goalrail.repl._event_tape import open_event_log
        ...
        # servers; None on any failure simply omits the row. Skipped entirely
        # when there's no header — the minimal banner never renders the
        # version, so there's no point paying even bounded probe latency.
        ...
        from goalrail_ui_sdk import StreamingText
    ...
    # Top-level ``goalrail resume`` only dispatches claude-native today;
...
async def _maybe_write_session_log(
    ...
    :param client: Connected GoalrailClient — REUSED, not opened
...
async def _cmd_help(
    ...
    client: GoalrailClient,  # noqa: ARG001
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    from rich.text import Text
...
async def _cmd_theme(
    ...
    client: GoalrailClient,  # noqa: ARG001 — dispatch-contract params
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
    ...
    """
    from goalrail_ui_sdk.terminal._theme import DARK_THEME, LIGHT_THEME
    from rich.text import Text

    from goalrail.repl._theme_picker import _build_preview, build_theme_confirmation
...
def _model_readout_harness(active_model: str | None) -> str:
    ...
    :func:`goalrail.llms.routing.infer_harness_from_model`, falling back
...
def _build_model_readout_lines(
    ...
    :param config: The parsed effective config mapping (``providers:``
        block), e.g. from
        :func:`goalrail.onboarding.provider_config.load_config`.
...
def _match_configured_provider(config: dict[str, object], token: str) -> str | None:
    ...
    """
    from goalrail.onboarding.configure_models import provider_display_name
...
def _model_validation_warning(model: str) -> str | None:
    ...
    (:func:`goalrail.onboarding.providers.get_chat_models`). An unknown
    ...
    from goalrail.onboarding.providers import get_chat_models
...
async def _cmd_model(
    ...
    if not value:
        from goalrail.onboarding.detected import effective_config_with_detected
        from goalrail.onboarding.provider_config import load_config
    ...
    from goalrail.onboarding.configure_models import kind_glyph, provider_display_name
    from goalrail.onboarding.detected import effective_config_with_detected
    from goalrail.onboarding.provider_config import (
...
async def _attach_to_conversation(
    ...
    *,
    ...
    - ``run_repl(resume_conversation_id=...)`` on startup
      ...
      designs/RUN_GOALRAIL_SESSION_RESUMPTION.md) — passes
    ...
    :param client: The :class:`GoalrailClient` for fetching
        items.
    ...
    if last_response_id is None:
        ...
        # `goalrail run` the daemon hands the REPL a freshly-created session
    ...
    if cw:
        ...
        if last_total is not None:
            ...
            from goalrail.runtime.compaction import count_tokens
...
# Mirrors _DEFAULT_TRIGGER_THRESHOLD from goalrail.runtime.compaction.
...
async def _refresh_session_metadata(
    ...
    :param client: Goalrail HTTP client used to fetch the snapshot.
...
async def _update_context_ring_estimate(
    ...
    :param client: Goalrail HTTP client used to query items.
...
def _build_github_issue_url(
    ...
    body_parts: list[str] = [
        ...
        *info_lines,
        ...
        "     --debug-events for a JSONL event log in ~/.goalrail/debug/ -->",
    ]

    body = "\n".join(body_parts)
    base = "https://github.com/heurema/goalrail/issues/new"
    return (
        f"{base}"
...
async def _cmd_logs(
    ...
    from goalrail.cli_diagnostics import current_cli_log_path
    from goalrail.repl._session_log import write_logs_zip, write_session_log
...
async def _cmd_report(
    arg: str,
    session: Session,
    client: GoalrailClient,  # noqa: ARG001 — dispatch-contract param
    host: TerminalHost,
    fmt: RichBlockFormatter,
) -> None:
...
async def _collect_terminals_for_conversations(
    ...
    *,
    ...
    :param client: The goalrail HTTP client.
...
class _TerminalInfo:
    ...
    :param target: Tmux target. Always ``"main"`` per the
        ...
        (``goalrail/inner/terminal.py:167``); kept on the
...
def _parse_terminal_tool_output(raw: object) -> dict[str, object] | None:
    ...
    string (default executor, goalrail builtins) and
...
async def _open_terminal_in_tmux(
    ...
    *,
    ...
    ``goalrail/inner/cli.py::_open_current_terminal_window``
    so users with muscle memory from the legacy CLI see the
    same behavior under Goalrail mode.
    ...
    :param client: Goalrail HTTP client — used to re-walk the
...
def _terminal_attach_command(info: _TerminalInfo) -> str:
    ...
    :meth:`goalrail.inner.cli._terminal_attach_command`
...
async def _build_terminal_overview(
    ...
    *,
    ...
    :meth:`goalrail.inner.cli._render_overview_terminal_text`
    output (``cli.py:2232``):
...
def _tmux_session_alive(socket: str, target: str) -> bool:
    ...
    :param socket: Tmux socket path the agent's
        ...
        ``"/tmp/goalrail-terminal-xyz/tmux.sock"``.
    :param target: Tmux session/target name. Always
        ``"main"`` per
        :class:`goalrail.inner.terminal.TerminalInstance.tmux_target`.
...
def _parse_sub_agent_handle(raw: str) -> dict[str, object] | None:
    ...
    Native goalrail builtins (``sys_session_send`` /
...
async def _build_debug_overview(
    ...
    *,
    ...
    The overview intentionally mirrors the ``goalrail run``
    ...
    2. **Event stream** — all items from the conversation,
       ...
       ``[N] type=...`` shape goalrail uses. Responses API
       items map onto the goalrail event vocabulary as
    ...
    :param client: The goalrail client used for the items fetch.
    ...
    :param cli_log_path: Optional path to the always-on CLI
        diagnostics log (``~/.goalrail/logs/cli-*.log``).
    ...
    # pair — matching the goalrail event view. When an output
...
def _render_overview_message_event(
    ...
    """
    Render a ``message`` item as an goalrail event.

    Splits on role so the header reads ``user_message`` or
    ``assistant_message`` (matching goalrail' event vocabulary)
...
def _coerce_arguments_dict(raw: object) -> dict[str, object]:
    ...
    The goalrail API surfaces tool-call arguments as a JSON
...
def _render_message_history_item(
    ...
    if role == "user":
        ...
        return
    if role == "assistant":
        # Skip empty assistant messages. The goalrail workflow
