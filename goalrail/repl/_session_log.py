"""
JSON dump of an goalrail conversation — the Goalrail mode port of the
legacy ``--log`` feature (see designs/RUN_GOALRAIL_REPL_PARITY.md).

The legacy non-AP path wrote a session-shaped JSON to
``~/.goalrail/logs/`` at REPL exit (`_write_session_log` /
``_session_log_dict`` in ``goalrail/inner/cli.py``). That format
was tied to the in-memory :class:`Session` machinery — goalrail
...
``goalrail/repl/_repl.py``), so spawn handles are recognized
...
from goalrail_ui_sdk import state_dir
...
_LOG_FORMAT = "goalrail-conversation"

# Default log directory. Derives from the shared ``state_dir()``
# (``~/.goalrail``) so the root is defined in one place. Mirrors
# the legacy non-AP path's ``~/.goalrail/logs/`` (see
# ``_default_session_log_path`` in ``goalrail/inner/cli.py``) so
...
def default_log_zip_path(
    ...
    *,
    ...
    Format: ``{output_dir}/goalrail-logs-{session_slug}-{timestamp}.zip``
    when a session id is available, otherwise
    ``{output_dir}/goalrail-logs-{timestamp}.zip``. ``None`` uses
    :data:`DEFAULT_LOG_ZIP_DIR`, i.e. ``~/.goalrail/logs``.
    ...
    if slug is not None:
        return base / f"goalrail-logs-{slug}-{timestamp}.zip"
    return base / f"goalrail-logs-{timestamp}.zip"
...
def collect_log_files(log_paths: list[Path]) -> list[tuple[Path, str]]:
    ...
    This helper does **not** walk ``~/.goalrail/logs`` or
    ``~/.goalrail/debug`` wholesale; doing so would include unrelated
    sessions.
...
def default_log_path(conversation_id: str, log_dir: Path | None = None) -> Path:
    ...
    treatment in ``goalrail/inner/cli.py::_default_session_log_path``.
...
async def write_session_log(
    ...
    *,
    ...
    in ``goalrail/repl/_repl.py`` — same parser the Ctrl+O debug
    ...
    :param client: A connected :class:`GoalrailClient`. The
    ...
    :param agent_name: Agent name to include in the dump (for
        ...
        e.g. ``"resume_test"``.
    :param log_dir: Override the default location. ``None`` uses
        :data:`DEFAULT_LOG_DIR` (``~/.goalrail/logs/``).
...
def _extract_child_conversation_ids(items: list[dict[str, object]]) -> list[str]:
    ...
    from goalrail.repl._repl import _parse_sub_agent_handle
...
def write_session_log_from_store(
    ...
    *,
    ...
    :class:`goalrail.stores.conversation_store.ConversationStore`
    directly instead of the SDK.

    Used by the one-shot ``goalrail run <yaml> -p "…" --log`` path
    where the in-process ASGI app doesn't have a connected
    :class:`GoalrailClient` — the run goes through raw httpx +
...
def _fetch_all_items_sync(conv_store: Any, conversation_id: str) -> list[dict[str, object]]:
    ...
    self-contained — a reader doesn't need any of Goalrail'
...
async def _fetch_all_items_via_sessions(
    ...
    :param client: The connected :class:`GoalrailClient`.
