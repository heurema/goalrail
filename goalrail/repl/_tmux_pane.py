...
1. Marks the pane with custom options so other tooling can identify
   it as an goalrail pane and recover the launch args.
2. Discovers the user's existing prefix-table ``split-window`` /
   ...
   - When the focused pane has the ``@goalrail-conv-id`` option set
     (i.e. it's an goalrail pane), runs ``goalrail pane-split``
...
non-goalrail panes (the else branch is the user's untouched
command), so the global mutation of the ``prefix`` table is
behaviorally invisible everywhere except inside an goalrail pane.
...
# branch is the user's exact original command, so non-goalrail
# panes keep working.
PANE_INTEGRATION_ENABLED = False

# Pane-option keys we set on the goalrail pane. Each is read by
# ``goalrail pane-picker`` to reconstruct the launch context for
# the new pane.
OPT_CONV_ID = "@goalrail-conv-id"
OPT_AGENT_NAME = "@goalrail-agent-name"
OPT_AGENT_YAML = "@goalrail-agent-yaml"
OPT_LAUNCH_ARGV = "@goalrail-launch-argv"
OPT_SERVER_URL = "@goalrail-server-url"

# Sentinel value for ``OPT_CONV_ID`` before the first conversation
# id is known. The wrapper's ``#{?#{@goalrail-conv-id},...}``
# truthiness check fires for any non-empty string, so this
# preserves "yes, this is an goalrail pane" even when there's no
# real conv id yet.
...
class SplitBinding:
    ...
    :param direction: One of ``'v'`` (vertical split), ``'h'``
        ...
        value passed to ``goalrail pane-split``.
    :param original_command: The full command string from the
        ...
        wrapper's else branch so non-goalrail panes get
...
def _resolve_goalrail_argv() -> list[str]:
    """
    Resolve a callable argv prefix for invoking the running
    goalrail installation from a context that doesn't share our
    ...
    1. ``sys.argv[0]`` is path-shaped (contains ``/``) — abspath
       it. Covers ``./.venv/bin/goalrail …`` and absolute-path
       invocations.
    2. ``sys.argv[0]`` is a bare name like ``"goalrail"`` — try
    ...
    3. Fallback — ``[sys.executable, "-m", "goalrail.cli"]``.
       Always works because if Python is running this code, the
       ``goalrail.cli`` module is importable. Trades aesthetics
       (the binding shows ``python -m goalrail.cli``) for
    ...
    :returns: A list of one or two argv elements. Length-1 means
        ...
        ``[python, "-m", "goalrail.cli"]``. The caller appends
        the subcommand and its args (e.g. ``["pane-split", "-v",
        ...]``) to this prefix.
    """
    argv0 = sys.argv[0] if sys.argv else ""
    # When the process was launched via ``python -m goalrail.cli``
    ...
    if argv0.endswith(".py"):
        return [sys.executable, "-m", "goalrail.cli"]
    ...
    # ``goalrail.cli`` module is importable since we're running
...
# match is the launcher prefix (the goalrail binary, ``python
# -m goalrail.cli``, etc.) and gets stripped during
...
def _user_args_after_launcher(launch_argv: list[str]) -> list[str]:
    ...
    - ``[<goalrail-binary>, run, …]`` (PATH-resolved console script)
    - ``[<python>, -m, goalrail.cli, run, …]`` (python-m fallback)
    ...
    :param launch_argv: ``sys.argv`` (or stored ``launch_argv``)
        of the running ``goalrail`` invocation.
...
def _unset_pane_options(pane_id: str) -> None:
    """
    Remove every ``@goalrail-*`` pane option this module sets.
    ...
    ``if-shell -F '#{?#{@goalrail-conv-id},...}'`` wrapper falls
...
def _classify(cmd_tokens: list[str]) -> str | None:
    ...
    routing in goalrail panes. Acceptable v1 limitation.

    :param cmd_tokens: The command portion of a bind-key line,
        e.g. ``['split-window', '-h', '-c', '#{pane_current_path}']``.
    :returns: Direction code consumed by ``goalrail pane-split``,
        or ``None``.
    """
    if not cmd_tokens:
        return None
...
_WRAPPER_MARKER_FORMAT = "#{?#{@goalrail-conv-id},1,0}"


def _unwrap_existing_wrapper(cmd_tokens: list[str]) -> list[str] | None:
    """
    Peel off an existing goalrail wrapper to recover the user's
...
def _discover_split_bindings() -> list[SplitBinding]:
    ...
    an unfamiliar flag set, are silently skipped. Non-goalrail
    panes still see those bindings unchanged (they're never
    rewritten); goalrail panes just don't get chooser routing
    on those keys.
    """
...
def _wrap_binding(binding: SplitBinding, goalrail_argv: list[str]) -> None:
    """
    Replace one of the user's prefix-table bindings with an
    ``if-shell -F`` wrapper.

    True branch (focused pane is an goalrail — the
    ``@goalrail-conv-id`` option is set and non-empty): run
    the resolved ``goalrail pane-split`` invocation via
    ...
    when the goalrail branch fires.

    :param binding: One of the user's prefix-table split-window /
        new-window bindings.
    :param goalrail_argv: Argv prefix for invoking the running
        goalrail installation, as returned by
        :func:`_resolve_goalrail_argv`. Length 1 (e.g.
        ``["/venv/bin/goalrail"]``) for a direct binary
        invocation, or length 3 (``[sys.executable, "-m",
        "goalrail.cli"]``) for the python-m fallback. Either
...
def register_pane(
    *,
    ...
    """
    Mark the current tmux pane as an goalrail pane and wrap the
    ...
    if not PANE_INTEGRATION_ENABLED:
        ...
        # ``#{?#{@goalrail-conv-id},1,0}`` check returns 0, so the
    ...
    pane_id = _tmux_pane_id()
    if pane_id is None:
        _LOGGER.warning("$TMUX is set but $TMUX_PANE is not — skipping goalrail pane integration")
        return
    if not _tmux_version_ok():
        _LOGGER.warning(
            "tmux >= %s required for goalrail pane integration; skipping",
            ".".join(str(n) for n in _MIN_TMUX_VERSION),
        )
        return

    # Resolve a callable ``goalrail`` argv prefix. tmux's
    ...
    # ``goalrail`` lives — so a bare ``goalrail`` exits 127.
    # ``sys.argv[0]`` is the bare name when the user invoked us
    # via ``$PATH`` (e.g. typed ``goalrail`` after activating
    # a venv), so we can't just pass through ``launch_argv[0]``.
    # See :func:`_resolve_goalrail_argv` for the resolution
    # ladder (abspath → ``shutil.which`` → ``python -m`` fallback).
    goalrail_argv = _resolve_goalrail_argv()
    ...
    # ``goalrail_argv``. The user's subcommand and args
    # (``run agent.yaml …``) survive untouched.
    #
    # Idempotency under repeat calls: ``_user_args_after_launcher``
    # walks past leading launcher tokens (the goalrail binary,
    # ``python``, ``-m``, ``goalrail.cli`` — including doubled
    ...
    # ``-m goalrail.cli`` markers each time is exactly what this
    # repairs.
    user_args = _user_args_after_launcher(launch_argv)
    normalized_launch_argv = list(goalrail_argv) + user_args
    ...
    for binding in _discover_split_bindings():
        _wrap_binding(binding, goalrail_argv)
