...
The ``goalrail cursor`` wrapper launches the real ``cursor-agent`` TUI in a
runner-owned tmux pane, and :mod:`goalrail.cursor_native_bridge` injects web-UI
...
:mod:`goalrail.claude_native_forwarder` (which tails Claude Code's JSONL
transcript) and :mod:`goalrail.codex_native_forwarder` (which subscribes to the
...
``_publish_turn_status`` in :mod:`goalrail.runner.app`), exactly as for
...
from goalrail.cursor_native_bridge import FORK_HISTORY_CLOSE_TAG, FORK_HISTORY_OPEN_TAG
...
#: ``goalrail.db.db_models.SqlConversationItem``). cursor's content-address blob
...
# first user message, fenced in <goalrail_fork_history>…</goalrail_fork_history>
...
def _read_last_used_model(store_path: Path) -> str | None:
    ...
    catalog (:func:`goalrail.cursor_native.cursor_base_model_options`) and the
    ...
    :func:`goalrail.cursor_native_bridge.inject_model_command`).
...
async def _post_model_change_if_new(
    ...
    *,
    ...
    un-pinned session falls back to goalrail's default (e.g. "fable") in the Web
...
async def _post_external_compaction_status(
    ...
    :func:`goalrail.claude_native_forwarder._post_external_compaction_status`.
...
async def supervise_cursor_forwarder(
    *,
    ...
    Mirrors :func:`goalrail.claude_native_forwarder.supervise_forwarder`: the
