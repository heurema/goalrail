...
to ``goalrail.codex_native._materialize_codex_agent_spec``). The runner
...
This module also hosts the interactive local ``goalrail opencode`` CLI wrapper
(:func:`run_opencode_native`, the analog of ``goalrail codex`` / ``goalrail pi``):
...
from goalrail._wrapper_labels import OPENCODE_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
from goalrail.opencode_native_state import read_launch_state, write_launch_state
...
def run_opencode_native(  # pragma: no cover
    *,
    ...
    """
    Launch the OpenCode TUI in a Goalrail terminal (the ``goalrail opencode`` path).

    Mirrors ``goalrail codex`` / ``goalrail pi``: ensure a local daemon + runner,
    ...
    :param server: Resolved Goalrail server URL. ``None`` is an error (the CLI
        must resolve a backend first).
    :param session_id: Optional existing Goalrail conversation id to resume.
    ...
    if server is None:
        raise click.ClickException(
            "OpenCode requires a resolved Goalrail server URL. The CLI should resolve "
            "a backend before run_opencode_native."
        )
    with TemporaryDirectory(prefix="goalrail-opencode-native-") as tmpdir:
...
def _run_with_remote_server(  # pragma: no cover
    ...
    *,
    ...
    """Launch OpenCode on a Goalrail server via a daemon-spawned runner."""
...
def _resolve_session_id_for_resume(
    *,
    ...
    # e2e, not unit tests (it opens an GoalrailClient and an arrow-key picker).
    from goalrail_client import GoalrailClient  # pragma: no cover
