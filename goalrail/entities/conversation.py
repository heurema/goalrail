...
# goalrail/inner/claude_native_executor.py and
# goalrail/inner/codex_native_executor.py.
...
class Conversation:
    ...
    :param host_id: Host that launched (or should launch) the
        ...
        sessions started via ``goalrail run`` (the CLI
    ...
    :param harness_override: Per-session harness override for the
        ...
        process. Only valid for ``executor.type: goalrail`` agents;
        the create route validates against ``GOALRAIL_HARNESSES``.
    ...
    :param terminal_launch_args: Pass-through CLI args for a native
        ...
        harness binary; the command and all bridge / Goalrail / auth wiring
...
class RoutingDecisionData(BaseModel):
    ...
    advised turn (see :func:`goalrail.runner.cost_advisor`) and persisted
