...
from goalrail.opencode_native_bridge import update_active_message_id, update_last_event_id
from goalrail.opencode_native_client import OpenCodeClient, OpenCodeEvent
...
# back to Goalrail so the web model pill stays in sync (claude-native contract).
...
class OpenCodeNativeForwarder:
    ...
    :param policy_evaluator: Optional async policy resolver. Production
        ...
        ``goalrail.runner.app._build_opencode_policy_evaluator``) — the SAME
    ...
    def __init__(
        ...
        *,
        ...
        # Last model mirrored to Goalrail (provider/id), to dedupe switches.
    ...
    async def _on_part_updated(self, event: OpenCodeEvent) -> None:
        ...
        if part_type == "text":
            ...
            # transcript (goalrail persists no separate user item for these
    ...
    async def _on_compaction_started(self, event: OpenCodeEvent) -> None:
        ...
        server-side. The Goalrail server maps ``external_compaction_status``
    ...
    async def _on_model_switched(self, event: OpenCodeEvent) -> None:
        ...
        When the user switches model in the opencode TUI, reflect it to Goalrail
    ...
    async def _on_permission_asked(self, event: OpenCodeEvent) -> None:
        ...
        try:
            await self._opencode.reply_permission(
                request.request_id, reply_body(reply, message="goalrail-policy")
            )
    ...
    async def _resolve_permission(self, *, request_dict: Any) -> PolicyDecision:
        ...
        normalized = normalize_for_policy(
            request_dict,
            goalrail_session_id=self._session_id,
...
_HANDLERS: dict[str, Callable[[OpenCodeNativeForwarder, OpenCodeEvent], Awaitable[None]]] = {
    ...
    # Mirror a TUI model switch back to Goalrail (in-harness session-cmd sync).
