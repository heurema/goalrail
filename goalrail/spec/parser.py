...
def parse(root: Path, *, expand_env: bool = True) -> AgentSpec:
    ...
    :returns: A fully populated :class:`AgentSpec` (not yet
        validated).
    :raises GoalrailError: If ``config.yaml`` is not valid YAML,
    ...
    # ``goalrail/inner/datamodel.py::AgentDef.async_enabled`` — the
    # same YAML must produce the same tool surface under Goalrail mode and
    ...
    # ``goalrail/inner/datamodel.py::AgentDef.timers`` — agents
    ...
    # ``_GOALRAIL_SYSTEM_PROMPT_KEYS``); ``instructions:`` wins if both set.
...
def _parse_llm(
    ...
    *,
    ...
    :raises GoalrailError: If the ``llm:`` block is present but
...
def _parse_builtin_tools(
    ...
    :raises GoalrailError: If a dict entry is missing ``name``.
    """
...
def _parse_executor(
    ...
    *,
    ...
    # ``goalrail`` we ALSO mirror it into ``config["profile"]``
    # so the existing goalrail executor (which still reads from
    # ``config["profile"]``) keeps working until it is migrated.
    profile_raw = raw.get("profile")
    profile: str | None = None
    if profile_raw is not None:
        profile = str(profile_raw)
    if etype == "goalrail" and profile is not None and "profile" not in config:
...
def _parse_os_env(
    ...
    Native Goalrail YAML mirrors the goalrail YAML shape so users
    ...
    :class:`goalrail.inner.datamodel.OSEnvSpec` for the
    ...
    :raises GoalrailError: If *raw* is not a mapping, or
...
def _parse_terminals(
    ...
    Native Goalrail YAML mirrors the goalrail-compat ``terminals:`` shape — a
    ...
    ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §3 (``_parse_terminals`` parallel to
    ``_parse_os_env``).
    ...
    :raises GoalrailError: If ``terminals`` (or any entry) is not a mapping,
...
def _parse_os_env_sandbox(
    ...
    if raw_type is None:
        ...
        from goalrail.inner.sandbox import _default_sandbox_for_platform
    ...
    if macos_reason is not None:
        raise GoalrailError(macos_reason, code=ErrorCode.INVALID_INPUT)
...
def _parse_cwd_allow_hidden(raw: object) -> list[str] | None:
    ...
    :raises GoalrailError: If ``raw`` isn't a list, contains a
...
def _parse_cwd_hidden_scan_max_entries(raw: object) -> int:
    ...
    :returns: Validated positive integer, or the dataclass default
        when ``raw`` is ``None``.
    :raises GoalrailError: If ``raw`` is not an int or is not
        strictly positive.
    """
    if raw is None:
...
def _parse_cwd_hidden_scan_overflow(raw: object) -> str:
    ...
    :raises GoalrailError: If ``raw`` is not one of the supported
        modes.
    """
    if raw is None:
...
def _parse_egress_rules(raw: object) -> list[str] | None:
    ...
    :func:`~goalrail.inner.egress.rules.parse_rule` so syntax
    ...
    :raises GoalrailError: If the value isn't a list or any
        rule fails to parse.
    """
    if raw is None:
        return None
    ...
    if not raw:
        return None
    from goalrail.inner.egress.rules import parse_rule
...
def _format_validation_error(exc: ValidationError) -> str:
    ...
    :class:`GoalrailError` so the CLI / loader surface a single error
...
def _parse_credential_proxy(raw: object) -> CredentialProxySpec | None:
    ...
    is re-raised as an :class:`GoalrailError` so callers see one error
    type.
    ...
    :raises GoalrailError: If the value isn't a list, an entry fails
...
def _normalize_https_bearer(
    ...
    *,
    ...
    :raises GoalrailError: If a host fails DNS-safety validation.
    """
...
def _normalize_gh_basic(
    ...
    *,
    ...
    :raises GoalrailError: If an explicit host fails DNS-safety validation.
    """
...
def _parse_credential_proxy_host(raw: str, *, field_path: str) -> str:
    ...
    :raises GoalrailError: If the value is empty or the host contains
        characters outside the DNS grammar ``[A-Za-z0-9.-]`` (wildcards
        included — credentials bind to an exact host).
    """
    from goalrail.inner.egress.rules import is_dns_safe_host
...
def _parse_share_policy(raw: object) -> SharePolicy:
    ...
    :raises GoalrailError: When the value is neither ``None`` nor one
...
def _discover_skills(
    ...
    *,
    ...
    for skill_dir in entries:
        ...
        except (GoalrailError, yaml.YAMLError) as exc:
            if skipped is None:
                raise
            msg = f"{skill_md}: {exc}"
...
def _parse_skill(skill_md: Path) -> SkillSpec:
    ...
    :raises GoalrailError: If the file cannot be read, or the
    ...
    try:
        ...
        # OSError — funnel it through GoalrailError too so the lenient
...
def expand_env_vars(
    ...
    Raises :class:`GoalrailError` if any value still contains an
    ...
    :raises GoalrailError: If a value contains an unresolved
...
def check_unresolved_env_vars(key: str, value: str) -> None:
    ...
    :param value: The expanded value to check, e.g.
        ``"Bearer ${MISSING}"``.
    :raises GoalrailError: If *value* contains an unresolved
...
def _discover_mcp_servers(
    ...
    *,
    ...
    :returns: A sorted list of parsed :class:`MCPServerConfig`
        ...
        exist.
    :raises GoalrailError: If any YAML file is malformed or
...
def _parse_http_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "http"``.
    :raises GoalrailError: If ``url`` is missing or a stdio-only
        field was supplied.
    """
...
def _parse_stdio_mcp_server(
    ...
    *,
    ...
    :returns: A fully populated :class:`MCPServerConfig` with
        ``transport == "stdio"``.
    :raises GoalrailError: If ``command`` is missing, ``args`` is
...
def _reject_wrong_transport_keys(
    ...
    *,
    ...
    :raises GoalrailError: When any *disallowed* key is present
        in *raw*.
    """
...
def _parse_guardrails(
    ...
    *,
    ...
    :returns: A populated :class:`GuardrailsSpec`, or ``None``
        when *raw* is ``None``.
    :raises GoalrailError: On any spec-load validation
...
def _parse_guardrails_ask_timeout(raw: Any) -> int:
    ...
    :raises GoalrailError: On non-integer or non-positive
        values.
    """
    try:
        value = int(raw)
...
def _parse_label_defs(
    ...
    :raises GoalrailError: On malformed entries — empty
...
def _parse_single_label_def(key: str, entry: Any) -> LabelDef:
    ...
    :raises GoalrailError: On any malformed value.
    """
...
def _coerce_label_values(key: str, raw: Any) -> list[str] | None:
    ...
    :returns: Every element str-coerced; ``None`` when
        *raw* is ``None``.
    :raises GoalrailError: When *raw* is a non-list
        non-None value.
    """
    if raw is None:
        return None
...
def _coerce_label_monotonic(
    ...
    :returns: The validated direction, or ``None`` when
        *raw* is ``None``.
    :raises GoalrailError: On any other value.
    """
    if raw is None:
        return None
    if raw == "increasing":
...
def _validate_label_def_cross_fields(
    ...
    :raises GoalrailError: On any cross-field violation.
    """
...
def _parse_policies(
    ...
    *,
    ...
    :returns: List of policy specs, or ``None`` when *raw*
        is ``None``.
    :raises GoalrailError: On any malformed policy entry.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_policy_spec(
    ...
    *,
    ...
    :raises GoalrailError: On malformed data or unknown
        policy type.
    """
    ...
    if policy_type == "prompt":
        raise GoalrailError(
            ...
            f"'goalrail.policies.builtins.prompt.prompt_policy' instead.",
            code=ErrorCode.INVALID_INPUT,
        )
...
def _parse_function_policy(
    ...
    :raises GoalrailError: On missing ``function:`` field
    ...
    # is the original goalrail YAML convention.
...
def _parse_on(
    ...
    *,
    ...
    :raises GoalrailError: On empty list, unknown phase,
...
def _parse_on_entry(
    ...
    *,
    ...
    :raises GoalrailError: On non-string entry, empty
...
def _resolve_phase(
    ...
    *,
    ...
    :raises GoalrailError: When *phase_str* is not a
        valid phase.
    """
    try:
...
def _parse_condition(
    ...
    *,
    ...
    stored ``"0"``. The coercion matches goalrail parity
    for label values.
    ...
    :raises GoalrailError: On a non-dict value.
    """
    if raw is None:
        return None
    if not isinstance(raw, dict):
...
def _parse_action_list(
    ...
    *,
    ...
    :raises GoalrailError: On empty list or unknown
        action value.
    """
    if isinstance(raw, str):
        strings = [raw]
...
def _parse_writable_labels(
    ...
    *,
    ...
    :returns: List of allowed label keys, or ``None`` when
        *raw* is absent.
    :raises GoalrailError: When *raw* is not a list.
    """
    if raw is None:
        return None
    if not isinstance(raw, list):
...
def _parse_function_ref(
    ...
    *,
    ...
    :raises GoalrailError: On malformed shape — non-string
...
def parse_default_policies(
    ...
    *,
    ...
    :raises GoalrailError: On any malformed policy entry — unknown
