"""
Compatibility shim: legacy spec ``(content, phase)`` policy
callables → goalrail ``(ctx, context)`` convention.

Legacy spec function policies were written like::
...
callable runs under goalrail it silently short-circuits to
``"allow"`` on the very first phase check (``phase`` is a dict
there, not a string) and any policy written in the old style
becomes a no-op.

This module bridges the gap. The spec-to-Goalrail translator in
:mod:`goalrail.spec.goalrail` routes every
...
every call. Modern callables (goalrail-native ``(ctx)`` /
...
from goalrail.spec.types import Phase

# Parameter names that identify a legacy spec policy
# callable — the first two positional parameters must be exactly
# ``content`` then ``phase``. A callable whose first two params
# are ``(foo, bar)`` is treated as goalrail-native even though
...
# Maps Goalrail' :class:`Phase` enum to the string literal
...
def build(
    ...
    loading an goalrail-sourced function policy.

    Imports *target*, optionally unwraps a factory call with
    *factory_kwargs*, then decides whether the resolved callable
    is a legacy spec policy (parameter names
    ``(content, phase)``) or an goalrail-native one
    ...
    :param configured_phases: The YAML ``on:`` list (verbatim
        ...
        :class:`goalrail.inner.policies.FunctionPolicy`
        ...
        (:func:`goalrail.spec.goalrail._translate_function_policy_yaml`)
        always passes a list.
    :returns: A callable matching Goalrail'
...
def _coerce_legacy_return(result: Any) -> Any:
    ...
    by :func:`goalrail.policies.function._coerce_to_policy_result`.
...
def _wrap_legacy(
    ...
    *,
    ...
    wrapper when present, so the goalrail ``FunctionPolicy``
    can call it at turn boundaries — preserving the legacy
    goalrail per-turn-reset contract for stateful policies
    ...
    # Capture once at wrap time. The goalrail FunctionPolicy
...
def _v0_event_to_legacy_phase(event: dict[str, Any]) -> str:
    ...
    Goalrail' ``Phase`` enum. Provide a direct mapping here.
...
def _convert_args(
    ...
    *,
    ...
    ``(content, phase, context)`` from Goalrail'
    ...
    - Inner system: the goalrail :class:`FunctionPolicy` builds an
...
def _legacy_content(ctx: EvaluationContext) -> Any:
    ...
    :func:`goalrail.runtime.workflow._enforce_tool_call_policy`
    and siblings). The legacy spec shape matches those
    normalized shapes almost exactly — this helper bridges the
    per-phase differences:

    - ``INPUT`` / ``OUTPUT``: goalrail passes the message
      text directly; legacy spec expects the same. Pass
      through.
    - ``TOOL_CALL``: goalrail passes
      ``{"tool": name, "args": parsed_args_dict}``; the legacy
      spec expects the same. Pass through.
    - ``TOOL_RESULT``: goalrail passes the raw tool output
      string. The legacy spec passes the parsed result *dict*
      via
...
def _maybe_parse_json(value: Any) -> Any:
    ...
    goalrail-native's behavior: when an MCP text block is
...
def _legacy_context(
    ...
    *,
    ...
    The legacy spec passed ``{"labels": {...},
    "configured_phases": [...], "tool_name": "sleep"}`` — but
    ``tool_name`` is *only* added on ``TOOL_RESULT`` (see
    :meth:`goalrail.inner.session.Session._apply_tool_result_policy`,
    which builds ``context = {"tool_name": tool_name}`` for
    that phase). On ``TOOL_CALL``, Goalrail-native execution passes no
    ...
    :class:`goalrail.inner.policies.FunctionPolicy.evaluate`,
