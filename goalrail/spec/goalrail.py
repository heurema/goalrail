"""Bidirectional translator between goalrail ``AgentSpec`` and goalrail ``AgentDef``.

Two functions live here:

- :func:`agent_spec_to_agent_def` — forward direction, consumed by
  ``GoalrailExecutor`` at executor-construction time.
- :func:`agent_def_to_agent_spec` — reverse direction, consumed by
  :func:`goalrail.spec.load` when handed an goalrail YAML so
  ``goalrail chat foo.yaml`` works transparently.

The bidirectional invariant
``agent_spec_to_agent_def(agent_def_to_agent_spec(d)) == d`` must
hold for every representative goalrail YAML; the round-trip test
under ``tests/spec/test_goalrail_roundtrip.py`` enforces it.

Design invariants (see designs/GOALRAIL_INTEGRATION.md §1 and §2):
...
- Unsupported concepts **fail loud** with an :class:`GoalrailError`
  naming the specific field. Currently unsupported (each is an
  goalrail spec gap to fix, not a translation to paper over):
...
from goalrail.inner.datamodel import AgentDef, OSEnvSpec, TerminalEnvSpec
...
from goalrail.llms.routing import infer_harness_from_model as _infer_harness_from_model
...
# selects ``GoalrailExecutor``. Both directions of the translator
# treat this as the discriminator between native goalrail specs
# and goalrail-sourced ones.
GOALRAIL_EXECUTOR_TYPE = "goalrail"

# Value placed in :attr:`LocalToolInfo.language` for tools that were
# sourced from an goalrail YAML. Distinguishes them from native
# goalrail tools (``"python"`` / ``"typescript"``) which live as
# files on disk under ``tools/python/``.
GOALRAIL_TOOL_LANGUAGE = "goalrail-python-callable"

# Version stamped on AgentSpecs synthesized from goalrail YAMLs.
# Goalrail YAMLs do not declare ``spec_version`` (it's an
# goalrail concept), so the adapter writes ``1`` — the currently
# valid goalrail schema version (see goalrail/spec/validator.py).
...
# translation. We resolve eagerly because goalrail spawns
# each child as an independent task — no live parent session
# exists at runtime to walk.
_OS_ENV_INHERIT_SENTINEL = "inherit"

# Goalrail → goalrail mapping for the ``monotonic`` label
# schema field. Goalrail uses ``max`` / ``min`` / ``none``
# (datamodel.LabelSchemaRule.monotonic); goalrail uses
...
_GOALRAIL_TO_AP_MONOTONIC: dict[str, str] = {
    "max": "increasing",
    "min": "decreasing",
}

# Goalrail loader policy-type discriminators. Used to dispatch
...
def agent_spec_to_agent_def(spec: AgentSpec) -> AgentDef:
    """
    Translate an goalrail ``AgentSpec`` into an goalrail
    ``AgentDef`` suitable for
    :func:`goalrail.executor_factory.create_executor`.
    ...
    :param spec: The goalrail spec to translate. ``spec.llm``
        must be set (the goalrail harness needs a model name);
        ``spec.executor.type`` must be ``"goalrail"`` with
        ``executor.config.harness`` populated.
    :returns: A populated :class:`goalrail.datamodel.AgentDef`
        ...
        rejected upstream.
    :raises GoalrailError: If the spec uses an unsupported
    ...
    if spec.executor.model is None:
        raise GoalrailError(
            "executor.type='goalrail' requires a model (set executor.model)",
    ...
    executor_spec = GoalrailExecutorSpec(
    ...
    return AgentDef(
        ...
        tools=_translate_tools_to_goalrail(spec),
...
def _reject_unsupported_concepts(spec: AgentSpec) -> None:
    ...
    to remove from the spec (or what goalrail bug to file to
    close the gap).

    :param spec: The goalrail spec to check.
    :raises GoalrailError: On any of:
    ...
    # Policies are enforced by the goalrail workflow layer
    # (see ``goalrail.runtime.policies.build_policy_engine``)
    # BEFORE the harness receives a turn — the
    # :class:`GoalrailExecutor` is invoked with tool calls
    ...
    # Sandbox declarations (goalrail ``tools.sandbox.container_image``
    # and the goalrail OSEnvSandboxSpec) are unsupported. Fail loud
    # if either is populated.
    if spec.tools.sandbox.container_image is not None:
        raise GoalrailError(
            "tools.sandbox translation to goalrail OSEnvSpec is unsupported; "
            "the adapter rejects specs with sandbox rather than silently dropping it",
            code=ErrorCode.INVALID_INPUT,
        )

    # ``cancellable_function`` is an goalrail concept — Goalrail'
    ...
    for tool in spec.local_tools:
        ...
        if _is_cancellable_function_path(tool.path):
            raise GoalrailError(
                ...
                "goalrail; cancellation support is an goalrail spec extension",
                code=ErrorCode.INVALID_INPUT,
            )
...
def _is_cancellable_function_path(path: str) -> bool:
    ...
    explicitly in the goalrail spec; goalrail has no spec
...
def _translate_tools_to_goalrail(spec: AgentSpec) -> dict[str, Tool]:
    """
    Build the ``AgentDef.tools`` dict from Goalrail' tool model.
    ...
    :class:`goalrail.tools.FunctionTool`. Filesystem paths
    (``"tools/python/foo.py"``) are not supported — the YAML adapter
    that produces them must encode dotted paths.

    :param spec: The goalrail spec. ``spec.local_tools`` lists
    ...
    :returns: A ``dict[str, Tool]`` mapping tool name to
        ...
        local tools.
    :raises GoalrailError: If a tool path is a filesystem path
    ...
    for tool_info in spec.local_tools:
        ...
        # the goalrail tool's ``input_schema``. Re-attach it so
    ...
    # so the goalrail harness surfaces them to the LLM via its
    # own MCP machinery (``goalrail.inner.mcp_tools``). The
    # forward direction translated ``MCPTool`` →
    # :class:`MCPServerConfig` (stdio or http); this is the inverse.
    # ``GoalrailExecutor.from_spec`` calls this reverse path when
    # wrapping an goalrail spec for an goalrail harness; if the
...
def _mcp_server_to_mcp_tool(config: MCPServerConfig) -> MCPTool:
    ...
    :returns: An :class:`MCPTool` with the equivalent transport
        shape for the inner goalrail runtime.
    :raises GoalrailError: If the config is missing the
...
def _resolve_dotted_attr(dotted_path: str, tool_name: str) -> Any:
    ...
    :returns: The resolved attribute object (callable or
        runner or other).
    :raises GoalrailError: If the path is not a dotted import
    ...
    if "/" in dotted_path or dotted_path.endswith(".py"):
        raise GoalrailError(
            ...
            "goalrail translator requires dotted import paths like "
            "'examples._shared.tool_functions.get_current_time'",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    try:
        ...
        # single GoalrailError so the spec author sees a
...
def _resolve_dotted_callable(
    ...
    :raises GoalrailError: If the module can't be imported,
...
# ── Policy translation (goalrail YAML dict → goalrail
# GuardrailsSpec) ────────────────────────────────────────────────
#
...
# parser (``goalrail.spec.parser._parse_guardrails``) is invoked
...
def _translate_guardrails_yaml(
    ...
    *,
    ...
    goalrail YAML into an goalrail :class:`GuardrailsSpec`.

    Goalrail declares labels/policies at the top level
    (``labels:``, ``label_schema:``, ``policies:``,
    ``ask_timeout:``); goalrail groups them under a single
    ``guardrails:`` block. This helper transforms the goalrail
    shape into the goalrail dict shape and delegates to
    :func:`goalrail.spec.parser._parse_guardrails` for
    ...
    :param raw_policies: Goalrail YAML ``policies:`` mapping,
    ...
    :param raw_labels: Goalrail YAML top-level ``labels:`` map
        ...
        ``initial: None``.
    :param raw_label_schema: Goalrail YAML top-level
        ``label_schema:`` map of
        ``{label_key: {values: [...], monotonic: "max"|"min"|"none"}}``.
    :param raw_ask_timeout: Goalrail YAML top-level
    ...
    :returns: A validated :class:`GuardrailsSpec`, or ``None``
        ...
        source YAML.
    :raises GoalrailError: On malformed entries — same error
        shape the goalrail YAML parser produces for a native
        ``guardrails:`` block, so YAML authors see consistent
        error messages whether their spec came from goalrail
        or goalrail.
    """
    if not raw_policies and not raw_labels and not raw_label_schema and raw_ask_timeout is None:
        return None
    # Build the goalrail-shaped guardrails dict, then hand it to
    ...
    from goalrail.spec.parser import _parse_guardrails
    ...
    # ``expand_env`` is False because the upstream goalrail
...
def _translate_labels_yaml(
    ...
    """
    Merge goalrail' separate ``labels:`` (initial values) and
    ``label_schema:`` (schemas) into Goalrail' unified
    ``guardrails.labels:`` shape.

    Agent-plane's :class:`LabelDef` bundles ``initial``,
    ``values``, and ``monotonic`` into one entry per key. The
    goalrail ``monotonic: none`` sentinel maps to "no
    monotonic constraint" on goalrail (field simply omitted
    from the dict).
    ...
    for key in keys:
        ...
        if isinstance(schema, dict):
            ...
            if monotonic_raw in _GOALRAIL_TO_AP_MONOTONIC:
                entry["monotonic"] = _GOALRAIL_TO_AP_MONOTONIC[monotonic_raw]
            elif monotonic_raw not in (None, "none"):
                # Unknown monotonic value — let the goalrail
...
def _translate_policies_yaml(
    ...
    *,
    ...
    """
    Translate the goalrail ``policies:`` mapping entry-by-entry
    into the goalrail shape.
...
def _translate_policy_entry_yaml(
    ...
    *,
    ...
    if policy_type not in _KNOWN_POLICY_TYPES:
        raise GoalrailError(
            f"goalrail policy {policy_name!r}: unknown type "
...
def _translate_function_policy_yaml(
    ...
    """
    Translate an goalrail ``type: function`` policy to the
    goalrail shape.
    ...
    - Other fields (``condition``, ``action``, ``set_labels``,
      ...
      longer accept ``on:`` on the goalrail side; the handler
    ...
    # goalrail parser side. ``on:`` is intentionally omitted —
    ...
    # If ``function:`` is already present in goalrail native
    ...
    if callable_path is None:
        # Let the goalrail parser surface the "function:
        # required" error — it already does so with a clear
        # message referencing the policy name.
        return out
    # Route every goalrail-sourced function policy through the
    # legacy-compat shim so author callables written with the
    # goalrail ``(content, phase)`` / ``(content, phase, context)``
    # convention keep working under Goalrail'
    # ``(ctx, context)`` convention. For goalrail-native
    # callables the shim is a pass-through (detected by parameter
    # names at policy-build time; see _goalrail_legacy_shim).
    ...
    out["function"] = {
        "path": "goalrail.spec._goalrail_legacy_shim.build",
        "arguments": shim_args,
    }
    return out
...
def _translate_prompt_policy_yaml(
    ...
    *,
    ...
    """
    Translate an goalrail ``type: prompt`` policy to the
    goalrail shape.
...
def agent_def_to_agent_spec(
    ...
    *,
    ...
    """
    Translate an goalrail :class:`AgentDef` into an goalrail
    :class:`AgentSpec`.

    Reverse direction of :func:`agent_spec_to_agent_def`. Used by
    :func:`goalrail.spec.load` when handed an goalrail YAML
    so ``goalrail chat foo.yaml`` routes the spec through
    ``GoalrailExecutor`` at runtime.

    The produced spec sets ``executor.type = "goalrail"`` so the
    runtime picks ``GoalrailExecutor``, with ``harness`` and
    ``profile`` carried in ``executor.config``.

    :param agent_def: Parsed goalrail agent definition,
        typically from :func:`goalrail.loader.load_agent_def`.
    ...
    :param raw_yaml: Optional raw YAML dict the ``agent_def`` was
        ...
        Goalrail' ``guardrails:`` block — the goalrail
        ...
        (``load_goalrail_yaml``) read the raw YAML themselves
        and pass it through.
    ...
    :raises GoalrailError: When *agent_def* uses an goalrail
        concept Goalrail' :class:`AgentSpec` cannot currently
    ...
    # ``goalrail.inner.loader.load_agent_def``) wins over ``prompt:``
    ...
    # translation time — the Goalrail path spawns each child as an
    ...
    # Split goalrail tools by subtype:
    #   FunctionTool / CancellableFunctionTool
    #       → AgentSpec.local_tools (goalrail treats every
    ...
    #         Goalrail' ToolManager, srt-wrapped when
    #         available — see goalrail/tools/mcp.py).
    ...
    # raw YAML. Policies/labels are lifted from the goalrail
    # top-level YAML fields into Goalrail' ``guardrails:``
    # block; enforcement then happens in the goalrail workflow
    # (outside the goalrail harness) via the standard
    ...
    # Per designs/GOALRAIL_TERMINAL_BRIDGE.md §6.1 step 2 + §3 non-goal
    ...
    # ``terminals`` field in the goalrail grammar — matches legacy
    # non-AP mode exactly.
    terminals = dict(agent_def.terminals) if agent_def.terminals else None

    # Top-level ``skills:`` from the raw YAML (the goalrail
    # loader doesn't carry this field on AgentDef — it's
    # goalrail spec config that controls Claude SDK harness
...
def _translate_skills_filter_from_yaml(
    ...
    goalrail-format YAML mapping and validate the same shapes
    the goalrail spec parser accepts (``"all"`` / ``"none"`` /
    list of names).

    The goalrail loader (``inner.loader.load_agent_def``) does
    NOT read this field — it's goalrail spec config that
    controls the Claude SDK harness's host-skill loading, not an
    goalrail runtime concept. So when the YAML is consumed via
    ...
    :raises GoalrailError: When the value isn't one of the
...
def _self_agent_tool_to_sub_spec(
    ...
    *,
    ...
    the Goalrail runtime spawns when the parent's LLM dispatches
...
def _agent_tool_to_sub_spec(
    ...
    *,
    ...
    """
    Translate an goalrail inline :class:`AgentTool` (sub-agent
    exposed as a tool) into a nested goalrail :class:`AgentSpec`.
    ...
    :class:`AgentSpec` with ``executor.type == "goalrail"`` so the
    :class:`GoalrailExecutor` runs it when spawned.

    Lossy fields (not modeled on Goalrail' AgentSpec yet):
    ``max_sessions``, ``pass_history``, ``pass_histories``.
    goalrail' runtime falls back to its defaults for these on
    the reverse trip.
    ...
    :param parent_harness: Goalrail harness carried on the
        ...
        goalrail resolves this at ``create_executor`` time;
        goalrail has to do it at spec-translation time because
    ...
    :param parent_os_env: OS environment on the parent agent.
        ...
        inline AgentTool — legacy spec resolves it from the
        live Session at runtime, but goalrail runs each child
    ...
    :param raw_executor: The raw ``executor:`` dict for this inline
        ...
        goalrail' dataclass parsing dropped unknown keys). When set,
        fields the goalrail :class:`~goalrail.inner.datamodel.ExecutorSpec`
    ...
    :returns: A nested :class:`AgentSpec` representing the
        sub-agent.
    """
    oa_executor = tool.executor if isinstance(tool.executor, GoalrailExecutorSpec) else None
    ...
    # (see ``goalrail/tools/manager.py:426`` — "if not
...
def _resolve_inline_agent_tool_os_env(
    ...
    Goalrail' :class:`AgentTool.os_env` is
    ...
    if isinstance(tool_os_env, str):
        ...
        # strings would be an goalrail YAML that the loader
...
def _fail_on_unsupported_concepts_def(agent_def: AgentDef) -> None:
    """
    Raise :class:`GoalrailError` for every goalrail concept
    Goalrail' :class:`AgentSpec` cannot currently represent.

    Each unsupported concept gets its own clear error message
    naming the specific field.

    :param agent_def: Parsed goalrail agent definition.
    :raises GoalrailError: On the first unsupported concept
...
def _fail_on_unsupported_tool(
    ...
    """
    Raise :class:`GoalrailError` when *tool* uses an goalrail
    tool concept goalrail cannot currently represent.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"glean_search"``.
    :param tool: The parsed goalrail tool instance.
...
def _translate_name_from_def(raw_name: str | None) -> str | None:
    """
    Translate goalrail ``AgentDef.name`` to :attr:`AgentSpec.name`.

    ``AgentDef.name`` is ``str | None`` on the goalrail side;
    goalrail also uses ``None`` for absence. An empty string on
    ...
    :param raw_name: The raw ``name`` field from
        ...
        ``None``.
    :returns: The goalrail name, or ``None`` when *raw_name*
        is absent or empty.
    """
...
def _translate_prompt_from_def(raw_prompt: str | None) -> str | None:
    """
    Translate goalrail ``AgentDef.prompt`` to
    :attr:`AgentSpec.instructions`.

    ``AgentDef.prompt`` is ``str | None`` on the goalrail side;
    goalrail also uses ``None`` for absence.

    :param raw_prompt: The raw ``prompt`` field from
        :class:`AgentDef`, e.g. ``"You are a friendly
        assistant."``, ``""``, or ``None``.
    :returns: The goalrail instructions text, or ``None``
...
def _translate_llm_from_def(
    oa_executor: GoalrailExecutorSpec | None,
    *,
    raw_executor: dict[str, Any] | None = None,
) -> LLMConfig | None:
    """
    Translate goalrail ``executor.model`` into an goalrail
    :class:`LLMConfig`.

    goalrail stores the model string on its own
    :class:`goalrail.datamodel.ExecutorSpec`. goalrail splits
    ...
    ``type = "goalrail"`` plus the ``harness`` / ``profile``
    config.
    ...
    :param raw_executor: Optional raw goalrail YAML ``executor:``
        ...
        through to :class:`goalrail.executor.ExecutorConfig.extra`.
        The goalrail loader drops unknown fields on
        ``ExecutorSpec`` at parse time, so we read them back from
        the raw dict here.
    :returns: An :class:`LLMConfig` with ``model`` populated, or
        ``None`` when goalrail declared no model.
    """
...
def _translate_executor_from_def(
    ...
    *,
    ...
    """
    Build the goalrail :class:`ExecutorSpec` for an goalrail
    agent.
    ...
    Empty-string coercion mirrors goalrail' own dataclass
    defaults (``ExecutorSpec.harness: str = ""``,
    ``ExecutorSpec.profile: str = ""``) so the round-trip is
    bit-exact against the goalrail side.

    :param oa_executor: The goalrail ``ExecutorSpec`` or
        ``None``.
    ...
    :param raw_executor: Optional raw YAML ``executor:`` mapping.
        ...
        API surface (chat/completions vs. responses). The goalrail
        loader silently drops unknown fields on its own
        :class:`~goalrail.inner.datamodel.ExecutorSpec`, so we
        have to recover this field from the raw dict here.
    :returns: An :class:`ExecutorSpec` with ``type="goalrail"``
        when a harness is known.
    :raises GoalrailError: When a model is declared but no
        harness can be inferred from it.
    """
    # Goalrail' :class:`ExecutorSpec.{harness,profile,model}`
    ...
    # ``use_responses`` is not a field on the goalrail inner
    ...
    if oa_executor is not None and oa_executor.auth is not None:
        ...
        from goalrail.spec.parser import _parse_executor_auth
...
def _translate_mcp_tool_from_def(
    ...
    """
    Translate one goalrail :class:`MCPTool` into a native
    ...
    :param tool: The parsed goalrail :class:`MCPTool` instance.
    ...
    raise GoalrailError(
        f"goalrail MCP tool {tool_name!r} has neither 'url' nor "
...
def _translate_function_tool_from_def(
    ...
    """
    Translate one goalrail function tool into a
    :class:`LocalToolInfo`.
    ...
    :param tool_name: The YAML key for this tool, e.g.
        ``"get_current_time"``.
    :param tool: The parsed goalrail tool. Must be a
    ...
    :returns: A :class:`LocalToolInfo` with ``name``, ``path``
        ...
        :data:`GOALRAIL_TOOL_LANGUAGE`.
    :raises GoalrailError: When *tool* is not a
        :class:`FunctionTool` (defensive guard).
    """
    if isinstance(tool, CancellableFunctionTool):
        raise GoalrailError(
            f"goalrail tool {tool_name!r}: "
    ...
    if not isinstance(tool, FunctionTool):
        raise GoalrailError(
            f"goalrail tool {tool_name!r}: expected FunctionTool, got {type(tool).__name__}.",
            code=ErrorCode.INVALID_INPUT,
        )
    ...
    # for typed-parameter tools. Without this fallback, goalrail
...
def _recover_callable_path(
    ...
    goalrail' loader resolves the YAML's ``callable:`` field
    ...
    :returns: The dotted module path, e.g.
        ``"examples._shared.tool_functions.get_current_time"``.
    :raises GoalrailError: When the callable is missing
    ...
    raise GoalrailError(
        f"goalrail tool {tool_name!r}: callable {callable_obj!r} "
