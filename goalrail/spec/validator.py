...
from goalrail.spec.types import AgentSpec, ToolRuntime
...
# goalrail/spec/_goalrail_compat.py. Placed after the module's
...
from goalrail.spec._goalrail_compat import (  # noqa: E402
    GOALRAIL_EXECUTOR_TYPE,
    validate_goalrail_executor,
)
...
def _validate_executor_type(
    ...
    elif etype == GOALRAIL_EXECUTOR_TYPE:
        validate_goalrail_executor(spec, result)
...
def _validate_local_tools(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    # spec load (the validator runs from ``goalrail.spec``,
    # which is a lower layer than ``goalrail.tools``).
    from goalrail.tools.builtins import BUILTIN_NAMES
...
# allow-list in ``goalrail/inner/loader.py``. ``none`` is excluded
...
def _validate_os_env(spec: AgentSpec, result: ValidationResult) -> None:
    ...
    The Goalrail parser already enforces these on the YAML-loaded path,
    but ``AgentSpec`` may also be constructed programmatically —
    by tests, by the Goalrail compat shim, or by future API
