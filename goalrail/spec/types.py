...
if TYPE_CHECKING:
    ...
    # goalrail.policies.types); PhaseSelector.matches takes
...
class ExecutorSpec:  # type: ignore[explicit-any]  # config: dict[str, Any] field (see below)
    ...
    :param type: Executor type. ``"goalrail"`` (default),
    ...
    :param config: Executor-type-specific configuration. For
        ...
        latter is a nested mapping mirroring the goalrail
        ...
        🚨 **TECH DEBT — REMOVE WHEN GOALRAIL COMPAT ENDS.**
        This field exists *solely* to carry harness / profile /
        os_env data for the Goalrail integration (see
        ``designs/GOALRAIL_INTEGRATION.md``). A free-form
        ``dict[str, Any]`` on a spec dataclass is the kind of
        bag-of-values escape hatch we'd otherwise reject in
        review — acceptable here only because the Goalrail
        executor is a temporary bridge with explicit sunset
        criteria. Once Goalrail is consolidated (phase 6 of the
    ...
    def harness_kind(self) -> str:
        ...
        For ``type == "goalrail"`` the kind lives in
...
class MCPServerConfig:
    ...
    - ``"stdio"`` — local subprocess, spawned via
      ...
      legacy inner stack at ``goalrail/inner/mcp_tools.py``
      ...
      sandboxing through the ``goalrail/environments/``
...
# PolicyEngine live under goalrail.runtime.policies in later
# phases.
...
class AgentSpec:  # type: ignore[explicit-any]  # params: dict[str, Any] field (see below)
    ...
    :param async_enabled: Whether the LLM-callable async-dispatch
        ...
        ``goalrail/inner/datamodel.py::AgentDef.async_enabled``
        — the same YAML must produce the same tool surface
        whether the user opts into Goalrail mode or runs the legacy
    ...
    :param os_env: The agent's OS environment, e.g.
        ...
        :mod:`goalrail.inner.bwrap_sandbox`), selected when the
        ...
        Native Goalrail YAML accepts this as a top-level ``os_env:``
        mapping; the goalrail-compat translator copies
        ``AgentDef.os_env`` into this field directly. The
        ``"inherit"`` string sentinel from goalrail inline
    ...
    :param terminals: Declared interactive tmux terminals
        ...
        Populated by the goalrail-compat translator from
        ``AgentDef.terminals``. Native Goalrail YAML support is
        deferred — see ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §3.
    :param timers: Whether the LLM-callable timer builtins
        ...
        ``goalrail/inner/datamodel.py::AgentDef.timers`` —
        ...
        stack uses in-memory asyncio tasks; Goalrail supersedes that
