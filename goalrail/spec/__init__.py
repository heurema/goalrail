...
# Goalrail compat: imported surgically from a dedicated module so
# the integration's tech debt is removable in one shot. See
# goalrail/spec/_goalrail_compat.py.
from goalrail.spec._goalrail_compat import (
    diagnose_yaml_rejection,
    is_goalrail_yaml,
    load_goalrail_yaml,
)
from goalrail.spec.parser import expand_env_vars, parse, parse_default_policies, parse_server_llm
...
from goalrail.spec.validator import ValidationResult, validate
...
def materialize_bundle(source: Path, dest: Path) -> Path:
    ...
    (``config.yaml`` + bundled assets) or a standalone goalrail
    YAML file. Downstream code (``_preregister_agent``,
    ``_prepare_goalrail_yaml_bundle``) always wants to operate on a
    ...
    :returns: *dest*, always as a populated directory. For the
        ...
        *source*. For the file case the YAML is placed at the root
        of *dest* so
        :func:`goalrail.spec._find_goalrail_yaml_in_dir` picks
...
def _find_goalrail_yaml_in_dir(root: Path) -> Path | None:
    """
    Return the goalrail YAML inside *root* when the directory is
    a single-file goalrail bundle, or ``None`` otherwise.

    A directory qualifies as a single-file goalrail bundle when
    there is no ``config.yaml`` at the root and exactly one file
    at the root is recognised by :func:`is_goalrail_yaml`. Any
    other shape (``config.yaml`` present, zero or multiple goalrail
    YAMLs, YAMLs in subdirectories) returns ``None`` and the caller
    falls through to the standard goalrail directory-parse path.

    :param root: Path to an extracted bundle directory.
    :returns: The matched goalrail YAML path, or ``None`` if the
        directory is not a single-file goalrail bundle.
    """
    if (root / "config.yaml").exists():
        return None
    matches = [p for p in root.iterdir() if p.is_file() and is_goalrail_yaml(p)]
    if len(matches) == 1:
        return matches[0]
    return None
...
def load(
    ...
    *,
    ...
    If *source* is a file path (tarball or goalrail YAML) or raw
    bytes, dispatch accordingly.

    :param source: Path to an agent image directory, ``.tar.gz``
        bundle, goalrail single-file YAML, or raw tarball bytes
    ...
    :param expand_env: Whether to expand ``${VAR}`` references in
        ...
        (local ``goalrail run``, ``--agent`` preregistration). It
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        :func:`goalrail.server.bundles.validate_agent_bundle`. It is
        applied for both bundle shapes: the goalrail single-file YAML
        ...
        so trusted spec loading (local ``goalrail run``, operator
    ...
    :param prune_invalid_sub_agents: When ``True``, a sub-agent that
        ...
        **execution** paths (runner spec resolution, server
        :class:`~goalrail.runtime.agent_cache.AgentCache`): a bundle
        ...
        (``goalrail run``,
        :func:`goalrail.server.bundles.validate_agent_bundle`) stay
        strict and surface real authoring mistakes to the author.
    :returns: A validated :class:`AgentSpec`.
    :raises GoalrailError: If the spec fails validation, if a policy
    ...
    if isinstance(source, bytes):
        ...
        # Goalrail single-file YAML dispatch — see
        # goalrail.spec._goalrail_compat. Tech-debt aside;
        # remove this branch when goalrail compat ends.
        if is_goalrail_yaml(source):
            return load_goalrail_yaml(
                source,
                enforce_handler_allowlist=enforce_handler_allowlist,
                prune_invalid_sub_agents=prune_invalid_sub_agents,
            )
        if source.suffix.lower() in {".yaml", ".yml"}:
            # The path is a YAML file but failed the goalrail check
            ...
            raise GoalrailError(
                f"{source}: not a valid goalrail YAML — {reason}",
                code=ErrorCode.INVALID_INPUT,
            )
        if dest is None:
            raise GoalrailError(
    ...
    # Goalrail single-file YAML dispatch (extracted-bundle variant)
    # — when a bundle (directory, tarball, or raw bytes) resolves to
    # a root that contains exactly one goalrail YAML and no
    # ``config.yaml``, route to the goalrail adapter. This is the
    # shape produced by ``goalrail server --agent`` when it wraps a
    # YAML into a single-file tarball for the agent store. See
    # goalrail/spec/_goalrail_compat.py for the tech-debt
    # ownership; remove when goalrail compat ends.
    # The goalrail single-file path (load_goalrail_yaml) copies
    ...
    candidate = _find_goalrail_yaml_in_dir(root)
    ...
    if enforce_handler_allowlist:
        ...
        # The single-file goalrail YAML path is guarded earlier, inside
    ...
    *spec* in place: the failing sub-agent is removed from
    ...
    for sa in spec.sub_agents:
        ...
        _logger.warning(
            ...
            "produced by a newer Goalrail server with a feature this client does "
...
def _reject_unregistered_spec_policy_handlers(spec: AgentSpec) -> None:
    ...
    :class:`~goalrail.spec.types.FunctionPolicySpec` entries whose
    ...
    :raises GoalrailError: If a function policy names an unregistered
        handler, e.g. ``"subprocess.Popen"``.
    """
    from goalrail.policies.registry import is_registered_handler
