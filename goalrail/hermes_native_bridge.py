...
by the headless ``hermes`` harness (:mod:`goalrail.inner.hermes_executor`). The
...
(:mod:`goalrail.hermes_native_permissions`), which reads the pane via
...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"goalrail-{os.getuid()}" / "hermes-native"
_TMUX_FILE = "tmux.json"
_TMUX_READY_TIMEOUT_S = 30.0
_TMUX_SEND_TIMEOUT_S = 10.0
_POLL_INTERVAL_S = 0.2
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "goalrail-hermes-paste"
...
def clone_hermes_session(
    ...
    *,
    ...
    try:
        ...
        # messages (Goalrail already has them from the fork item copy).
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/goalrail-<uid>/hermes-native/<hash>``."""
...
def build_hermes_native_spawn_env(session_id: str) -> dict[str, str]:
    ...
    :class:`~goalrail.inner.hermes_native_executor.HermesNativeExecutor` can find
...
def write_policy_hook_config(
    ...
    2. An ``mcp_servers.goalrail`` entry that launches the Goalrail MCP stdio
    ...
    :func:`goalrail.inner.hermes_executor._populate_hermes_home`.
...
def _write_mcp_bridge_config(bridge_dir: Path) -> None:
    ...
    :func:`goalrail.codex_native_bridge.write_mcp_bridge_config`.
    """
...
def capture_hermes_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`goalrail.hermes_native_permissions`) to detect Hermes' in-terminal
