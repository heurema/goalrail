...
- ``goalrail resume <conv_id>`` — fetch the conversation, read its
  ``goalrail.wrapper`` label, and dispatch by runtime.
- ``goalrail resume`` (no id) — open the cross-agent picker (see
  :func:`goalrail.repl._resume_picker.pick_conversation_cross_agent_from_sdk`)
...
def run_resume(
    *,
    ...
    leak state and collide with any other in-flight ``goalrail``
    ...
    *,
    ...
    async def _drive() -> str | None:
        ...
        :returns: Selected conversation id, ``None`` on cancel /
            empty list.
        """
        # Deferred import: ``goalrail_client`` carries the full SDK
...
def _dispatch_wrapper(
    *,
    ...
    :param wrapper: Value from ``labels.goalrail.wrapper``.
...
def _read_wrapper_label_remote(
    *,
    ...
    :returns: Wrapper label string, or ``None`` when no
        ``goalrail.wrapper`` label is present on the row.
