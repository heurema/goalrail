...
:mod:`goalrail.inner.bwrap_sandbox` calls
...
:mod:`goalrail.sandbox.seatbelt`).

The bwrap backend is the Linux platform default when the ``bwrap``
binary is on ``PATH`` (selected by
:func:`goalrail.inner.sandbox._default_sandbox_for_platform`). When
...
from goalrail.inner.bwrap_sandbox import BwrapSandboxBackend
