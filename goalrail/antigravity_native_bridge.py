...
ANTIGRAVITY_NATIVE_BRIDGE_ID_LABEL_KEY = "goalrail.antigravity_native.bridge_id"
...
# onboarding is a distinct flow Goalrail does not drive, so it stays ``False``.
...
def ensure_agy_onboarding_complete() -> None:
    ...
    (see :func:`goalrail.runner.app._auto_create_antigravity_terminal` and the CLI
...
class AntigravityNativeBridgeState:
    ...
    .. note::
        ...
        driver that superseded it (:mod:`goalrail.antigravity_native_reader`) keeps
    ...
    .. note::
        ...
        :func:`goalrail.antigravity_native_rpc.resolve_language_server_port`).
...
# ── Goalrail MCP relay wiring (sys_* tools) ──────────────────────────────────
#
# agy is otherwise tool-isolated from Goalrail: unlike claude/codex/cursor it
# wires no MCP relay, so the wrapped agy cannot reach any ``sys_*`` tool (spawn
# sub-agent sessions, drive Goalrail terminals, list agents/models, ``sys_os_*``).
# This section wires the SAME relay cursor #742 uses — the shared
# ``goalrail.claude_native_bridge serve-mcp`` stdio server — into agy.
#
...
# its ``/mcp`` panel shows ``✓ goalrail`` with the ``sys_*`` tools discovered. agy
...
# Goalrail's own TOOL_CALL policy + elicitation gate still applies on the server
# side, so auto-approving the agy-side MCP gate only avoids a hidden in-terminal
# prompt blocking the call before Goalrail ever sees it.
_AGY_ENABLED_TOOLS = [
    "list_comments",
    "sys_add_policy",
    "sys_agent_download",
...
def build_mcp_config(
    ...
    *,
    ...
    """Build agy's ``mcp_config.json`` payload for the Goalrail relay server.

    Mirrors cursor #742's :func:`goalrail.cursor_native_bridge.build_mcp_config`
    ...
    ``<python> -I -m goalrail.claude_native_bridge serve-mcp --bridge-dir <dir>``.
    ...
    **HOME pinning (critical for the isolated-HOME design).** agy spawns this relay
    ...
    against ``bridge_root()`` (``$HOME/.goalrail/antigravity-native``), which must
...
def write_mcp_bridge_config(bridge_dir: Path) -> None:
    """Write the token config the shared Goalrail MCP relay requires.
    ...
    Mirrors :func:`goalrail.cursor_native_bridge.write_mcp_bridge_config`.
...
def write_mcp_config(
    ...
    *,
    ...
    ``mcp_config.json`` for the Goalrail relay into the per-session isolated agy
    ...
    :func:`goalrail.cursor_native_bridge.write_mcp_config`, adapted to agy's
...
def seed_isolated_agy_home(bridge_dir: Path) -> dict[str, str]:
    ...
    user from the seeded token and its ``/mcp`` panel shows ``✓ goalrail`` with the
...
def update_conversation_id(
    ...
    Goalrail session stays the same (e.g. the runner cold-start replacing the
...
_PASTE_BUFFER = "goalrail-agy-paste"
...
def send_interaction_keys_via_tui(
    ...
    **in-process permission / question prompt**. When agy runs attended (the
    ...
    Goalrail web UI AND as agy's own numbered TUI prompt ("Do you want to
    proceed?", 1.Yes … 4.No). Delivering the verdict over
    :func:`goalrail.antigravity_native_rpc.handle_user_interaction` flips the
    ...
    :func:`goalrail.cursor_native_bridge.send_cursor_pane_keys`.
