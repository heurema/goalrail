"""WebSocket ``Origin`` enforcement for the Goalrail server.
...
Goalrail server and drive the agent, read session updates, or attach to a
...
- :data:`GOALRAIL_INTERNAL_WS_ORIGIN` — the sentinel ``Origin`` the
...
from goalrail.server.auth import local_single_user_enabled
...
# ``goalrail.runner.identity`` and re-exported here for the server-side
# policy. See :data:`GOALRAIL_INTERNAL_WS_ORIGIN` there for rationale.
__all__ = [
...
_ALLOWED_ORIGINS_ENV = "GOALRAIL_WS_ALLOWED_ORIGINS"
...
def parse_allowed_origins() -> frozenset[str]:
    ...
    Reads ``GOALRAIL_WS_ALLOWED_ORIGINS`` (comma-separated). Whitespace
...
def origin_allowed(
    ...
    *,
    ...
    - The first-party sentinel (:data:`GOALRAIL_INTERNAL_WS_ORIGIN`) and
    ...
    :param local_mode: Whether the server is a single-user local runtime
        (``GOALRAIL_LOCAL_SINGLE_USER`` truthy).
    ...
    """
    if origin == GOALRAIL_INTERNAL_WS_ORIGIN:
        return True
