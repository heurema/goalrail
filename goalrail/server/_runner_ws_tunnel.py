...
The server's :mod:`goalrail.server.routes.terminal_attach` route
...
def make_tunnel_ws_factory(
    ...
    Install via :func:`goalrail.runtime.set_runner_ws_factory`. The
    factory shape (callable ``(runner_path) -> async-context``) is
    what :mod:`goalrail.server.routes.terminal_attach` already
    expects.
...
class _TunneledWSConn:
    ...
    :func:`goalrail.server.routes.terminal_attach._shuttle_ws_frames`
