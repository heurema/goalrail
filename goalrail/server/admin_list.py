...
def resolve_data_dir() -> Path:
    ...
    1. The parent directory of ``GOALRAIL_ADMIN_CREDENTIALS_PATH`` if
       that env var is set (Docker compose mounts
       ``/data/admin-credentials``, so this yields ``/data``).
    2. ``GOALRAIL_DATA_DIR`` when set.
    ...
    """
    explicit_creds = os.environ.get("GOALRAIL_ADMIN_CREDENTIALS_PATH", "").strip()
    if explicit_creds:
...
def resolve_admin_list_path() -> Path:
    ...
    :returns: ``GOALRAIL_ADMIN_LIST_PATH`` if set, else
        ``<data_dir>/admins`` (see :func:`resolve_data_dir`).
    """
    explicit = os.environ.get("GOALRAIL_ADMIN_LIST_PATH", "").strip()
    if explicit:
...
class AdminFlagStore(Protocol):
    ...
    Both :class:`~goalrail.stores.permission_store.PermissionStore`
    and :class:`~goalrail.server.accounts_store.SqlAlchemyAccountStore`
