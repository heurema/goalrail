...
Mounted at ``/auth`` when ``GOALRAIL_AUTH_PROVIDER=accounts``.
...
from goalrail.server.admin_list import AdminList, promote_if_listed
from goalrail.server.auth import _RESERVED_USERS, RESERVED_USER_LOCAL, UnifiedAuthProvider
...
def create_accounts_auth_router(
    ...
    :param admin_list: File-backed admin roster, consulted on each
        ...
        :mod:`goalrail.server.admin_list`).
    ...
    async def setup(body: SetupRequest) -> Response:
        ...
        that pre-seeds ``GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD`` also
        ...
        # `goalrail run` needs a CLI token too, not just the browser cookie.
