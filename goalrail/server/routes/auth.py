...
/ ``cli-poll`` pair supports the ``goalrail login`` CLI command.

See ``designs/OIDC_AUTH.md`` for the complete design.

These routes are only mounted when ``GOALRAIL_AUTH_PROVIDER=oidc``.
"""
...
def create_auth_router(
    ...
    :param admin_list: File-backed admin roster. Consulted on each
        ...
        :mod:`goalrail.server.admin_list`). OIDC's only admin signal.
    :param account_store: Invite-token persistence, required only when
        ``GOALRAIL_OIDC_ALLOW_INVITES`` is on. ``None`` disables the
    ...
    :param allowed_domains: Domains from the server config's
        ...
        ``GOALRAIL_OIDC_ALLOWED_DOMAINS`` and the runtime-editable file
    ...
    # decides who may sign in — see goalrail/server/oidc_access.py.
    ...
    async def logout() -> Response:
        ...
        If ``GOALRAIL_OIDC_LOGOUT_REDIRECT_URI`` is configured,
