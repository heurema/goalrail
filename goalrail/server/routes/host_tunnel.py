...
Hosts (machines running ``goalrail host``) connect here via
...
from goalrail.host.identity import MANAGED_HOST_TOKEN_HEADER
...
def create_host_tunnel_router(
    ...
    *,
    ...
    :param local_single_user: When ``True``, allow a host to re-own a
        ...
        ``None`` (the default) resolves from ``GOALRAIL_LOCAL_SINGLE_USER``
...
async def _receive_loop(
    ...
    while True:
        ...
        if isinstance(frame, HostCodeIntelSearchResultFrame):
            search_future = conn.pending_code_intel_search.pop(frame.request_id, None)
            if search_future is not None and not search_future.done():
        ...
        if isinstance(frame, HostCodeIntelFileResultFrame):
            file_future = conn.pending_code_intel_files.pop(frame.request_id, None)
            if file_future is not None and not file_future.done():
...
async def _ping_loop(
    ...
    (:data:`goalrail.stores.host_store.HOST_LIVENESS_TTL_S`): when a
