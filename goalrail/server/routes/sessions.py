...
from goalrail.db.utils import generate_agent_id, generate_task_id
...
from goalrail.entities.permission import SessionPermission
...
from goalrail.model_override import model_family_mismatch, validate_model_override
...
from goalrail.reasoning_effort import (
    EFFORT_CLEAR_VALUES,
    EFFORT_VALUES,
    validate_effort,
)
...
from goalrail.runner.routing import RunnerRouter
...
from goalrail.runtime.policies.builder import build_policy_engine, load_session_usage
from goalrail.runtime.policies.engine import PolicyEngine
from goalrail.runtime.tool_output import cap_tool_output
...
from goalrail.server.bundles import bundle_location, validate_agent_bundle
from goalrail.server.host_registry import HostConnection, HostRegistry, RunnerExitReports
...
from goalrail.server.mcp_pool import ServerMcpPool
from goalrail.server.permissions import check_session_access
from goalrail.server.routes._auth_helpers import (
...
from goalrail.server.routes._codex_elicitation import parse_codex_elicitation_request
from goalrail.server.routes._content_type import (
    require_json_content_type,
    require_json_or_multipart_content_type,
)
from goalrail.server.routes._host_worktree import CreatedWorktree
...
from goalrail.session_lifecycle import (
    is_session_closed,
...
from goalrail.stores import AgentStore, ConversationStore
...
from goalrail.stores.host_store import Host, HostStore
from goalrail.stores.permission_store import PermissionStore
from goalrail.tools.client_specified import parse_client_side_tool_specs
...
# assistant response outside the Goalrail task runtime and need to
...
# semantic item observed outside the Goalrail task runtime. Unlike a
...
# assistant text delta observed outside the Goalrail task runtime. The
...
# explicit ``session.interrupted`` edge observed outside the Goalrail
...
# session.status event observed outside the Goalrail task runtime
...
# ``.meta.json`` appears so the Goalrail server can mint a child Conversation
...
# sub-agents from goalrail-spawned ones at the data layer.
_CLAUDE_NATIVE_SUBAGENT_WRAPPER_LABEL_VALUE = "claude-code-native-ui-subagent"
_CLAUDE_NATIVE_SUBAGENT_ID_LABEL_KEY = "goalrail.claude_native.subagent_id"
_CLAUDE_NATIVE_TOOL_USE_ID_LABEL_KEY = "goalrail.claude_native.tool_use_id"
...
_CLAUDE_NATIVE_DESCRIPTION_LABEL_KEY = "goalrail.claude_native.description"
...
# forwarder converts each receiver thread into this event so Goalrail can mint a
...
# goalrail-spawned and claude-native sub-agents.
...
_CODEX_NATIVE_SUBAGENT_THREAD_ID_LABEL_KEY = "goalrail.codex_native.subagent_thread_id"
_CODEX_NATIVE_SUBAGENT_PARENT_THREAD_ID_LABEL_KEY = "goalrail.codex_native.parent_thread_id"
_CODEX_NATIVE_SUBAGENT_TOOL_CALL_ID_LABEL_KEY = "goalrail.codex_native.collab_tool_call_id"
_CODEX_NATIVE_SUBAGENT_PROMPT_LABEL_KEY = "goalrail.codex_native.prompt"
_CODEX_NATIVE_SUBAGENT_NICKNAME_LABEL_KEY = "goalrail.codex_native.agent_nickname"
_CODEX_NATIVE_SUBAGENT_ROLE_LABEL_KEY = "goalrail.codex_native.agent_role"
# Current Codex collaboration mode kind (``"plan"`` or ``"default"``)
# mirrored from app-server ``thread/settings/updated``.
_CODEX_NATIVE_COLLABORATION_MODE_LABEL_KEY = "goalrail.codex_native.collaboration_mode"
...
# reload for sessions where no Goalrail task carries usage (claude-native).
_LAST_CONTEXT_TOKENS_LABEL_KEY: str = "goalrail.last_context_tokens"
_LAST_CONTEXT_WINDOW_LABEL_KEY: str = "goalrail.last_context_window"
...
_LAST_TASK_ERROR_CODE_LABEL_KEY: str = "goalrail.last_task_error_code"
_LAST_TASK_ERROR_MESSAGE_LABEL_KEY: str = "goalrail.last_task_error_message"
...
_CLAUDE_NATIVE_WRAPPER_LABEL_KEY = "goalrail.wrapper"
...
_CLAUDE_NATIVE_UI_LABEL_KEY = "goalrail.ui"
...
# Maximum time Goalrail waits for its runner->AP SSE relay to observe the
...
def _native_ask_gate_lock(conversation_id: str, deciding_policy: str) -> asyncio.Lock:
    ...
    :param conversation_id: Goalrail conversation id whose ASK gate is being
...
def _publish_and_persist_resource_event(
    ...
    """
    from goalrail.entities.conversation import ResourceEventData
...
def _signal_terminal_resolved_harness_elicitation(
    ...
    :param session_id: Goalrail conversation id whose forwarder mirrored the
...
def _client_supplied_hook_elicitation_id(
    ...
    :param payload: Parsed PermissionRequest hook body. Reads the
        optional ``_goalrail_elicitation_id`` key.
    ...
    :raises GoalrailError: 400 when the id is malformed or is
        currently parked by a different session.
    """
    raw = payload.get("_goalrail_elicitation_id")
    if raw is None:
        return None
    if not isinstance(raw, str) or not _HOOK_ELICITATION_ID_RE.fullmatch(raw):
        raise GoalrailError(
            "PermissionRequest hook '_goalrail_elicitation_id' must match "
...
def _signal_harness_elicitation_resolved_by_id(
    ...
    :param elicitation_id: Harness elicitation id, e.g.
        ``"elicit_codex_abc123"``.
    :returns: None.
    :raises GoalrailError: If the id is malformed or belongs to a
        different session.
    """
...
def _permission_level_from_grants(
    ...
    Mirrors :func:`goalrail.server.routes._auth_helpers._get_permission_level_sync`
...
def _owner_from_grants(grants: list[SessionPermission]) -> str | None:
    ...
    Mirrors :func:`goalrail.server.routes._auth_helpers.get_session_owner_id`
    but operates on grants already held in memory so callers can batch the
    permission-store query across many sessions at once.
...
def _build_session_response(
    ...
    :raises GoalrailError: If ``conv.agent_id`` is ``None``.
    """
    ...
    # ``goalrail.ui = "terminal"`` label. That flag is fully determined by the
...
def _publish_input_consumed(
    ...
    :param cleared_pending_id: When this message drained a
        :mod:`goalrail.runtime.pending_inputs` entry (native-terminal
...
def _validated_harness_override(value: str | None, agent: Agent) -> str | None:
    ...
    in ``goalrail/chat.py``): the canonical name must be a known bundle
    harness, and the bound agent must be an ``executor.type: goalrail``
    ...
    :raises GoalrailError: ``invalid_input`` for an unknown harness, a
        non-goalrail executor type, or an unloadable agent bundle.
    """
    if value is None:
        return None
    from goalrail.harness_aliases import canonicalize_harness
    from goalrail.runtime import get_agent_cache
    from goalrail.spec._goalrail_compat import (
        GOALRAIL_EXECUTOR_TYPE,
        GOALRAIL_HARNESSES,
    )

    canonical = canonicalize_harness(value) or value
    if canonical not in GOALRAIL_HARNESSES:
        raise GoalrailError(
            f"invalid harness_override: must be one of "
            f"{sorted(GOALRAIL_HARNESSES)}, got {value!r}",
            code=ErrorCode.INVALID_INPUT,
        )
    try:
        loaded = get_agent_cache().load(
    ...
    if executor_type != GOALRAIL_EXECUTOR_TYPE:
        raise GoalrailError(
            f"harness_override only applies to executor.type "
            f"{GOALRAIL_EXECUTOR_TYPE!r} agents; agent {agent.name!r} "
...
def _accumulate_session_usage(
    ...
    if llm_model:
        from goalrail.llms.context_window import compute_llm_cost, fetch_model_pricing
...
def _persist_native_cumulative_usage(
    ...
    Unlike the Goalrail relay path (:func:`_accumulate_session_usage`), which adds
    ...
    :raises GoalrailError: When a cumulative field is the wrong type.
    """
...
def _coerce_cumulative_field(
    ...
    *,
    ...
    :raises GoalrailError: When present but the wrong type / negative.
    """
    value = data.get(key)
    if value is None:
        return None
...
async def _persist_external_session_usage(
    ...
    :raises GoalrailError: On missing / malformed fields.
    """
    ...
    # from the Goalrail relay's per-response accumulation. Persist this session's
...
async def _persist_external_model_change(
    ...
    Goalrail session: writes ``model_override`` so the value survives reload
    ...
    :raises GoalrailError: If ``data.model`` is missing or not a
        non-empty string.
    """
...
def _validate_external_reasoning_effort(body: SessionEventInput) -> str | None:
    ...
    :raises GoalrailError: If the payload is missing or unsupported.
    """
...
async def _persist_external_reasoning_effort_change(
    ...
    Mirrors a native-terminal thinking-level change onto the Goalrail session.
...
async def _persist_external_codex_collaboration_mode_change(
    ...
    :returns: None.
    :raises GoalrailError: If ``data.mode`` is missing or unsupported.
    """
...
async def _persist_model_change_note(
    ...
    ``goalrail.wrapper`` native label, via ``_is_native_terminal_session``)
    ...
    keys on ``goalrail.wrapper`` rather than ``goalrail.ui == "terminal"``
...
def _handle_external_session_todos(
    ...
    :raises GoalrailError: When ``data.todos`` is missing or not a list.
    """
...
def _publish_external_output_text_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises GoalrailError: If ``data.delta`` is not a string, or any
...
def _publish_external_output_reasoning_delta(session_id: str, body: SessionEventInput) -> None:
    ...
    :returns: None.
    :raises GoalrailError: If ``data.delta`` is not a string, or ``data.started``
...
def _parse_external_assistant_message(
    ...
    :raises GoalrailError: If required fields are missing or
        malformed.
    """
...
async def _persist_external_assistant_message(
    ...
    """
    Persist and broadcast assistant text produced outside Goalrail tasks.

    The event is append-only conversation history. It intentionally
    bypasses the legacy persist path so mirroring a
    Claude terminal response does not create or steer an Goalrail
    agent task.
...
async def _persist_external_subagent_start(
    ...
    never POSTs to Goalrail to register them. The forwarder watches the
    ...
    goalrail agent), stamp identifying labels, and publish the
    same ``session.created`` event goalrail-spawned children fire
    ...
    :raises GoalrailError: 400 if the payload is missing any of
    ...
    # Title format mirrors goalrail-spawned children
...
async def _persist_external_codex_subagent_start(
    ...
    :raises GoalrailError: If ``thread_id`` is missing or parent has
        no bound agent.
    """
...
async def _persist_external_conversation_item(
    ...
    without starting or steering the placeholder Goalrail agent.
...
def _is_kiro_native_session(conv: Conversation) -> bool:
    ...
    return conv.labels.get("goalrail.wrapper") == "kiro-native-ui"
...
def _latest_assistant_text_from_store(
    ...
    helper lets Goalrail forward the durable assistant output with the
...
def _require_external_status_forward(
    ...
    :returns: None.
    :raises GoalrailError: If the runner was unavailable or
...
def _require_collaboration_mode_forward(
    ...
    app-server. Persisting the Goalrail label without a successful runner
    ...
    :param runner_result: HTTP result returned by the runner, or ``None``
        when no runner could be reached.
    :returns: None.
    :raises GoalrailError: If no runner was reachable or the runner rejected
...
def _drive_terminal_resolved_elicitation(session_id: str, persisted: ConversationItem) -> None:
    ...
    :param session_id: Goalrail conversation id the item was mirrored for,
        e.g. ``"conv_abc123"``.
...
def _publish_sandbox_status(session_id: str, stage: str, error: str | None = None) -> None:
    ...
    :param stage: The launch stage just entered, e.g.
        ``"provisioning"`` — one of
        :data:`goalrail.server.schemas.SandboxLaunchStage`.
...
def _publish_session_superseded(session_id: str, target_conversation_id: str) -> None:
    ...
    ``goalrail/claude_native_forwarder.py``): a client actively viewing
...
async def _get_runner_client(
    ...
    """
    from goalrail.runtime import get_runner_client
    ...
    if runner_router is not None:
        ...
        except (LookupError, httpx.HTTPError, GoalrailError):
            _logger.debug(
...
async def _validate_session_workspace(
    *,
    ...
    :mod:`goalrail.server.routes._workspace_validation` and
    raises :class:`GoalrailError` on failure so the route layer
    ...
    :raises GoalrailError: With ``ErrorCode.INVALID_INPUT`` on
    ...
    """
    from goalrail.server.routes._workspace_validation import (
    ...
    from goalrail.server.routes._host_launch import resolve_host_owner
...
async def _launch_runner_on_host(
    ...
    """
    from goalrail.host.frames import HostLaunchRunnerFrame, encode_host_frame
...
async def _provision_managed_sandbox(
    *,
    ...
    """
    from goalrail.server.managed_hosts import launch_managed_host, relaunch_managed_host
...
async def _bind_and_launch_managed_runner(
    *,
    ...
    :param tunnel_registry: Runner-tunnel registry used to await the
        ...
        wirings (the rendezvous then settles at frame-send).
    """
    from goalrail.server.managed_hosts import terminate_managed_host
...
async def _await_settled_managed_launch(launch: ManagedLaunch) -> None:
    ...
    :raises GoalrailError: 503 when the launch failed or is still
        running at the timeout.
    """
    from goalrail.server.managed_hosts import MANAGED_LAUNCH_RENDEZVOUS_TIMEOUT_S
...
async def _maybe_relaunch_managed_sandbox(
    *,
    ...
    :raises GoalrailError: 503 when the relaunch failed or timed out.
    """
...
def _kick_managed_relaunch(
    *,
    ...
    """
    from goalrail.server.managed_hosts import MANAGED_REPO_LABEL_KEY, parse_repo_workspace
...
async def _run_managed_wake(
    *,
    ...
    """
    from goalrail.server.managed_hosts import resume_managed_host
...
async def _get_runner_client_for_resource_access(
    ...
    """
    from goalrail.runtime import get_runner_client, get_runner_router
...
async def _reset_runner_resources_after_switch(session_id: str) -> None:
    ...
    except (httpx.HTTPError, HTTPException, GoalrailError, RuntimeError):
        # Best-effort: a runner hiccup must not break the (already-committed)
        # switch. GoalrailError covers the session-not-runner-bound / runner-
...
def _native_terminal_runtime(conv: Conversation) -> tuple[str, str, str]:
    ...
    :raises GoalrailError: If the wrapper label is unsupported.
    """
    wrapper = conv.labels.get(_CLAUDE_NATIVE_WRAPPER_LABEL_KEY)
...
def _native_terminal_name_for_harness(harness: str) -> str:
    ...
    :raises GoalrailError: If *harness* is not a supported native
        terminal harness.
    """
...
async def _forward_native_subagent_terminal_failure(
    ...
    :returns: None.
    :raises GoalrailError: If the parent's runner could not be reached
...
def _build_native_terminal_message_event(
    ...
    :raises GoalrailError: If the event is not a user message.
    """
...
async def _forward_native_terminal_message(
    ...
    """
    Forward one Goalrail web-chat message to the native terminal harness.
...
def _extract_claude_native_runner_failure(resp: httpx.Response) -> str | None:
    ...
    ``response.failed``. Claude-native Goalrail forwarding must treat that
...
async def _forward_session_change_to_runner(
    ...
    * Explicit ``compact`` — the caller inspects the returned status
      ...
      200) or the Goalrail server must run its own in-process compaction
    ...
    skipped — the persisted value on the Goalrail side is the authoritative
    ...
    in the Goalrail log — otherwise the POST succeeds at the httpx layer
...
async def _stop_session_via_runner(
    ...
    :raises GoalrailError: ``RUNNER_UNAVAILABLE`` (HTTP 503) if the
...
async def _stop_session_host_runner(
    ...
    the host's ``goalrail host`` tunnel is open while the user drives
    ...
    if conn is None:
        ...
        return
    from goalrail.host.frames import HostStopRunnerFrame, encode_host_frame
...
def _parse_skill_slash_command(body: SessionEventInput) -> tuple[str, str]:
    ...
    :raises GoalrailError: If the payload is not a skill command
...
def _build_skill_slash_command_policy_body(body: SessionEventInput) -> SessionEventInput:
    ...
    :raises GoalrailError: If the slash-command payload is invalid.
    """
...
async def _resolve_skill_meta_text_via_runner(
    ...
    :returns: The hidden ``<skill>`` meta text for a single
        ``input_text`` block.
    :raises GoalrailError: If the skill is not exposed for the session
...
async def _dispatch_skill_slash_command_to_runner(
    ...
    * a hidden ``message`` item with ``is_meta=True`` containing the
    ...
    :raises GoalrailError: If the skill is not exposed for the
...
async def _forward_event_to_runner(
    ...
    if (
        ...
        from goalrail.server.smart_routing import route_turn
...
class _SessionEventDispatchResult:
    ...
    :param pending_id: Id of the :mod:`goalrail.runtime.pending_inputs`
...
async def _dispatch_session_event_to_runner(
    ...
    * **transcript-forwarded native + ``type == "message"``**: web-chat user
      ...
      server. The Goalrail would otherwise persist an AP-side copy AND
    ...
    * **All other cases**: persist the item AP-side (invariant I1:
    ...
    :mod:`goalrail.runtime.pending_inputs`). It is rolled back if the
    ...
    :param created_by: Authenticated identity of the posting actor,
        ...
        :func:`goalrail.runtime.pending_inputs.record` and applied
        ...
        async with runner_client.stream(
            ...
            async for chunk in resp.aiter_text():
                ...
                while "\n\n" in buffer:
                    ...
                    # (the Goalrail subscriber generates its own heartbeats).
...
async def _ensure_runner_relay_ready(
    ...
    The runner stream has no replay buffer. For item events, Goalrail must
    subscribe to runner output before it forwards the input event; a
    fast harness can otherwise complete before Goalrail is listening, leaving
    ...
    :returns: The active relay handle, or ``None`` when no runner is
        bound.
    :raises GoalrailError: If the relay cannot observe the
...
async def _run_compact_locked(
    ...
    """
    if conv.agent_id is None:
        raise GoalrailError("Session has no agent binding", code=ErrorCode.INTERNAL_ERROR)
    if agent_cache is None:
        raise GoalrailError(
    ...
    if spec.llm is not None:
        ...
        from goalrail.spec.types import LLMConfig
    ...
    from goalrail.runtime.workflow import compact_conversation_now
...
def _agent_provider_family(agent: Agent) -> str | None:
    ...
    """
    from goalrail.onboarding.provider_config import provider_family_for_harness
...
def _same_provider_family(a: Agent, b: Agent) -> bool:
    ...
    native transcript from Goalrail items instead).
...
# Goalrail items and relaunch the CLI with --resume, so prior turns reappear as
...
# session file from copied items (goalrail/pi_native_resume.py), the same
...
def _agent_carries_native_fork_history(agent: Agent) -> bool:
    ...
    session file that the runner rebuilds from the copied Goalrail items on
...
def _presentation_labels_for_agent(agent: Agent) -> dict[str, str]:
    ...
    main view), gated on ``goalrail.ui == "terminal"`` plus the matching
    ``goalrail.wrapper`` value; an SDK agent runs as plain chat (no such
...
def _build_evaluation_context(
    ...
    *,
    ...
    if phase == Phase.TOOL_CALL:
        tool_call_data = data if isinstance(data, dict) else {}
    ...
    if phase == Phase.TOOL_RESULT:
        tool_result_data = data if isinstance(data, dict) else {}
...
def _publish_input_deny_terminal(session_id: str, conv: Conversation, reason: str) -> None:
    ...
    :class:`goalrail_client.SessionsChat.send`) iterate until a
...
async def _wake_parent_for_blocked_child(
    ...
    *outcome* is reported back so the notifier can release its per-block
    ...
    except (httpx.HTTPError, GoalrailError):
        _logger.warning(
...
def configure_subagent_block_notifier(
    ...
    :mod:`goalrail.runtime.pending_elicitations` so a sub-agent that
    ...
    """
    from goalrail.runtime import pending_elicitations as _pending_elicitations
    from goalrail.runtime.subagent_block_notifier import SubagentBlockNotifier
...
def _validate_terminal_launch_args(value: list[str] | None) -> list[str] | None:
    ...
    a caller to smuggle internal launch wiring (bridge dir, Goalrail URL,
...
def _validated_cost_control_mode_override(value: str | None) -> str | None:
    ...
    :raises GoalrailError: 400 (``invalid_input``) when *value* is
...
def _parse_session_create_metadata(metadata: str) -> SessionCreateMetadata:
    ...
    :raises GoalrailError: If the JSON fails the request schema.
    """
    try:
...
def _require_host_conn_for_worktree(host_id: str | None, request: Request) -> HostConnection:
    ...
    :raises GoalrailError: ``invalid_input`` when ``host_id`` is
...
async def _create_session_worktree(
    *,
    ...
    :raises GoalrailError: ``invalid_input`` for a bad branch name,
        ...
        is configured.
    """
    from goalrail.host.git_worktree import WorktreeError, validate_branch_name
    ...
    try:
        ...
        raise GoalrailError(exc.message, code=ErrorCode.INVALID_INPUT) from exc
    ...
    try:
        ...
        raise GoalrailError(exc.message, code=ErrorCode.CONFLICT) from exc
...
def _spec_config_flag_explicitly_disabled(spec: AgentSpec, key: str) -> bool:
    ...
    ``goalrail/spec/parser.py`` — ``{str(k): str(v) ...}``), so a YAML
...
def _derive_terminal_launch_args_from_spec(sub_spec: AgentSpec) -> list[str] | None:
    ...
    if harness == _CODEX_NATIVE_HARNESS:
        ...
        # session; the goalrail executor's later turn/start inherits that
...
def _native_subagent_wrapper_labels(
    *,
    ...
    The pill is gated on the conversation's ``goalrail.wrapper`` +
    ``goalrail.ui`` labels (see ``web`` ``TerminalFirstContext``), but
...
def _reject_reserved_cost_control_label_seed(labels: dict[str, str]) -> None:
    ...
    :param labels: The client-supplied initial labels, e.g.
        ``{"team": "ml"}``.
    :raises GoalrailError: 400 when any ``cost_control.*`` key is
        present.
    """
...
def _require_cost_control_label_authority(
    *,
    ...
    :param tunnel_token: Value of the ``X-Goalrail-Runner-Tunnel-Token``
    ...
    :raises GoalrailError: 403 when the caller presents no acceptable
...
async def _create_session_from_existing_agent(
    ...
    :raises GoalrailError: 404 if no agent matches ``body.agent_id``;
    ...
    # Validated against the loaded spec (known harness + goalrail
...
def _create_session_from_bundle(
    ...
    reusing a template by name would make a fresh ``goalrail run
    ...
    :raises GoalrailError: If bundle validation or agent insert
    ...
    # ``goalrail run`` uploads the operator's own bundle through this same
...
def _persist_stored_session_bundle(
    ...
    *,
    ...
    :returns: Response with the new session id.
    :raises GoalrailError: If the agent insert violates integrity
...
async def _authorize_bundled_parent_and_inherit_runner(
    ...
    *,
    ...
    :raises GoalrailError: 403/404 when the caller may not access the
        parent session.
    """
    await _require_access(
        user_id,
...
def _registered_runner_id(
    ...
    *,
    ...
    :raises GoalrailError: If the id is empty, the router is
...
def _child_session_summary_from_conversation(
    ...
    :func:`goalrail.tools.builtins.spawn._spawn_one`, plus the
...
async def _handle_mcp_tools_list(
    ...
    env, and tooling). The Goalrail server's role here is routing only —
...
async def _handle_mcp_tools_call(
    ...
    *,
    ...
    # tunnel the runner already opened to the Goalrail server at startup.
    runner_client = await _get_runner_client(session_id, runner_router)
    if runner_client is None:
        from goalrail.runtime import get_runner_client
    ...
    if runner_client is None:
        return _mcp_error_response(rpc_id, -32000, f"No runner bound for session {session_id!r}")
    try:
        from goalrail.runner.tool_dispatch import MCP_PROXY_FORWARD_TIMEOUT_S
...
def create_sessions_router(
    ...
    async def create_session(
        ...
        :raises GoalrailError: If metadata, bundle, or agent lookup
        ...
        if launch_host_id is not None and resp.runner_id is None:
            ...
            if host_registry is not None and host_store_inst is not None:
                ...
                from goalrail.runner.identity import token_bound_runner_id
                from goalrail.server.routes._host_launch import resolve_host_launch
    ...
    async def _create_bundled_session_from_multipart(
        ...
        :raises HTTPException: 422 when a required multipart part is
            absent.
        :raises GoalrailError: If metadata or bundle validation
    ...
    async def list_session_projects(
        ...
        has a ``conversation_labels`` row with ``key="goalrail.project"``.
    ...
    async def get_session(
        ...
        :raises GoalrailError: 404 if no session exists.
        """
    ...
    from goalrail.server.routes.codex.sessions import register_codex_session_routes
    ...
    async def update_session(
        ...
        :raises GoalrailError: 400 if the runner is not
        ...
        if body.runner_id is not None and permission_store is not None:
            if not check_session_access(
                ...
                raise GoalrailError(
                    ...
                    f"To fork this session instead, run: goalrail run --fork {session_id}",
                    code=ErrorCode.FORBIDDEN,
                )
        if body.labels:
        ...
        # dispatch — Goalrail server stays harness-agnostic; the runner
        ...
        if live_forward and (model_override is not None or clear_model):
            ...
            # whose history Goalrail writes. Gate on the wrapper label (NOT
            # goalrail.ui, which chat-first SDK terminal-view sessions like
    ...
    async def fork_session(
        ...
        :raises GoalrailError: 404 if *source_id* does not exist
        ...
        if body.model_override is not None:
            ...
            if mismatch is not None:
                raise GoalrailError(mismatch, code=ErrorCode.INVALID_INPUT)
        ...
        # the copied Goalrail transcript on its own — mark the fork so the
        ...
        # copied Goalrail items. Cross-family: the source's native transcript
        # is the wrong format, so ALWAYS rebuild from the copied Goalrail
        # items (the converters consume Goalrail's normalized item shape, so
    ...
    async def switch_session_agent(
        ...
        :raises GoalrailError: 404 if the session or target agent does
        ...
        # Goalrail items (a format-agnostic conversion). SDK targets replay
    ...
    async def claude_permission_request_hook(
        ...
        the same Bearer token used for every other Goalrail request. For
        ...
        :raises GoalrailError: 404 if the session doesn't exist,
    ...
    async def evaluate_policy(
        ...
        command hooks (via ``goalrail.claude_native_hook``) to
        ...
        raw_elicitation_id = payload.get("_goalrail_elicitation_id")
        ...
        if raw_elicitation_id is not None:
            if not isinstance(raw_elicitation_id, str) or not (
                ...
                raise GoalrailError(
                    "Policy evaluate '_goalrail_elicitation_id' must match "
    ...
    async def codex_elicitation_request_hook(
        ...
        :raises GoalrailError: 404 if the session does not exist,
    ...
    async def antigravity_elicitation_request_hook(
        ...
        :class:`~goalrail.server.schemas.ElicitationResult` so the bridge
        ...
        :class:`~goalrail.server.schemas.ElicitationRequestParams` dict.
        ...
        :raises GoalrailError: 404 if the session does not exist, 400 if
    ...
    async def cursor_permission_request_hook(
        ...
        (:mod:`goalrail.cursor_native_permissions`), publishes the standard
        ...
        :returns: An ``ElicitationResult`` (``{"action": …}``) on a web
            ...
            disconnect.
        :raises GoalrailError: 404 if the session does not exist, 400 if the
            body is malformed.
        """
        user_id = _get_user_id(request, auth_provider)
        await _require_access(
    ...
    async def list_child_sessions(
        ...
        :raises GoalrailError: 403 if the caller lacks READ on
    ...
    async def list_session_resources(
        ...
        if runner_client is not None:
            ...
            from goalrail.runtime import get_terminal_registry
    ...
    async def _validate_session(
        ...
        :raises GoalrailError: 401/403/404 on auth or access failure.
        """
    ...
    async def create_session_terminal(
        ...
        :raises GoalrailError: 400 when the requested terminal is not
        ...
        if status >= 400:
            error = payload.get("error", {})
            # GoalrailError derives http_status from code; pass the runner's code, not a status.
            raise GoalrailError(
    ...
    async def _proxy_fs_response(
        ...
        *,
        ...
        if status >= 400:
            ...
            if status == 404:
                raise GoalrailError(message, code=ErrorCode.NOT_FOUND)
    ...
    async def get_elicitation(
        ...
        :mod:`goalrail.runtime.pending_elicitations` index — no
    ...
    async def post_event(
        ...
        - ``"external_output_reasoning_delta"`` publishes a transient
          ...
          observed outside the Goalrail task runtime, without persisting an
          item or starting / steering a task.
        - ``"external_session_interrupted"`` publishes a
          ``session.interrupted`` event observed outside the Goalrail task
        ...
        - ``"external_codex_collaboration_mode_change"`` persists the
          ...
          (``goalrail.codex_native.collaboration_mode``).
        ...
        if body.tools:
            try:
                ...
                raise GoalrailError(str(exc), code=ErrorCode.INVALID_INPUT) from exc
        ...
        if body.type == _COMPACT_TYPE:
            ...
            # The Goalrail server stays harness-agnostic: it runs its own
        ...
        if runner_client is None:
            ...
            if body.type == "message" and _is_native_terminal_session(conv):
                ...
                return {"queued": True, "item_id": item_id}
            # Raise so the Goalrail server doesn't persist an item the
    ...
    async def delete_session(
        ...
        :raises GoalrailError: 404 if no session or no access,
    ...
    def _policy_description(spec: PolicySpec) -> str | None:
        ...
        if isinstance(spec, FunctionPolicySpec) and spec.function:
            from goalrail.policies.registry import get_entry
    ...
    async def get_session_agent(
        ...
        :raises GoalrailError: If the session or agent is not found.
        """
        user_id = _require_user(request, auth_provider)
        access = await _require_access_and_level(
            user_id, session_id, LEVEL_READ, permission_store, conversation_store
    ...
    async def get_session_agent_contents(
        ...
        :raises GoalrailError: If the session, agent, or bundle is
            not found.
        """
        user_id = _require_user(request, auth_provider)
        access = await _require_access_and_level(
    ...
    async def update_session_agent(
        ...
        if spec.name is None:
            raise GoalrailError("spec missing name", code=ErrorCode.INVALID_INPUT)
    ...
    async def mcp_proxy(
        ...
        - Policy runs on the Goalrail server where the ConversationStore and
          label state live.
        ...
        if method == "initialize":
            ...
            return _mcp_ok_response(
                rpc_id,
                {
                    ...
                    "capabilities": {"tools": {}},
                    "serverInfo": {"name": "goalrail-mcp-proxy", "version": "1.0.0"},
                },
            )
...
async def _fetch_model_options(
    ...
    * **cursor-native** — a curated *static* base catalog
      (:func:`goalrail.cursor_native.cursor_base_model_options`), returned
    ...
    * **codex-native** — a *live*, account-scoped catalog only the bound runner
    ...
    if wrapper == _CURSOR_NATIVE_WRAPPER_LABEL_VALUE:
        from goalrail.cursor_native import cursor_base_model_options
...
async def _get_session_snapshot(
    ...
    :raises GoalrailError: 404 if no session exists, 500 if the
