...
- **In-process runner**: the server passes
  :func:`goalrail.runtime.get_terminal_registry` into
  :func:`goalrail.runner.create_runner_app`, so the registry is
...
- **Out-of-process runner** over the WebSocket tunnel: the runner
  ...
  (``goalrail/server/_runner_ws_tunnel.py``), and the runner's
  resource-addressed WS route runs ``tmux attach`` and bridges the
  PTY.

The proxy uses a factory configured via
:func:`goalrail.runtime.set_runner_ws_factory` in the server
...
from goalrail.server.auth import LEVEL_OWNER, LEVEL_READ, AuthProvider
from goalrail.server.routes._auth_helpers import require_access
...
def create_terminal_attach_router(
    *,
    ...
    :func:`goalrail.server.app.create_app`. The list/CRUD endpoints
