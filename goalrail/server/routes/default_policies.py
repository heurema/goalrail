...
from goalrail.entities import Policy
from goalrail.errors import ErrorCode, GoalrailError
from goalrail.policies.registry import is_registered_handler, validate_factory_params
from goalrail.server.auth import AuthProvider
from goalrail.server.routes._auth_helpers import get_user_id
...
async def _require_admin(
    ...
    :raises GoalrailError: 401 if unauthenticated, 403 if
        not an admin.
    """
...
def create_default_policies_router(
    ...
    async def create_policy(
        ...
        :raises GoalrailError: 401/403 if the user lacks admin
        ...
        if body.type == "python":
            ...
            if validation_error:
                raise GoalrailError(validation_error, code=ErrorCode.INVALID_INPUT)
    ...
    async def list_policies(
        ...
        :raises GoalrailError: 401 if unauthenticated in
            multi-user mode.
        """
    ...
    async def get_policy(
        ...
        :raises GoalrailError: 401 if unauthenticated, or
        ...
        if policy is None:
            raise GoalrailError("Policy not found", code=ErrorCode.NOT_FOUND)
        return _entity_to_response(policy)
