...
The JSON Content-Type guard in :mod:`goalrail.server.routes._content_type`
...
enforces (:func:`goalrail.server.ws_origin.origin_allowed`) so HTTP and
...
- the first-party sentinel (:data:`GOALRAIL_INTERNAL_WS_ORIGIN`,
  ``"goalrail://internal"``) and any origin in
  ``GOALRAIL_WS_ALLOWED_ORIGINS`` pass;
...
from goalrail.server.ws_origin import origin_allowed, parse_allowed_origins
...
def require_trusted_origin(request: Request) -> None:
    ...
    may announce themselves with ``Origin: goalrail://internal``
    (:data:`GOALRAIL_INTERNAL_WS_ORIGIN`), but are not required to.
