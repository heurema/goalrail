...
registered at startup with ``goalrail server --agent``. They are the
...
from goalrail.server.routes._auth_helpers import require_user as _require_user
from goalrail.server.schemas import AgentObject, MCPServerSummary, PaginatedList, SkillSummary
from goalrail.stores import AgentStore
...
def _to_agent_object(agent: Agent, agent_cache: AgentCache) -> AgentObject:
    ...
    return AgentObject(
        ...
        # by a same-named ``goalrail run`` upload, but lets a newer
