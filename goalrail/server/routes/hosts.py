...
from goalrail.server.host_registry import HostConnection, HostRegistry
...
from goalrail.server.schemas import SessionGitOptions
from goalrail.stores import AgentStore, ConversationStore
from goalrail.stores.host_store import HostStore, host_is_live
...
def create_hosts_router(
    ...
    *,
    ...
    :param agent_store: Agent store used to resolve a session's agent
        ...
        :func:`goalrail.server.app.create_app` always supplies it.
    ...
    async def launch_runner(
        ...
        if result.get("status") == "failed":
            ...
            if result.get("error_code") == HARNESS_NOT_CONFIGURED_ERROR_CODE:
                ...
                # (`goalrail setup` on the host machine). Surface the
