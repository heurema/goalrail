...
class RequestDurationAccessFormatter(AccessFormatter):
    """
    Uvicorn access formatter that appends Goalrail request duration.
    ...
    def formatMessage(self, record: logging.LogRecord) -> str:
        ...
        :returns: Uvicorn's access message with a millisecond suffix when
            Goalrail recorded a duration for the current request.
        """
...
def _create_request_otel_instruments(meter: MeterLike) -> _RequestOtelInstruments:
    ...
    return _RequestOtelInstruments(
        started=meter.create_counter(
            "goalrail.server.http.requests.started",
            unit="{request}",
            description="HTTP requests started by the Goalrail server.",
        ),
        completed=meter.create_counter(
            "goalrail.server.http.requests.completed",
            unit="{request}",
            description="HTTP requests completed by the Goalrail server.",
        ),
        failed=meter.create_counter(
            "goalrail.server.http.requests.failed",
            unit="{request}",
...
def _create_processing_otel_instruments(
    ...
    return _ProcessingOtelInstruments(
        ...
        avg_30s_ms=meter.create_gauge(
            "goalrail.server.http.request.processing.avg_30s",
            unit="ms",
            description=(
