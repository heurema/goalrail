...
Run once at server startup when ``GOALRAIL_AUTH_PROVIDER=accounts``.
...
1. **Password flag** — ``--admin-password`` /
   ``GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD``. Headless / CI. Bootstrap
   creates the admin from it directly (this module).
2. **Terminal prompt** — ``goalrail server`` on a TTY asks for a
...
bootstrap writes a session JWT to ``~/.goalrail/auth_tokens.json``
(via :mod:`goalrail.cli_auth`) keyed to this spawn's URL, so the next
``goalrail run`` is signed in without a prompt. Skipped for
...
from goalrail.server.auth import _RESERVED_USERS
...
def resolve_admin_username() -> str:
    ...
    1. ``GOALRAIL_ACCOUNTS_INIT_ADMIN_USERNAME`` env var — explicit
    ...
    """
    explicit = os.environ.get("GOALRAIL_ACCOUNTS_INIT_ADMIN_USERNAME", "").strip()
    if explicit:
...
class BootstrapResult:
    ...
    :param needs_setup: ``True`` when no admin exists yet and none was
        ...
        ``goalrail server`` terminal prompt, or a password flag.
    ...
    :param tui_token_written: ``True`` when a CLI session JWT was
        written to ``~/.goalrail/auth_tokens.json`` (loopback only),
        so the next ``goalrail run`` is signed in without a prompt.
    """
...
def _mint_loopback_cli_token(
    ...
    *,
    ...
    ``~/.goalrail/auth_tokens.json`` keyed by the **exact** server
    ...
    per spawn, ``goalrail run`` 401s against its own loopback
    ...
    try:
        ...
        logger.warning(
            ...
            "`goalrail run` may need `goalrail login`",
            exc,
        )
        return False
...
def bootstrap_admin(
    ...
    *,
    ...
    :param account_store: The accounts-specific store backing user
        ...
        for accounts concerns — see ``goalrail/server/accounts_store.py``
        for the boundary rationale.
    :param init_admin_password: Optional operator-provided
        password from ``GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD``.
    ...
    :param session_ttl_hours: TTL for the CLI session token
        written to ``~/.goalrail/auth_tokens.json``.
    ...
    if any(u.has_password for u in account_store.list_users()):
        ...
        if init_admin_password:
            ...
            print(
                ...
                "    (--admin-password / GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD).\n"
        ...
        # one-time. Without this, `goalrail run` 401s against its own
    ...
    #      GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD — headless / CI;
    #   2. the `goalrail server` terminal prompt (cli.py, on a TTY);
    ...
    # Loopback CLI handoff so `goalrail run` is signed in without a
