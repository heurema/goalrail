"""FastAPI application — main entry point for the goalrail server."""
...
from goalrail._platform import resolve_repo_symlink
...
from goalrail.resources import examples as _examples_resources
...
from goalrail.server.managed_hosts import ManagedSandboxConfig
...
from goalrail.server.ws_origin import WebSocketOriginMiddleware
from goalrail.stores import (
    AgentStore,
    ArtifactStore,
    ConversationStore,
    FileStore,
...
from goalrail.stores.conversation_store import SessionConnectivity
from goalrail.stores.host_store import HostStore
from goalrail.stores.permission_store import PermissionStore
from goalrail.stores.policy_store import PolicyStore
...
# goalrail.resources.examples (see pyproject package-data), so they resolve
...
def _normalize_tarinfo(tarinfo: tarfile.TarInfo) -> tarfile.TarInfo:
    ...
    ``goalrail.spec.tar_utils.extract_safe``), so on-disk permissions
    don't depend on it.
...
def _ensure_builtin_agent(
    ...
    *,
    ...
    one. Mirrors the upsert in :func:`goalrail.cli._register_yaml_bundle`.
...
_EXTRA_BUILTIN_AGENTS_ENV = "GOALRAIL_BUILTIN_AGENT_DIRS"
...
def _build_opencode_native_bundle() -> bytes:
    ...
    from goalrail.opencode_native import _materialize_opencode_agent_spec
    from goalrail.spec import materialize_bundle
...
def _build_pi_native_bundle() -> bytes:
    ...
    from goalrail.pi_native import _materialize_pi_agent_spec
    from goalrail.spec import materialize_bundle
...
def _ensure_default_cursor_agent(
    ...
    ``goalrail cursor`` CLI first registers it). Content-aware via
...
def _build_antigravity_native_bundle() -> bytes:
    ...
    from goalrail.antigravity_native import _materialize_antigravity_agent_spec
    from goalrail.spec import materialize_bundle
...
def _build_qwen_native_bundle() -> bytes:
    ...
    from goalrail.qwen_native import _materialize_qwen_agent_spec
    from goalrail.spec import materialize_bundle
...
def _ensure_default_qwen_agent(
    ...
    built-in native-terminal agent on every deployment (not only after the
    ``goalrail qwen`` CLI first registers it). Content-aware via
    :func:`_ensure_builtin_agent`.
...
def _build_kimi_native_bundle() -> bytes:
    ...
    from goalrail.kimi_native import _materialize_kimi_agent_spec
    from goalrail.spec import materialize_bundle
...
def _ensure_default_kimi_native_agent(
    ...
    built-in native-terminal agent on every deployment (not only after the
    ``goalrail kimi`` CLI first registers it). Content-aware via
    :func:`_ensure_builtin_agent`.
...
def create_app(
    ...
    :param allowed_domains: Allowed email domains from the server
        ...
        Union'd with ``GOALRAIL_OIDC_ALLOWED_DOMAINS`` and the
    ...
    if auth_provider is not None and account_store is not None:
        ...
        if isinstance(auth_provider, UnifiedAuthProvider) and auth_provider._source == "accounts":
            from goalrail.server.accounts_bootstrap import bootstrap_admin
    ...
    from goalrail.server.host_registry import HostRegistry, RunnerExitReports
    ...
    async def _lifespan(
        ...
        ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §4.4.
        ...
        # Apply GOALRAIL_LOG_LEVEL to the goalrail namespace after
        # uvicorn's dictConfig runs (dictConfig resets existing handlers,
        # making a pre-run basicConfig call ineffective).
        import os as _os

        _log_level_name = _os.environ.get("GOALRAIL_LOG_LEVEL", "INFO").upper()
        logging.getLogger("goalrail").setLevel(getattr(logging, _log_level_name, logging.INFO))
        ...
        from goalrail.runtime import set_resource_registry
        ...
        from goalrail.server._runner_ws_tunnel import make_tunnel_ws_factory
        ...
        from goalrail.policies.registry import load_registry
        ...
        # passes GOALRAIL_ACCOUNTS_AUTO_OPEN=0 for --no-open). Broad
        # try so a missing display / browser never blocks startup.
        if _bootstrap_result is not None and _bootstrap_result.open_url:
            from goalrail.server.auth import env_var_is_truthy

            if env_var_is_truthy("GOALRAIL_ACCOUNTS_AUTO_OPEN", default=True):
                import webbrowser
        ...
        try:
            ...
            from goalrail.server.routes.sessions import cancel_managed_launch_tasks
    ...
    from goalrail.server.managed_hosts import ManagedLaunchTracker
    ...
    async def health(
        ...
        :class:`~goalrail.server.routes.sessions.SessionLiveness`.
    ...
    async def version() -> dict[str, str]:
        """
        Return the installed goalrail package version.
        ...
        return {"version": _pkg_version("goalrail")}

    @app.get("/v1/info")
    async def info() -> dict[str, bool | str | None]:
        """Runtime capabilities probe for the SPA + CLI.

        Returned at app boot by the frontend (and by ``goalrail
        ...
        booleans (``managed_sandboxes_enabled``), the short sandbox
        ...
        # server_version is the installed goalrail package version (same
        ...
        # a RoutingClient is explicitly configured (GOALRAIL_SMART_ROUTING=1
        ...
        try:
            from goalrail.runtime._globals import _caps
        ...
        return {
            ...
            "server_version": _pkg_version("goalrail"),
    ...
    async def _on_runner_connect(runner_id: str) -> None:
        ...
        """
        from goalrail.server.routes.sessions import _ensure_runner_relay
    ...
    if auth_provider is not None and getattr(auth_provider, "login_url", None):
        from goalrail.server.admin_list import load_admin_list
        from goalrail.server.auth import UnifiedAuthProvider
        ...
        if (
            ...
            from goalrail.server.routes.accounts_auth import (
            ...
            # OIDC invites are opt-in (GOALRAIL_OIDC_ALLOW_INVITES) and
    ...
    # built into ``goalrail/server/static/web-ui/`` by ``web/``'s Vite
