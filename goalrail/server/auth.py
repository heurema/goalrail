...
selected via the ``GOALRAIL_AUTH_PROVIDER`` env var:
...
- ``"header"`` (default): reads the ``X-Forwarded-Email`` header
  ...
  (``GOALRAIL_LOCAL_SINGLE_USER=1``), in which case they fall back
...
# Opt-in multi-user switch. ``GOALRAIL_AUTH_ENABLED`` is the current
# name; ``GOALRAIL_ACCOUNTS_ENABLED`` is the pre-rename name, still
# honored as a deprecated alias (see :func:`_auth_enabled`).
_AUTH_ENABLED_ENV = "GOALRAIL_AUTH_ENABLED"
_AUTH_ENABLED_ENV_DEPRECATED = "GOALRAIL_ACCOUNTS_ENABLED"
...
# paths (`goalrail run` in chat.py, the daemon's
# host/local_server.py) and by the canonical bare loopback
# `goalrail server` (cli.py) — never by deployed multi-user servers.
...
_LOCAL_SINGLE_USER_ENV = "GOALRAIL_LOCAL_SINGLE_USER"
...
_AUTH_HEADER_STRIP_PREFIX_ENV = "GOALRAIL_AUTH_HEADER_STRIP_PREFIX"
...
def local_single_user_enabled() -> bool:
    ...
    Reads ``GOALRAIL_LOCAL_SINGLE_USER``, the marker the managed
...
def resolve_auth_header() -> str:
    ...
    Reads ``GOALRAIL_AUTH_HEADER`` and falls back to
    ...
    e.g. ``GOALRAIL_AUTH_HEADER=Cf-Access-Authenticated-User-Email`` for
...
def resolve_auth_header_strip_prefix() -> str:
    ...
    Reads ``GOALRAIL_AUTH_HEADER_STRIP_PREFIX`` and returns it
    ...
    ``GOALRAIL_AUTH_HEADER=X-Goog-Authenticated-User-Email`` at IAP's
    identity header and set
    ``GOALRAIL_AUTH_HEADER_STRIP_PREFIX=accounts.google.com:`` so the
...
def _auth_enabled() -> bool:
    ...
    Reads ``GOALRAIL_AUTH_ENABLED``. The pre-rename name
    ``GOALRAIL_ACCOUNTS_ENABLED`` is still honored as a deprecated
    ...
    semantics — ``GOALRAIL_AUTH_ENABLED=0`` disables auth even though
...
def resolve_auth_source() -> str:
    ...
    - An explicit ``GOALRAIL_AUTH_PROVIDER`` (case-insensitive) always
      wins, e.g. ``"header"`` / ``"oidc"`` / ``"accounts"``. This is the
      low-level escape hatch.
    - Otherwise ``header`` is the default, unless the opt-in switch
      ``GOALRAIL_AUTH_ENABLED`` is truthy (see :func:`_auth_enabled`,
      which also honors the deprecated ``GOALRAIL_ACCOUNTS_ENABLED``
      alias). When enabled, the mode depends on whether OIDC config was
      supplied:

      - ``GOALRAIL_OIDC_ISSUER`` is set → ``"oidc"`` (the operator
    ...
    :returns: The resolved source string, e.g. ``"accounts"``,
        ...
        ``GOALRAIL_AUTH_PROVIDER``). The caller is responsible for
        rejecting unknown values.
    """
    raw_source = os.environ.get("GOALRAIL_AUTH_PROVIDER")
    ...
    if _auth_enabled():
        ...
        if os.environ.get("GOALRAIL_OIDC_ISSUER", "").strip():
            return "oidc"
        return "accounts"
    return "header"
...
class UnifiedAuthProvider(AuthProvider):
    ...
    ``GOALRAIL_AUTH_PROVIDER``. OIDC and accounts modes share
    ...
    :param local_single_user: When ``True``, header mode falls back
        ...
        ``GOALRAIL_LOCAL_SINGLE_USER`` at construction (see
    ...
    :param header_name: The trusted identity header read in header
        ...
        ``GOALRAIL_AUTH_HEADER`` at construction, falling back to
    ...
    :param header_strip_prefix: A prefix stripped from the identity
        ...
        ``GOALRAIL_AUTH_HEADER_STRIP_PREFIX`` at construction, falling
    ...
    def _check_cookie(self, request: HTTPConnection) -> str | None:
        ...
        authenticated via ``goalrail login``). Both carry the same
        HS256-signed JWT.
        ...
        from goalrail.server.oidc import hmac_digest
    ...
    def _check_header(self, request: HTTPConnection) -> str | None:
        ...
        by default, overridable via ``GOALRAIL_AUTH_HEADER`` — e.g.
        ``Cf-Access-Authenticated-User-Email`` for Cloudflare Access).

        When :attr:`_header_strip_prefix` is set (from
        ``GOALRAIL_AUTH_HEADER_STRIP_PREFIX``), it is removed from the
        ...
        ``GOALRAIL_LOCAL_SINGLE_USER=1``): there the absent header
...
def create_auth_provider() -> AuthProvider:
    ...
    -----------------------------------
    Set ``GOALRAIL_AUTH_ENABLED=1`` (or any truthy value) to turn on
    multi-user auth. With no OIDC config present this selects
    ``accounts`` mode — the built-in login flow with
    first-user-is-admin setup. Set the ``GOALRAIL_OIDC_*`` env vars
    (at minimum ``GOALRAIL_OIDC_ISSUER``) alongside it and the same
    ...
    (``GOALRAIL_AUTH_ENABLED`` is the renamed opt-in gate,
    commit ``b23e886e``, formerly ``GOALRAIL_ACCOUNTS_ENABLED``:
    ...
    if source not in ("header", "oidc", "accounts"):
        raise RuntimeError(
            f"Unknown GOALRAIL_AUTH_PROVIDER={source!r}. Valid: 'header', 'oidc', 'accounts'"
        )
    ...
    if source == "oidc":
        ...
        # — either GOALRAIL_AUTH_PROVIDER=accounts or the
        # GOALRAIL_AUTH_ENABLED=1 opt-in without OIDC config
