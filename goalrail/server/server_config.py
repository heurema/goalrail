...
from goalrail.server.admin_list import resolve_data_dir
...
def resolve_config_path() -> Path | None:
    ...
    :returns: ``GOALRAIL_CONFIG`` if set; else ``<data_dir>/config.yaml``
        when that file exists; else ``None``.
    """
    explicit = os.environ.get("GOALRAIL_CONFIG", "").strip()
    if explicit:
