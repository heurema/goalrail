...
A user-set ``GOALRAIL_ACCOUNTS_COOKIE_SECRET`` always wins —
...
Caller pattern:
    ...
    os.environ.setdefault("GOALRAIL_ACCOUNTS_COOKIE_SECRET", secret_hex)
...
def load_or_generate_cookie_secret(data_dir: str | os.PathLike[str]) -> str:
    ...
    1. ``GOALRAIL_ACCOUNTS_COOKIE_SECRET`` env var — operator-set,
    ...
    """
    env_value = os.environ.get("GOALRAIL_ACCOUNTS_COOKIE_SECRET")
    if env_value:
        return env_value
    ...
    logger.info(
        ...
        "(set GOALRAIL_ACCOUNTS_COOKIE_SECRET to share across instances)",
        path,
    )
    return fresh
