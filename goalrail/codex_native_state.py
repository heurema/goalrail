"""Persistent client-side state for ``goalrail codex`` sessions.

The native Codex wrapper records the cwd used to create a session so a
later ``goalrail codex --resume <conv_id>`` can launch Codex from the
...
_STATE_ROOT_ENV_VAR = "GOALRAIL_CODEX_NATIVE_STATE_DIR"
