...
# codex-native ``goalrail.codex_native.bridge_id`` label.
OPENCODE_NATIVE_BRIDGE_ID_LABEL_KEY = "goalrail.opencode_native.bridge_id"
...
# Token file the shared ``goalrail.claude_native_bridge serve-mcp`` reads to
...
# Goalrail builtin tools (``sys_*``/``load_skill``/``web_fetch``) advertised in
...
# the elicitation against the Goalrail server (mirrors codex-native's
# ``policy_hook.json``; consumed by ``goalrail.native_cost_popup``).
_COST_POPUP_CONFIG_FILE = "cost_popup.json"
# Filename of the opencode plugin that bridges opencode's lifecycle hooks to the
# Goalrail policy engine (REQUEST + TOOL_RESULT phases the reactive
# ``permission.asked`` path can't reach).
_POLICY_PLUGIN_FILE = "goalrail-policy.js"
...
# function exports as plugins (legacy shape). It reads its Goalrail coordinates
...
// Goalrail policy bridge for opencode-native (generated; do not edit).
// Forwards opencode lifecycle hooks to the Goalrail policy engine so
...
const BASE = (process.env.GOALRAIL_POLICY_URL || "").replace(/\/+$/, "");
const SESSION = process.env.GOALRAIL_SESSION_ID || "";
const AUTH = process.env.GOALRAIL_POLICY_AUTH || "";
...
export const GoalrailPolicyPlugin = async () => ({
  ...
  "chat.message": async (_input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      ...
      throw new Error(
        "Goalrail policy blocked this prompt: " + (verdict.reason || "request denied"),
      );
    }
  },
  ...
  "tool.execute.after": async (input, output) => {
    ...
    if (verdict.result === "POLICY_ACTION_DENY") {
      output.output = "[Goalrail policy withheld this tool result: " +
...
def write_opencode_policy_plugin(bridge_dir: Path) -> Path:
    """
    Write the Goalrail policy-bridge plugin into *bridge_dir* and return its path.

    The runner registers the returned path in the synthesized ``opencode.json``
    ``plugin`` field and stamps ``GOALRAIL_POLICY_URL`` / ``GOALRAIL_SESSION_ID``
    / ``GOALRAIL_POLICY_AUTH`` on the ``opencode serve`` process so the plugin
...
def write_relay_bridge_config(bridge_dir: Path) -> None:
    ...
    The shared ``goalrail.claude_native_bridge serve-mcp`` stdio server (which
...
def write_cost_popup_config(
    ...
    ``goalrail.native_cost_popup`` subprocess inside a ``tmux display-popup`` on
    the opencode pane; it must POST the verdict to the Goalrail server but cannot
    ...
    :param bridge_dir: OpenCode-native bridge directory.
    :param ap_server_url: Goalrail server base URL, e.g. ``"http://127.0.0.1:6767"``.
...
def update_model_override(bridge_dir: Path, model_override: str | None) -> bool:
    """
    Persist a new per-session model override (Goalrail→opencode model switch).
