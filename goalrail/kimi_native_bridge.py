...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"goalrail-{os.getuid()}" / "kimi-native"
...
# read by :mod:`goalrail.kimi_native_hook` (PreToolUse deny-gate + the
...
_PASTE_BUFFER = "goalrail-kimi-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/goalrail-<uid>/kimi-native/<hash>``."""
...
def write_hook_config(
    ...
    *,
    ...
    :func:`goalrail.claude_native_bridge` ``permission_hook.json`` plumbing.
