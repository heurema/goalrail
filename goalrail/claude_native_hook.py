"""Claude Code hook recorder for the native Goalrail wrapper."""
...
# budget — an unresponsive Goalrail server would otherwise hang the banner.
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"goalrail claude hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("goalrail claude hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    if payload.get("hook_event_name") == "SessionStart" and payload.get("source") == "clear":
        ...
        if rotated_session_id:
            payload["goalrail_clear_rotated_to"] = rotated_session_id
    elif _is_claude_branch_session_start(payload):
        payload["goalrail_fork_detected"] = True
        rotated_session_id = _rotate_session_on_fork(bridge_dir)
        if rotated_session_id:
            payload["goalrail_fork_rotated_to"] = rotated_session_id
    try:
        record_hook_event(bridge_dir, payload)
    except Exception as exc:  # noqa: BLE001 - hook must not break Claude Code.
        print(f"goalrail claude hook: failed to record hook: {exc}", file=sys.stderr)
...
def _annotate_resume_session_context(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    if (
        ...
        payload["goalrail_previous_claude_session_id"] = current_claude_session_id
    payload["goalrail_claude_session_was_seen"] = new_claude_session_id in seen
...
def _is_claude_branch_session_start(payload: dict[str, object]) -> bool:
    ...
    :param payload: Hook payload read from Claude Code stdin, e.g.
        ``{"hook_event_name": "SessionStart", "source": "resume"}``.
    :returns: ``True`` when the event should fork the active Goalrail session.
    """
    if payload.get("hook_event_name") != "SessionStart" or payload.get("source") != "resume":
        return False
    new_claude_session_id = payload.get("session_id")
    if not isinstance(new_claude_session_id, str) or not new_claude_session_id:
        return False
    current_claude_session_id = payload.get("goalrail_previous_claude_session_id")
    if not isinstance(current_claude_session_id, str) or not current_claude_session_id:
        return False
    if payload.get("goalrail_claude_session_was_seen") is True:
        return False
...
def _rotate_session_on_clear(bridge_dir: Path) -> str | None:
    """
    Rotate Goalrail sessions synchronously for a Claude ``/clear`` SessionStart.

    The SessionStart hook output is what Claude renders as the welcome
    banner. Rotating here lets the banner point at the new Goalrail session
    ...
    :param bridge_dir: Native Claude bridge directory.
    :returns: New Goalrail session id, e.g. ``"conv_new"``, or ``None`` when
    ...
    try:
        ...
        print(f"goalrail claude clear hook: Goalrail rotation failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"goalrail claude clear hook: rotation failed: {exc}", file=sys.stderr)
        return None
...
def _rotate_session_on_fork(bridge_dir: Path) -> str | None:
    """
    Fork Goalrail sessions synchronously for a Claude ``/fork``/``/branch``.
    ...
    forked Goalrail session before Claude prints it. Failures return
    ``None`` so the background forwarder can perform the fork from the
    annotated hook record.

    :param bridge_dir: Native Claude bridge directory.
    :returns: New Goalrail session id, e.g. ``"conv_fork"``, or ``None``
    ...
    try:
        ...
        print(f"goalrail claude fork hook: Goalrail fork failed: {exc}", file=sys.stderr)
        return None
    except RuntimeError as exc:
        print(f"goalrail claude fork hook: fork failed: {exc}", file=sys.stderr)
        return None
    return new_session_id
...
def _create_clear_replacement_session(
    ...
    """
    Create and activate the fresh Goalrail session for ``/clear``.

    :param client: Sync Goalrail HTTP client.
    :param ap_server_url: Goalrail server base URL without a trailing slash,
    ...
    :raises httpx.HTTPError: If Goalrail rejects session creation,
        new-session binding, or terminal transfer.
    :raises RuntimeError: If Goalrail returns malformed session data.
    """
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "goalrail claude clear hook: failed to clear old runner binding: "
...
def _create_fork_replacement_session(
    ...
    """
    Create and activate the forked Goalrail session for Claude ``/fork``.
    ...
    if clear_resp.status_code >= 400:
        print(
            (
                "goalrail claude fork hook: failed to clear old runner binding: "
                f"{clear_resp.status_code} {clear_resp.text}"
            ),
            file=sys.stderr,
        )
...
def _conversation_url_for_active_session(
    ...
    """
    Build the web URL for the bridge's current active Goalrail session.
    ...
    if isinstance(ap_server_url, str) and ap_server_url and session_id:
        ...
        # builder so workspace-hosted servers land on the ``/goalrail``
...
def _build_reauth(
    ...
    """
    Build a callable that re-mints the Goalrail bearer for ``ap_server_url``.
    ...
    which dies once the stored OIDC token expires. When a later hook POST is
    bounced to a login-flow redirect (302→``/oidc/``) or returns ``401``, this
    callable re-mints a fresh bearer through the SAME factory the runner's
    refresh-capable ``_RunnerBearerAuth`` uses
    (:func:`goalrail.runner._entry._make_auth_token_factory`), merged over the
    other headers so neither half is dropped. Best-effort: returns ``None``
    when no refresh mechanism is available (local unauthenticated servers,
    import failure, or a transient mint failure), letting the caller fail
    closed.

    :param ap_server_url: Goalrail server base URL the hook POSTs to — also
        the key the token factory uses to resolve the stored OIDC token.
    ...
    def _reauth() -> dict[str, str] | None:
        ...
        # per-tool-call hot path free of the runner import.
        try:
        ...
        try:
            token = factory()
        except Exception:  # noqa: BLE001 — transient refresh failure; fail closed
            return None
        if not token:
            return None
...
def _post_hook_with_reattach(
    ...
    ``_goalrail_elicitation_id`` so the server re-parks the SAME
    ...
    :param headers: Outbound auth headers for the Goalrail server.
    ...
    body = {
        **payload,
        "_goalrail_elicitation_id": f"elicit_claude_{secrets.token_hex(16)}",
    }
    ...
    while True:
        try:
            with httpx.Client(headers=headers, timeout=timeout) as client:
                ...
                if (
                    ...
                    # One-shot ``ap_auth_headers`` token lapsed: re-mint and
                    # retry once rather than fail-asking into a terminal
                    # prompt no one watches. Mirrors the evaluate-policy hook
                    # and ``_RunnerBearerAuth``.
                    refreshed = reauth()
                    if refreshed:
                        headers = refreshed
                        reauthed = True
                        print(
                            f"goalrail {hook_label} hook: Goalrail auth expired "
            ...
            if exc.response.status_code < 500:
                print(
                    f"goalrail {hook_label} hook: Goalrail request rejected: {exc}",
                    file=sys.stderr,
                )
                return None
            print(
                f"goalrail {hook_label} hook: Goalrail request failed; retrying: {exc}",
                file=sys.stderr,
            )
        except httpx.HTTPError as exc:
            print(
        ...
        if time.monotonic() + backoff_s >= deadline:
            print(
                f"goalrail {hook_label} hook: retry budget exhausted",
                file=sys.stderr,
            )
            return None
...
def _main_permission_request(argv: list[str]) -> int:
    """
    Forward one Claude ``PermissionRequest`` hook to the active Goalrail session.

    :param argv: CLI argv after the ``permission-request`` subcommand,
        e.g. ``["--bridge-dir", "/tmp/x", "--goalrail-server-url",
        "http://127.0.0.1:8787"]``.
    ...
    try:
        ...
        print(f"goalrail claude permission hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("goalrail claude permission hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("goalrail claude permission hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.goalrail_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("goalrail claude permission hook: Goalrail server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.goalrail_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
        if isinstance(raw_headers, dict):
...
def _main_ask_user_question(argv: list[str]) -> int:
    ...
    in all permission modes), POSTs the same payload to the Goalrail server's
    ...
    try:
        ...
        print(f"goalrail ask-user-question hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("goalrail ask-user-question hook: expected JSON object", file=sys.stderr)
        return 0
    ...
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
        print("goalrail ask-user-question hook: active session missing", file=sys.stderr)
        return 0
    config = read_permission_hook_config(bridge_dir)
    ap_server_url = args.goalrail_server_url or config.get("ap_server_url")
    if not isinstance(ap_server_url, str) or not ap_server_url:
        print("goalrail ask-user-question hook: Goalrail server URL missing", file=sys.stderr)
        return 0
    headers = _parse_headers(args.goalrail_auth_headers_json)
    if not headers:
        raw_headers = config.get("ap_auth_headers")
    ...
    if resp is None or not resp.content:
        return 0
    # The Goalrail server returns a PermissionRequest-shaped response:
    ...
    try:
        ...
        print(f"goalrail ask-user-question hook: invalid JSON from AP: {exc}", file=sys.stderr)
        return 0
    decision = (
...
def _main_evaluate_policy(argv: list[str]) -> int:
    ...
    :func:`goalrail.native_policy_hook.evaluation_response_to_hook_output`.
    ...
    ``mcp__goalrail__*`` tool already gated on the relay path — still
    return exit 0 with no output ("no opinion") so non-Goalrail tool
    ...
    try:
        ...
        print(f"goalrail evaluate-policy hook: malformed JSON: {exc}", file=sys.stderr)
        return 0
    if not isinstance(payload, dict):
        print("goalrail evaluate-policy hook: expected JSON object", file=sys.stderr)
        return 0
    bridge_dir = Path(args.bridge_dir)
    session_id = read_active_session_id(bridge_dir)
    if not session_id:
    ...
    if resp is None:
        return _fail_closed()
    if not resp.content:
        print("goalrail evaluate-policy hook: empty Goalrail response", file=sys.stderr)
        return _fail_closed()

    try:
        eval_response = resp.json()
    except json.JSONDecodeError:
        print("goalrail evaluate-policy hook: malformed Goalrail response", file=sys.stderr)
        return _fail_closed()
...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m goalrail.claude_native_hook")
...
def _parse_evaluate_policy_args(argv: list[str]) -> argparse.Namespace:
    ...
    :returns: Parsed argparse namespace.
    """
    parser = argparse.ArgumentParser(prog="python -m goalrail.claude_native_hook evaluate-policy")
    parser.add_argument("--bridge-dir", required=True)
    return parser.parse_args(argv)
...
def _parse_permission_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        prog="python -m goalrail.claude_native_hook permission-request"
    )
    parser.add_argument("--bridge-dir", required=True)
    parser.add_argument("--goalrail-server-url")
    parser.add_argument("--goalrail-auth-headers-json")
    return parser.parse_args(argv)


def _parse_headers(raw: str | None) -> dict[str, str]:
    """
    Parse serialized Goalrail auth headers for the permission hook.
    ...
    try:
        ...
        print(f"goalrail claude permission hook: bad headers JSON: {exc}", file=sys.stderr)
        return {}
