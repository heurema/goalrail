...
from goalrail.llms.adapters.base import BaseAdapter
...
def get_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    :raises GoalrailError: If the provider is not supported.
    """
...
def _create_adapter(provider: str, **kwargs: Any) -> BaseAdapter:
    ...
    if provider in openai_compat_providers:
        ...
        from goalrail.llms.adapters.openai import OpenAICompatibleAdapter

        return OpenAICompatibleAdapter(base_url=resolved_url)

    if provider == "anthropic":
        from goalrail.llms.adapters.anthropic import AnthropicAdapter
    ...
    if provider == "bedrock":
        from goalrail.llms.adapters.bedrock import BedrockAdapter
