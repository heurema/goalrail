...
# Goalrail's authoritative context-window registry, consulted BEFORE litellm
...
def _registry_context_window(model: str) -> int | None:
    """Resolve a model's context window from Goalrail's authoritative registry.
    ...
    # (covers ``claude-*``); other providers fall through to litellm/catalog
    # rather than being forced to 1M.
...
def _fetch_context_window_from_mlflow(model: str) -> int | None:
    ...
    """
    if os.environ.get("GOALRAIL_DISABLE_CATALOG_LOOKUP") == "1":
        return None
...
def get_model_context_window(model: str) -> int:
    ...
    2. :func:`_registry_context_window` — Goalrail's own authoritative
    ...
    3. ``litellm.get_model_info()`` — fast, local, no network.
