...
Usage::
    ...
    from goalrail.llms import Client
...
def __getattr__(name: str) -> Any:
    ...
    Eager top-level imports here pull in ``goalrail.llms.client``,
    which imports ``goalrail.reasoning_effort``, which imports
    ``goalrail.llms.errors`` -- a submodule load that re-triggers
    ...
    raise AttributeError(f"module 'goalrail.llms' has no attribute {name!r}")
