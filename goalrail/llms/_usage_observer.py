...
2. **Auto-recorder** that activates whenever
   ``GOALRAIL_TOKEN_USAGE_JSON=<path>`` is set in the environment.
...
- :meth:`goalrail.llms.client._ResponsesNamespace.create` (in-process
  LLM client).
...
runner, harness) inherit both ``GOALRAIL_TOKEN_USAGE_JSON`` and
...
_ENV_VAR = "GOALRAIL_TOKEN_USAGE_JSON"
...
def _current_test_path() -> Path | None:
    ...
    Derived from the ``GOALRAIL_TOKEN_USAGE_JSON`` base path plus the
