"""Persistent client-side state for ``goalrail claude`` sessions.
...
``/tmp/goalrail-<uid>/claude-native/``: tmpfs gets cleared on
...
defaults to ``~/.goalrail/`` during the rename and can be overridden
...
_STATE_ROOT_ENV_VAR = "GOALRAIL_CLAUDE_NATIVE_STATE_DIR"
...
# :mod:`goalrail.claude_native_bridge`.
_ID_HASH_CHARS = 32
