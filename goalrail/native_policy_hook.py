...
entrypoints (:mod:`goalrail.claude_native_hook`,
:mod:`goalrail.codex_native_hook`) share one implementation.
...
def hook_payload_to_evaluation_request(
    ...
    (``mcp__goalrail__*``) are skipped because they are already
    ...
    :returns: An ``EvaluationRequest`` dict suitable for POSTing to
        ...
        policy-relevant (unknown event or an ``mcp__goalrail__*`` tool).
    """
    ...
    if isinstance(tool_name, str) and tool_name.startswith("mcp__goalrail__"):
        return None
...
def fail_closed_hook_output(hook_event: str) -> dict[str, object] | None:
    ...
    :data:`goalrail.policies.types.FAIL_CLOSED_PHASES` (the runner-side
...
def post_evaluate_with_retry(
    ...
    A stable ``_goalrail_elicitation_id`` is minted once and stamped on
    ...
    request_body = {**eval_request, "_goalrail_elicitation_id": elicitation_id}
    ...
    while True:
        try:
            ...
            if exc.response.status_code < 500:
                ...
                print(
                    f"goalrail {hook_label}: Goalrail returned {exc.response.status_code}"
                    + (f": {body_preview}" if body_preview else ""),
                    file=sys.stderr,
                )
                return None
            print(
                f"goalrail {hook_label}: Goalrail returned {exc.response.status_code}; retrying",
                file=sys.stderr,
            )
        except (httpx.ConnectError, httpx.ConnectTimeout) as exc:
            print(
                f"goalrail {hook_label}: Goalrail request failed; retrying: {exc}",
                file=sys.stderr,
            )
            ...
            print(
                f"goalrail {hook_label}: Goalrail request failed: {exc}",
                file=sys.stderr,
            )
            return None
        if time.monotonic() + backoff_s >= deadline:
            print(
                f"goalrail {hook_label}: retry budget exhausted",
                file=sys.stderr,
            )
            return None
