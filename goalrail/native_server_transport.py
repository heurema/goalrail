...
(:class:`goalrail.opencode_http_transport.OpenCodeHttpTransport`);
:class:`NativeServerTransport` is the seam so the orchestration in
:class:`goalrail.native_server_harness.NativeServerHarness` stays
protocol-agnostic.
"""
...
class NativeLaunchConfig:
    ...
    :param goalrail_session_id: Goalrail conversation id, e.g.
        ``"conv_abc123"``.
...
class NativeServerTransport(Protocol):
    ...
    :class:`~goalrail.native_server_harness.NativeServerHarness` calls only
    these methods.
    """
