...
from goalrail._wrapper_labels import WRAPPER_LABEL_KEY as _WRAPPER_LABEL_KEY
...
from goalrail.codex_native_state import read_launch_state, write_launch_state
...
def _resolve_codex_auth_source() -> _CodexAuthSource:
    ...
    This is the seam for future managed credentials: once Goalrail can provide
    ...
    inherited private Goalrail homes map back to the user's real Codex home.

    :returns: Local Codex auth source to inspect synchronously.
    """
    from goalrail.inner.codex_executor import _codex_home_config_source_from_env
...
def run_codex_native(
    *,
    ...
    with TemporaryDirectory(prefix="goalrail-codex-native-") as tmpdir:
...
def _materialize_codex_agent_spec(
    ...
    *,
    ...
    """
    Write the terminal-first agent spec used by ``goalrail codex``.
        ...
        try:
            ...
            if thread_id is None:
                event_client = client_for_transport(
                    codex_ws_url,
                    client_name="goalrail-codex-native",
                )
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> None:
    ...
    environment so users who run ``goalrail codex`` inside their own
...
def _find_codex_rollout(codex_home: Path, thread_id: str) -> Path | None:
    ...
    :param codex_home: A per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.goalrail/codex-native/<hash>/codex-home")``.
...
def _clone_codex_rollout(
    ...
    *clone_workspace* (see :func:`_copy_rollout_with_cwd`), preserving the
    ...
    :func:`goalrail.claude_native._clone_claude_transcript`. See
    ...
    :param clone_codex_home: The clone's per-session private ``CODEX_HOME``,
        e.g. ``Path("~/.goalrail/codex-native/<hash>/codex-home")``.
...
async def _ensure_local_codex_resume_rollout(
    ...
    *,
    ...
    if codex_path is not None:
        from goalrail.inner.codex_executor import _codex_cli_version
...
def _codex_resume_rollout_path(codex_home: Path, external_session_id: str) -> Path:
    ...
    :param codex_home: Per-session private ``CODEX_HOME``, e.g.
        ``Path("~/.goalrail/codex-native/x/codex-home")``.
...
def _codex_turn_id_for_session_item(
    *,
    ...
    return (
        ...
        + uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"goalrail-codex-resume:{session_id}:{external_session_id}:{stable}",
        ).hex
    )
...
async def _wait_for_thread_started(client: CodexAppServerClient) -> str:
    ...
    :func:`goalrail.codex_native_forwarder.wait_for_thread_started`
    ...
    """
    from goalrail.codex_native_forwarder import wait_for_thread_started
...
async def _start_initial_turn(app_server_url: str, thread_id: str, prompt: str) -> None:
    ...
    :returns: None.
    """
    client = client_for_transport(app_server_url, client_name="goalrail-codex-native")
