...
import goalrail.onboarding.gemini_auth as _gemini_auth
from goalrail.harness_aliases import HARNESS_ALIASES, canonicalize_harness
...
# ``goalrail.onboarding.gemini_auth.gemini_login_detected`` and have the patch
...
# Native Cursor harnesses. These boot the ``cursor-agent`` TUI (``goalrail cursor``)
...
# Native Goose harnesses. Boot the ``goose session`` TUI (``goalrail goose``) and
...
# Native Kimi TUI harnesses (``goalrail kimi``). Like the other native CLIs,
...
# Native Hermes harnesses. Boot the ``hermes`` TUI (``goalrail hermes``) and can't
...
# ``qwen-native`` / ``native-qwen`` (the native TUI via ``goalrail qwen``) all resolve
...
def _install_key(canonical: str) -> str:
    ...
    :returns: ``"anthropic"`` / ``"openai"`` for the claude/codex CLIs,
        :data:`~goalrail.onboarding.harness_install.KIMI_KEY` for kimi,
        :data:`~goalrail.onboarding.harness_install.OPENCODE_KEY` for
        opencode-native,
        :data:`~goalrail.onboarding.harness_install.QWEN_KEY` for qwen, or
        :data:`~goalrail.onboarding.harness_install.PI_KEY` for pi.
    """
...
def harness_is_configured(harness: str) -> bool:
    ...
    if canonical in _CURSOR_NATIVE_HARNESSES:
        # Native Cursor (``goalrail cursor``) wraps the ``cursor-agent`` CLI — gate
    ...
    if canonical in _GOOSE_NATIVE_HARNESSES or canonical == GOOSE_KEY:
        ...
        # ``goalrail goose``) and the headless ACP harness (``goose``, drives
    ...
    if canonical in _HERMES_NATIVE_HARNESSES or canonical == HERMES_KEY:
        ...
        # via ``goalrail hermes``) and the headless subprocess harness (``hermes``)
    ...
    if canonical == CURSOR_KEY:
        ...
        # (the ``cursor:`` block — see :mod:`goalrail.onboarding.cursor_auth`)
        ...
        # (:mod:`goalrail.inner.cursor_executor`); gating here would only
        # duplicate that, less actionably. So cursor keeps its single key check.
        from goalrail.onboarding.cursor_auth import cursor_api_key_configured
    ...
    if canonical == COPILOT_KEY:
        ...
        # block — see :mod:`goalrail.onboarding.copilot_auth`) or inherited from
...
def _harness_availability(canonical: str) -> HarnessAvailability:
    ...
    if (
        ...
        from goalrail.codex_native import _codex_auth_unavailable_reason
