"""OpenCode readiness + credential reporting for ``goalrail setup``.

Like :mod:`goalrail.onboarding.goose_auth`, Goalrail stores **no** OpenCode
...
read-only reporter so ``goalrail setup`` can show which providers OpenCode can
...
from goalrail.onboarding.harness_install import OPENCODE_KEY, harness_cli_installed
