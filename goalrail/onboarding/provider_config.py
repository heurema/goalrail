...
from goalrail._env_compat import config_home_path
...
# :mod:`goalrail.onboarding.gemini_auth`, not a ``gemini:`` family here).
...
_HARNESS_FAMILY: dict[str, str] = {
    ...
    # Native CLI harnesses. Specs (and the goalrail-compat translator)
    ...
    # (file-based, checked in :mod:`goalrail.onboarding.gemini_auth`) and the
...
class FamilyConfig:
    ...
    :param api_key_ref: A reference to a secret stored outside the config
        ...
        ``"keychain:<name>"`` (read from the goalrail secret store — see
        :func:`resolve_secret` and :mod:`goalrail.onboarding.secrets`),
...
class ProviderEntry:
    ...
    def family(self, name: str) -> FamilyConfig | None:
        ...
        :raises GoalrailError: If the family references an unset
...
def resolve_secret(ref: str) -> str:
    ...
    :raises GoalrailError: If an ``env:`` / ``$VAR`` reference names an
...
def _expand(key: str, value: str) -> str:
    ...
    :raises GoalrailError: If a referenced variable is unset.
    """
...
def _expand_family(provider_name: str, family_name: str, family: FamilyConfig) -> FamilyConfig:
    ...
    :returns: A new :class:`FamilyConfig` with ``base_url`` expanded and,
        ...
        ``api_key_ref`` cleared).
    :raises GoalrailError: If a referenced environment variable is unset,
...
def _parse_family(provider_name: str, family_name: str, raw: dict[str, object]) -> FamilyConfig:
    ...
    :raises GoalrailError: If ``base_url`` is missing, the credential
...
def _parse_default_families(
    ...
    :raises GoalrailError: If ``default`` is an unsupported type, or names
...
def _parse_provider(name: str, raw: dict[str, object]) -> ProviderEntry:
    ...
    :raises GoalrailError: If ``kind`` is missing/invalid or the
    ...
    # goalrail/inner/loader.py), so a YAML ``default: true`` arrives as
...
def load_config() -> dict[str, object]:
    """Load the global ``~/.goalrail/config.yaml`` mapping.
    ...
    ``$GOALRAIL_CONFIG_HOME`` for test isolation, exactly like the rest
...
def get_default_provider(config: dict[str, object], family: str) -> ProviderEntry | None:
    ...
    :raises GoalrailError: If any provider is malformed, or more than one
...
def default_provider_for_harness(config: dict[str, object], harness: str) -> ProviderEntry | None:
    ...
    harness can't read, and ``bedrock`` is native-``goalrail claude`` only.
    ...
    :raises GoalrailError: If a provider is malformed, or more than one
    ...
    for fam in _PI_FALLBACK_FAMILIES:
        ...
        # wrap; a bedrock provider is native-``goalrail claude`` only
...
def _source_descriptor(family: FamilyConfig) -> str:
    ...
    :raises GoalrailError: If the family has no credential source (should
...
def describe_active_credential(
    ...
    :raises GoalrailError: If the resolved provider is malformed, or more
...
def set_default_provider(
    ...
    :raises GoalrailError: If *name* is absent, an entry is malformed, or
    ...
    if not isinstance(target_raw, dict):
        raise GoalrailError(f"provider {name!r} must be a mapping.", code=ErrorCode.INVALID_INPUT)
...
def provider_entry_settings(
    ...
    ``~/.goalrail/config.yaml`` under ``providers:``. When *make_default*
