"""Hermes readiness + config reporting for ``goalrail setup``.

Like :mod:`goalrail.onboarding.goose_auth`, Goalrail manages **no** Hermes
...
provider/model so ``goalrail setup`` can show Hermes as ready (and which model
...
:func:`goalrail.hermes_native_bridge._load_user_hermes_config`. A fresh install
...
from goalrail.onboarding.harness_install import HERMES_KEY, harness_cli_installed
