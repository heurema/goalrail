---
name: build-goalrail
description: Patterns and templates for generating valid Goalrail agent directories. Load when ready to create files.
---
...
that `goalrail server` uses. If your environment exposes the
...
**`goalrail-knowledge`** skill if you need the deeper field reference
...
- `guardrails` — runtime policy gates (see `goalrail-knowledge`).

## Step 2a: Choose an executor

`executor.type` must be one of **`claude_sdk`**, **`agents_sdk`**, or
**`goalrail`**. There is **no `llm` executor** — do not use it.
...
| A CLI/coding harness, shell + file tools, sub-agents | `goalrail` + a `config.harness` |

When `executor.type: goalrail`, **`config.harness` is required** and must
...
Use the `goalrail` executor with a coding harness when the agent needs to
...
Sub-agents need the `goalrail` executor (it provides the spawn tools).
...
- [ ] `executor.type` is one of `claude_sdk`, `agents_sdk`, `goalrail`
- [ ] If `executor.type: goalrail`, `executor.config.harness` is set to a
      valid harness
- [ ] `instructions` (or `prompt`) points to a file that exists or is
      inline text
- [ ] When declaring `tools.agents`, the parent uses `executor.type:
      goalrail`, and each entry has a matching `agents/` subdirectory
