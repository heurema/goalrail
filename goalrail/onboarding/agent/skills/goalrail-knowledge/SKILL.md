...
executor:
  # REQUIRED area. type must be one of: claude_sdk | agents_sdk | goalrail.
  ...
  # type: goalrail     — subprocess harness; requires config.harness below

  # Only for type: goalrail — pick the harness that runs the loop.
...
| `goalrail` | Coding/CLI harnesses, shell + file tools, sub-agents | Spawns a subprocess harness selected by `config.harness` |

**There is no `llm` executor type** — the only valid values are
`claude_sdk`, `agents_sdk`, and `goalrail`. For **most new simple agents**,
use `claude_sdk` (or `agents_sdk`) — in-process, no extra config. Use
`goalrail` when the agent needs a specific harness, shell/file access, or
...
from goalrail.tools import tool
...
`executor.type: goalrail` — that's what provides the spawn tools. Each
sub-agent is a full agent and may use any executor (`claude_sdk`,
`agents_sdk`, or `goalrail`).
...
goalrail server --agent ./my-agent/

# Or start an interactive session with the agent
goalrail run ./my-agent/
```
