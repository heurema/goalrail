"""Goose readiness + config reporting for ``goalrail setup``.

Unlike :mod:`goalrail.onboarding.cursor_auth`, Goalrail manages **no** Goose
...
from goalrail.onboarding.harness_install import GOOSE_KEY, harness_cli_installed

#: Override for the goose binary path (mirrors goalrail.goose_native).
...
def _goose_binary() -> str | None:
    """Resolve the goose executable (``GOALRAIL_GOOSE_PATH`` override, else PATH)."""
