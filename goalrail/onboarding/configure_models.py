...
- the ``/model`` REPL readout/switch (in :mod:`goalrail.repl._repl`).
...
:mod:`goalrail.cli`; this module owns the look/feel and the
...
# see goalrail.inner.banner._display_width.)
...
# :func:`goalrail.onboarding.providers.get_all_providers`) to the
# goalrail family it serves for a ``key`` provider. Anthropic serves the
# Claude (anthropic) surface; OpenAI and OpenAI-compatible vendors serve
# the Codex (openai) surface. Providers absent here have no goalrail
...
def key_providers() -> list[str]:
    ...
    goalrail family. Only providers in :data:`_CATALOG_PROVIDER_FAMILY`
...
def add_menu_options() -> list[AddOption]:
    ...
    return [
        ...
        # native ``goalrail claude`` terminal in Bedrock mode. Listed last so it
        ...
        _opt(
            ...
            "terminal (goalrail claude). Claude only.",
            BEDROCK_KIND,
        ),
    ]
...
def render_provider_listing_by_harness(
    ...
    several surfaces is listed under each. Used by ``goalrail config list``.
    ...
    :param providers: Parsed provider entries keyed by name (from
        :func:`~goalrail.onboarding.provider_config.load_providers`).
    :returns: None. Side effect: writes the listing to the shared console.
    """
    from goalrail.onboarding.provider_config import provider_families
...
def render_provider_listing(
    ...
    Prints (via the shared :data:`~goalrail.onboarding.interactive.console`)
    ...
    :param detected: Ambient detections from
        :func:`~goalrail.onboarding.ambient.detect_providers`; entries
...
def build_bedrock_provider_entry(
    ...
    native ``goalrail claude`` terminal in AWS Bedrock mode (the in-process /
