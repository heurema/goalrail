...
``0600`` JSON file; see :mod:`goalrail.onboarding.secrets`) — and referenced
from a dedicated top-level ``cursor:`` block in ``~/.goalrail/config.yaml``::
...
(:func:`goalrail.runtime.workflow._load_global_auth`) — a Cursor key parked
...
CURSOR_EXTRA = "cursor"
CURSOR_EXTRA_INSTALL_COMMAND = 'pip install "goalrail[cursor]"'
...
def cursor_install_command() -> list[str]:
    ...
    Mirrors :func:`goalrail.onboarding.antigravity_auth.antigravity_install_command`.

    :returns: The install argv, e.g.
        ``["uv", "pip", "install", "goalrail[cursor]"]`` or
        ``[sys.executable, "-m", "pip", "install", "goalrail[cursor]"]``.
    """
    target = f"goalrail[{CURSOR_EXTRA}]"
...
def install_cursor_sdk() -> bool:
    ...
    visible. Mirrors
    :func:`goalrail.onboarding.antigravity_auth.install_antigravity_sdk`.
...
def cursor_api_key_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`goalrail.cli._save_global_config` (a shallow update, so
