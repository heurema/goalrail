...
Implements :class:`~goalrail.onboarding.sandboxes.base.SandboxLauncher`
...
'goalrail[modal]'``) imported lazily, so the provider can be listed and
...
MODAL_APP_NAME: str = "goalrail-sandboxes"
...
HOST_IMAGE_ENV_VAR: str = "GOALRAIL_MODAL_HOST_IMAGE"
"""Environment variable overriding :data:`DEFAULT_HOST_IMAGE`, e.g. for
an org-internal copy of the host image
(``ghcr.io/<your-org>/goalrail-host:latest``)."""

REGISTRY_SECRET_ENV_VAR: str = "GOALRAIL_MODAL_REGISTRY_SECRET"
...
SANDBOX_SECRETS_ENV_VAR: str = "GOALRAIL_MODAL_SANDBOX_SECRETS"
...
def _ensure_sdk() -> None:
    ...
    an optional dependency — the base ``goalrail`` install does not
    pull it in.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "Install it with `pip install 'goalrail[modal]'`, then "
...
def _build_sandbox_image(image_ref: str | None = None) -> modal.Image:
    ...
    Pulls the prebaked Goalrail host image — the full goalrail
    ...
    :param image_ref: Explicit registry image reference, e.g.
        ``"docker.io/me/goalrail-host:latest"``, or ``None`` to
...
class ModalSandboxLauncher(SandboxLauncher):
    ...
    def __init__(self, *, image: str | None = None, secrets: Sequence[str] | None = None) -> None:
        ...
        :param image: Optional registry image reference to provision
            sandboxes from, e.g. ``"docker.io/me/goalrail-host:latest"``
        ...
        :param secrets: Optional Modal secret names whose env vars are
            injected into every sandbox, e.g. ``["goalrail-llm"]`` —
    ...
    def provision(self, name: str) -> str:
        ...
        :param name: Human-readable label, e.g. ``"goalrail-host"``.
        ...
        handle.set_tags({"goalrail-name": name})
    ...
    def keep_alive(self, sandbox_id: str) -> None:
        ...
        """
        click.echo(
            f"  → Modal caps sandbox lifetime at 24 hours; re-run `goalrail "
    ...
    def wheel_install_command(self, remote_tgz_path: str) -> str:
        ...
        :func:`~goalrail.onboarding.sandboxes.base.host_image_wheel_install_command`
