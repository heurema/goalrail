...
:class:`~goalrail.onboarding.sandboxes.base.SandboxLauncher` for an
...
dependency (``pip install 'goalrail[kubernetes]'``) imported lazily, so the
...
- **Least privilege.** ``automountServiceAccountToken: false`` keeps the runner
  ...
  filesystem stays writable (the host writes ``/tmp`` and ``~/.goalrail``).
...
HOST_IMAGE_ENV_VAR: str = "GOALRAIL_KUBERNETES_HOST_IMAGE"
"""Environment variable overriding
:data:`~goalrail.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE` for Kubernetes
sandbox Pods (amd64-only)."""

NAMESPACE_ENV_VAR: str = "GOALRAIL_KUBERNETES_NAMESPACE"
...
SANDBOX_SECRET_ENV_VAR: str = "GOALRAIL_KUBERNETES_SECRET"
...
SANDBOX_ENV_PASSTHROUGH_ENV_VAR: str = "GOALRAIL_KUBERNETES_SANDBOX_ENV"
...
SERVICE_ACCOUNT_ENV_VAR: str = "GOALRAIL_KUBERNETES_SERVICE_ACCOUNT"
...
KUBECONFIG_ENV_VAR: str = "GOALRAIL_KUBERNETES_KUBECONFIG"
...
_DEFAULT_NAMESPACE: str = "goalrail-sandboxes"
_DEFAULT_SERVICE_ACCOUNT: str = "goalrail-runner"
...
# (or a future reconciler) can select goalrail-managed objects for GC.
_MANAGED_BY_LABEL: str = "app.kubernetes.io/managed-by"
_MANAGED_BY_VALUE: str = "goalrail"
_ROLE_LABEL: str = "goalrail.dev/role"
...
# in :mod:`goalrail.server.managed_hosts` (which validate config-sourced names);
...
def _ensure_sdk() -> None:
    ...
    optional dependency — the base ``goalrail`` install does not pull it in.
    ...
    try:
        ...
        raise click.ClickException(
            ...
            "provider. Install it with `pip install 'goalrail[kubernetes]'`."
        ) from exc
...
def _new_pod_name(label: str) -> str:
    ...
    :returns: A Pod name like ``"goalrail-managed-a1b2c3d4-1a2b3c"``.
    """
    base = re.sub(r"[^a-z0-9-]+", "-", label.lower()).strip("-")
    base = re.sub(r"-+", "-", base) or "host"
    return f"goalrail-{base[:40]}-{uuid.uuid4().hex[:6]}"
...
def _token_secret_name(pod_name: str) -> str:
    ...
    :returns: The Secret name, e.g. ``"goalrail-managed-a1b2c3d4-1a2b3c-token"``.
    """
...
def _render_workspace_prep_command(
    ...
    :param workspace: The workspace root to create, e.g. ``"/home/goalrail/workspace"``.
...
def build_pod_manifest(
    *,
    ...
    - Pod + container ``securityContext`` satisfy Pod Security "restricted"
      ...
      root filesystem stays writable (the host writes ``/tmp`` + ``~/.goalrail``).
...
class KubernetesSandboxLauncher(SandboxLauncher):
    ...
    def __init__(
        ...
        *,
        ...
        :param image: Host image reference — the ``sandbox.kubernetes.image``
            ...
            :data:`~goalrail.onboarding.sandboxes.base.DEFAULT_HOST_IMAGE`.
    ...
    def start_host(
        ...
        *,
        ...
        :meth:`~goalrail.onboarding.sandboxes.base.SandboxLauncher.start_host`
    ...
    def _delete_with_retry(self, kind: str, name: str, delete: Callable[[], object]) -> None:
        ...
        click.echo(
            ...
            "and carries the goalrail managed-by/role labels for GC.",
            err=True,
        )
