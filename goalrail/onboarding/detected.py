...
:mod:`goalrail.onboarding.ambient` discovers credentials already on the
...
``providers:`` shape consumed by :mod:`goalrail.onboarding.provider_config`,
...
def codex_config_provider_dismissed(config: dict[str, object]) -> bool:
    ...
    :returns: ``True`` when ``~/.codex/config.toml`` selects a custom,
        ...
        dismissed in *config*.
    """
    from goalrail.onboarding.ambient import codex_config_detection
...
def _synthesize_entry(det: DetectedProvider) -> dict[str, object] | None:
    ...
    :returns: A raw provider entry body (config shape), or ``None`` when the
        detection maps to no goalrail harness surface (a ``family``-less
        ``key`` detection).
    """
    ...
    if det.family is None:
        ...
        # whose provider maps to no goalrail family).
        return None
...
def synthesize_detected_entries(
    ...
    :param detected: Detections from
        :func:`goalrail.onboarding.ambient.detect_providers`, in priority
        order.
    :returns: Raw provider entries keyed by the detection name, e.g.
        ...
        surface) — see :data:`goalrail.onboarding.ambient._ENV_KEY_FAMILY`.
        Only detections that map to no goalrail family at all (a ``family``-less
...
def effective_config_with_detected(
    ...
    :param detected: Detections to merge; defaults to a live
        :func:`goalrail.onboarding.ambient.detect_providers` call.
