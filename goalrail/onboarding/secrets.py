...
:func:`goalrail.onboarding.provider_config.resolve_secret`). The
...
- **OS keychain** (macOS Keychain, GNOME Keyring, Windows Credential
  ...
  actually works and ``GOALRAIL_DISABLE_KEYRING`` is not set. Secrets
...
- **A ``0600`` JSON file** at ``<config_home>/secrets.json`` (a flat
  ...
  ``GOALRAIL_DISABLE_KEYRING`` or a keyring call raises
...
:mod:`goalrail.onboarding.provider_config` — that module imports *this*
one lazily inside :func:`~goalrail.onboarding.provider_config.resolve_secret`
...
# service groups all goalrail secrets; the per-secret ``name`` is the
# keychain "username".
_KEYRING_SERVICE = "goalrail"
...
_DISABLE_KEYRING_ENV = "GOALRAIL_DISABLE_KEYRING"
...
def _keyring_disabled() -> bool:
    ...
    :func:`goalrail.runtime.telemetry._env_bool`): ``"true"`` / ``"1"`` /
    ``"yes"`` (case-insensitive) count as set; anything else (including
    unset) does not.

    :returns: ``True`` when ``GOALRAIL_DISABLE_KEYRING`` is truthy, e.g.
        with ``GOALRAIL_DISABLE_KEYRING=1`` set.
    """
...
def _use_keyring() -> bool:
    ...
    :returns: ``True`` unless ``GOALRAIL_DISABLE_KEYRING`` is truthy.
    """
...
def _config_home() -> str:
    """Return the Goalrail config home directory.
