...
from goalrail.onboarding.providers import PROVIDER_ENV_VARS
...
class DetectedProvider:
    ...
    :param family: The model family this credential serves
        ...
        credential is detected but maps to no goalrail harness surface.
...
def _provider_table_has_self_contained_auth(table: dict[str, object]) -> bool:
    ...
    environment variable that goalrail would have to thread into the codex
...
def _claude_login_detected() -> bool:
    ...
    (:func:`goalrail.onboarding.harness_install.harness_cli_logged_in`, which
    ...
    if sys.platform == "darwin":
        ...
        from goalrail.onboarding.harness_install import harness_cli_logged_in
...
def detect_providers() -> list[DetectedProvider]:
    ...
    1. Vendor API keys in the environment, via
       :data:`goalrail.onboarding.providers.PROVIDER_ENV_VARS`. Only
