"""
Databricks profile onboarding for Goalrail.
...
:mod:`goalrail.onboarding.internal_beta`, which is excluded from the public
...
SKIP_ENV_VAR = "GOALRAIL_SKIP_ONBOARD"
...
class ProfileSpec:
    """
    One Databricks profile Goalrail depends on.
...
def run_onboarding() -> bool:
    ...
    console.print(
        Panel(
            "[bold]Goalrail onboarding[/bold]\n\n"
            "Setting up the Databricks profiles Goalrail needs "
...
def _derive_workspace_profile_name(workspace_url: str, existing: dict[str, str]) -> str:
    ...
    2. A bundled :data:`DEFAULT_PROFILES` spec whose host matches — use its
       ...
       in
       :func:`goalrail.onboarding.databricks_config.get_workspace_url_for_profile`.
...
def login_databricks_workspace(workspace_url: str, *, console: Console | None = None) -> str:
    ...
    place Goalrail triggers a Databricks CLI login: it fires solely when
    a user explicitly adds a Databricks provider in
    ``goalrail setup --no-internal-beta``, never on a bare ``goalrail run``.
...
def maybe_run_onboarding() -> None:
    """
    Pre-flight onboarding for ``goalrail run``.
    ...
    Skipped when ``$GOALRAIL_SKIP_ONBOARD`` is set or stdin isn't a
    ...
    if actions.aliasable:
        ...
        if n:
            console.print(
                f"goalrail: aliased {n} existing profile(s) "
                f"to Goalrail names: "
    ...
    console.print(
        f"\n  [yellow]Goalrail needs Databricks profiles: {', '.join(missing_parts)}[/yellow]"
    )
    try:
