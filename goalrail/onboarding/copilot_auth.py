...
machinery (see :func:`goalrail.runtime.workflow._build_copilot_spawn_env`): the
GitHub Copilot SDK (``github-copilot-sdk``) talks only to GitHub's Copilot
backend, authenticated by a **GitHub token**. It therefore has no
``providers:`` family entry, but a user should still be able to register a
Copilot token once through ``goalrail setup`` rather than exporting it in every
shell.

This module is that home. The token is stored exactly like the api-key
providers' secrets — in the goalrail secret store (OS keychain, else a ``0600``
JSON file; see :mod:`goalrail.onboarding.secrets`) — and referenced from a
dedicated top-level ``copilot:`` block in ``~/.goalrail/config.yaml``::
...
from goalrail.onboarding.provider_config import load_config, resolve_secret
...
COPILOT_EXTRA_INSTALL_COMMAND = 'pip install "goalrail[copilot]"'
...
def copilot_sdk_installed() -> bool:
    ...
    (:mod:`goalrail.inner.copilot_executor`), so a token can be set with no SDK;
    setup uses this to detect that and offer to install it. The
    ``github-copilot-sdk`` package is imported as ``copilot``. Mirrors
    :func:`goalrail.onboarding.cursor_auth.cursor_sdk_installed` /
...
def copilot_install_command() -> list[str]:
    ...
    Mirrors :func:`goalrail.onboarding.cursor_auth.cursor_install_command`.

    :returns: The install argv, e.g.
        ``["uv", "pip", "install", "goalrail[copilot]"]`` or
        ``[sys.executable, "-m", "pip", "install", "goalrail[copilot]"]``.
    """
    target = f"goalrail[{COPILOT_EXTRA}]"
...
def install_copilot_sdk() -> bool:
    ...
    visible. Mirrors :func:`goalrail.onboarding.cursor_auth.install_cursor_sdk`.
...
def copilot_github_token_ref(config: dict[str, object] | None = None) -> str | None:
    ...
    :param config: A pre-loaded config mapping; ``None`` loads
        ``~/.goalrail/config.yaml`` via :func:`load_config`.
...
def copilot_github_token_settings(ref: str) -> dict[str, object]:
    ...
    Handed to :func:`goalrail.cli._save_global_config` (a shallow update, so it
