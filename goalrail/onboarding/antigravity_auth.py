...
the goalrail secret store and is referenced from a dedicated top-level
``antigravity:`` block in ``~/.goalrail/config.yaml`` (``keychain:`` / ``env:``),
...
:mod:`goalrail.onboarding.cursor_auth`.
"""
...
ANTIGRAVITY_EXTRA_INSTALL_COMMAND = 'pip install "goalrail[antigravity]"'
...
def antigravity_install_command() -> list[str]:
    ...
    :returns: The install argv, e.g.
        ``["uv", "pip", "install", "goalrail[antigravity]"]`` or
        ``[sys.executable, "-m", "pip", "install", "goalrail[antigravity]"]``.
    """
    target = f"goalrail[{ANTIGRAVITY_EXTRA}]"
...
def install_antigravity_sdk() -> bool:
    ...
    :func:`goalrail.onboarding.harness_install.install_harness_cli`.
...
def antigravity_api_key_settings(ref: str) -> dict[str, object]:
    ...
    :returns: The settings dict for :func:`goalrail.cli._save_global_config`.
    """
