"""Single source of truth for Goalrail wrapper-session labels.

Wrapper-style sessions (``goalrail claude`` today; future
``codex`` / ``pi`` wrappers tomorrow) stamp an ``goalrail.wrapper``
label on the conversation row at creation time. The server reads it
to gate behavior (claude-native message bypass at
``goalrail/server/routes/sessions.py:182-183``); the chat redirect
...
* keep ``goalrail.repl._resume_picker`` decoupled from the
  ``goalrail.claude_native`` import graph (which pulls in tmux /
...
# for the ``goalrail.*`` namespace; never reused for guardrails /
# policy labels.
WRAPPER_LABEL_KEY = "goalrail.wrapper"
...
# ``labels["goalrail.ui"] == "terminal"``). Stamped at creation for the
...
UI_MODE_LABEL_KEY = "goalrail.ui"
UI_MODE_TERMINAL_VALUE = "terminal"

# Value the ``goalrail claude`` wrapper writes into
...
# Value the ``goalrail codex`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
CODEX_NATIVE_WRAPPER_VALUE = "codex-native-ui"

# Value the ``goalrail pi`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
PI_NATIVE_WRAPPER_VALUE = "pi-native-ui"

# Value the ``goalrail opencode`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
OPENCODE_NATIVE_WRAPPER_VALUE = "opencode-native-ui"

# Value the ``goalrail cursor`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
CURSOR_NATIVE_WRAPPER_VALUE = "cursor-native-ui"

# Value the ``goalrail kiro`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
KIRO_NATIVE_WRAPPER_VALUE = "kiro-native-ui"

# Value the ``goalrail goose`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
GOOSE_NATIVE_WRAPPER_VALUE = "goose-native-ui"

# Value the ``goalrail antigravity`` native (agy TUI) wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
ANTIGRAVITY_NATIVE_WRAPPER_VALUE = "antigravity-native-ui"

# Value the ``goalrail qwen`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
QWEN_NATIVE_WRAPPER_VALUE = "qwen-native-ui"

# Value the ``goalrail kimi`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
KIMI_NATIVE_WRAPPER_VALUE = "kimi-native-ui"
# Value the ``goalrail hermes`` wrapper writes into
# ``conversations.labels[WRAPPER_LABEL_KEY]``.
