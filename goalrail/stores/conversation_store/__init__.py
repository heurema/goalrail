...
FORK_SOURCE_LABEL_KEY = "goalrail.fork.source_id"
...
# (see ``goalrail.claude_native._clone_claude_transcript`` and the
# fork-resume branch in ``goalrail.runner.app``), so the clone opens with
...
FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY = "goalrail.fork.source_external_session_id"
...
# CLI ignores the Goalrail transcript, so the runner rebuilds the target's
...
# the native transcript from the fork's copied Goalrail items
# (``_ensure_local_claude_resume_transcript`` /
# ``_ensure_local_codex_resume_rollout`` — the converters consume Goalrail's
...
FORK_CARRY_HISTORY_LABEL_KEY = "goalrail.fork.carry_history"
...
SWITCH_PREVIOUS_BUILTIN_LABEL_KEY = "goalrail.switch.previous_builtin_id"
...
# ``goalrail.runner.app._codex_native_launch_config`` and
...
CODEX_NATIVE_BYPASS_SANDBOX_LABEL_KEY = "goalrail.codex_native.bypass_sandbox"
...
PROJECT_LABEL_KEY = "goalrail.project"
...
_INSTANCE_SCOPED_LABEL_KEYS = frozenset(
    {
        ...
        "goalrail.codex_native.bridge_id",
        "goalrail.last_context_tokens",
        "goalrail.last_context_window",
...
class SessionConnectivity:
    ...
    :param needs_workspace: ``True`` when this is an unbound fork of a
        ...
        ``goalrail.fork.source_id`` label is set). Forces the online
...
class ConversationStore(ABC):
    ...
    def list_conversations(
        ...
        :param agent_id: When set, only return conversations
            ...
            *conversation* columns (``created_at`` /
            ...
            Goalrail mode ``--continue`` flag (resume the
            most-recent conversation for the agent that
            *this YAML* registers as) — see
            ``designs/RUN_GOALRAIL_SESSION_RESUMPTION.md``. ``None``
            disables the filter.
        ...
        :param project: When set to a non-empty string, only return
            ...
            ``key="goalrail.project"`` and ``value=project`` (the sidebar's
    ...
    def delete_label(
        ...
        ``goalrail.project`` label).

        :param conversation_id: The conversation to update,
            e.g. ``"conv_abc123"``.
        :param key: The label key to remove, e.g. ``"goalrail.project"``.
        """
        ...
    ...
    def list_projects(
        ...
        *non-archived* conversation carries a
        ``conversation_labels`` row with ``key="goalrail.project"``
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param carry_history_into_native: When ``True``, stamp
            ...
            native transcript, or build from the copied Goalrail items) instead
        ...
        :param resume_source_native_session: When ``True`` (default), a
            ...
            native transcript from the copied Goalrail items instead.
        :param presentation_labels: When not ``None``, replace the source's
            harness-presentation labels (``goalrail.ui`` /
            ``goalrail.wrapper``) on the clone with these. Used when the
    ...
    def switch_conversation_agent(
        ...
        *,
        ...
        :param presentation_labels: Replace the session's
            ``goalrail.ui`` / ``goalrail.wrapper`` labels with these so
