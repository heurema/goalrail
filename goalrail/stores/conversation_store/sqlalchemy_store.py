...
from goalrail._wrapper_labels import UI_MODE_LABEL_KEY, WRAPPER_LABEL_KEY
...
class SqlAlchemyConversationStore(ConversationStore):
    ...
    def get_session_owner(self, conversation_id: str) -> str | None:
        ...
        :returns: The owner's user id, e.g. ``"alice@example.com"``,
            ...
            permission grants.
        """
        from goalrail.db.db_models import SqlSessionPermission
    ...
    def list_projects(
        ...
        *non-archived* ``conversation_labels`` row with ``key="goalrail.project"``
        ...
        (``goalrail.*``) to keep this internal storage key distinct from the
    ...
    def list_conversations(
        ...
        :param project: When set to a non-empty string, only return
            ...
            ``key="goalrail.project"`` and ``value=project``. When set to an
    ...
    def fork_conversation(
        ...
        *,
        ...
        :param resume_source_native_session: When ``True`` (default), a
            ...
            copied Goalrail items instead.
        :param presentation_labels: When not ``None``, drop the source's
            ``goalrail.ui`` / ``goalrail.wrapper`` labels from the clone
