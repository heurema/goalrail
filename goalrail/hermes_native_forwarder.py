...
The ``goalrail hermes`` wrapper launches the real ``hermes`` TUI in a runner-owned
tmux pane, and :mod:`goalrail.hermes_native_bridge` injects web-UI messages into
it. That covers the web→TUI direction, but the *embedded terminal* is then the
only surface that reflects the agent's work — the Goalrail conversation view (chat
...
:mod:`goalrail.goose_native_forwarder`. Hermes stores all sessions in a single
...
class _HermesUsageTracker:
    ...
    Follows the :class:`goalrail.codex_native_forwarder._SessionUsageCoalescer`
...
async def forward_hermes_store_to_session(
    *,
    ...
    # Goalrail server so we do it at most once per forwarder lifetime.
