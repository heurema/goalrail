...
CODEX_NATIVE_BRIDGE_ID_LABEL_KEY = "goalrail.codex_native.bridge_id"
...
def codex_mcp_config_overrides(
    ...
    *,
    ...
    :mod:`goalrail.claude_native_bridge` as a stdio MCP server.
    ...
    :returns: Codex ``-c`` config override strings, e.g.
        ``['mcp_servers.goalrail.command="python"', ...]``.
    """
    python = python_executable or sys.executable
    args_toml = json.dumps(
        ["-m", "goalrail.claude_native_bridge", "serve-mcp", "--bridge-dir", str(bridge_dir)]
    )
    return [
        f'mcp_servers.goalrail.command="{python}"',
        f"mcp_servers.goalrail.args={args_toml}",
    ]
...
def read_codex_config_model(bridge_dir: Path) -> str | None:
    ...
    ``goalrail.inner.codex_executor``), then seeded with the session's
    launch model by ``_pin_codex_config_model`` in
    ``goalrail.codex_native_app_server``. An in-TUI ``/model`` writes
