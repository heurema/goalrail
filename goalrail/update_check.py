...
* **Installed wheel** (``uv tool install goalrail``,
  ``pip install goalrail``, ``pipx install``, Homebrew, etc.): no clone
  ...
  old. The index is resolved from ``GOALRAIL_INDEX_URL`` /
...
_ENV_SKIP = "GOALRAIL_NO_UPDATE_CHECK"
_CACHE_DIR = Path.home() / ".goalrail"
...
# explicit ``GOALRAIL_INDEX_URL`` wins; then uv's and pip's. When none is
...
def _effective_cache_dir() -> Path:
    ...
    current_default_cache_dir = Path.home() / ".goalrail"
    if (
...
class _InstalledWheelInfo:
    ...
    :param vcs_url: The git URL recorded in ``direct_url.json``
        ...
        ``"git+https://github.com/heurema/goalrail.git"``.
...
def _resolve_index_url() -> str:
    ...
    1. index env vars (:data:`_INDEX_ENV_VARS`) — explicit
       ``GOALRAIL_INDEX_URL`` first, then uv's and pip's;
...
def fetch_latest_version(
    ...
    *,
    ...
    """Fetch the latest ``goalrail`` release from the configured index.
...
def _versions_from_filenames(filenames: Iterable[str]) -> list[Version]:
    ...
    :param filenames: Distribution filenames, e.g.
        ``["goalrail-0.2.0-py3-none-any.whl", "goalrail-0.2.0.tar.gz"]``.
...
def _spawn_background_refresh() -> None:
    ...
    Uses ``python -c`` rather than re-invoking the ``goalrail`` CLI so the
    ...
    with contextlib.suppress(OSError, ValueError):
        subprocess.Popen(
            [
                sys.executable,
                "-c",
                "from goalrail.update_check import refresh_update_cache as r; r()",
            ],
...
def _find_repo_root() -> Path | None:
    ...
    DIRECT parent of the ``goalrail/`` package, and must contain
    both ``.git/`` AND a ``pyproject.toml``. The layout we accept
    is exactly what a dev clone of this repo looks like::

        <repo>/.git/
        <repo>/pyproject.toml
        <repo>/goalrail/update_check.py    ← this file

    We do NOT walk further up the filesystem tree. The previous
    implementation did, which caused a real bug: when ``goalrail``
    is installed via ``uv tool install`` at
    ``~/.local/share/uv/tools/goalrail/…``, the unbounded walk-up
    ...
    """
    package_dir = Path(__file__).resolve().parent  # <candidate>/goalrail/
...
def _read_build_info() -> tuple[float, str] | None:
    ...
    The ``goalrail/_build_info.py`` module is generated by
    ...
    """
    try:
        from goalrail import _build_info  # type: ignore[attr-defined]
    except ImportError:
        return None
    try:
...
def _get_distribution() -> importlib.metadata.Distribution | None:
    ...
    :returns: The ``Distribution`` for the ``goalrail`` package,
...
def _unredact_ssh_userinfo(vcs_url: str) -> str:
    ...
    :param vcs_url: The normalized ``<vcs>+<scheme>://…`` reinstall URL,
        e.g. ``"git+ssh://****@github.com/heurema/goalrail.git"``.
    :returns: The same URL with a redacted SSH user restored to ``git``,
        e.g. ``"git+ssh://git@github.com/heurema/goalrail.git"``;
...
class _UpgradeSuggestion:
    ...
    :param command: The shell command (or prose) we display in the
        nag panel, e.g. ``"uv tool upgrade goalrail"`` or
        ``"reinstall goalrail from your original source"``.
        Always populated.
...
def _pip_invocation() -> str:
    ...
    *other* environment's ``pip`` shadows the one running ``goalrail`` (a
    ...
    copy of goalrail while the running one stays put. ``<sys.executable>
...
def upgrade_command_for_installed() -> _UpgradeSuggestion | None:
    ...
    upgrade command (``uv tool upgrade goalrail``, ``pip install -U
    goalrail``, etc.).
...
def _probe_installed_distribution() -> tuple[str | None, str | None]:
    ...
    ``goalrail`` and cached its metadata — re-reading in-process returns the
    *pre-upgrade* version. A fresh ``sys.executable`` subprocess loads the
    ...
    The version is read via ``importlib.metadata`` (no ``goalrail`` import,
    ...
    probe = (
        ...
        "try:\n"
        "    print(m.version('goalrail'))\n"
        "except Exception:\n"
        "    print('')\n"
        "try:\n"
        "    from goalrail.update_check import _read_installed_wheel_info as r\n"
        "    i = r()\n"
...
def _remote_git_head(vcs_url: str) -> str | None:
    ...
    :param vcs_url: A normalized VCS URL, e.g.
        ``"git+https://github.com/heurema/goalrail.git"`` or
        ``"git+https://…/goalrail.git@main"``.
