...
from goalrail.codex_native_bridge import write_policy_hook_config
from goalrail.codex_native_process_registry import (
...
# might declare.
_POLICY_HOOK_MODULE = "goalrail.codex_native_hook"
...
# (``goalrail/_startup_profile.py``, ``goalrail/cli.py``).
_MODEL_FLAG_ENV_VAR = "GOALRAIL_CODEX_NATIVE_MODEL_FLAG"
...
def _toml_table_header_name(line: str) -> str | None:
    ...
    :param line: One config line, e.g.
        ``"[mcp_servers.goalrail] # generated\n"``.
    :returns: The table name, e.g. ``"mcp_servers.goalrail"``, or
...
def _remove_toml_table(text: str, table_name: str) -> str:
    ...
    ``[mcp_servers.goalrail]`` sections across terminal relaunches.

    :param text: TOML document text.
    :param table_name: Table name to remove, e.g.
        ``"mcp_servers.goalrail"``.
...
def _codex_mcp_server_config_section(
    ...
    :param python_executable: Python executable for serve-mcp, e.g.
        ...
        :data:`sys.executable`.
    :returns: TOML text for ``[mcp_servers.goalrail]``.
    """
...
def _inject_mcp_server_config(
    ...
    Writes a ``[mcp_servers.goalrail]`` section that points Codex
    ...
    if config_path.exists():
        ...
        existing = ""
    updated = _remove_toml_table(existing, "mcp_servers.goalrail")
...
class CodexAppServerClient:
    ...
    :param client_name: App-server client name for the initialize
        handshake, e.g. ``"goalrail-codex-native"``.
    """
...
class CodexNativeAppServer:
    ...
    :param bridge_dir: Native Codex bridge directory, e.g.
        ``Path("~/.goalrail/codex-native/<hash>")``. The policy hook
    ...
    async def start(self) -> None:
        ...
        # Read the opt-in from the goalrail server's OWN process environment
        # (``os.environ``, the default), NOT ``self.env``: ``self.env`` is the
        # cleaned codex spawn env from ``_clean_codex_env``, whose prefix
        # allowlist strips ``GOALRAIL_*`` keys -- so the flag would never be
        # visible there. The flag is an operator knob for goalrail, not
    ...
    async def _trust_policy_hooks(self) -> None:
        ...
        if self.listen_url and self.listen_url.startswith("ws://"):
            client = CodexAppServerClient(
                ...
                client_name="goalrail-policy-trust",
            )
        else:
            client = CodexAppServerClient(
                self.socket_path,
...
def _codex_policy_hook_command(bridge_dir: Path, python_executable: str | None) -> str:
    ...
    :returns: A shell-escaped command string, e.g.
        ``"/path/python -m goalrail.codex_native_hook evaluate-policy
        --bridge-dir /home/u/.goalrail/codex-native/abc"``.
    """
    python = python_executable or sys.executable
    return shlex.join(
...
def resolve_native_codex_launch(*, model: str | None) -> NativeCodexLaunch:
    ...
    (:func:`goalrail.runtime.workflow._resolve_provider_for_build`) for the
    ``openai`` surface, so ``goalrail codex`` and a host-spawned native
    Codex session route through ``goalrail setup``:
...
def client_for_transport(
    ...
    *,
    ...
    :param transport: App-server transport from bridge state, e.g.
        ...
        ``"/home/u/.goalrail/codex-native/x/app-server.sock"``.
    :param client_name: App-server initialize-handshake client name,
        e.g. ``"goalrail-codex-native"``.
...
async def preload_codex_thread_for_resume(transport: str, thread_id: str) -> None:
    ...
    client = client_for_transport(
        transport,
        client_name="goalrail-codex-native-preload",
    )
    await client.connect()
    try:
        await client.request(
