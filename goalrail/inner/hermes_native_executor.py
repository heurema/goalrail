"""Executor that bridges Goalrail web-chat turns into the native Hermes TUI.

It does not launch ``hermes`` — the ``goalrail hermes`` wrapper already launched
...
:class:`goalrail.inner.goose_native_executor.GooseNativeExecutor`.
"""
...
from goalrail.hermes_native_bridge import BRIDGE_DIR_ENV_VAR, inject_interrupt, inject_user_message
...
class HermesNativeExecutor(Executor):
    """Harness-side executor for ``goalrail hermes`` web-UI turns.
