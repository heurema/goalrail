"""Executor that bridges Goalrail web-chat turns into the native Goose TUI.

It does not launch ``goose`` — the ``goalrail goose`` wrapper already launched
...
from goalrail.goose_native_bridge import BRIDGE_DIR_ENV_VAR, inject_user_message
...
class GooseNativeExecutor(Executor):
    """Harness-side executor for ``goalrail goose`` web-UI turns.
