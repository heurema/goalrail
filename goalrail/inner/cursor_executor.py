...
``AsyncAgent`` per Goalrail conversation, created on a
...
evaluates PHASE_TOOL_CALL and cancels the run on DENY.  Bridged Goalrail
tools (MCP-wrapped) are already gated server-side via the dispatch bridge
and are skipped to avoid double evaluation.

Crucially, Goalrail's spec-declared tools (``sys_session_send`` et al.) are
bridged into Cursor **in-process** via the SDK's ``custom_tools``: each
:class:`~goalrail.inner.executor.ToolSpec` becomes a ``cursor_sdk.CustomTool``
...
# Goalrail's bridged-tool callback: (tool_name, args) -> awaitable result.
...
def _normalize_cursor_usage(raw: dict[str, Any], model: str) -> dict[str, Any]:
    """Map Cursor SDK usage fields to the standard Goalrail usage dict."""
    ...
    # map to the Goalrail-standard cache_read_input_tokens /
...
def _build_cursor_prompt(
    ...
    *,
    ...
    first turn the Goalrail system prompt is prepended (the SDK has no separate
...
def _sdk_message_to_events(message: Any) -> list[ExecutorEvent]:  # type: ignore[explicit-any]
    ...
    if mtype == "tool_call":
        ...
        # Goalrail tool name + args so the observed events (and any name-keyed
...
def _write_cursor_hooks(cwd: str, hook_script_path: str, server_url: str, session_id: str) -> Path:
    ...
    :param server_url: Goalrail server URL, e.g. ``"http://127.0.0.1:6767"``.
    ...
    wrapper = hooks_dir / "goalrail-hook.sh"
    wrapper.write_text(
        f"#!/bin/sh\n"
        f"export _GOALRAIL_SERVER_URL='{server_url}'\n"
        f"export _GOALRAIL_SESSION_ID='{session_id}'\n"
...
class _CursorSessionState:
    """Per-Goalrail-conversation SDK session state."""
...
class CursorExecutor(Executor):
    ...
    def __init__(
        ...
        *,
        ...
        # Goalrail's session (policy gating, sub-agent dispatch, logging).
    ...
    async def _evaluate_native_tool_policy(
        ...
        <goalrail.inner.claude_sdk_executor.ClaudeSDKExecutor>` exposes
    ...
    def _make_execute(
        ...
        """Build a sync ``execute`` that bridges a cursor tool call to Goalrail.
    ...
    async def _close_state(self, state: _CursorSessionState) -> None:
        ...
        if state.hooks_file is not None:
            try:
                ...
                wrapper = state.hooks_file.parent / "goalrail-hook.sh"
