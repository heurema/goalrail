...
from the resolved :class:`goalrail.inner.sandbox.SandboxPolicy`.
...
This is the **macOS parity** for :mod:`goalrail.inner.bwrap_sandbox`.
...
- The same cwd dotfile/escaping-symlink walker
  (:func:`goalrail.inner._cwd_scan.scan_cwd_mask_entries`).
- The same egress filtering pipeline: parent-side
  :class:`goalrail.inner.egress.proxy.EgressProxy`, Unix socket
  bridge in the scratch tmpdir, in-helper TCP→Unix relay via
  :func:`goalrail.inner.egress.relay.start_relay`, and
...
- The per-helper scratch tmpdir (added by the spawn site via
  :func:`goalrail.inner.sandbox.with_additional_write_roots`) is
...
- **AF_UNIX path-length cap.** macOS limits ``sun_path`` to ~104
  ...
  ``/var/folders/.../T/goalrail-osenv-XXXXXX/.egress.sock`` is
...
class SeatbeltSandboxBackend(SandboxBackend):
    ...
    def wrap_launcher_argv(
        ...
        :param argv: The unwrapped helper command, typically
            ``[sys.executable, "-m", "goalrail.inner.os_env",
        ...
        :param policy: The :class:`SandboxPolicy` produced by
            ...
            :func:`goalrail.inner.sandbox.with_additional_write_roots`
    ...
    def activate(self, policy: SandboxPolicy) -> None:
        ...
        Unlike :meth:`goalrail.inner.bwrap_sandbox.BwrapSandboxBackend.activate`,
        ...
        if policy.egress_relay_port is not None and policy.egress_socket_path is not None:
            from goalrail.inner.egress.relay import start_relay
...
def _build_profile(
    ...
    *,
    ...
    # ``goalrail.inner.os_env`` even starts. The bwrap backend
...
def _ensure_executable_visible(
    ...
    Parallel to
    :func:`goalrail.inner.bwrap_sandbox._ensure_executable_visible`,
...
def _seatbelt_safe_roots(
    ...
    Mirrors :func:`goalrail.inner.bwrap_sandbox._bwrap_safe_roots`
...
def _interpreter_safe_roots(argv: Sequence[str]) -> list[Path]:
    ...
    Mirrors :func:`goalrail.inner.bwrap_sandbox._interpreter_safe_roots`:
...
def _per_user_dyld_cache_subpath() -> Path | None:
    ...
    dirs (which live under ``<per-user-folder>/T/goalrail-osenv-*``,
...
def _scratch_tmpdir(write_roots: list[Path]) -> Path | None:
    ...
    :func:`goalrail.inner.sandbox.create_private_tmpdir` returns a
    path under ``tempfile.gettempdir()`` (typically ``/var/folders/.../T``
    on macOS). The helper-spawn site adds it to ``write_roots`` via
    :func:`goalrail.inner.sandbox.with_additional_write_roots`, so
...
def _ensure_profile_tempdir() -> str:
    ...
    ``<system tmpdir>/goalrail-seatbelt-profiles-XXXX`` with mode
    ...
    if _PROFILE_TEMPDIR is None:
        _PROFILE_TEMPDIR = tempfile.mkdtemp(prefix="goalrail-seatbelt-profiles-")
