...
Goalrail session, persists the Codex thread across turns, and exposes
Goalrail tools to Codex as App Server ``dynamicTools``.
"""
...
from goalrail.reasoning_effort import CODEX_EFFORTS, validate_effort
...
# to running sessions without any action from Goalrail.
...
async def _create_subprocess_exec(*args: Any, **kwargs: Any) -> asyncio.subprocess.Process:
    ...
    ``goalrail.inner.codex_executor.asyncio.create_subprocess_exec``
...
def _is_goalrail_private_codex_home(path: Path) -> bool:
    """
    Return whether *path* is an Goalrail-created private ``CODEX_HOME``.

    Goalrail launches Codex with private homes for session state
    ...
    :param path: Candidate ``CODEX_HOME`` path, e.g.
        ``"/home/user/.goalrail/codex-native/<hash>/codex-home"``.
    :returns: ``True`` when *path* matches a native bridge home or the
        wrapped executor's temporary ``goalrail-codex-home-*`` naming.
    """
    ...
    if (
        ...
        and parts[-4] == ".goalrail"
    ):
        return True
    return expanded.name.startswith("goalrail-codex-home-")
...
def _private_codex_home_config_source(path: Path) -> Path | None:
    ...
    A parent Goalrail launch bridges ``auth.json`` and ``config.toml`` into
...
def _resolve_codex_home_config_source(source_dir: Path, home_codex_home: Path) -> Path:
    ...
    points at an Goalrail private home from a parent session. In that
    ...
    """
    if source_dir != home_codex_home and _is_goalrail_private_codex_home(source_dir):
...
def _codex_home_config_source_from_env() -> Path:
    ...
    Codex stores subscription login state in ``CODEX_HOME``. Goalrail
    launches Codex with private per-session homes for isolation, then bridges
    only auth/config from the user's source home. Nested Goalrail processes
...
def _provider_codex_config_overrides(
    *,
    ...
    :returns: Codex TOML-fragment override strings.
    """
    provider_name = "goalrail_provider"
    auth_command_json = json.dumps(auth_command)
...
class _PendingToolResult:
    ...
    :param name: The tool name Codex asked Goalrail to run.
...
class _CodexAppServerSession:
    ...
    async def start(self) -> None:
        ...
        self._codex_home_dir = Path(
            tempfile.mkdtemp(prefix="goalrail-codex-home-", dir=str(codex_home_root))
        )
    ...
    def __init__(
        ...
        *,
        ...
        if not enable_web_search:
            ...
            # tools exposed by Goalrail as dynamicTools. The top-level web_search
