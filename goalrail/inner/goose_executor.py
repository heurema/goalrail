...
output streams into the Goalrail conversation as chat, and Goose's mid-turn tool
...
internally. This executor translates the ACP event stream into Goalrail
ExecutorEvents and routes Goose's permission requests through Goalrail's
...
class GooseExecutor(Executor):
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``session/request_permission`` — decide via Goalrail's TOOL_CALL policy
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here, executed through the Goalrail OSEnvironment so
    ...
    def _usage_from_result(result: dict[str, Any]) -> dict[str, Any] | None:  # type: ignore[explicit-any]
        """Map Goose's final ``result.usage`` to Goalrail's usage keys.

        Goose reports ``{totalTokens, inputTokens, outputTokens}``; Goalrail's
