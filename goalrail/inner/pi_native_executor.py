"""Executor that bridges Goalrail messages into a native Pi TUI."""
...
class PiNativeExecutor(Executor):
    """
    Harness-side executor for ``goalrail pi`` web UI turns.

    The native Pi process is already running in the session terminal with
    the Goalrail Pi extension loaded. Each turn queues the latest user
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from Goalrail. Ignored for now; native
