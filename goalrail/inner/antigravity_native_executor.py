"""Executor that delivers Goalrail web/mobile turns into a native Antigravity agy.

``goalrail antigravity`` runs the Antigravity ``agy`` CLI in a runner-owned tmux
terminal and mirrors its transcript into the Goalrail session via the RPC read
driver (the read path). This executor is the **write path**: when a turn is
submitted from the Goalrail web/mobile UI it delivers the user's message by
TYPING IT INTO the agy TUI pane over tmux
(:func:`goalrail.antigravity_native_bridge.inject_user_message_via_tui`), exactly
...
so the agy TUI and the Goalrail web mirror share ONE conversation in both
...
``GetAvailableModels``. The Goalrail ``ExecutorConfig.model``/``reasoning_effort``
...
from goalrail.llms.errors import PermanentLLMError
from goalrail.reasoning_effort import ANTIGRAVITY_EFFORTS, validate_effort_or_llm_error
...
class AntigravityNativeExecutor(Executor):
    """
    Harness-side executor for ``goalrail antigravity`` web UI turns.
    ...
    async def interrupt_session(self, session_key: str) -> bool:
        ...
        (:func:`goalrail.antigravity_native_rpc.cancel_cascade_steps`).
        ...
        .. note:: **Scope — RUNNING cascades only (live-verified, C3).**
           ...
           **NO-OP on a step that is WAITING for a user interaction**
           ...
           (:mod:`goalrail.antigravity_native_interactions`), NOT here — this
    ...
    async def run_turn(
        ...
        (:func:`goalrail.antigravity_native_rpc.send_user_cascade_message`), which
        ...
        :param tools: Tool schemas from Goalrail. Ignored; native agy owns its
            own tool surface.
    ...
    async def _deliver(self, text: str) -> str | None:
        ...
        :func:`goalrail.antigravity_native_bridge.inject_user_message_via_tui`)
        ...
        discovers/binds — see :mod:`goalrail.antigravity_native_reader`).
    ...
    async def _resolve_plan_model(self, port: int, cascade_id: str) -> str | None:
        ...
        1. **Echo agy's current model** — read the latest ``USER_INPUT`` step's
           ...
           :func:`goalrail.antigravity_native_rpc.get_trajectory_steps`. This
        ...
        2. **Recommended fallback** — when no prior model is observable (a first
           ...
           :func:`goalrail.antigravity_native_rpc.get_available_models`.
...
def _request_session_id_from_env() -> str | None:
    """
    Resolve the Goalrail session id that requested this harness process.

    :returns: Goalrail session id, e.g. ``"conv_abc123"``, or ``None``.
    """
...
def _latest_requested_model(steps: list[dict[str, object]]) -> str | None:
    ...
    field :func:`goalrail.antigravity_native_rpc.send_user_cascade_message` sends
    ...
    :param steps: Trajectory steps as returned by
        :func:`goalrail.antigravity_native_rpc.get_trajectory_steps`.
...
def _content_to_text(content: EnqueuedContent, bridge_dir: Path) -> str:
    ...
    if isinstance(content, list):
        from goalrail.inner.native_attachments import materialize_attachment
