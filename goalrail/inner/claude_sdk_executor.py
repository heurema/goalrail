...
underlying agent harness.  Goalrail tools are bridged into the SDK session
...
This executor translates the SDK message stream into Goalrail ExecutorEvents
...
from goalrail._platform import stable_user_id
...
# with the Goalrail tool executor and the SDK's MCP bridge.
...
# Tool executor callable wired in by ``goalrail.Session``.
...
_NO_SANDBOX_ENV = "GOALRAIL_CLAUDE_SDK_NO_SANDBOX"
...
class PreparedClaudeCli:
    """Result of wrapping the Claude CLI in an Goalrail sandbox.
...
def _build_mcp_tools(
    ...
    """Build SdkMcpTool objects from Goalrail tool schemas.

    Each tool is backed by a handler that calls the Goalrail tool_executor
    ...
    for schema in tool_schemas:
        ...
        # itself does not accept ``None``. Goalrail tool schemas always
...
def _augment_system_prompt_for_goalrail_mcp_tools(
    system_prompt: str,
    tool_schemas: list[ToolSpec],
) -> str:
    """
    ...
    Goalrail schemas use bare names such as ``sys_session_send``. The
    Claude SDK exposes tools from our in-process MCP server to the model
    as ``mcp__goalrail__<bare_name>``. Bundled agent prompts and skills use
    ...
    if examples:
        example_text = "; ".join(
            f"use `mcp__goalrail__{name}` when instructions say `{name}`" for name in examples
        )
        note = (
            "Claude SDK tool naming: Goalrail tools are exposed as MCP tools. "
            f"{example_text}. For any other Goalrail tool, use "
            "`mcp__goalrail__<tool_name>` rather than the bare name."
        )
    else:
        note = (
            "Claude SDK tool naming: Goalrail tools are exposed as MCP tools. "
            "When instructions mention a bare Goalrail tool name, invoke "
            "`mcp__goalrail__<tool_name>` rather than the bare name."
        )
...
class ClaudeSDKExecutor(Executor):
    ...
    The SDK runs Claude Code's full agent loop internally.  Goalrail tools
        ...
        *,
        ...
        Args:
            ...
            os_env: If set, enable built-in OS tools (Bash, Read, Edit, …)
                ...
                spec's sandbox is enabled, Goalrail wraps the Claude CLI
                in the same sandbox. If omitted, Goalrail still tries to
        ...
        # the Goalrail elicitation system rather than silently allowed.
        # ``None`` until the adapter installs it on first use.
        self._elicitation_handler: ElicitationHandler | None = None
        # Live Claude SDK clients keyed by Goalrail session id.
    ...
    async def _can_use_tool_for_permission(
        ...
        """
        Route a Claude SDK permission request through the Goalrail elicitation system.

        Installed as ``options.can_use_tool`` when ``permission_mode`` is
        not ``"bypassPermissions"`` and an elicitation handler has been wired
        in by :class:`goalrail.runtime.harnesses._executor_adapter.ExecutorAdapter`.
        ...
        if allowed:
            return PermissionResultAllow()
        return PermissionResultDeny(message="Denied via Goalrail elicitation")
    ...
    async def _evaluate_tool_call_policy(
        ...
        evaluated against Goalrail TOOL_CALL-phase policies BEFORE they
        ...
        Double-evaluation guard: Goalrail's OWN tools are exposed as the
        single ``goalrail`` SDK MCP server (the model sees
        ``mcp__goalrail__*``). When the model calls one, the SDK wrapper
        routes it back through Goalrail's dispatch bridge
        ...
        (see ``goalrail/runner/app.py`` "All tool calls go through AP:/mcp
        ... which enforces TOOL_CALL + TOOL_RESULT policies server-side").
        Spec-declared MCP tools are surfaced through that same
        ``mcp__goalrail__*`` server, so they are covered there too.
        Evaluating ``mcp__goalrail__*`` here as well would double-count
        ...
        :returns: :class:`claude_agent_sdk.PermissionResultDeny` when a
            ...
            existing Goalrail elicitation handler before returning
            ...
            ``mcp__goalrail__*`` tool already gated on the dispatch path, or
        ...
        if _policy_eval is None:
            return None
        # Goalrail's own tools are already TOOL_CALL-gated server-side via
        # the dispatch bridge / ProxyMcpManager — don't evaluate them twice.
        if tool_name.startswith("mcp__goalrail__"):
            return None
        ...
        if _action == "POLICY_ACTION_ASK":
            ...
            reason = _verdict.reason or "Approval required by Goalrail TOOL_CALL policy"
            if self._elicitation_handler is None:
                logger.warning(
        ...
        if _action == "POLICY_ACTION_DENY":
            ...
            reason = _verdict.reason or "Denied by Goalrail TOOL_CALL policy"
            logger.info("TOOL_CALL policy denied tool=%s reason=%s", tool_name, reason)
            return PermissionResultDeny(message=reason)
        from claude_agent_sdk import PermissionResultDeny

        reason = f"Unexpected Goalrail TOOL_CALL policy verdict: {_action!r}"
    ...
    async def run_turn(
        ...
        # Build MCP tools from Goalrail tool schemas
        mcp_tools = _build_mcp_tools(tools, self._tool_executor) if tools else []

        # Create MCP server config for Goalrail tools. The SDK's
        ...
        if mcp_tools:
            mcp_servers["goalrail"] = sdk.create_sdk_mcp_server(
                name="goalrail",
                version="1.0.0",
                tools=mcp_tools,
            )
            system_prompt = _augment_system_prompt_for_goalrail_mcp_tools(
                system_prompt,
                tools,
            )

        # Build allowed_tools list.  OS-environment operations route
        # through Goalrail ``sys_os_*`` MCP tools rather than the
        ...
        # stays because goalrail owns conversation persistence
        # via its own conversation store.
        # OS-environment tools are provided via Goalrail ``sys_os_*``
        ...
        # the CLI subprocess are evaluated against Goalrail TOOL_CALL
        ...
        # callback, call it with the request data so the Goalrail server
