"""Hermes ``pre_tool_call`` shell hook for Goalrail policy enforcement.
...
The hook evaluates ``PHASE_TOOL_CALL`` policy via the Goalrail server.
...
Environment variables (set by the wrapper shell script):
    ...
    _GOALRAIL_SERVER_URL  : Base URL of the Goalrail server
                            (e.g. ``http://127.0.0.1:6767``).
    _GOALRAIL_SESSION_ID  : Session / conversation ID for policy
                            evaluation.
"""
...
def main() -> None:
    server_url = os.environ.get("_GOALRAIL_SERVER_URL", "")
    session_id = os.environ.get("_GOALRAIL_SESSION_ID", "")
    ...
    try:
        from goalrail.native_policy_hook import post_evaluate_with_retry
    ...
    if action == "POLICY_ACTION_DENY":
        ...
        if reason:
            out["reason"] = f"Tool '{tool_name}' denied by Goalrail policy: {reason}"
        else:
            out["reason"] = f"Tool '{tool_name}' denied by Goalrail policy"
