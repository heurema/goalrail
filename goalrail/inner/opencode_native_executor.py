"""Executor that bridges Goalrail web turns into a native OpenCode session.

Built on :class:`goalrail.native_server_harness.NativeServerHarness`: the
runner owns the ``opencode serve`` process + SSE forwarder, and this
executor injects the latest web turn over the
:class:`goalrail.opencode_http_transport.OpenCodeHttpTransport` using the
...
from goalrail.opencode_http_transport import OpenCodeHttpTransport
...
class OpenCodeNativeExecutor(NativeServerHarness):
    """
    Harness-side executor for ``goalrail opencode`` web UI turns.
