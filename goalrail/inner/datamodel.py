"""Core data model types for Goalrail, which are all exposed to agents and in configs."""
...
# whose concrete types live in other modules (goalrail.tools,
# goalrail.policies, ...). Importing them here would create import cycles,
...
class History:
    ...
    def get_context_window(self, max_tokens: int | None = None) -> list[Message]:  # noqa: ARG002
        ...
        :mod:`goalrail.runtime.compaction`, which operates on the
...
class OSEnvSandboxSpec:
    ...
    # :func:`goalrail.inner.sandbox._default_sandbox_for_platform`,
    ...
    #   the rest of the tree first (see :mod:`goalrail.inner._cwd_scan`).
    ...
    # see :data:`goalrail.inner.os_env._DEFAULT_ENV_PASSTHROUGH`).
    ...
    # ``goalrail/inner/egress/proxy.py``). Defends against
...
class AgentDef:
    ...
    :param instructions: Resolved system-prompt text loaded from
        ...
        See :func:`goalrail.inner.loader.load_agent_def` for
