"""Executor that bridges Goalrail messages into a native Codex TUI thread."""
...
from goalrail.inner.native_attachments import materialize_attachment, parse_data_uri
...
class CodexNativeExecutor(Executor):
    """
    Harness-side executor for ``goalrail codex`` web UI turns.
    ...
    async def run_turn(
        ...
        :param tools: Tool schemas from Goalrail. Ignored here;
        ...
        :param config: Per-turn executor config. Its ``model`` and
            ``extra["reasoning_effort"]`` (carrying the Goalrail web
...
def _model_effort_overrides(config: ExecutorConfig | None) -> dict[str, Any]:
    ...
    A model or reasoning-effort change selected in the Goalrail web UI is
...
def _latest_user_input_items(messages: list[Message], bridge_dir: Path) -> list[dict[str, Any]]:
    ...
    :param bridge_dir: Bridge directory for materializing image/file
        attachments, e.g. ``Path("/tmp/goalrail/codex-native/<digest>")``.
...
def _file_block_to_input_item(block: dict[str, Any], bridge_dir: Path) -> dict[str, Any] | None:
    ...
    if path is not None:
        ...
        # goalrail/entities/conversation.py. Keep in sync.
