...
event stream into Goalrail ExecutorEvents.

Goalrail tools are bridged into Pi via a generated JavaScript extension
that registers each tool with ``pi.registerTool()``.  Tool execution is
proxied over a local TCP socket to the Goalrail Python process, so
policies, history recording, sub-agents, runtime, and all other Goalrail
...
# ---------------------------------------------------------------------------
# TCP tool server — serves Goalrail tools to the Pi extension
# ---------------------------------------------------------------------------
...
class _ToolServer:
    ...
    async def _evaluate_policy(  # type: ignore[explicit-any]
        ...
        Goalrail outage can't wedge the agent mid-turn.
        """
...
# ---------------------------------------------------------------------------
# Pi extension generator — bridges Goalrail tools into Pi
# ---------------------------------------------------------------------------
...
def _generate_extension_js(port: int, tool_schemas: list[ToolSpec], token: str) -> str:
    """Generate a JavaScript Pi extension that registers Goalrail tools.

    Each tool is forwarded to the TCP tool server at ``127.0.0.1:<port>``.

    :param port: TCP port the Goalrail tool server is listening on, e.g.
        ``54321``.
    :param tool_schemas: Goalrail tool schemas to register with Pi.
    ...
    return f"""\
// Auto-generated Goalrail tool bridge extension for Pi.
// Connects to the Goalrail TCP tool server on port {port}.
...
/**
 * Ask the Goalrail tool server for a TOOL_CALL policy verdict on a native
...
module.exports = function(pi) {{
  // Gate native (non-bridged) tool calls through Goalrail policy. Pi's
  ...
  pi.on("tool_call", async (event) => {{
    ...
    if (verdict && verdict.block) {{
      return {{ block: true, reason: verdict.reason || "blocked by Goalrail policy" }};
    }}
  }});

  for (const tool of TOOLS) {{
    // Pi passes tool.parameters directly to the LLM as JSON Schema, so we
    // can use the Goalrail schema as-is without TypeBox conversion.
    pi.registerTool({{
      name: tool.name,
      label: tool.name,
...
async def _create_subprocess_exec(*args: Any, **kwargs: Any) -> asyncio.subprocess.Process:  # type: ignore[explicit-any]
    ...
    ``goalrail.inner.pi_executor.asyncio.create_subprocess_exec``
...
def _clean_pi_env(extra_allowed: Sequence[str] | None = None) -> dict[str, str]:
    ...
    :func:`goalrail.inner.codex_executor._clean_codex_env`. The previous
...
class PiSubprocessConfig:
    ...
    :param extra_args: Extra CLI arguments to append to the Pi invocation,
        e.g. ``["--extension", ".../goalrail_tools.js"]``.
    """
...
def _build_pi_prompt(
    ...
    :param messages: Goalrail conversation history for the
        turn.
...
def _extract_pi_turn_usage(
    ...
    carries the resolved ``model``. This translates those into goalrail's
    usage schema (see :class:`goalrail.inner.executor.TurnComplete`).
...
def _aggregate_pi_turn_usage(
    ...
    A single Goalrail turn drives Pi's full agent loop, which may make
...
class PiExecutor(Executor):
    ...
    async def _gate_native_tool(  # type: ignore[explicit-any]
        ...
        """Evaluate a native Pi tool call against Goalrail TOOL_CALL policy.
        ...
        PreToolUse hooks: the verdict is computed by the Goalrail server
    ...
    def _build_env_and_dir(
        ...
        :param tools: Goalrail tool schemas to bridge into Pi.
        :param tool_server_port: TCP port the Goalrail tool server is
        ...
        """
        env = dict(self._env)
        tmp_dir = tempfile.mkdtemp(prefix="goalrail_pi_")
        ...
        if self._gateway:
            ...
            from goalrail.inner.pi_settings import prepare_managed_pi_agent_dir
        ...
        # Generate the Goalrail tool bridge extension if tools are available.
        if tools and tool_server_port is not None:
            ...
            ext_path = os.path.join(tmp_dir, "goalrail_tools.js")
            ...
            # an Goalrail TOOL_CALL policy verdict; see
