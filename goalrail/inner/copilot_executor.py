...
per Goalrail conversation, created once and reused turn to turn. Each
...
:class:`~goalrail.inner.executor.ToolSpec` becomes a :class:`copilot.Tool`
...
``edit`` / ``bash``) run *inside* the SDK and never flow through Goalrail's
...
def _build_copilot_prompt(messages: list[Message], *, is_first_turn: bool) -> str:
    ...
    Goalrail system prompt is delivered separately (``system_message``), so on
...
class CopilotExecutor(Executor):
    ...
    def _make_tools(self, tools: list[ToolSpec]) -> list[Any]:  # type: ignore[explicit-any]
        """Build the SDK ``tools`` list from Goalrail ToolSpecs.
        ...
        ``skip_permission=True`` because Goalrail's policy layer (and the
    ...
    def _make_handler(self, tool_name: str) -> Callable[[Any], Awaitable[Any]]:  # type: ignore[explicit-any]
        """Build an async ``handler`` that bridges a Copilot tool call to Goalrail.
    ...
    async def _ensure_session(
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or `pip install 'goalrail[copilot]'`)."
            ) from exc
        ...
        try:
            ...
            # its built-in tools) and layers the Goalrail agent's system prompt
...
def _unwrap_tool_result(raw: Any) -> Any:  # type: ignore[explicit-any]
    ...
    Goalrail-convention result (e.g. ``{"blocked": True}``) passes through
