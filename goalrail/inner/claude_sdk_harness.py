...
around a :class:`goalrail.inner.claude_sdk_executor.ClaudeSDKExecutor`
...
than per-spec data on the request body. Goalrail sets these env vars
...
- ``HARNESS_CLAUDE_SDK_OS_ENV``: JSON-encoded :class:`OSEnvSpec`
  ...
  Goalrail mode parity with the legacy non-AP path holds for
...
# Set by the Goalrail workflow layer when executor.auth: {type: api_key, …} is
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    Goalrail serialized via :func:`dataclasses.asdict` on its
...
def _resolve_retry_policy() -> RetryPolicy:
    ...
    missing — Goalrail omits the env var when ``llm.retry`` matches
