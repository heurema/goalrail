"""Executor that bridges Goalrail web-chat turns into the native Cursor TUI.

It does not launch cursor-agent — the ``goalrail cursor`` wrapper already
...
class CursorNativeExecutor(Executor):
    """Harness-side executor for ``goalrail cursor`` web-UI turns.
