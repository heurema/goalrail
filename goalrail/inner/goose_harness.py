...
Wraps a :class:`goalrail.inner.goose_executor.GooseExecutor`, which drives
...
(:mod:`goalrail.inner.goose_native_harness`). Goose's mid-turn tool approvals
surface as web elicitation cards (via Goalrail's TOOL_CALL policy + ``ctx.elicit``
...
``~/.config/goose/config.yaml``); Goalrail stores no Goose credential. A spec
...
- ``HARNESS_GOOSE_CWD``: working directory for the goose subprocess. ``None``
  falls back to ``GOALRAIL_RUNNER_WORKSPACE`` then the inherited cwd.
...
def _build_goose_executor() -> Executor:
    ...
    cwd_raw = os.environ.get(_ENV_CWD) or os.environ.get("GOALRAIL_RUNNER_WORKSPACE")
    cwd = cwd_raw or None
