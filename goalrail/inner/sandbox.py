...
from goalrail.runner.identity import RUNNER_AUTH_SECRET_ENV_VARS
...
_SANDBOX_STRACE_ENV = "GOALRAIL_SANDBOX_STRACE"
...
_LAUNCHER_WRAPPED_ENV = "GOALRAIL_LAUNCHER_SPAWN_WRAPPED"
...
class SandboxPolicy:
    ...
    :param deny_unix_socket_paths: Absolute paths to AF_UNIX (pathname)
        ...
        ``[Path("/tmp/goalrail-terminal-ab12/tmux.sock")]``.
    ...
    dropped — see :func:`goalrail.inner.os_env._HelperProcessClient.
...
class SandboxBackend(ABC):
    ...
    def wrap_launcher_argv(
        ...
        Called by :class:`goalrail.inner.os_env._HelperProcessClient`
        ...
        :param argv: The unwrapped command argv the parent intends to
            ...
            "goalrail.inner.os_env", "helper", "<encoded>"]``.
    ...
    def post_spawn(self, policy: SandboxPolicy, pid: int) -> ContainmentHandle | None:
        ...
        Called by :class:`goalrail.inner.os_env._HelperProcessClient` on
...
def create_private_tmpdir() -> Path:
    return Path(tempfile.mkdtemp(prefix="goalrail-osenv-"))
...
def run_launcher(encoded_sandbox: str, target_path: str, argv: list[str]) -> int:
    ...
    *argv* and returns its exit code.
    ...
    if (
        ...
        # ``/tmp/goalrail-sandbox-*.py`` written by
        ...
        logger.info(
            "[goalrail-sandbox] spawn-time wrap re-exec backend=%s wrap_head=%s",
    ...
    logger.info(
        "[goalrail-sandbox] activating backend=%s active=%s target=%s",
        sandbox.backend_type,
        sandbox.active,
        target_path,
    )
    ...
    if os.environ.get(_SANDBOX_STRACE_ENV):
        ...
        if strace_bin is None:
            ...
            logger.warning(
                "[goalrail-sandbox] strace active; wrapping target with %s",
                strace_bin,
            )
    try:
        activate_sandbox(sandbox)
        logger.info("[goalrail-sandbox] activated; spawning target=%s", target_path)
        completed = subprocess.run(target_argv)
        logger.info("[goalrail-sandbox] target exited rc=%s", completed.returncode)
...
def create_exec_launcher(target_path: str, sandbox: SandboxPolicy) -> str:
    ...
    fd, path = tempfile.mkstemp(prefix="goalrail-sandbox-", suffix=".py")
    project_root = repr(str(_project_root()))
...
def get_backend(type_name: str) -> SandboxBackend:
    ...
    (:meth:`goalrail.inner.os_env._HelperProcessClient._start_locked`)
...
def _default_sandbox_for_platform() -> OSEnvSandboxSpec:
    ...
    (e.g. :func:`goalrail.inner.loader._parse_os_env_sandbox_spec`
    and :func:`goalrail.spec.parser._parse_os_env_sandbox`) to fill
    ...
    :meth:`goalrail.inner.bwrap_sandbox.BwrapSandboxBackend.resolve`,
...
def _project_root() -> Path:
    # File lives at goalrail/inner/sandbox.py; climb two levels to the
    # repo root that hosts `goalrail/` as a package.
