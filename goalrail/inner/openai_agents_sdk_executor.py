...
Goalrail as the system of record for tools, policies, and session history.
Goalrail tools are exposed as SDK FunctionTools whose callbacks route back
through the Session's tool registry.

Conversation state is delegated to the SDK's session layer. Each Goalrail
Session gets its own SDK SQLiteSession keyed by the Goalrail session id.
"""
...
from goalrail.reasoning_effort import OPENAI_AGENTS_EFFORTS, validate_effort
...
# Tool executor callable wired in by ``goalrail.Session``. The result
# is JSON-ish (dict[str, Any]) but the static type leaks ``Any`` through
# the kwargs dict. Constrained here to match goalrail.executor.
...
def _normalize_responses_items_for_chat(
    ...
    :func:`~goalrail.inner.open_responses_sdk._convert_messages_to_responses`
...
def _normalize_content_blocks_for_chat(
    ...
    Goalrail-only metadata (for example ``filename``) from known Responses
    content block types before the OpenAI Agents SDK serializes them to the
    provider API.

    Background: :mod:`goalrail.runtime.content_resolver` resolves uploaded
    file IDs to inline ``data:<mime>;base64,<data>`` URIs stored in
    ``file_data`` / ``image_url`` while preserving UI/history metadata such as
    ``filename``.  That metadata is useful in Goalrail, but the OpenAI API
    ...
    for block in content:
        ...
        if block_type == "input_file":
            ...
            # Preserve inline images; strip Goalrail-only metadata.
...
def _copy_known_keys(block: dict[str, Any], keys: tuple[str, ...]) -> dict[str, Any]:
    ...
    Goalrail content blocks can carry UI/history metadata such as
