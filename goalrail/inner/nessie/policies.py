...
(``goalrail/runner/policy.py``) and add no server routes. See
...
def blast_radius(
    *,
    ...
    def _evaluate(event: _Json, config: _Json) -> _Json:  # noqa: ARG001
        ...
        """
        # Match the Goalrail built-in OS shell, the Claude/Codex native
...
def spawn_bounds(
    *,
    ...
    hook the runner calls (``goalrail/runner/policy.py``). This is the v1
...
def worktree_guard(
    *,
    ...
    # Match Goalrail built-in OS write/edit, Claude/Codex native Write/Edit
    # (surfaced via the PreToolUse hook), and Pi's native lowercase
    # write/edit (surfaced via the pi ``tool_call`` hook). Pi uses the same
    # ``path`` argument key as the Goalrail tools, so no Pi-specific arg
...
POLICY_REGISTRY: list[dict[str, Any]] = [
    {
        "handler": "goalrail.inner.nessie.policies.blast_radius",
        "kind": "factory",
    ...
    {
        "handler": "goalrail.inner.nessie.policies.spawn_bounds",
        "kind": "factory",
    ...
    {
        "handler": "goalrail.inner.nessie.policies.headless_subagent_purpose_guard",
        "kind": "factory",
