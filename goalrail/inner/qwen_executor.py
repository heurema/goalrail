...
class QwenExecutor(Executor):
    ...
    def __init__(
        ...
        :param gateway_base_url: OpenAI-compatible base URL of an Goalrail
        ...
        # reads/writes back to us (executed through the Goalrail OSEnvironment,
        ...
        # ``session/request_permission`` routes through Goalrail's TOOL_CALL
    ...
    async def _start_process(self) -> None:
        ...
        env = os.environ.copy()
        # Translate Goalrail's provider/gateway routing into the OpenAI-compatible
    ...
    def _sandbox_launch_path(self, spawn_env_names: Sequence[str]) -> str:
        ...
        Mirrors :func:`goalrail.inner.pi_executor._try_sandbox_pi`. When
    ...
    async def _ensure_initialized(self) -> None:
        ...
        resp = await self._rpc(
            _AGENT_METHOD_INITIALIZE,
            {
                ...
                "clientInfo": {"name": "goalrail", "version": "1.0"},
    ...
    async def _respond_to_agent_request(self, request: dict[str, Any]) -> None:  # type: ignore[explicit-any]
        ...
        - ``session/request_permission`` — decide via Goalrail's TOOL_CALL
        ...
        - ``fs/read_text_file`` / ``fs/write_text_file`` — when fs delegation is
          ...
          routes its file I/O here. Executed through the Goalrail OSEnvironment
          so the spec's sandbox read/write roots are enforced at the Python
          layer and the I/O flows through Goalrail rather than qwen touching
        ...
        try:
            if method == "session/request_permission":
                ...
                error = {
                    "code": -32601,
                    "message": f"goalrail: unsupported ACP request method {method!r}",
                }
    ...
    async def _ensure_os_environment(self) -> OSEnvironment:
        ...
        if self._os_environment is None:
            ...
            if env is None:
                raise _AcpRequestError(-32603, "goalrail: no os_env for fs delegation")
    ...
    def _accumulate_usage(  # type: ignore[explicit-any]
        ...
        ``MessageEmitter.emitUsageMetadata``). A single Goalrail turn can drive
