...
class _GoalrailYamlLoader(yaml.SafeLoader):
...
_GoalrailYamlLoader.yaml_implicit_resolvers = {
    key: value[:] for key, value in yaml.SafeLoader.yaml_implicit_resolvers.items()
}
for key, resolvers in list(_GoalrailYamlLoader.yaml_implicit_resolvers.items()):
    _GoalrailYamlLoader.yaml_implicit_resolvers[key] = [
...
_GoalrailYamlLoader.add_implicit_resolver(  # type: ignore[no-untyped-call]
    ...
    *,
    ...
    :param enforce_handler_allowlist: When ``True``, reject any
        ...
        **calls** it. This is the guard for the untrusted
        agent-bundle upload path: ``goalrail.spec.load`` routes a
        single-file goalrail YAML bundle here during
        ...
        ``goalrail run``, operator specs, the CLI) keep working with
    ...
    if isinstance(path_or_dict, (str, Path)):
        ...
        with open(path) as f:
            data = yaml.load(f, Loader=_GoalrailYamlLoader)
...
def _read_contained_file(root: Path, value: str) -> str | None:
    ...
    Mirrors :func:`goalrail.spec.parser._read_contained_file`: resolves
...
def _resolve_instructions(
    ...
    Mirrors :func:`goalrail.spec.parser._resolve_instructions`
    so goalrail-flavored YAMLs and native Goalrail YAMLs treat the
    field identically:
    ...
    - File-path-shaped value that doesn't resolve: silently
      treated as inline text. Matches native Goalrail behavior so users
...
def _parse_os_env_spec(data: YamlData | str | bool | None) -> OSEnvSpec | None:
    ...
    # Mirror the Goalrail YAML parser's cross-field validation
    # (goalrail/spec/parser.py:_parse_os_env). Keeping the two
    ...
    # accepts with weaker semantics than the Goalrail parser would have
    # allowed).
...
def _effective_terminal_sandbox(
    ...
    :func:`goalrail.inner.terminal.build_terminal_os_env_spec`: if the
...
def _parse_os_env_sandbox_spec(data: YamlData | str | bool | None) -> OSEnvSandboxSpec:
    ...
    if raw_type is None:
        ...
        # (same behavior as the Goalrail YAML parser, kept in sync so legacy
        # and Goalrail loaders agree on what an "untyped" sandbox block means).
    ...
    # Mirror the Goalrail parser's hard reject of ``egress_rules`` paired with
    ...
    # (``goalrail.spec.parser._parse_credential_proxy``) rather than a
    # second copy so the single-file goalrail-YAML path and the
...
def load_agent_def_from_path(path_str: str) -> AgentDef:
    ...
    - ``foo.yaml`` (single-file goalrail YAML) — parsed directly.
    - ``my-agent/`` (goalrail AGENTSPEC directory with
      ``config.yaml``, or a directory containing exactly one
      goalrail YAML at its root).
    ...
    from goalrail.spec import _find_goalrail_yaml_in_dir

    if path.is_dir():
        resolved_dir = path
        cleanup: tempfile.TemporaryDirectory[str] | None = None
    else:
        from goalrail.spec.tar_utils import extract_safe

        cleanup = tempfile.TemporaryDirectory(prefix="goalrail-bundle-")
    ...
    try:
        ...
        if config_yaml.is_file():
            from goalrail.spec import load as load_agent_spec
        ...
        single = _find_goalrail_yaml_in_dir(resolved_dir)
        if single is None:
            raise FileNotFoundError(
                f"{path}: directory has neither a config.yaml "
                f"nor a single-file goalrail YAML at its root"
            )
