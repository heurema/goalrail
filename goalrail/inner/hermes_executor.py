...
turns to maintain conversational context across the Goalrail session without
re-serialising the full history.

Each turn yields text output as ``TextChunk`` / ``TurnComplete`` events.
Goalrail policies are enforced on Hermes' native tool calls via Hermes'
...
# either irrelevant to a headless Goalrail turn or actively harmful
...
def _populate_hermes_home(
    ...
    Creates a ``config.yaml`` that registers the Goalrail policy hook
    ...
    :param server_url: Goalrail server URL.
    ...
    # Layer Goalrail's policy hook config on top.
...
class HermesExecutor(Executor):
    ...
    history is maintained without Goalrail re-serializing the full
    message list.
    ...
    ``config.yaml`` that registers an Goalrail policy hook as a
    ...
    def _setup_hermes_home(self) -> None:
        """Create a per-session ``HERMES_HOME`` with Goalrail policy hooks.

        When the Goalrail server URL and conversation ID are available,
        creates a temp directory with a ``config.yaml`` that registers the
        Goalrail policy hook as a Hermes ``pre_tool_call`` shell hook.
    ...
    def _hermes_session_id(self, session_key: str) -> str | None:
        """Return the stored Hermes session ID for an Goalrail session key."""
    ...
    def handles_tools_internally(self) -> bool:
        ...
        through Goalrail's tool dispatch.  Goalrail policies are enforced
        via Hermes' native ``pre_tool_call`` shell hook that evaluates
        ``PHASE_TOOL_CALL`` against the Goalrail server before each tool
        execution.
        """
        return True
    ...
    async def run_turn(
        ...
        :param messages: Conversation history from Goalrail.
    ...
    def _session_key(self, messages: list[Message]) -> str:
        """
        Derive a stable Goalrail session key from the message list.
    ...
    async def close_session(self, session_key: str) -> None:
        ...
        via `hermes --resume` outside Goalrail.
        """
