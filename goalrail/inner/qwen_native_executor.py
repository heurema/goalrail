"""Executor that bridges Goalrail web-chat turns into the native qwen TUI.

It does not launch ``qwen`` — the ``goalrail qwen`` wrapper already launched the
...
:mod:`goalrail.qwen_native_forwarder` mirrors the JSON event stream.
...
class QwenNativeExecutor(Executor):
    """Harness-side executor for ``goalrail qwen`` web-UI turns.
