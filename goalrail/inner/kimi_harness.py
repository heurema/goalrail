...
:mod:`goalrail.runtime.harnesses._runner` invokes after the parent
process resolves ``"kimi"`` to this module via
:data:`goalrail.runtime.harnesses._HARNESS_MODULES`.

Wraps a :class:`goalrail.inner.kimi_executor.KimiExecutor` that drives
...
``~/.kimi/config.toml`` (out-of-band from Goalrail) — upstream kimi
has no per-spawn ``--config-file`` or env-var provider override.
Goalrail-side provider injection remains a deferred follow-up.
"""
...
from goalrail.inner.kimi_executor import KimiExecutor, _resolve_skills_dirs
...
def _resolve_os_env() -> OSEnvSpec:
    ...
    has its own internal sandbox / approval flow, so Goalrail does not
...
def _build_kimi_executor() -> Executor:
    ...
    return KimiExecutor(
        ...
        # else the runner's GOALRAIL_RUNNER_WORKSPACE (the cwd the user launched
        ...
        cwd=os.environ.get(_ENV_CWD) or os.environ.get("GOALRAIL_RUNNER_WORKSPACE") or None,
