...
def _parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    """
    parser = argparse.ArgumentParser(prog="python -m goalrail.claude_native_status")
    ...
    # status bar still renders for goalrail-launched sessions.
...
def _write_context_atomic(bridge_dir: Path, payload: dict[str, object]) -> None:
    ...
    try:
        ...
        print(f"goalrail claude status: write failed: {exc}", file=sys.stderr)
...
def _chain(command: str, stdin_payload: str) -> None:
    ...
    try:
        ...
        print(f"goalrail claude status: chain failed: {exc}", file=sys.stderr)
        return
    if proc.stdout:
