"""Native Kimi TUI wrapper for the Goalrail CLI.

``goalrail kimi`` launches the Kimi CLI's interactive TUI (``kimi``
with no args) inside a Goalrail-runner-owned tmux terminal and attaches the
local TTY — the kimi analog of ``goalrail codex`` / ``goalrail pi``. The runner
spawns the process (see :func:`goalrail.runner.app._auto_create_kimi_terminal`);
...
from goalrail._wrapper_labels import KIMI_NATIVE_WRAPPER_VALUE as _WRAPPER_LABEL_VALUE
...
_KIMI_PATH_ENV = "GOALRAIL_KIMI_PATH"
...
def run_kimi_native(
    *,
    ...
    """
    Launch the Kimi TUI in a Goalrail terminal.
    ...
    if server is None:
        raise click.ClickException(
            "Kimi requires a resolved Goalrail server URL. The CLI should call "
            "_ensure_backend before run_kimi_native."
        )
    with TemporaryDirectory(prefix="goalrail-kimi-native-") as tmpdir:
...
def _materialize_kimi_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``goalrail kimi``.
...
def _run_with_remote_server(
    ...
    *,
    ...
    """
    Launch Kimi on a Goalrail server via a daemon-spawned runner.
