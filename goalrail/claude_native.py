...
from goalrail._native_resume_hint import echo_native_resume_hint
from goalrail._runner_startup import RunnerStartupProgress, runner_startup_progress
from goalrail._startup_profile import StartupProfiler
from goalrail._terminal_picker_theme import (
...
from goalrail.claude_native_state import (
    read_launch_state,
    redirect_launch_state,
    write_launch_state,
)
from goalrail.conversation_browser import conversation_url, open_conversation_link_if_enabled
...
from goalrail.native_terminal import (
...
class PreparedClaudeTerminal:
    ...
    :param tmux_socket: Runner tmux server socket path when the
        ...
        e.g. ``Path("/tmp/goalrail-501/.../tmux.sock")``. ``None``
...
def run_claude_native(
    *,
    ...
    :param startup_profiler: Optional shared startup profiler from the
        ...
        ``GOALRAIL_CLAUDE_STARTUP_PROFILE``.
    ...
    with TemporaryDirectory(prefix="goalrail-claude-native-") as tmpdir:
...
def _resolve_session_id_for_resume(
    *,
    ...
    if not resume_picker:
        return None
    # Deferred — goalrail_client / repl pull in heavy graphs we don't want at startup.
...
def _align_working_directory_with_session(
    ...
    *,
    ...
    :mod:`goalrail.claude_native_state`); this helper reads it back
...
def _strip_resume_from_claude_args(args: tuple[str, ...]) -> tuple[str, ...]:
    ...
    ``goalrail claude -- --resume <id>``) must not have it reach
...
def _provider_config_for_native_claude(entry: ProviderEntry) -> ClaudeNativeProviderConfig | None:
    ...
    harness does (:func:`goalrail.runtime.workflow.configure_agent_harness_with_provider`).
    ...
    """
    from goalrail.onboarding.provider_config import ANTHROPIC_FAMILY
...
def resolve_native_claude_config(
    *,
    ...
    ``goalrail claude`` and the runner's host-spawned auto-create), so the
    native harness honors ``goalrail setup`` exactly like the in-process
    ...
    ``goalrail setup`` provider config — there is no CLI/env profile
    override.

    :param spec: The agent spec, or ``None`` for the bare ``goalrail
        claude`` launch.
    :returns: The launch config, or ``None`` to use Claude's own login.
    """
...
def _materialize_claude_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first session agent spec used by ``goalrail claude``.
    ...
    raw = {
        ...
        # ``goalrail/runner/app.py``. Claude Code already operates
...
def _run_with_local_server(
    ...
    *,
    ...
    startup_profiler = startup_profiler or StartupProfiler(name="goalrail claude", enabled=False)
    ...
    try:
        ...
        if resolved_session_id is not None:
            ...
            # independent (writes to ``~/.goalrail/``), so chdiring
            # now is safe.
...
def _can_attach_direct_tmux(prepared: PreparedClaudeTerminal) -> bool:
    ...
    :func:`goalrail.codex_native._can_attach_direct_tmux`.
...
async def _attach_direct_tmux(
    ...
    *,
    ...
    environment so a user who runs ``goalrail claude`` from inside
    ...
    """
    from goalrail.terminals.ws_bridge import _tmux_session_alive
...
async def _sleep(seconds: float) -> None:
    ...
    loop — see ``goalrail-testing`` skill rule 14 for why globally
...
# (``goalrail host``) rather than this CLI: ensure the daemon, ask
...
async def _prepare_claude_terminal_via_daemon(
    *,
    ...
    """
    from goalrail.claude_native_bridge import bridge_dir_for_conversation_id
...
def _run_with_remote_server(
    ...
    *,
    ...
    :returns: None.
    """
    from goalrail.chat import _bundle_agent, _remote_headers, _server_auth
    from goalrail.cli import _ensure_host_daemon
...
async def _ensure_local_claude_resume_transcript(
    ...
    *,
    ...
    :param workspace: Resolved directory Claude will run in — its
        ...
        ``GOALRAIL_RUNNER_WORKSPACE``. Pass an already-resolved
...
def _synthetic_claude_transcript_uuid(
    *,
    ...
    return str(
        uuid.uuid5(
            uuid.NAMESPACE_URL,
            f"goalrail-claude-resume:{session_id}:{external_session_id}:{stable_item_id}",
        )
    )
...
async def _create_claude_session(
    ...
    *,
    ...
    ``goalrail.wrapper = claude-code-native-ui`` label until the
...
class _ClaudeTerminalTmux:
    ...
    :param socket: tmux server socket path the runner advertised in
        ...
        ``Path("/tmp/goalrail-501/.../tmux.sock")``. ``None`` when
        absent.
...
def _claude_terminal_request(
    ...
    *,
    ...
    spec: dict[str, Any] = {
        ...
        # deployments; ``goalrail claude`` is a local-only
...
def _websocket_connect(attach_url: str, *, headers: dict[str, str]) -> Any:
    ...
    handshake_headers = {**headers, "Origin": GOALRAIL_INTERNAL_WS_ORIGIN}
    try:
...
def claude_terminal_resource_id() -> str:
    """
    Return the deterministic terminal id used by ``goalrail claude``.
