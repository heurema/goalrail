...
- carries a ``config.toml`` that is the user's config text with two Goalrail
  ...
  read-only surface, both dispatched to :mod:`goalrail.kimi_native_hook`.
...
def render_kimi_hooks_toml(*, bridge_dir: Path, python_executable: str | None = None) -> str:
    ...
    Both hooks dispatch to :mod:`goalrail.kimi_native_hook` with the bridge
    ...
    :param python_executable: Interpreter to run the hook module; ``None`` uses
        :data:`sys.executable` (the runner's interpreter, which has goalrail).
    ...
    # ``sys.path[0]``. A workspace that contains its own ``goalrail/`` directory
    ...
    # from the path, importing only the interpreter's own goalrail. Mirrors
    # claude-native's ``python -I -m goalrail.claude_native_hook``.
    base = f"{shlex.quote(python)} -I -m goalrail.kimi_native_hook"
