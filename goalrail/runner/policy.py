...
Pre-refactor the Goalrail server's :class:`PolicyEngine` enforced
...
``evaluate_policy=True`` to the Goalrail server, which independently
re-evaluates and parks an elicitation; the runner then awaits the
verdict via :mod:`goalrail.runner.pending_approvals`. The dual
...
from goalrail.policies import FunctionPolicy, resolve_function_policy
...
class PolicyVerdict:
    ...
    on ASK (and escalate to the Goalrail server) rather than treating
    ...
    :param action: ``"allow"`` (tool may proceed), ``"deny"`` (tool
        ...
        the Goalrail server for a user verdict; ``policy_name`` and
