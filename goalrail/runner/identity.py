...
RUNNER_ID_ENV_VAR = "GOALRAIL_RUNNER_ID"
RUNNER_PARENT_PID_ENV_VAR = "GOALRAIL_RUNNER_PARENT_PID"
...
RUNNER_WORKSPACE_ENV_VAR = "GOALRAIL_RUNNER_WORKSPACE"
RUNNER_TUNNEL_BINDING_TOKEN_ENV_VAR = "GOALRAIL_RUNNER_TUNNEL_BINDING_TOKEN"
RUNNER_TUNNEL_TOKEN_HEADER = "X-Goalrail-Runner-Tunnel-Token"
...
GOALRAIL_INTERNAL_WS_ORIGIN = "goalrail://internal"
...
RUNNER_ISOLATE_SESSION_ENV_VAR = "GOALRAIL_RUNNER_ISOLATE_SESSION"

# Marker env var stamped into every agent-facing environment so any
# process launched inside an Goalrail agent session can detect it is
# running under Goalrail. This is the analog of Claude Code's
# ``CLAUDE_CODE`` / ``CLAUDECODE`` and Codex's ``CODEX``. It is set once
# on the runner process (see :mod:`goalrail.runner._entry`) and inherited
...
# scrub.
GOALRAIL_SESSION_ENV_VAR = "GOALRAIL"
GOALRAIL_SESSION_ENV_VALUE = "1"
...
def token_bound_runner_id(token: str) -> str:
    ...
    digest = hashlib.sha256(f"goalrail-runner:{stripped}".encode()).hexdigest()
