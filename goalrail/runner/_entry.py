...
- ``RUNNER_SERVER_URL``: Goalrail server base URL for outbound calls
...
if TYPE_CHECKING:
    ...
    from goalrail.runner.transports.ws_tunnel.serve import _ASGIApp
...
_RUNNER_CONFIG_HOME_ENV_VAR = "GOALRAIL_CONFIG_HOME"
...
def _server_url_from_env() -> str:
    """Return the required Goalrail server URL from the runner environment.
...
def _runner_config_path() -> Path:
    ...
    :envvar:`GOALRAIL_CONFIG_HOME` for test isolation and subprocess
...
def _runner_parent_pid_from_env() -> int | None:
    ...
    """
    from goalrail.runner.identity import RUNNER_PARENT_PID_ENV_VAR
...
def _runner_workspace_from_env() -> Path | None:
    ...
    """
    from goalrail.runner.identity import RUNNER_WORKSPACE_ENV_VAR
...
def _runner_isolate_session_from_env() -> bool:
    """Return ``True`` when ``GOALRAIL_RUNNER_ISOLATE_SESSION`` is ``"1"``.

    See :data:`RUNNER_ISOLATE_SESSION_ENV_VAR` for the contract.
    """
    from goalrail.runner.identity import RUNNER_ISOLATE_SESSION_ENV_VAR
...
async def _resolve_agent_spec_from_server(
    ...
    """
    Fetch, cache, and parse one agent spec bundle from the Goalrail server.
...
def create_app(
    ...
    # Stamp the Goalrail session marker into the runner's environment so
    # every process this runner spawns can detect it is running inside an
    # Goalrail agent session, the way Claude Code sets CLAUDE_CODE and
    ...
    os.environ[GOALRAIL_SESSION_ENV_VAR] = GOALRAIL_SESSION_ENV_VALUE

    # Keep the harness manager on its default /tmp/goalrail root.
    ...
    # The Goalrail server's POST /v1/sessions/{id}/mcp handles policy
    # evaluation and delegates execution here via
    # POST /v1/sessions/{id}/mcp/execute (tunneled through the WS
    # tunnel the runner opened to the Goalrail server at startup).
    ...
    async def spec_resolver(agent_id: str, session_id: str | None = None) -> ResolvedSpec | None:
        """
        Fetch agent spec from the Goalrail server, extract under the
    ...
    from goalrail.inner.terminal import reap_orphaned_terminals
    ...
    async def _start_pm() -> None:
        ...
        if prewarm_path and mcp_manager is not None:
            try:
                from goalrail.spec import load as _load_spec
...
async def _run_tunnel_from_env() -> None:
    ...
    :returns: None.
    """
    from goalrail.runner.identity import get_stable_runner_id
    from goalrail.runner.transports.ws_tunnel.serve import serve_tunnel
...
def main() -> None:
    ...
    :returns: None.
    """
    log_level = os.environ.get("GOALRAIL_LOG_LEVEL", "INFO").upper()
    logging.basicConfig(
