...
from goalrail.tools.base import is_valid_tool_name
from goalrail.tools.mcp import McpServerConnection
...
def _build_accept_content(
    ...
    :func:`goalrail.tools._elicitation_schema.build_accept_content_from_schema`.
...
def _mcp_tool_schema(
    ...
    """
    from goalrail.tools.mcp import _normalize_input_schema
...
class RunnerMcpManager:
    ...
    def __init__(
        ...
        :param server_client: ``httpx.AsyncClient`` pointed at the
            Goalrail server. When provided, inline MCP elicitations are
            surfaced to the user via the Goalrail server's session events
    ...
    def _build_elicitation_callback(
        ...
        async def _elicit(
            ...
            When an Goalrail server client is available, POSTs a
            ...
            if server_client is None:
                _logger.warning(
                    "MCP elicitation callback: no Goalrail server client available — declining",
                )
            ...
            try:
                ...
                _logger.warning(
                    "MCP elicitation callback: Goalrail server POST failed (%s) — declining",
                    exc,
                )
                return ElicitResult(action="decline")
            ...
            if not elicitation_id:
                _logger.warning(
                    "MCP elicitation callback: Goalrail server returned no "
            ...
            # won't fire on timeout/cancellation, so the Goalrail server's
            # sidebar badge may stay stale. Same pattern as
            # proxy_mcp_manager. A future enhancement could POST
            # the resolved event back to the Goalrail server here.
