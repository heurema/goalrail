...
Routes all MCP calls through the Goalrail server's
...
The Goalrail server holds the live connections in its
:class:`goalrail.server.mcp_pool.ServerMcpPool` and enforces
...
interface as :class:`goalrail.runner.mcp_manager.RunnerMcpManager` so
it can be substituted transparently at every dispatch site in
``runner/app.py``.

When to use each implementation:

- **AP mode** (out-of-process runner paired with Goalrail server):
  ``ProxyMcpManager(session_id, server_client)`` — all tool calls flow
  through the Goalrail server, policy enforced centrally.
- **No-AP / test mode**:
...
from goalrail.runner.tool_dispatch import MCP_PROXY_CALL_TIMEOUT_S
...
class ProxyMcpManager:
    """Routes runner-side MCP calls through the Goalrail server MCP proxy endpoint.

    Drop-in substitute for :class:`goalrail.runner.mcp_manager.RunnerMcpManager`
    used in Goalrail mode.  The runner creates one instance per session on first MCP use
    and passes it to :func:`goalrail.runner.tool_dispatch.execute_tool` in place
    of a direct manager.
    ...
    def __init__(
        ...
        :param ap_client: HTTP client pointed at the Goalrail server.
        ...
        self._goalrail_client = ap_client
    ...
    async def schemas_for(self, spec: AgentSpec) -> McpSchemasResult:
        """Fetch tool schemas from the Goalrail server MCP proxy (``tools/list``).

        Sends a ``tools/list`` JSON-RPC 2.0 request to the Goalrail server's MCP
        proxy endpoint.  Returns tool schemas in the same flat OpenAI
        function-tool format as
        :meth:`goalrail.runner.mcp_manager.RunnerMcpManager.schemas_for`.

        Tool names are returned with the server namespace prefix applied by
        the Goalrail server (e.g. ``github__search``).  This ensures the harness
        ...
        try:
            resp = await self._goalrail_client.post(
                self._mcp_url,
                json=payload,
                timeout=30.0,
            )
        ...
        for tool in tools_list:
            ...
            if not name:
                continue
            # The Goalrail server returns ``inputSchema`` (JSON Schema from MCP).
    ...
    async def call_tool(
        ...
        """Dispatch a tool call via the Goalrail server MCP proxy (``tools/call``).

        Sends a ``tools/call`` JSON-RPC 2.0 request.  The Goalrail server enforces
        TOOL_CALL and TOOL_RESULT policies before forwarding to the real MCP
        server.

        **ASK policy / approval flow**: when the Goalrail server returns an
        ...
        :returns: Tool output string.  On denial the Goalrail server returns an
        ...
        """
        del spec  # Goalrail server resolves spec from session context
        ...
        for _attempt in range(2):
            ...
            # ── ASK: Goalrail server returned InputRequiredResult ───────────────
            ...
            if result.get("resultType") == "input_required":
                ...
                # The Goalrail server uses the elicitation_id as the key in
    ...
    async def prewarm(self, spec: AgentSpec) -> None:
        """No-op — the Goalrail server warms connections lazily via ServerMcpPool.
    ...
    async def shutdown(self) -> None:
        """No-op — the Goalrail server owns and manages MCP connections."""
