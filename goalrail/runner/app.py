...
if TYPE_CHECKING:
    ...
    from goalrail.codex_native_app_server import CodexAppServerClient
    from goalrail.runner.cost_advisor import AdvisorTurnResult
    from goalrail.terminals.registry import TerminalListEntry

import httpx
from fastapi import FastAPI, HTTPException, Query, Request, WebSocket
from fastapi.responses import JSONResponse, Response, StreamingResponse

from goalrail._platform import IS_WINDOWS
...
from goalrail.llms.summarize import (
    build_summarization_input,
    build_summarization_prompt,
    extract_summary_text,
)
from goalrail.model_override import validate_model_override
from goalrail.policies.types import FAIL_CLOSED_PHASES
from goalrail.runner import pending_approvals
from goalrail.runner.codex.goal import CodexGoalRunner
from goalrail.runner.proxy_mcp_manager import ProxyMcpManager
...
from goalrail.spec.skill_sources import SkillSourceContext, resolve_harness_skills
from goalrail.spec.types import AgentSpec, LocalToolInfo, SkillSpec
from goalrail.terminals.ws_bridge import (
    WS_CLOSE_TERMINAL_NOT_FOUND,
    bridge_tmux_pty_to_websocket,
)
from goalrail.tools.builtins.load_skill import (
    find_skill_by_name,
...
# ASK gate: policy evaluation (``_evaluate_policy_via_goalrail``) and sub-agent
...
# Terminal resource hosting the framework's own TUI (the Goalrail REPL,
# ``goalrail attach``) for runner-hosted SDK sessions — the SDK mirror of
...
# delivery signal for the last child of a fan-out, and Goalrail routinely
...
def _publish_tmux_target_for_bridge(
    *,
    ...
    from goalrail.claude_native_bridge import bridge_dir_for_bridge_id, write_tmux_target
...
class _CodexNativeLaunchConfig:
    ...
    :param policy_server_url: Goalrail server URL for the Codex policy hook and
    ...
    :param fork_source_id: SOURCE conversation id stamped on a forked
        clone (``goalrail.fork.source_id``), used to locate the
    ...
    :param fork_source_external_id: SOURCE Codex thread id stamped on a
        forked clone (``goalrail.fork.source_external_session_id``),
    ...
    :param fork_carry_history: ``True`` on a forked clone bound to a
        native target (``goalrail.fork.carry_history``); when no source
        rollout exists to clone (an SDK or cross-family source) the runner
        builds the clone's rollout from the copied Goalrail items instead (see
        ``_ensure_local_codex_resume_rollout``).
    :param bypass_sandbox: ``True`` when the session opted into Codex's
        DANGEROUS full-bypass stance (``goalrail.codex_native.bypass_sandbox``
...
class _PiNativeLaunchConfig:
    ...
    :param server_url: Goalrail server URL for the extension/forwarder.
    ...
    :param fork_source_external_id: SOURCE Pi session id stamped on a forked
        clone (``goalrail.fork.source_external_session_id``); consulted only
        when the clone has no native session of its own yet.
    :param fork_carry_history: ``True`` on a forked clone bound to a native
        target (``goalrail.fork.carry_history``); when no source session
        exists to clone, the clone's session is rebuilt from its OWN copied
        Goalrail items (see :func:`_auto_create_pi_terminal`). Also consumed by
...
def _codex_session_workspace(session_workspace: str | None) -> Path:
    ...
    runner's ``GOALRAIL_RUNNER_WORKSPACE``.
    ...
    :raises RuntimeError: If no workspace is available (neither the
        session snapshot nor ``GOALRAIL_RUNNER_WORKSPACE``).
    """
    raw = session_workspace or _required_runner_env("GOALRAIL_RUNNER_WORKSPACE")
...
async def _pi_native_launch_config(
    *,
    ...
    :param server_client: Runner Goalrail server client.
...
class _OpenCodeNativeLaunchConfig:
    ...
    :param policy_server_url: Goalrail server URL for the forwarder.
    ...
    :param fork_carry_history: ``True`` on a forked clone whose prior
        ...
        (``goalrail.fork.carry_history``); opencode has no native session to
        clone, so the runner rehydrates from the copied Goalrail transcript.
    """
...
async def _opencode_native_launch_config(
    *,
    ...
    from goalrail.stores.conversation_store import FORK_CARRY_HISTORY_LABEL_KEY
...
async def _auto_create_opencode_terminal(
    ...
    *,
    ...
    :param server_client: Runner Goalrail server HTTP client.
    :param ensure_comment_relay: Callback that starts the Goalrail builtin-tool
        relay for this session's bridge dir (the nested
        ``_ensure_comment_relay_started``). ``None`` skips wiring the Goalrail
        MCP relay (tests / no server).
    :returns: The created terminal resource view.
    """
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    from goalrail.opencode_native_forwarder import OpenCodeNativeForwarder
    ...
    # Goalrail builtin-tool relay (wired below) can start. Safe to call before
    ...
    # Accumulate the synthesized opencode.json: a pinned default model + the
    ...
    # Build opencode's ``mcp`` block: the Goalrail builtin-tool relay (so the
    # model can call sys_*/load_skill/web_fetch — the real "connects to Goalrail
    ...
    # through Goalrail's policy engine via the forwarder's permission gate —
    ...
    if server_client is not None and ensure_comment_relay is not None:
        mcp_block.update(build_opencode_goalrail_mcp_server(bridge_dir))
    if mcp_block:
        config.setdefault("$schema", "https://opencode.ai/config.json")
        config["mcp"] = mcp_block
        config["permission"] = "ask"

    # Load the Goalrail policy-bridge plugin so opencode's lifecycle hooks reach
    ...
    # PostToolUse hooks); coordinates come from the GOALRAIL_* env stamped on
    ...
    if server_client is not None and runner_server_url:
        ...
        policy_env["GOALRAIL_POLICY_URL"] = runner_server_url
        policy_env["GOALRAIL_SESSION_ID"] = session_id
        ...
        from goalrail.runner._entry import _make_auth_token_factory

        _policy_factory = _make_auth_token_factory()
        _policy_token = _policy_factory() if _policy_factory is not None else None
        if _policy_token:
            policy_env["GOALRAIL_POLICY_AUTH"] = f"Bearer {_policy_token}"
    ...
    # Start the Goalrail builtin-tool relay BEFORE opencode boots, so
    ...
    # relay POSTs each call back through the Goalrail server (policy enforced).
    if server_client is not None and ensure_comment_relay is not None:
        ...
        try:
            ...
            if launch_config.external_session_id is not None:
                ...
                if existing is not None:
                    ...
                    # XDG store) — we'll rehydrate from the Goalrail transcript
                    # below instead of silently starting empty.
                    resume_lost_history = True
            if opencode_session_id is None:
                created = await client.create_session({"title": f"goalrail:{session_id}"})
                ...
                # seed the copied Goalrail transcript as a noReply preamble.
                if resume_lost_history or launch_config.fork_carry_history:
                    await _rehydrate_opencode_session_from_transcript(
                        ...
                        goalrail_session_id=session_id,
                        server_client=server_client,
                        model_override=model_override,
                    )
    ...
    # lifetime; ``server_client`` is the runner's Goalrail client. The
...
def _build_opencode_policy_evaluator(
    *,
    ...
    :param server_client: Runner's Goalrail server HTTP client.
    :param conversation_id: Owning Goalrail session id, e.g. ``"conv_abc"``.
    :returns: An async evaluator returning a verdict mapping, or a deny
        verdict on failure.
    """
    from goalrail.opencode_native_permissions import OPENCODE_NATIVE_HARNESS
...
def _opencode_native_model_from_spec(agent_spec: Any | None) -> str | None:
    ...
    if agent_spec is None:
        return None
    try:
        from goalrail.runtime.workflow import _resolve_spec_model
...
def _render_opencode_transcript_text(items: list[Any]) -> str:
    """
    Render committed Goalrail message items into a plain-text transcript.
...
async def _rehydrate_opencode_session_from_transcript(
    *,
    ...
    persisted opencode session is gone) inject the Goalrail transcript as a
    ...
    try:
        resp = await server_client.get(
            f"/v1/sessions/{urllib.parse.quote(goalrail_session_id, safe='')}/items",
    ...
    try:
        ...
        _logger.warning(
            "opencode resume: rehydration seed failed for %s", goalrail_session_id, exc_info=True
        )
        return False
    return True
...
def _pi_args_have_session_control(args: list[str]) -> bool:
    ...
    :returns: ``True`` when Goalrail should not add resume/session flags.
    """
    session_flags = {
        "--session-dir",
        "--session",
...
def _pi_args_have_provider(args: list[str]) -> bool:
    ...
    Goalrail must not inject the ``goalrail setup`` provider on top — the
    explicit choice wins.

    :param args: User pass-through Pi CLI args.
    :returns: ``True`` when Goalrail should not add provider/model args.
    """
...
def _build_pi_native_args(
    *,
    ...
    :param extension_path: Generated Goalrail Pi extension path.
    :param session_dir: Per-Goalrail-session Pi session directory.
...
async def _resolve_pi_resume_session(
    *,
    ...
    1. **Cold resume** — the session already carries a captured Pi
       ...
       file from committed Goalrail items so ``pi --session <id>`` opens with
    ...
    2. **Fork rebuild** — a forked clone bound to a pi-native target with NO
       ...
       session id, build its file from the clone's OWN copied Goalrail items,
       and patch the server so Goalrail reflects the clone's session id and a
    ...
    from goalrail.pi_native_resume import ensure_local_pi_resume_session, mint_pi_session_id
    ...
    model = ""
    try:
        from goalrail.pi_native_credentials import resolve_pi_native_provider
    ...
    # yet. Build the clone's session from its OWN copied Goalrail items under a
    ...
    if launch_config.fork_carry_history:
        ...
        if built is not None:
            # Record the minted id so Goalrail reflects the clone's own Pi
...
async def _auto_create_pi_terminal(
    ...
    *,
    ...
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from goalrail.pi_native import resolve_pi_executable
    from goalrail.pi_native_bridge import (
    ...
    from goalrail.pi_native_bridge import extension_path as pi_extension_path
    from goalrail.runner._entry import _make_auth_token_factory
    ...
    # Build the Goalrail tool surface (sys_* tools) the Pi extension registers
    ...
    # can call Goalrail tools with centralized server-side policy enforcement
    ...
    try:
        from goalrail.runner.tool_dispatch import build_native_relay_tool_schemas
    ...
    # Rebuild the local Pi session JSONL from committed Goalrail items so a
    ...
    if not _pi_args_have_provider(launch_config.terminal_launch_args or []):
        from goalrail.pi_native_credentials import (
...
async def _auto_create_cursor_terminal(
    ...
    *,
    ...
    :returns: Created terminal resource view.
    """
    from goalrail.cursor_native import resolve_cursor_executable
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from goalrail.cursor_native import is_valid_cursor_chat_id
    ...
    from goalrail.cursor_native_forwarder import clear_cursor_bridge_state, preseed_resume_state
    from goalrail.cursor_native_usage import clear_cursor_usage_state
    ...
    # Render the copied Goalrail items once and stash them; the executor prepends
    ...
    if launch_config.fork_carry_history and not preseeded and server_client is not None:
        try:
            from goalrail.claude_native import _fetch_all_session_items_for_claude_resume
    ...
    # passthrough launch args (``goalrail cursor -- --model X`` or the joined
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from goalrail.cursor_native_bridge import write_tmux_target
    ...
    # Mirror the Cursor TUI's conversation back into the Goalrail session so the
    ...
    from goalrail.cursor_native_usage import supervise_cursor_usage_forwarder
    ...
    async def _supervise_cursor_native_bridges() -> None:
        ...
        calls (see :mod:`goalrail.cursor_native_permissions`) — more reliable
        ...
        :mod:`goalrail.cursor_native_usage`).
        """
...
async def _auto_create_goose_terminal(
    ...
    *,
    ...
    ``~/.config/goose/config.yaml``), so HOME is inherited and Goalrail writes no
    ...
    :returns: Created terminal resource view.
    """
    from goalrail.goose_native import resolve_goose_executable
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    await _cancel_auto_forwarder_task(session_id)
    from goalrail.goose_native_bridge import bridge_dir_for_session_id, write_tmux_target
    from goalrail.goose_native_forwarder import clear_goose_bridge_state
    ...
    # Mirror the Goose TUI's conversation back into the Goalrail session so the
    ...
    async def _supervise_goose_native_bridges() -> None:
        ...
        a web elicitation (see :mod:`goalrail.goose_native_permissions`).
        """
        await asyncio.gather(
...
async def _auto_create_hermes_terminal(
    ...
    *,
    ...
    ``~/.hermes/config.yaml``), so HOME is inherited and Goalrail writes no vendor
    ...
    ``cwd`` + ``started_at`` floor (see :mod:`goalrail.hermes_native_forwarder`).
    ...
    :returns: Created terminal resource view.
    """
    from goalrail.hermes_native import resolve_hermes_executable
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    ...
    # drop the prior terminal's stale forward cursor.
    await _cancel_auto_forwarder_task(session_id)
    from goalrail.hermes_native_bridge import (
        bridge_dir_for_session_id,
        read_hermes_home,
        write_policy_hook_config,
        write_tmux_target,
    )
    from goalrail.hermes_native_forwarder import clear_hermes_bridge_state

    bridge_dir = bridge_dir_for_session_id(session_id)
    clear_hermes_bridge_state(bridge_dir)

    # Write a per-session HERMES_HOME with the Goalrail policy hook so the
    # native TUI evaluates tool calls against Goalrail policies.
        ...
        if _source_db is not None and _source_db.is_file():
            ...
            if _target_db is not None:
                try:
                    ...
                    # the forwarder only mirrors NEW messages (Goalrail already
                    # has the cloned ones from the fork item copy).
                    if _clone_max_id > 0:
                        from goalrail.hermes_native_forwarder import (
                            _ForwardState,
                            _write_state,
                        )
    ...
    # Mirror the Hermes TUI's conversation back into the Goalrail session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from goalrail.runner._entry import _make_auth_token_factory, _RunnerBearerAuth
    ...
    from goalrail.hermes_native_permissions import supervise_hermes_approval_mirror
    ...
    async def _supervise_hermes_native_bridges() -> None:
        ...
        elicitation (see :mod:`goalrail.hermes_native_permissions`).
        """
...
async def _auto_create_kiro_terminal(
    ...
    *,
    ...
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from goalrail.kiro_native import build_kiro_launch
    ...
    if terminal_registry is not None:
        ...
        if instance is not None and instance.running:
            from goalrail.kiro_native_bridge import write_tmux_target
...
async def _persist_qwen_external_session_id(
    ...
    """Record the qwen session id on the Goalrail session as ``external_session_id``.
    ...
    ``fork_conversation`` stamps as ``goalrail.fork.source_external_session_id``
    ...
    :param server_client: Runner Goalrail server client (``None`` skips the write).
...
async def _auto_create_qwen_terminal(
    ...
    *,
    ...
    env vars or ``~/.qwen`` from ``/auth``), so HOME is inherited and Goalrail
    ...
    """
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from goalrail.qwen_native import resolve_qwen_executable
    ...
    from goalrail.qwen_native_forwarder import clear_qwen_bridge_state
    ...
    # is persisted on the Goalrail session and read back from the snapshot
    # (``launch_config.external_session_id``), which also lets a fork carry history
    # (``goalrail.fork.source_external_session_id``). qwen is cleaner than
    ...
    # the default in-terminal prompt (the embedded pane shows it) — Goalrail-side
    ...
    # Mirror the qwen TUI's conversation back into the Goalrail session so the
    # chat view tracks the embedded terminal. Host-spawned sessions have no CLI
    # client to start this, so the runner owns it — reusing the runner's own
    # server URL + refresh-capable auth.
    from goalrail.runner._entry import _make_auth_token_factory, _RunnerBearerAuth
    ...
    from goalrail.qwen_native_bridge import qwen_session_recording_path
    ...
    async def _supervise_qwen_native_bridges() -> None:
        ...
        elicitations (see :mod:`goalrail.qwen_native_permissions`); the compaction
        ...
        :func:`goalrail.qwen_native_forwarder.supervise_qwen_compaction_mirror`).
        """
        await asyncio.gather(
            supervise_qwen_forwarder(
                base_url=server_url,
...
async def _auto_create_kimi_terminal(
    ...
    *,
    ...
    :func:`goalrail.kimi_native_credentials.build_kimi_session_home`) that
    mirrors the user's global ``kimi login`` (symlinked ``oauth`` / providers)
    and adds the Goalrail tool-policy hooks — a ``PreToolUse`` deny-gate and a
    ``PermissionRequest`` read-only surface dispatched to
    :mod:`goalrail.kimi_native_hook`. The hook subprocess reads its routing
    from ``hook_config.json`` in the bridge dir.

    A background forwarder (:func:`goalrail.kimi_native_forwarder.
    supervise_kimi_forwarder`) tails kimi's per-session ``wire.jsonl`` transcript
    and mirrors each user prompt + assistant reply into the Goalrail chat, so the
    ...
    :param resource_registry: Session resource registry for launching the
        terminal.
    :param publish_event: Runner session event publisher.
    :param server_client: Runner Goalrail server client (used only for the
    ...
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec
    from goalrail.kimi_native import resolve_kimi_executable
    from goalrail.kimi_native_bridge import (
        bridge_dir_for_session_id,
        write_hook_config,
        write_tmux_target,
    )
    from goalrail.kimi_native_credentials import build_kimi_session_home
    from goalrail.kimi_native_forwarder import clear_kimi_bridge_state, supervise_kimi_forwarder
    from goalrail.runner._entry import _make_auth_token_factory
    ...
    # launch args (``goalrail kimi -- <args>``) are persisted on the session
    # snapshot and threaded here.
    kimi_args = list(launch_config.terminal_launch_args or [])

    # Wire the Goalrail tool-policy hooks: kimi reads a single
    ...
    # ``goalrail.kimi_native_hook``. The hook subprocess reads the server URL +
    ...
    # Mirror the kimi TUI transcript into the Goalrail chat: tail the per-session
...
async def _auto_create_codex_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        surface the new terminal on the live stream (the Goalrail relay
    ...
    :param server_client: Runner's Goalrail server HTTP client. Used to read
    ...
    from goalrail.inner.codex_executor import _find_codex_cli
    ...
    if (
        ...
        from goalrail.codex_native import _clone_codex_rollout, _mint_codex_thread_id
        ...
        if cloned_rollout is not None:
            ...
            # Record the assigned thread id now so Goalrail reflects the clone's
            ...
            if server_client is not None:
                try:
                    ...
                    # unset on the Goalrail session and a future relaunch of this
        ...
        # rollout from its OWN copied Goalrail items under a thread id we mint, then flip
        ...
        from goalrail.codex_native import (
            _ensure_local_codex_resume_rollout,
    ...
    if launch_config.external_session_id is not None and original_external_session_id is not None:
        from goalrail.codex_native import _ensure_local_codex_resume_rollout
    ...
    # Goalrail coordinates for the codex-native policy hook. The hook runs as a
    ...
    event_client = CodexAppServerClient(
        ws_url=codex_ws_url,
        client_name="goalrail-codex-native-auto",
    )
    if launch_config.external_session_id is None:
        ...
        from goalrail.codex_native_bridge import CodexNativeBridgeState, write_bridge_state
        ...
        terminal_view = await resource_registry.launch_auxiliary_terminal(
            ...
            spec=TerminalEnvSpec(
                ...
                args=build_codex_remote_args(
                    ...
                    # Goalrail provider instead of falling back to the
                ...
                # Match the local ``goalrail codex`` terminal scrollback.
                scrollback=100_000,
...
async def _codex_discover_thread_and_forward(
    *,
    ...
    """
    Adopt the fresh Codex TUI's thread, then mirror it into the Goalrail session.
    ...
    from goalrail.codex_native_forwarder import (
        supervise_forwarder,
    ...
    try:
        ...
        # Mirror the discovered Codex thread id onto the Goalrail session as its
        ...
        # ``goalrail.fork.source_external_session_id`` from
        ...
        # would resume fresh. Best-effort: a transient Goalrail failure here still
...
async def _run_antigravity_reader(
    *,
    ...
    :func:`goalrail.antigravity_native_reader.run_reader_with_bridge` (used by both
    this runner path and the CLI ``goalrail antigravity`` attach fallback); it
    ...
    """
    from goalrail.antigravity_native_reader import run_reader_with_bridge
...
async def _auto_create_antigravity_terminal(
    ...
    *,
    ...
    :func:`goalrail.antigravity_native._launch_antigravity_terminal`, which
    ...
    *attended* (``headless=False``). Auto-bypass comes only from the user's
    ...
    :param ensure_comment_relay: The runner's relay starter
        (``_ensure_comment_relay_started``). When provided, the Goalrail MCP
    ...
    from goalrail.antigravity_native_launch import build_agy_launch
    ...
    # Wire the Goalrail MCP relay so the wrapped agy gets the sys_* tools
    # (spawn sub-agent sessions, drive Goalrail terminals, list agents/models,
    ...
    terminal_view = await resource_registry.launch_required_terminal(
        ...
        spec=TerminalEnvSpec(
            ...
            args=list(argv[1:]),
            env=env_overrides,
            # Match the local ``goalrail antigravity`` terminal scrollback.
            scrollback=100_000,
            ...
            # ``goalrail antigravity`` path keeps start-on-attach: there a human
            # TTY is the driver.)
            tmux_start_on_attach=False,
        ),
    )
    ...
    # elicitations via the Task 9 hook. The reader owns its own Goalrail client
...
def _mint_runner_agy_conversation_id() -> str:
    ...
    :func:`goalrail.antigravity_native._mint_agy_conversation_id`.
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    :func:`goalrail.antigravity_native_rpc.resolve_cold_start_agy_rpc_port`:
    ...
    *timeout_s*, or ``StartCascade`` erroring) must NOT abort the auto-create:
    ...
    :param server_client: Runner Goalrail server client used for the
    ...
    from goalrail.antigravity_native_rpc import (
        AntigravityRpcError,
...
def _terminal_tmux_pane(
    ...
    # instance (see goalrail.inner.terminal). Guard defensively so a registry
...
async def _session_payload_for_host_spawn_check(
    ...
    :param server_client: The runner's Goalrail server HTTP client, or
...
async def _codex_session_needs_runner_terminal(
    ...
    - **Sub-agent children** (dispatched server-side via
      ...
      e.g. polly run via ``goalrail run --server`` — silently never got
    ...
    :param server_client: The runner's Goalrail server HTTP client, or ``None`` in
...
def _pi_native_model_from_spec(agent_spec: AgentSpec | ResolvedSpec | None) -> str | None:
    ...
    Reads the canonical ``spec.executor.model`` field (the same field the
    in-process harnesses and cursor-native consume). The resolved model is
    threaded into ``resolve_pi_native_provider(model=...)`` (see
    :func:`goalrail.pi_native_credentials.resolve_pi_native_provider`) so the
    generated ``models.json`` (and the appended ``--model``) selects it.

    :param agent_spec: Agent spec object, or a resolved wrapper carrying a
        ``spec`` attribute. ``None`` means no spec was available.
    :returns: A model id, e.g. ``"claude-opus-4-7"``, or ``None`` when the
        spec declares no model (Pi then uses the provider default).
    """
    spec = agent_spec.spec if isinstance(agent_spec, ResolvedSpec) else agent_spec
    if spec is None:
        return None
...
def _cursor_fork_history_preamble(items: list[dict[str, Any]]) -> str:
    ...
    :func:`goalrail.cursor_native_bridge.wrap_fork_preamble`.

    :param items: Committed Goalrail items (``GET /v1/sessions/{id}/items``),
        chronological.
...
def _build_claude_native_base_args(
    *,
    ...
    bridge / MCP / hook / Goalrail wiring. The order is: ``--resume`` for a
    ...
    :param resume_external_session_id: Claude-native session id to
        ...
        :func:`goalrail.claude_native._clone_claude_transcript`), so
    ...
    """
    from goalrail.reasoning_effort import CLAUDE_EFFORTS
...
def _publish_terminal_pending(
    ...
    fails. The Goalrail relay caches the latest value and republishes it, and
...
def _publish_native_terminal_start_error(
    ...
    outside a transcript turn; Goalrail writes and publishes the turn-scoped
...
def _codex_ensure_response_with_policy_notice(
    ...
    ``policy_hook_disabled_reason`` exactly once so Goalrail can post a single
...
def _ensure_orchestrator_skills_in_bundle(
    ...
    """
    Link the ``build-goalrail`` skill into a bundle's ``skills/`` dir.

    Called before native bridge launches so ``--plugin-dir`` (claude) or
    ``CODEX_HOME/skills/`` (codex) picks up the skill. Injects
    unconditionally for every agent — every ``goalrail claude`` /
    ``goalrail codex`` user should be able to author new agents. The
    ...
    :param bundle_dir: Materialized agent-bundle root, e.g.
        ``/tmp/goalrail-ap-chat-xyz/bundle``.
...
async def _auto_create_claude_terminal(
    ...
    *,
    ...
    :param publish_event: The runner's per-session SSE emitter, used to
        ...
        republishes it to the web UI) so the Terminal toggle enables
        without a refresh.
    :param server_client: Goalrail server client used to fetch the session
    ...
    from goalrail.claude_native_forwarder import reset_transcript_forward_state
    from goalrail.inner.datamodel import OSEnvSpec, TerminalEnvSpec

    workspace = os.environ.get("GOALRAIL_RUNNER_WORKSPACE", str(Path.cwd()))
    ...
    # build the clone's native transcript from the copied Goalrail items
    ...
    if server_client is not None and session_external_id is not None:
        from goalrail.claude_native import _ensure_local_claude_resume_transcript
        ...
        from goalrail.claude_native import _clone_claude_transcript
        ...
        if _cloned is not None:
            ...
            # Record the assigned id now so Goalrail reflects the clone's own
        ...
        # native transcript from its OWN copied Goalrail items under a uuid we
        ...
        if _built is not None:
            resume_external_session_id = our_uuid
            # Record the assigned id so Goalrail reflects the clone's own Claude
    ...
    from goalrail.entities.session_resources import session_resource_view_to_dict
    ...
    # back to the Goalrail server. Normally the CLI client runs this,
    ...
    # offset 0 already holds every item Goalrail has. Starting the forwarder at
    ...
    # ``claude_native.py``.
    from goalrail.claude_native_forwarder import supervise_forwarder
...
async def _auto_create_repl_terminal(
    ...
    *,
    ...
    """
    Auto-create an Goalrail REPL terminal for a runner-hosted SDK session.
    ...
    terminal hosts the framework's own TUI (``goalrail attach
    ...
    like the claude-/codex-native terminals — with the Goalrail REPL as
    the TUI.
    ...
    attach the session is fully live (``goalrail attach`` fails loud on a
    ...
    :param publish_event: The runner's per-session SSE emitter,
        ...
        without a refresh.
    :param server_client: Goalrail server client used to stamp the
        ``goalrail.ui: terminal`` presentation label that makes the web
    ...
    env_spec = TerminalEnvSpec(
        ...
        # The runner's interpreter is the venv with goalrail installed;
        # ``python -m goalrail`` avoids depending on the console script
        # being on the tmux pane's PATH.
        command=sys.executable,
        args=["-m", "goalrail", "attach", session_id, "--server", server_url],
        scrollback=50000,
    ...
    terminal_view = await resource_registry.launch_auxiliary_terminal(
        ...
        resource_role=GOALRAIL_REPL_TERMINAL_ROLE,
    )
    ...
    # show a dead pill. The ``goalrail.wrapper`` label is deliberately
    ...
    _logger.info(
        "Auto-created goalrail REPL terminal for session %s: terminal_id=%s "
...
async def _claude_native_bridge_id_for_session(
    *,
    ...
    :returns: Opaque bridge id from
        ``goalrail.claude_native.bridge_id`` when present, otherwise
...
async def _claude_native_terminal_arrives_via_transfer(
    *,
    ...
    :param server_client: Goalrail client to resolve the bridge id label;
...
async def _antigravity_native_terminal_arrives_via_transfer(
    *,
    ...
    :func:`goalrail.antigravity_native_reader._rotate_session_for_cascade`) binds the
...
# marker.
_RUNNER_DISPATCHED_FIELD = "goalrail_runner_dispatched"
...
async def _evaluate_policy_via_goalrail(
    *,
    server_client: httpx.AsyncClient,
    ...
    """
    Proxy a policy evaluation request from the harness to the Goalrail server.

    Called by the runner's ``proxy_stream`` when it intercepts a
    ``policy_evaluation.requested`` SSE event from the harness. Posts
    the evaluation request to the Goalrail server's
    ...
    - ``PHASE_LLM_REQUEST`` / ``PHASE_LLM_RESPONSE`` fail OPEN
      (``POLICY_ACTION_ALLOW``) so a transient Goalrail outage does not
    ...
    :param server_client: HTTP client pointed at the Goalrail server.
    ...
    verdict_reason: str | None = (
        f"Goalrail policy evaluation unavailable; failing closed for {phase}."
        if _fail_closed
        else None
    )
...
def _is_spec_local_native_python_tool(spec: Any, tool_name: str) -> bool:
    ...
    return any(
        ...
        and getattr(info, "language", None) in ("python", "goalrail-python-callable")
...
# Language constant the goalrail YAML translator stamps on callable-backed
# tools (goalrail/spec/goalrail.py:GOALRAIL_TOOL_LANGUAGE). Duplicated rather
# than imported to avoid pulling the heavy translator module in for one
# string — same rationale as goalrail/tools/local_callable.py.
_GOALRAIL_CALLABLE_LANGUAGE = "goalrail-python-callable"
...
def _looks_like_file_path(path: str) -> bool:
    ...
    source extension (see :func:`goalrail.spec.parser._discover_local_tools`).
...
def _spec_with_workdir_paths(spec: Any, workdir: Path | None) -> Any:
    ...
    for info in local_tools:
        ...
        if (
            path
            and getattr(info, "language", None) != _GOALRAIL_CALLABLE_LANGUAGE
...
def _merge_advisor_note(
    ...
    :param note_item: The advisor's note message item (see
        :func:`goalrail.runner.cost_advisor._advisor_note_item`), e.g.
...
async def _resolve_forwarded_message_content(
    ...
    *,
    ...
    Remote Goalrail servers can forward session messages with raw file IDs
...
def register_subagent_work(
    *,
    ...
    :param wrapper_label: Optional child ``goalrail.wrapper``
...
def _wake_post_is_retryable(exc: httpx.HTTPError) -> bool:
    ...
    Goalrail returns while the parent's runner tunnel is reconnecting), as
    ...
    if not isinstance(exc, httpx.HTTPStatusError):
        # Transport failure — the POST may never have reached Goalrail.
        return True
...
async def _deliver_subagent_wake_post(
    ...
    :param server_client: Goalrail HTTP client for the runner subprocess.
...
def _subagent_delivery_not_confirmed_response(
    ...
    *,
    ...
    was created as a sub-agent. For known sub-agents, Goalrail must not receive a
...
def create_runner_app(
    *,
    ...
    :param server_client: httpx.AsyncClient pointed at the AP
        ...
        In-process: pointed at the Goalrail ASGI app.
    ...
    app = FastAPI(title="goalrail-runner")
    ...
    if terminal_registry is not None:
        from goalrail.runtime import _globals as _rt_globals
    ...
    # Same guard for the Goalrail REPL (``goalrail attach``) terminal
    ...
    # ``goalrail.runner.pending_approvals`` so the runner-side
    # policy gate (``goalrail.runner.tool_dispatch``) can register
    ...
    def _child_preview_from_status(
        ...
        *,
        ...
        history fallback because Goalrail owns native transcript persistence. The
    ...
    def _publish_session_status(session_id: str, status: str) -> None:
        ...
        own turns lets the Goalrail server relay it through the normal status
        ...
        if event.last_output:
            ...
            parts.extend(
                [
                    ...
                    "Goalrail captured a pane snapshot.",
                ]
            )
        return "\n".join(parts)
    ...
    from goalrail.entities.environment_filesystem import (
        FilesystemEntry,
        ResourceError,
    )
    ...
    async def create_session(request: Request) -> JSONResponse:
        ...
        if spec is not None:
            ...
            if _sa_name_assign:
                from goalrail.runtime.workflow import _find_spec_by_name
            ...
            if harness_name == "pi-native" and spawn_env is None:
                from goalrail.pi_native_bridge import build_pi_native_spawn_env
            ...
            if harness_name == "cursor-native" and spawn_env is None:
                from goalrail.cursor_native_bridge import build_cursor_native_spawn_env

                spawn_env = build_cursor_native_spawn_env(session_id)
            if harness_name == "kiro-native" and spawn_env is None:
                from goalrail.kiro_native_bridge import build_kiro_native_spawn_env
            ...
            if harness_name == "goose-native" and spawn_env is None:
                from goalrail.goose_native_bridge import build_goose_native_spawn_env
            ...
            if harness_name == "qwen-native" and spawn_env is None:
                from goalrail.qwen_native_bridge import build_qwen_native_spawn_env

                spawn_env = build_qwen_native_spawn_env(session_id)
            if harness_name == "kimi-native" and spawn_env is None:
                from goalrail.kimi_native_bridge import build_kimi_native_spawn_env
        ...
        # created: the Goalrail relay's ``GET /stream`` can race ahead of this
        ...
        if harness_name == "claude-native":
            ...
            async with _ensure_lock:
                ...
                if not _has_terminal and not _terminal_inbound:
                    ...
                    # Auto-inject orchestrator skills (build-goalrail)
        ...
        # Auto-bootstrap the Goalrail REPL terminal for non-native
        # (SDK-harness) top-level sessions: host the framework's own TUI
        # (``goalrail attach``) in a tmux pane so the web UI can embed it
            ...
            async with _repl_lock:
                ...
                if not _has_repl_terminal:
                    ...
                    try:
                        ...
                        _logger.exception(
                            "Failed to auto-create goalrail REPL terminal for %s",
                            session_id,
                        )
                    finally:
                        _publish_terminal_pending(_publish_event, session_id, False)
        ...
        # turn with no assistant item, not an unanswered Goalrail task to replay.
    ...
    async def stream_session(session_id: str) -> StreamingResponse:
        ...
        async def _event_generator() -> AsyncIterator[bytes]:
            ...
            HTTP connection, leaving the Goalrail relay on a half-open
            ...
            # Immediate ready ack: Goalrail waits for this frame before
    ...
    async def _codex_native_bridge_state_for_session(
        ...
        *,
        ...
        can have a bridge id distinct from the Goalrail session id, so the
        lookup first resolves ``goalrail.codex_native.bridge_id`` from
    ...
    async def _handle_codex_native_interrupt(conv_id: str) -> Response:
        ...
        injecting a ``[System: interrupted]`` bubble into the Goalrail mirror would
        ...
        *own* ``[Request interrupted by user]`` record, which is real.
        ...
        """
        from goalrail.codex_native_app_server import client_for_transport
    ...
    async def _handle_codex_native_settings_update(
        ...
        injection path: model/effort changes persisted by Goalrail are
    ...
    async def _handle_pi_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from goalrail.pi_native_bridge import bridge_dir_for_session_id, enqueue_interrupt
    ...
    async def _teardown_session_terminals(conv_id: str) -> None:
        ...
        from goalrail.runner.tool_dispatch import _publish_terminal_deleted_event
    ...
    async def _handle_claude_native_stop(conv_id: str) -> Response:
        ...
        This is the runner-side handler for the Goalrail web UI's "Stop
    ...
    async def _handle_cursor_native_interrupt(conv_id: str) -> Response:
        ...
        """
        from goalrail.cursor_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_cursor_native_stop(conv_id: str) -> Response:
        ...
        """
        from goalrail.cursor_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_goose_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from goalrail.goose_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_kimi_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from goalrail.kimi_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_goose_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from goalrail.goose_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_kimi_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from goalrail.kimi_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_hermes_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from goalrail.hermes_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_hermes_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from goalrail.hermes_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_qwen_native_interrupt(conv_id: str) -> Response:
        ...
        :returns: 204 when Escape was sent; 503 if the tmux target is unavailable.
        """
        from goalrail.qwen_native_bridge import bridge_dir_for_session_id, inject_interrupt
    ...
    async def _handle_qwen_native_stop(conv_id: str) -> Response:
        ...
        :returns: 204 on success; 503 if the tmux target is unavailable.
        """
        from goalrail.qwen_native_bridge import bridge_dir_for_session_id, kill_session
    ...
    async def _handle_claude_native_effort_change(
        ...
        * *effort* is in ``EFFORT_VALUES`` but not in
        ...
        if effort is None or effort not in CLAUDE_EFFORTS:
            # Persistence already happened on the Goalrail server; the
    ...
    async def _handle_claude_native_compact(conv_id: str) -> Response:
        ...
        The Goalrail server's own compaction path (``compact_conversation_now``)
        ...
        Returns 200 (not 204) on successful injection so the Goalrail server
        can tell the control was handled in the terminal and skip its
        own AP-side compaction. Other harnesses 204 no-op in the
        ``post_session_events`` dispatch and the Goalrail server runs its
    ...
    async def _handle_codex_native_compact(conv_id: str) -> Response:
        ...
        Returns 200 on successful injection so the Goalrail server
    ...
    async def _handle_opencode_native_compact(conv_id: str) -> Response:
        ...
        compact, and return 200 so the Goalrail server skips its AP-side
        ...
        """
        from goalrail.opencode_native_bridge import bridge_dir_for_bridge_id, read_bridge_state
        from goalrail.opencode_native_client import OpenCodeClientError
    ...
    async def _handle_opencode_native_model_change(conv_id: str, model: str | None) -> Response:
        """
        Apply a Goalrail-initiated model switch to an opencode-native session.
    ...
    async def _handle_cursor_native_compact(conv_id: str) -> Response:
        ...
        :mod:`goalrail.cursor_native_forwarder` when it observes cursor write the
        ...
        complete it).  Returns 200 so the Goalrail server knows the control was
        ...
        """
        from goalrail.cursor_native_bridge import bridge_dir_for_session_id, inject_user_message
    ...
    async def _handle_pi_native_compact(conv_id: str) -> Response:
        ...
        process, so explicit ``/compact`` must run there. The Goalrail
        ...
        Returns 200 (not 204) on successful enqueue so the Goalrail server
        ...
        """
        from goalrail.pi_native_bridge import bridge_dir_for_session_id, enqueue_compact
    ...
    def _inject_codex_compact(socket_path: str, target: str) -> None:
        ...
        sequence that :func:`~goalrail.claude_native_bridge.inject_slash_command`
        ...
        """
        from goalrail.claude_native_bridge import _run_tmux
    ...
    async def _handle_hermes_native_compact(conv_id: str) -> Response:
        ...
        injection so the Goalrail server knows the control was handled in the
    ...
    async def _handle_qwen_native_compact(conv_id: str) -> Response:
        ...
        :func:`goalrail.qwen_native_forwarder.supervise_qwen_compaction_mirror` when
        ...
        """
        from goalrail.qwen_native_bridge import bridge_dir_for_session_id, submit_user_message
    ...
    async def _handle_claude_native_cost_popup(
        ...
        terminal — who never sees the web card — the Goalrail server forwards a
    ...
    async def _handle_codex_native_cost_popup(
        ...
        :func:`goalrail.native_cost_popup.launch_cost_popup`.
        ...
        """
        from goalrail.codex_native_bridge import _POLICY_HOOK_FILE, bridge_dir_for_bridge_id
        from goalrail.native_cost_popup import launch_cost_popup
    ...
    async def _handle_opencode_native_blocked_notice(
        ...
        :returns: 204 once dispatched (or skipped); 503 if launching raised.
        """
        from goalrail.native_cost_popup import launch_blocked_notice
    ...
    async def _native_cost_popup_config_file(conv_id: str, harness: str) -> Path:
        ...
        if harness == "claude-native":
            from goalrail import claude_native_bridge as _cnb
        ...
        from goalrail import codex_native_bridge as _cxb
    ...
    async def _repop_pending_cost_popup_on_attach(
        ...
        if harness not in ("claude-native", "codex-native", "opencode-native"):
            return
        from goalrail.native_cost_popup import launch_cost_popup, wait_for_tmux_client
    ...
    async def _cancel_inprocess_turn(conv_id: str) -> None:
        ...
        try:
            ...
            await harness_client.post(
                f"/v1/sessions/{conv_id}/events",
                json={"type": "interrupt"},
                # Bounded under the Goalrail server's 5s stop deadline.
                timeout=3.0,
            )
    ...
    async def _ensure_comment_relay_started(
        ...
        *,
        ...
        - ``create_session_terminal`` (the ``bridge_inject_dir`` branch), which
          ...
          initial ``tools/list``. This is the normal ``goalrail claude``
        ...
        # giving centralized policy evaluation on the Goalrail server. The exact
        ...
        async def _relay_tool_executor(
            ...
            """
            Relay one MCP tool call through the Goalrail server's /mcp endpoint.

            Routes the call through
            :class:`~goalrail.runner.proxy_mcp_manager.ProxyMcpManager`
            so the Goalrail server evaluates TOOL_CALL and TOOL_RESULT policies
    ...
    async def _run_turn_advisor(
        ...
        from goalrail.runner.cost_advisor import maybe_run_advisor
    ...
    def _emit_routing_decision(conv: str, result: AdvisorTurnResult | None) -> None:
        ...
        if result is None:
            return
        from goalrail.runner.cost_advisor import routing_decision_event
    ...
    async def _advisor_spec_for_session(conv: str) -> Any:  # type: ignore[explicit-any]  # resolved AgentSpec or None
        ...
        except (GoalrailError, httpx.HTTPError, RuntimeError):
            _logger.warning(
    ...
    async def _run_turn_bg(
        ...
        try:
            ...
            # with no resolvable model raising GoalrailError from
    ...
    async def _run_turn_bg_setup_and_stream(
        ...
        if cached_spec is not None:
            ...
            # create, forwarded by the Goalrail server in the message
            ...
            spawn_env = _build_spawn_env_from_spec(
                ...
                # readout). Forwarded by the Goalrail server in the message body.
                model_override=msg_body.get("model_override"),
            )
            from goalrail.runtime.prompt import (
                build_instructions,
            )
        ...
        if conv not in _session_tool_schemas:
            ...
            if cached_spec is not None:
                try:
                    from goalrail.tools.manager import (
                        ToolManager,
                    )
        ...
        if cached_spec and cached_spec.mcp_servers:
            from goalrail.runner.mcp_manager import compute_spec_hash
        ...
        # user message is injected. The normal ``goalrail claude`` /
        # ``goalrail codex`` path already started it at terminal launch, in
    ...
    async def _stream_message_to_harness(
        ...
        if harness_name == "claude-native" and spawn_env is None:
            from goalrail.claude_native_bridge import build_claude_native_spawn_env
        ...
        async def proxy_stream():
            ...
            try:
                async with client.stream(
                    ...
                    _goalrail_task_id: str | None = body.get("task_id")
                    _buffer = ""
                            ...
                            if event is not None:
                                ...
                                if is_action_required(event):
                                    ...
                                    if _should_dispatch and _response_id:
                                        ...
                                        # (ProxyMcpManager in Goalrail mode), which
                                        ...
                                        _dispatch_tasks.append(
                                            _asyncio.create_task(
                                                dispatch_tool_locally(
                                                    ...
                                                    conversation_id=conv_id,
                                                    task_id=_goalrail_task_id or _response_id,
                                ...
                                # proxies the request to the Goalrail server's
    ...
    async def post_session_events(
        ...
        """
        Inbound surface for the Goalrail server's post-migration session
        ...
        / ``ApprovalEvent``) — see
        :class:`goalrail.runtime.harnesses._scaffold.InboundEventRequest`.
        ...
        * ``interrupt`` / ``tool_result`` / ``approval``: control
        ...
        if body_type == "message" or body_type is None:
            ...
            try:
                ...
                if conversation_id in _session_histories:
                    ...
                    # store (invariant I1, goalrail/server/routes/sessions.py:
        ...
        if body_type == "external_session_status":
            ...
            # Keep this allowlist in sync with Goalrail server's
            ...
            if status == "idle":
                # Native transcripts are owned by AP. If Goalrail did not forward
        ...
        if body_type == "stop_session":
            # Goalrail server forwards a "stop session" request here. Native harnesses
        ...
        if body_type == "effort_change":
            # Goalrail server forwards the persisted reasoning_effort here
        ...
        if body_type == "model_change":
            # Goalrail server forwards the persisted model_override here so
        ...
        if body_type == "compact":
            # Goalrail server forwards explicit /compact here. claude-native
        ...
        if body_type == "clear":
            # Goalrail server forwards an explicit /clear here. opencode-native
        ...
        if body_type == "cost_approval_popup":
            # Goalrail server forwards a cost-budget checkpoint here so it can
            ...
            # body) and is always set by the Goalrail server forwarder; fall back
        ...
        try:
            ...
            # 502 so the Goalrail route's "best-effort cancel" branch
    ...
    async def list_session_resources(
        ...
        """
        from goalrail.entities.pagination import paginate_in_memory
    ...
    async def create_session_terminal(
        ...
        if (
            ...
            # ``goalrail antigravity`` CLI wrapper POSTs ``ensure_native_terminal``
        ...
        if declared_terminal is not None:
            ...
            # (goalrail claude) lands here; the launched terminal
        ...
        # Opt-in: callers (e.g. the ``goalrail claude`` wrapper) can ask the
        ...
        try:
            ...
            resource_view = await launch_method(
                ...
                parent_os_env=agent_os_env,
                # The bridge-inject path is the ``goalrail claude``
    ...
    async def transfer_session_terminal(
        ...
        checks: the runner has no Goalrail permission store. Public callers
        must use the Goalrail session-resource transfer route, which validates
    ...
    async def _recreate_repl_terminal(
        ...
        """Re-create a dead embedded Goalrail REPL terminal for attach.
        ...
        exits — the user pressing Ctrl+C inside the REPL, or ``goalrail
        ...
        async with lock:
            ...
            if existing is None or not existing.running or not await existing.is_alive():
                ...
                try:
                    ...
                    _logger.exception(
                        "Failed to recreate goalrail REPL terminal for %s",
                        session_id,
                    )
                    return None
    ...
    async def terminal_resource_attach_ws(
        ...
        The embedded Goalrail REPL terminal (role
        :data:`GOALRAIL_REPL_TERMINAL_ROLE`) gets recreate-on-attach
    ...
    async def search_environment_files(
        ...
        """
        from goalrail.runner.environment_filesystem import (
    ...
    async def read_environment_file_diff(
        ...
        from goalrail.entities.environment_filesystem import InvalidPath
        from goalrail.runner.environment_filesystem import _validate_path
    ...
    async def _resolve_session_spec_entry(session_id: str) -> Any | None:
        ...
        :raises GoalrailError: If the server returns malformed data
    ...
    async def _resolve_session_skills(session_id: str) -> list[SkillSpec]:
        ...
        * user-global host skills (``~/.claude/skills`` etc.), scanned by
          the harness's :func:`~goalrail.spec.skill_sources.resolve_harness_skills`
          provider.
        ...
        :raises GoalrailError: If the session's spec cannot be
            resolved.
        """
        ...
        # Both are unioned; ``resolve_harness_skills`` also scans ``~`` on
    ...
    async def get_session_skills(session_id: str) -> JSONResponse:
        ...
        ``~/.claude/skills/``), not the Goalrail server's. The server overlays
    ...
    # ``_fetch_model_options`` in goalrail/server/routes/sessions.py), so it
    ...
    async def resolve_session_skill(session_id: str, request: Request) -> JSONResponse:
        ...
        resolves at runtime. The Goalrail server calls this, persists the
    ...
    async def reset_session_agent_cache(session_id: str, request: Request) -> JSONResponse:
        ...
        if not isinstance(agent_id, str) or not agent_id:
            agent_id = _session_agent_ids.get(session_id)
        if not agent_id:
            with contextlib.suppress(GoalrailError, httpx.HTTPError, RuntimeError):
                snapshot = await _session_snapshot(session_id)
    ...
    async def mcp_execute(session_id: str, request: Request) -> JSONResponse:
        ...
        Called by the Goalrail server's ``POST /v1/sessions/{id}/mcp`` handler
        **after** TOOL_CALL policy evaluation.  The Goalrail server owns policy
        ...
        - **Runner-local tools** (bare names: ``sys_os_read``,
          ...
          :func:`~goalrail.runner.tool_dispatch.execute_tool` using
        ...
        - ``tools/list`` — return namespaced MCP tool schemas for the
          ...
          injected by the Goalrail server in the turn request body).
        ...
        if method == "tools/list":
            ...
            # Return schemas + failures so the Goalrail server can surface
        ...
        if method == "tools/call":
            ...
            # MRTR retry: Goalrail server forwards inputResponses + requestState
            ...
            if "__" in tool_name:
                ...
                try:
                    from goalrail.tools.mcp import McpElicitationRequired

                    if input_responses is not None:
                        # MRTR retry: the Goalrail server already showed the
                    ...
                    # (MRTR). Pass it back to the Goalrail server so it can
    ...
    def _resolve_provider_connection(
        ...
        Loads providers from ``~/.goalrail/config.yaml`` and extracts
        ...
        """
        try:
            from goalrail.onboarding.detected import effective_config_with_detected
...
def create_runner_app_from_env() -> FastAPI:
    ...
    minimal :class:`httpx.AsyncClient` for the Goalrail server, then delegates
    ...
    :class:`~goalrail.runner.transports.tcp.RunnerTCPSubprocess` and
    :class:`~goalrail.runner.transports.uds.RunnerSubprocess`.  It is
    intentionally lighter than :func:`goalrail.runner._entry.create_app`
...
_HARNESS_MODEL_ENV_KEY: dict[str, str] = {
    ...
    # :mod:`goalrail.inner.kimi_executor`; without this mapping a per-session
...
async def _evaluate_agent_start_gate(
    ...
    """
    from goalrail.runner.policy import RunnerToolPolicyGate
