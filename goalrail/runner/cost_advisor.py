...
:class:`~goalrail.runner.cost_judge.Judge` over the turn's user query
and produces ONE :class:`~goalrail.cost_plan.AdvisorVerdict` sizing the
...
:func:`~goalrail.runner.cost_judge.resolve_advisor_mode`).
"""
...
from goalrail.runner.cost_judge import build_llm_judge, resolve_advisor_mode
...
class Judge(Protocol):
    ...
    The production :class:`~goalrail.runner.cost_judge.LLMJudge`
...
def routing_decision_event(verdict: AdvisorVerdict) -> dict[str, Any]:  # type: ignore[explicit-any]  # JSON-shaped SSE event
    ...
    The item type is in :data:`~goalrail.entities.conversation.NON_CONTENT_ITEM_TYPES`
...
async def maybe_run_advisor(
    *,
    ...
    :param judge: Judge override; ``None`` builds the production
        :class:`~goalrail.runner.cost_judge.LLMJudge` from the spec's
        advisor config.
...
def _runner_identity_headers() -> dict[str, str]:
    ...
    :returns: ``{X-Goalrail-Runner-Tunnel-Token: <token>}`` when the
...
async def _persist_verdict_label(
    ...
    (see ``update_session`` in :mod:`goalrail.server.routes.sessions`).
