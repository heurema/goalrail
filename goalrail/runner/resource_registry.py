...
# Role marker for the embedded Goalrail REPL terminal auto-created for
# runner-hosted SDK sessions (``goalrail attach`` in a tmux pane — the
...
GOALRAIL_REPL_TERMINAL_ROLE = "goalrail-repl"
...
class SessionResourceRegistry:
    ...
    def set_session_status_publisher(
        ...
        event onto the session's SSE queue (which the Goalrail server relays
    ...
    def compute_default_env_root(
        ...
        1. ``self._runner_workspace`` (sourced from
           ``GOALRAIL_RUNNER_WORKSPACE``) — when set, ALWAYS
