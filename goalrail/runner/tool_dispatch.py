...
if TYPE_CHECKING:
    from goalrail.runtime.filesystem_registry import FilesystemRegistry
...
from goalrail.tools.builtins.async_inbox import (
    SysCallAsyncTool,
    SysCancelAsyncTool,
    SysCancelTaskTool,
    SysReadInboxTool,
...
from goalrail.tools.builtins.upload_file import UploadFileTool, safe_resolve
...
_SESSION_WRAPPER_LABEL_KEY = "goalrail.wrapper"
...
# runner (runner → Goalrail server → runner). ``sys_os_shell`` accepts caller-provided
...
# state via the Goalrail server's existing REST endpoints (GET /items, GET
...
# (goalrail.spec.types.SharePolicy) that enable the sys_session_share
...
# these proxy the Goalrail server's REST endpoints (GET /v1/sessions/{id}/agent,
...
# The runner proxies the Goalrail server's session policy REST endpoint.
...
def build_native_relay_tool_schemas(spec: Any | None) -> list[dict[str, Any]]:
    """Build the flat Goalrail tool surface for native harness bridges.
    ...
    centralized policy enforcement on the Goalrail server.
    ...
    try:
        ...
        if _os_env is None:
            raise RuntimeError("create_os_environment returned None for a non-None spec")
        for _tool in (
...
# sys_os_write, e.g. following the ``build-goalrail`` skill).
_AGENT_CONFIG_SUBDIR = ".goalrail/agent-configs"
...
async def _list_child_sessions(
    *,
    ...
    :param server_client: Goalrail server client.
...
def _subagent_harness(sub_agent_name: str, agent_spec: Any | None) -> str | None:
    ...
    """
    from goalrail.model_catalog import spec_harness
...
def _normalize_subagent_model(
    ...
    *,
    ...
    """
    from goalrail.model_catalog import resolve_model_provider
...
async def _execute_list_models_tool(*, agent_spec: Any | None) -> str:
    ...
    cached in :mod:`goalrail.model_catalog`).
    ...
    from goalrail.model_catalog import catalog_for_spec
...
async def _execute_subagent_tool(
    ...
    *,
    ...
    :param server_client: httpx client pointed at the Goalrail server.
    ...
    from goalrail.runner import app as _runner_app
    ...
    if existing is not None:
        ...
        # and the requested harness must canonicalize into GOALRAIL_HARNESSES.
        ...
        # override against the GLOBAL ``GOALRAIL_HARNESSES`` (plus the goalrail
        ...
        if harness_override is not None:
            ...
            if canonical not in GOALRAIL_HARNESSES:
                return (
                    f"Error: sys_session_send 'harness' {harness_override!r} is "
                    f"not a known harness; must be one of {sorted(GOALRAIL_HARNESSES)}."
                )
        ...
        from goalrail.onboarding.harness_install import missing_harness_cli
    ...
    from goalrail.server.schemas import SessionCreatedEvent
    ...
    if created_child:
        ...
        # has no subscribers (they live in the Goalrail server), so a direct
        # publish here is silently dropped. ``publish_event`` enqueues onto
        # the parent's queue, which the Goalrail server's relay republishes onto
...
async def _execute_session_create(
    ...
    *,
    ...
    :param server_client: HTTP client pointed at the Goalrail server; ``None``
...
def _bundle_local_agent_source(source: Path) -> bytes:
    ...
    :param source: Local agent config YAML, agent directory, or
        bundle file, e.g.
        ``Path("/work/.goalrail/agent-configs/helper.yaml")``.
...
async def _upload_config_bundle(
    ...
    *,
    ...
    :param config_path: Caller-supplied path to the agent config YAML,
        ...
        cwd, e.g. ``".goalrail/agent-configs/helper.yaml"``.
...
def _has_subagent(
    ...
    dict (goalrail inner loader) for a matching name.
    ...
    for sa in sub_agents:
        if getattr(sa, "name", None) == sub_agent_name:
            return True
    # Goalrail inner loader: tools dict with AgentTool entries
    tools = getattr(agent_spec, "tools", None)
...
async def _execute_timer_set(
    ...
    *,
    ...
    """
    from goalrail.runner import app as _app
...
async def _execute_comment_tool(
    ...
    *,
    ...
    The runner is a separate subprocess from the Goalrail server and has no
    in-process ``CommentStore``. This handler uses ``server_client`` to
    call the Goalrail server's REST API (``GET/PATCH
...
def _project_api_item(item: dict[str, Any]) -> dict[str, str | None]:
    ...
    Mirrors :func:`goalrail.tools.builtins.spawn._project_activity_item`
...
async def _execute_session_query_tool(
    ...
    *,
    ...
    Goalrail server's existing REST endpoints over ``server_client``:
    ...
    :mod:`goalrail.tools.builtins.spawn` so the LLM sees identical
...
def _goalrail_error_message(resp: httpx.Response) -> str | None:
    """
    Extract the human-readable message from an Goalrail error response.

    The server renders :class:`goalrail.errors.GoalrailError` as
    ``{"error": {"code": ..., "message": ...}}`` (see the exception
    handler in ``goalrail/server/app.py``). Return that ``message`` so a
...
async def _session_share_via_rest(
    ...
    if resp.status_code >= 400:
        ...
        detail = _goalrail_error_message(resp)
...
async def _execute_agent_tool(
    ...
    *,
    ...
    these proxy the Goalrail server's REST endpoints over ``server_client``:

    - ``sys_agent_get`` → ``GET /v1/sessions/{id}/agent`` (project the
      :class:`~goalrail.server.schemas.AgentObject`)
...
async def _agent_get_via_rest(
    ...
    Projects the :class:`~goalrail.server.schemas.AgentObject` fields
...
async def _session_get_history_via_rest(
    ...
    # lives only in the Goalrail server's pending-elicitations index, replayed
...
async def _session_close_via_rest(
    ...
    On success marks the child with ``goalrail.closed=true`` and
...
class _PeekMeta:
    ...
    :param pending_elicitations: Outstanding
        ...
        parked on, replayed on the snapshot from the Goalrail server's
        :mod:`goalrail.runtime.pending_elicitations` index. Empty list
...
def _clone_os_env_spec(spec: Any) -> Any:
    ...
    Symmetric with :func:`goalrail.inner.terminal._clone_sandbox_spec`;
...
def _runner_default_os_env_cwd(conversation_id: str | None) -> str:
    ...
    root = Path(
        os.environ.get(
            "GOALRAIL_RUNNER_OS_ENV_ROOT",
            str(Path(tempfile.gettempdir()) / "goalrail-runner-os-envs"),
        )
    )
...
def _effective_runner_os_env_spec(
    ...
    - When ``runner_workspace`` is set, it ALWAYS wins — whether
      ...
      passes it via ``GOALRAIL_RUNNER_WORKSPACE``) and
    ...
    :param runner_workspace: Authoritative runtime cwd for the
        runner, sourced from ``GOALRAIL_RUNNER_WORKSPACE``.
...
async def _seed_os_env_snapshot(
    ...
    :meth:`~goalrail.runtime.filesystem_registry.FilesystemRegistry.seed_snapshot`
    ...
    :param os_env: The :class:`~goalrail.inner.os_env.OSEnvironment` used for
...
async def _execute_os_env_tool(
    ...
    *,
    ...
    """
    from goalrail.inner.os_env import _DEFAULT_READ_LIMIT, create_os_environment
...
async def _execute_file_tool(
    ...
    *,
    ...
    :param server_client: HTTP client for the Goalrail server.
    ...
    :param runner_workspace: Authoritative runtime cwd for the runner,
        sourced from ``GOALRAIL_RUNNER_WORKSPACE``. Combined with
...
async def _execute_terminal_tool(
    ...
    *,
    ...
    # the Goalrail server's relay republishes onto ``session_stream``.
...
async def _emit_terminal_resource_event(
    *,
    ...
    (:func:`goalrail.server.routes.sessions._publish_and_persist_resource_event`)
...
async def _publish_terminal_created_event(
    *,
    ...
    if resource_registry is not None:
        ...
        from goalrail.entities.session_resources import terminal_resource_view
...
def _subagent_tool_result_policy_request(
    ...
    """
    Build the Goalrail policy-evaluation request for delayed child output.
...
async def _post_subagent_policy_verdict(
    *,
    ...
    """
    POST delayed sub-agent output to Goalrail policy evaluation.

    :param server_client: HTTP client pointed at Goalrail server.
...
def _apply_subagent_policy_verdict(
    ...
    """
    Apply an Goalrail policy verdict to a sub-agent inbox payload.

    :param payload: Original completed sub-agent payload.
    :param verdict: Parsed Goalrail policy response, e.g.
...
def _inject_orchestrator_skills(
    ...
    """
    Auto-inject built-in platform skills for every goalrail agent.

    The ``build-goalrail`` skill teaches the LLM how to author valid
    agent configs. Every agent on the platform should have access to it
    — whether it declares ``tools.agents`` or not — so that any
    ``goalrail claude`` user can author and launch new agents. The
    skill is injected from the canonical source at
    ``goalrail/onboarding/agent/skills/build-goalrail/`` when not
    ...
    if "build-goalrail" in existing_names:
        return skills
    from goalrail.spec.parser import _discover_skills
...
def _execute_skill_tool(
    ...
    *,
    ...
    :returns: Tool output string.
    """
    from goalrail.tools.builtins.load_skill import LoadSkillTool
    from goalrail.tools.builtins.read_skill_file import ReadSkillFileTool

    bundled_skills = list(getattr(agent_spec, "skills", None) or [])
    skills_filter = getattr(agent_spec, "skills_filter", "all")
    # Auto-inject the build-goalrail skill for agents that opt into the
