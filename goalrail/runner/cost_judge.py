...
The runner-side cost advisor (:mod:`goalrail.runner.cost_advisor`)
...
:class:`~goalrail.cost_plan.AdvisorVerdict` (or ``None``) sizing the
...
:class:`~goalrail.llms.client.Client`, which the runner already depends
on.
"""
...
from goalrail.llms.types import MessageOutput, Response, ResponseStreamEvent
...
class LLMClientLike(Protocol):
    ...
    Matches :class:`goalrail.llms.client.Client`'s ``responses.create``
    surface; declared structurally so tests can pass a scripted stub that
    returns canned :class:`~goalrail.llms.types.Response` objects without
...
class _ResponsesLike(Protocol):
    ...
    :meth:`goalrail.llms.client._ResponsesNamespace.create` the judge
    uses, so the real :class:`~goalrail.llms.client.Client` satisfies
    ...
    async def create(  # type: ignore[explicit-any]  # mirrors Client.responses.create's Any-typed input/kwargs
        ...
        **kwargs: Any,
        ...
        :class:`~goalrail.llms.client.Client` (which also streams); the
...
class LLMJudge:
    ...
    Implements the :class:`goalrail.runner.cost_advisor.Judge` protocol.
    ...
    def __init__(self, config: _JudgeConfig, client: LLMClientLike) -> None:
        ...
        :param client: The LLM client the judge calls; the real
            :class:`goalrail.llms.client.Client` in production, a
...
def resolve_advisor_mode(spec_mode: str, override: str | None) -> str | None:
    ...
    :param spec_mode: The mode the spec marker configured, one of
        :data:`~goalrail.cost_plan.ADVISOR_MODES` (``"advise"`` /
