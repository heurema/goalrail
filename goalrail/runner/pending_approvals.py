...
Lifted out of ``goalrail.runner.app`` so the gate that lives in
``goalrail.runner.tool_dispatch`` can register and wait without
...
async def wait_for_user_approval(
    *,
    ...
    Goalrail server's pending-elicitations index (powers the sidebar
    ...
    :param elicitation_id: Correlation id minted by the Goalrail server's
    ...
    try:
        ...
        # Signal the Goalrail server's pending-elicitations index that
