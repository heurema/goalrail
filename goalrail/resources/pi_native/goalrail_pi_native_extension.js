 ...
 * It does NOT require an active Goalrail turn context on the harness side —
...
async function evalNativePolicyHttp(config, toolName, args) {
  ...
  try {
    ...
    if (json.result === "POLICY_ACTION_DENY") {
      return { block: true, reason: json.reason || "blocked by Goalrail policy" };
    }
    return { block: false, reason: "" };
  } catch (_err) {
    // Keep Pi responsive if Goalrail is temporarily unavailable.
    return null;
  }
}
