...
//
// Run with: node goalrail/resources/pi_native/goalrail_pi_native_extension.test.js
//
// Manual reproduction of the original bug (for context):
//   1. Start a native Pi session linked to Goalrail and let it go idle.
...
const EXT_PATH = path.resolve(__dirname, "goalrail_pi_native_extension.js");
...
(async () => {
  try {
    ...
    for (const h of harnesses) {
      if (h.pi.__goalrailInboxPoller) clearInterval(h.pi.__goalrailInboxPoller);
      try {
