...
_BRIDGE_ROOT = Path(tempfile.gettempdir()) / f"goalrail-{stable_user_id()}" / "cursor-native"
...
_PASTE_SETTLE_S = 0.3
_PASTE_BUFFER = "goalrail-cursor-paste"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/goalrail-<uid>/cursor-native/<hash>``."""
...
FORK_HISTORY_OPEN_TAG = "<goalrail_fork_history>"
FORK_HISTORY_CLOSE_TAG = "</goalrail_fork_history>"
...
def _neutralize_fork_sentinels(text: str) -> str:
    ...
    could literally contain ``<goalrail_fork_history>`` /
    ``</goalrail_fork_history>`` (a user typed it, pasted logs, etc.). If left
    ...
    """
    return text.replace(FORK_HISTORY_OPEN_TAG, "[goalrail_fork_history]").replace(
        FORK_HISTORY_CLOSE_TAG, "[/goalrail_fork_history]"
    )
...
def build_hooks_config(
    ...
    ``goalrail.cursor_native_usage record-usage``, which appends the usage to
...
def approve_mcp_server_for_workspace(workspace: Path) -> None:
    ...
    ask ``cursor-agent mcp enable goalrail`` to write the exact approval entry
...
def capture_cursor_pane(bridge_dir: Path) -> str | None:
    ...
    (:mod:`goalrail.cursor_native_permissions`) to detect cursor-agent's native
...
def kill_session(bridge_dir: Path, *, timeout_s: float = _TMUX_READY_TIMEOUT_S) -> None:
    ...
    affordance. Mirrors :func:`goalrail.claude_native_bridge.kill_session`.
