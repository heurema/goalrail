...
- ``--json-file`` (:func:`events_file_path`): ``qwen`` streams structured JSON
  events here while the TUI renders normally; :mod:`goalrail.qwen_native_forwarder`
  tails it.
...
_BRIDGE_ROOT = Path(os.environ.get("TMPDIR", "/tmp")) / f"goalrail-{os.getuid()}" / "qwen-native"
...
def bridge_dir_for_session_id(session_id: str) -> Path:
    """Return the per-session bridge dir, e.g. ``/tmp/goalrail-<uid>/qwen-native/<hash>``."""
...
def qwen_session_recording_path(session_id: str, workspace: Path | str) -> Path:
    ...
    ``chat_compression`` marker (see :mod:`goalrail.qwen_native_forwarder`).
    """
    return (
        Path.home()
        / ".qwen"
