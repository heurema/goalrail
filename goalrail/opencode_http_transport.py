...
:class:`goalrail.native_server_harness.NativeServerHarness` drives it through
...
from goalrail.opencode_native_bridge import read_bridge_state
    ...
    async def create_or_resume_session(self, launch: NativeLaunchConfig) -> str:
        ...
        try:
            ...
            created = await client.create_session(
                {"title": f"goalrail:{launch.goalrail_session_id}"}
            )
            return created.id
        finally:
            await client.aclose()
