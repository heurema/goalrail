...
if TYPE_CHECKING:
    ...
    from goalrail.update_check import _InstalledWheelInfo
...
def _server_uvicorn_log_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    log_config["formatters"]["access"]["()"] = (
        "goalrail.server.performance_metrics.RequestDurationAccessFormatter"
    )
    return log_config


# Path to the user-level global config file, analogous to ~/.gitconfig.
# Tests may set ``GOALRAIL_CONFIG_HOME`` to isolate subprocesses from a
# developer's real config.
_CONFIG_HOME_ENV_VAR = "GOALRAIL_CONFIG_HOME"
_GLOBAL_CONFIG_PATH: Path = Path.home() / ".goalrail" / "config.yaml"
...
# pidfile, runner identity, native session state, logs) lives under this dir.
_STATE_DIR: Path = Path.home() / ".goalrail"
_DATA_DIR_ENV_VAR = "GOALRAIL_DATA_DIR"


def _migrate_legacy_state_dir() -> None:
    """Retained as a startup hook; Goalrail no longer migrates legacy state dirs."""


# Project-level config relative to cwd, analogous to .git/config.
# Resolved at call time so tests can control cwd.
_LOCAL_CONFIG_RELPATH: Path = Path(".goalrail") / "config.yaml"
...
# Overridable via GOALRAIL_SERVER_SHUTDOWN_TIMEOUT_S for deployments that
...
_SERVER_GRACEFUL_SHUTDOWN_TIMEOUT_S = int(
    os.environ.get(
        "GOALRAIL_SERVER_SHUTDOWN_TIMEOUT_S",
...
def _effective_global_agents_dir() -> Path:
    ...
    """
    current_default_agents_dir = Path.home() / ".goalrail" / "agents"
    if (
        os.environ.get(_CONFIG_HOME_ENV_VAR)
...
def _effective_global_config_path() -> Path:
    ...
    """
    current_default_config_path = Path.home() / ".goalrail" / "config.yaml"
    if (
        os.environ.get(_CONFIG_HOME_ENV_VAR)
...
def _display_path(path: Path) -> str:
    ...
    hardcoded ``~/.goalrail/...`` literal, this reflects the *actual*
    effective path — so a state dir outside ``$HOME`` (an
    ``GOALRAIL_*`` / ``GOALRAIL_*`` path override) renders as its real
    location rather than a misleading ``~``.

    :param path: The path to display, e.g.
        ``Path("/Users/alice/.goalrail/logs/server/local-server-ab12.log")``.
    :returns: ``"~/.goalrail/..."`` when *path* is under ``$HOME``,
        otherwise ``str(path)``.
    """
    try:
...
def _display_config_path(path: Path) -> str:
    ...
    :param path: The config path to display, e.g.
        ``Path("/Users/alice/.goalrail/config.yaml")``.
    :returns: ``"~/.goalrail/config.yaml"`` when *path* is under
...
def _load_global_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    """
    Load the global goalrail config from ``~/.goalrail/config.yaml``.
...
def _load_local_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    """
    Load the project-level config from ``.goalrail/config.yaml`` in cwd.
...
def _load_effective_config() -> dict[str, Any]:  # type: ignore[explicit-any]
    ...
    Precedence (highest last): global (``~/.goalrail/config.yaml``)
    → local (``.goalrail/config.yaml`` in cwd).  Project config
...
def _bundled_example_path(name: str) -> str:
    ...
    Located via the packaged ``goalrail.resources.examples`` (symlinks to
    ...
    resource = importlib.resources.files("goalrail.resources.examples").joinpath(name)
...
def _pick_first_run_harness() -> _FirstRunPlan | None:
    ...
    from goalrail.onboarding.harness_install import KIMI_KEY, harness_cli_installed
...
def _resolve_default_agent_target(
    ...
    if default_harness is not None:
        click.echo(
            f"goalrail: default agent '{default_agent}' uses harness "
...
def _save_global_config(  # type: ignore[explicit-any]
    ...
    """
    Merge *settings* into ``~/.goalrail/config.yaml`` and remove any
    keys listed in *unset_keys*.

    Creates the ``~/.goalrail/`` directory if it does not exist.
    ...
    *deep_merge_keys*, the incoming mapping is instead merged one level
    ...
    :func:`~goalrail.onboarding.provider_config.set_default_provider`,
...
def _materialize_bundled_example(name: str) -> Path:
    ...
    resource = resources.files("goalrail.resources.examples").joinpath(name)
    text = resource.read_text(encoding="utf-8")
    executable_placeholder = "__GOALRAIL_PYTHON_EXECUTABLE__"
    text = text.replace('"${GOALRAIL_HOME:-$PWD}/.venv/bin/python"', executable_placeholder)
    text = text.replace("${GOALRAIL_HOME:-$PWD}/.venv/bin/python", executable_placeholder)
...
def _save_local_config(
    ...
    """
    Merge *settings* into ``.goalrail/config.yaml`` in cwd and remove
...
def _default_db_uri() -> str:
    ...
    ``GOALRAIL_DATA_DIR`` → else ``~/.goalrail``).
    Pinning ``server``
    to the same DB as ``run`` means there is **one local DB — and so one
    accounts admin — per machine**, instead of a fresh CWD-relative
    ``goalrail.db`` (and a fresh admin) for every directory you launch
    from. ``--database-uri`` / the config file still override.

    :returns: e.g. ``"sqlite:////home/alice/.goalrail/chat.db"``.
    """
    from goalrail.host.local_server import _local_data_dir
...
def _default_artifact_location() -> str:
    ...
    :returns: e.g. ``"/home/alice/.goalrail/artifacts"``.
    """
    from goalrail.host.local_server import _local_data_dir
...
def _ensure_sqlite_parent_dir(db_uri: str) -> None:
    ...
    ``GOALRAIL_DATA_DIR``), so a first-ever run — or any run after the data dir
    was cleared — must create that dir before the stores connect. The
    daemon-spawned server handles this in ``ensure_local_goalrail_server``;
    ...
    :param db_uri: The resolved store DB URI, e.g.
        ``"sqlite:////home/alice/.goalrail/chat.db"`` or
        ``"postgresql://host/db"``.
    :returns: None.
    """
...
def _maybe_prompt_first_admin(account_store: Any, auth_provider: Any, *, auto_open: bool) -> None:  # type: ignore[explicit-any]  # SqlAlchemyAccountStore | None, AuthProvider
    ...
    from goalrail.server.accounts_bootstrap import (
    ...
    from goalrail.server.routes.accounts_auth import _MIN_PASSWORD_LENGTH
...
def _preregister_agent(  # type: ignore[explicit-any]  # agent_store / artifact_store / agent_cache typed Any to avoid import cycle
    ...
    :func:`goalrail.spec.materialize_bundle`, tars it, validates
    ...
    :param agent_source: Either an agent-image directory containing
        ``config.yaml`` (standard goalrail shape) or a standalone
        goalrail YAML file (e.g.
    ...
    # :func:`goalrail.inner.cli._goalrail_register_yaml_bundle` —
    # see designs/RUN_GOALRAIL_SESSION_RESUMPTION.md. Reusing the
    ...
    # (``ondelete=CASCADE`` in
    # :class:`goalrail.db.db_models.SqlTask`), wiping every
    ...
    if existing is not None:
        ...
        if existing.bundle_location != new_loc:
            ...
            # ``goalrail/server/routes/agents.py:248``.
...
def _format_version() -> str:
    ...
    ``setup.py`` wrote ``goalrail/_build_info.py``, the line is
    ...
    :returns: Either ``"goalrail 0.1.0"`` (no build info), or
        ``"goalrail 0.1.0 (010cf77c, built 2026-05-21T14:34:45Z)"``.
    """
    import datetime
    import importlib.metadata

    from goalrail.update_check import _read_build_info

    version_str = importlib.metadata.version("goalrail")
    info = _read_build_info()
    if info is None:
        return f"goalrail {version_str}"
    epoch, sha = info
    ...
    if sha:
        ...
        return f"goalrail {version_str} ({sha[:8]}, built {when})"
    # _build_info exists but has no SHA (built without git available).
    return f"goalrail {version_str} (built {when})"
...
def _print_version_callback(ctx: click.Context, _param: click.Parameter, value: bool) -> None:
    ...
    it ``_read_build_info()`` — during ``goalrail.cli`` import. The
    successful sub-import would then set ``goalrail._build_info`` as
    an attribute on the ``goalrail`` package object. Once that
    attribute exists, ``from goalrail import _build_info`` short-
...
class _GoalrailCLI(click.Group):
    ...
    def format_help(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        if ui.show_banner():
            ...
            try:
                version = importlib.metadata.version("goalrail")
...
def main() -> None:
    ...
    # Relocate pre-rename ~/.goalrail state before anything reads ~/.goalrail
    # (update-check cache, diagnostics logs, config). No-op once migrated.
    _migrate_legacy_state_dir()

    argv = sys.argv[1:]

    # Bare ``goalrail`` with no args behaves like ``goalrail run`` on an
    ...
    # Shorthand: ``goalrail --harness claude [opts]`` →
    ...
    # Shorthand: ``goalrail myagent.yaml [opts]`` → ``run myagent.yaml [opts]``.
    # Allows ``goalrail`` to act as a transparent alias for ``goalrail run``
    ...
    # and warnings to ~/.goalrail/logs/cli-*.log even when --log
    ...
    from goalrail.cli_diagnostics import (
        log_cli_error_hint,
        log_cli_exception,
        print_setup_hint,
        setup_cli_logging,
    ...
    if not _should_skip_update_check(argv) and sys.stderr.isatty():
        from goalrail.update_check import maybe_show_update_notice
...
def _is_removed_ad_hoc_invocation(argv: list[str]) -> bool:
    ...
    - The first non-flag token isn't a known click subcommand and is
      ...
      ``goalrail "what does this repo do?"``) — the free-text shape
    ...
    (``goalrail --help``, ``goalrail --version``), or when the
    token is a single command-shaped word (e.g. ``goalrail blah``)
    ...
    # tokens are flags (e.g. ``goalrail --system-prompt "..."``),
...
_HOST_PID_PATH = Path.home() / ".goalrail" / "host.pid"
...
def _effective_host_pid_path() -> Path:
    ...
    """
    current_default_host_pid_path = Path.home() / ".goalrail" / "host.pid"
    if (
        os.environ.get("GOALRAIL_DATA_DIR")
        or os.environ.get("GOALRAIL_DATA_DIR")
...
def _daemon_host_online(record: _HostDaemonRecord, *, timeout_s: float = 2.0) -> bool:
    ...
    """
    from goalrail.claude_native_bridge import url_component
...
def _read_daemon_record(path: Path) -> _HostDaemonRecord | None:
    ...
    :param path: JSON file path to read, e.g.
        ``Path("~/.goalrail/daemons/abc.json")``.
...
def _load_existing_host_id() -> str | None:
    ...
    from goalrail.host.identity import CONFIG_PATH, default_config_path
...
def _daemon_host_identity_changed(record: _HostDaemonRecord) -> bool:
    ...
    A live daemon can outlast edits to ``~/.goalrail/config.yaml``. Reusing
...
class _DaemonReuseDecision:
    ...
    :param config_changed: ``True`` when the existing daemon was torn down
        ...
        invocation (e.g. the user flipped ``GOALRAIL_AUTH_ENABLED``).
...
def _reuse_existing_daemon_record(target: str) -> _DaemonReuseDecision:
    ...
    ``GOALRAIL_AUTH_ENABLED``) would silently keep its old auth
...
def _ensure_host_daemon(server_url: str | None) -> bool:
    ...
    args = [sys.executable, "-m", "goalrail.host._daemon_entry", *mode_args]
...
def _build_host_daemon_env(
    *,
    ...
    through :func:`goalrail.host.connect._build_runner_env`, so these
...
# free-port respawn cold-boots — see ensure_local_goalrail_server.
...
def _exit_for_auth_mode_change(base_url: str) -> None:
    ...
    secret) at boot, so an ``GOALRAIL_AUTH_ENABLED`` flip restarts it
...
def _start_cli_runner_process(
    *,
    ...
    :param runner_id: Optional runner id to advertise. ``None``
        ...
        :func:`goalrail.runner.identity.get_stable_runner_id`
    ...
    :param log_dir: Optional base log directory to use when
        ...
        ``~/.goalrail/logs`` location; tests should pass a
    ...
    if runner_proc.poll() is not None:
        from goalrail._runner_startup import format_runner_log_tail
...
def _adopt_cli_runner_process(proc: subprocess.Popen[bytes]) -> None:
    ...
    :returns: None.
    """
    from goalrail.runner.identity import RUNNER_ADOPT_SIGNAL
...
@click.option(
    ...
    help=(
        ...
        "(alternative to GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD). Only "
...
def server(
    ...
    :param auto_open: Whether to open the magic-redeem URL in the
        ...
        into the ``GOALRAIL_ACCOUNTS_AUTO_OPEN`` env var so the
    ...
    :param admin_password: Optional first-run accounts admin password
        ...
        ``GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD`` env var that
    ...
    if admin_password:
        os.environ.setdefault("GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD", admin_password)
    ...
    os.environ["GOALRAIL_ACCOUNTS_AUTO_OPEN"] = "1" if auto_open else "0"
    ...
    # DEDICATED server here" — the daemon's own spawn (ensure_local_goalrail_server)
    ...
    # accounts/oidc) and an explicit GOALRAIL_AUTH_PROVIDER=header
    # deploy behind an identity-injecting proxy. setdefault so an
    # operator's explicit GOALRAIL_LOCAL_SINGLE_USER=0 wins. Must run
    # before create_auth_provider() below, which reads the var.
    from goalrail.server.auth import resolve_auth_source as _resolve_auth_source

    _is_loopback_bind = host in ("127.0.0.1", "localhost", "::1")
    # Compose-style deploys pass GOALRAIL_AUTH_PROVIDER as an empty
    # string when unset ("${VAR:-}"), so empty and missing both mean
    # "not explicitly pinned".
    _raw_auth_provider = os.environ.get("GOALRAIL_AUTH_PROVIDER")
    ...
    from goalrail.runner.transports.ws_tunnel.limits import (
        RUNNER_TUNNEL_MAX_MESSAGE_BYTES,
    )
    from goalrail.server.app import create_app
    from goalrail.server.auth import create_auth_provider
    from goalrail.server.server_config import config_str_list
    ...
    from goalrail.spec import parse_default_policies, parse_server_llm
    ...
    # GOALRAIL_SMART_ROUTING=1.  Hidden by default — managed deployments
    # override RuntimeCaps.routing_client with their own implementation.
    routing_client = None
    if server_llm is not None and os.environ.get("GOALRAIL_SMART_ROUTING") == "1":
        ...
        if _policy_client is not None:
            from goalrail.server.smart_routing import LLMRoutingClient
    ...
    from goalrail.server.managed_hosts import parse_sandbox_config
    ...
    # (GOALRAIL_AUTH_ENABLED=1 without OIDC config, or an explicit
    # GOALRAIL_AUTH_PROVIDER=accounts), supply sensible defaults
    ...
    # set this explicitly.
    from goalrail.server.auth import resolve_auth_source

    if resolve_auth_source() == "accounts":
        from goalrail.server.accounts_secret import load_or_generate_cookie_secret

        os.environ.setdefault(
            "GOALRAIL_ACCOUNTS_COOKIE_SECRET",
            load_or_generate_cookie_secret(art_loc),
        )
        os.environ.setdefault("GOALRAIL_ACCOUNTS_BASE_URL", f"http://{host}:{port}")
    ...
    account_store = None
    from goalrail.server.auth import UnifiedAuthProvider as _UAP
    ...
    # always-on diagnostics (goalrail.* loggers, captured warnings) also land
    ...
    # path (`-m goalrail.cli server`, no setup_cli_logging), whose captured
    # log `server start` already reports.
    from goalrail.cli_diagnostics import current_cli_log_path
    ...
    from goalrail.server.app import _WEB_UI_DIST
...
def _stop_local_server_and_daemon(*, force: bool) -> bool:
    ...
    recorded in ``~/.goalrail/local_server.pid``. Best-effort and
...
def server_start() -> None:
    ...
    :returns: None.
    """
    startup = ensure_local_goalrail_server()
    verb = (
...
def server_stop(force: bool) -> None:
    ...
    in ``~/.goalrail/local_server.pid`` — its web UI and sessions become
...
def server_status(json_output: bool) -> None:
    ...
    local host daemon is attached. Reads ``~/.goalrail/local_server.pid``
    and probes ``/health``.
...
def _upgrade_vcs_install(
    ...
    if remote_sha and current_sha and remote_sha == current_sha:
        click.echo(f"goalrail is up to date (git {cur_short}, tracking {info.vcs_url}).")
        return
    if known_behind:
        click.echo(
...
def upgrade(check_only: bool, force: bool, pre: bool) -> None:
    ...
    if info is None:
        raise click.ClickException(
            "Couldn't determine how goalrail is installed; upgrade it manually."
        )
    if info.is_editable:
        raise click.ClickException(
    ...
    current = importlib.metadata.version("goalrail")
    ...
    if latest is None:
        raise click.ClickException(
            ...
            "connection (or GOALRAIL_INDEX_URL / your configured index) and try again."
        )
    if not _is_newer(latest, current):
        click.echo(f"goalrail is up to date (v{current}).")
        return
    ...
    if _is_newer(new_version, current):
        ...
        return
    raise click.ClickException(
        f"The upgrade command ran but goalrail is still v{new_version} (expected "
        ...
        "Reinstall it explicitly — e.g. `uv tool upgrade --reinstall goalrail` or "
        f"`pip install --force-reinstall 'goalrail=={latest}'`."
    )
...
def _bundle(source: Path) -> bytes:
    ...
    :raises GoalrailError: If a required env var is
    ...
    if source.is_file() and source.suffix.lower() in {".yaml", ".yml"}:
        from goalrail.spec import materialize_bundle
...
def _resolve_bundle_env_vars(source: Path) -> dict[str, str]:
    ...
    :raises GoalrailError: If a ``${VAR}`` reference
...
class _ExecutorDeploy(BaseModel):  # type: ignore[explicit-any]  # auth is a free-form mapping
    ...
    expands (``goalrail/spec/parser.py`` — ``_parse_executor`` /
...
def _expand_config_env_vars(  # type: ignore[explicit-any]  # raw is parsed YAML (heterogeneous values)
    ...
    :param expand_fn: Callable that expands env var references
        ...
        :func:`goalrail.spec.expand_env_vars`.
...
def _reject_native_on_windows(harness: str) -> None:
    ...
    if IS_WINDOWS:
        raise click.ClickException(
            f"`goalrail {harness}` (native tmux/PTY terminal) is not supported on "
...
def claude(
    ...
    from goalrail.claude_native import run_claude_native
...
def codex(
    ...
    from goalrail.codex_native import run_codex_native
...
def pi(
    ...
    from goalrail.pi_native import run_pi_native
...
def _ensure_bundled_agent_brain_credential(name: str) -> None:
    ...
    adoption (see :func:`goalrail.setup`).
    ...
    try:
        ...
        for entry_name, entry in load_providers(config).items():
            ...
            click.echo(
                ...
                f"the default (change anytime with: goalrail /model).",
                err=True,
            )
            return
    except (OSError, yaml.YAMLError, GoalrailError):
        return
...
def cursor(
    ...
    from goalrail.cursor_native import run_cursor_native
...
def kiro(
    ...
    from goalrail.kiro_native import run_kiro_native
...
def goose(
    ...
    from goalrail.goose_native import run_goose_native
...
def hermes(
    ...
    from goalrail.hermes_native import run_hermes_native
...
def antigravity(
    ...
    from goalrail.antigravity_native import run_antigravity_native
...
def qwen(
    ...
    from goalrail.qwen_native import run_qwen_native
...
def _run_bundled_agent(name: str, run_args: tuple[str, ...]) -> None:
    ...
    ``prog_name`` stays pinned to ``"goalrail run"`` so context-derived output,
...
def kimi(
    ...
    from goalrail.kimi_native import run_kimi_native
...
# ``goalrail.chat.run_chat`` for how local-agent options get baked
...
_LOG_HELP = "Write a JSON dump of the conversation to ~/.goalrail/logs/ on exit."
...
def _validate_harness(harness: str) -> None:
    ...
    if canonicalize_harness(harness) in GOALRAIL_HARNESSES:
        return
    allowed = ", ".join(sorted(GOALRAIL_HARNESSES))
...
def _materialize_harness_launcher_file(
    *,
    ...
    tmpdir = Path(tempfile.mkdtemp(prefix="goalrail-harness-launcher-"))
...
def _build_resume_parts() -> list[str]:
    ...
    :returns: Argument list prefix, e.g.
        ``["goalrail", "run", "agent.yaml", "--server",
        "https://example.com"]``.
    """
...
def _dispatch_native_terminal_harness(
    *,
    ...
    if session_id is None and not resume_picker and resume_latest:
        from goalrail.chat import _remote_headers, _resolve_latest_conversation_id
...
def _reject_agent_with_native_terminal_harness(harness: str) -> None:
    ...
    raise click.ClickException(
        ...
        f"`goalrail {native_agent.terminal_name}` (or `run --harness {harness}`)."
    )
...
def _dispatch_run(
    *,
    ...
    if target is None:
        if server_from_cli and server is not None and harness is None:
            ...
            if resume_conversation_id is not None and is_interactive_shape:
                from goalrail.chat import _redirect_native_resume_if_needed, run_attach
            ...
            from goalrail.chat import run_chat
    ...
    if prompt is not None:
        ...
        from goalrail.chat import run_prompt
...
@click.option(
    ...
    help=(
        ...
        "overlay, JSONL event log (~/.goalrail/debug/), and "
...
def attach(
    ...
    auto_open_conversation = _resolve_auto_open_conversation_from_config(cfg)
    from goalrail.chat import run_attach
...
def _prompt_stop_local_server() -> None:
    ...
    server (:func:`ensure_local_goalrail_server`) that survives the daemon's exit
...
def _parse_config_settings(
    ...
    *,
    ...
    for item in settings:
        ...
        # Resolve ``default_agent`` to an absolute path so ``goalrail`` works from
...
def _print_config_defaults() -> None:
    ...
    The ``KEY=VALUE`` defaults from ``~/.goalrail/config.yaml`` (user) and
    ``.goalrail/config.yaml`` in the cwd (project, takes precedence).
    ...
    # (``cwd/.goalrail/config.yaml``) resolves to the SAME file as the
    # user-level path (``~/.goalrail/config.yaml``). Dedup on the resolved
...
def config_grp() -> None:
    ...
    (``.goalrail/config.yaml`` in the cwd, like ``.git/config``) overrides
    user-level config (``~/.goalrail/config.yaml``, like ``~/.gitconfig``).
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Write to ~/.goalrail/config.yaml (user-level) instead of the project config.",
)
...
def config_set(is_global: bool, settings: tuple[str, ...]) -> None:
    ...
    Without ``--global``, pairs are written to ``.goalrail/config.yaml``
    in the current directory (project-level, like ``.git/config``); with
    ``--global`` to ``~/.goalrail/config.yaml`` (user-level, like
    ``~/.gitconfig``). Project values take precedence.

    Supported keys: auto_open_conversation, default_agent, harness,
    model, server.

    :param is_global: When ``True``, write to ``~/.goalrail/config.yaml``;
        when ``False``, to ``.goalrail/config.yaml`` in cwd.
    :param settings: ``KEY=VALUE`` pairs to set, e.g.
        ``("default_agent=examples/hello.yaml", "model=gpt-5.4-mini")``.
...
@click.option(
    ...
    is_flag=True,
    default=False,
    help="Remove from ~/.goalrail/config.yaml (user-level) instead of the project config.",
)
...
def config_unset(is_global: bool, keys: tuple[str, ...]) -> None:
    ...
    :param is_global: When ``True``, remove from ``~/.goalrail/config.yaml``;
        when ``False``, from ``.goalrail/config.yaml`` in cwd.
...
def _warn_missing_harness_dependencies() -> None:
    ...
    wrapper needs it (Node when a harness CLI runs, tmux when ``goalrail
    ...
    :returns: None. Side effect: writes a yellow warning block to stderr
        via :mod:`goalrail.inner.ui` when one or more dependencies are
        missing.
    """
    problems: list[str] = []
...
def _print_credentials_by_harness() -> None:
    ...
    :returns: None. Side effect: writes the listing to the onboarding
        console.
    """
    from goalrail.onboarding.configure_models import render_provider_listing_by_harness
    from goalrail.onboarding.detected import effective_config_with_detected
...
def _existing_key_name_for_ref(  # type: ignore[explicit-any]  # config is a yaml-boundary mapping
    ...
    """
    from goalrail.onboarding.provider_config import KEY_KIND, load_providers
...
def _configure_harness_add(family: str | None = None) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        status. Side effect: writes to ``~/.goalrail/config.yaml`` and,
        for a pasted API key, the secret store.
    """
    from goalrail.onboarding import secrets as secret_store
    from goalrail.onboarding.ambient import detect_providers
    ...
    from goalrail.onboarding.interactive import console, prompt_text, select
    from goalrail.onboarding.provider_config import (
        ANTHROPIC_FAMILY,
        BEDROCK_KIND,
        CHAT_WIRE_API,
...
def _compact_credential_label(det: DetectedProvider) -> str:
    ...
    Unlike :func:`goalrail.onboarding.configure_models.credential_label`
    ...
    :param det: A credential found by
        :func:`goalrail.onboarding.ambient.detect_providers`.
    :returns: A short human label, e.g. ``"Anthropic API Key"``,
        ``"Claude Subscription"``, or ``"ChatGPT Subscription"``.
    """
    from goalrail.onboarding.ambient import SUBSCRIPTION_KIND
...
def _announce_auto_configured_credentials(adopted: list[str]) -> None:
    ...
    """
    from goalrail.onboarding.ambient import detect_providers
    from goalrail.onboarding.interactive import console
    ...
    console.print(
        ...
        f"auto-configured for goalrail: {', '.join(labels)}[/dim]"
    )
...
def _harness_credential_rows(config: dict[str, Any], family: str) -> list[_HarnessMenuRow]:  # type: ignore[explicit-any]
    ...
    """
    from goalrail.onboarding.configure_models import kind_glyph
    from goalrail.onboarding.provider_config import (
        load_providers,
        provider_families,
        surface_default_provider,
...
def _prompt_install_harness(family: str) -> bool:
    ...
    """
    from goalrail.onboarding.configure_models import family_label
    from goalrail.onboarding.harness_install import (
        harness_install_command,
        install_harness_cli,
    )
    from goalrail.onboarding.interactive import console, select
...
def _manage_harness_providers(family: str) -> None:
    ...
    """
    from goalrail.onboarding.configure_models import family_label
    from goalrail.onboarding.harness_install import harness_cli_installed
...
def _prompt_install_cursor() -> str | None:
    ...
    :func:`goalrail.onboarding.cursor_auth.cursor_install_command`.
...
def _manage_cursor_harness() -> None:
    ...
    API key stored in the goalrail secret store, mirroring how the other
    harnesses persist their api keys (the secret in the store, a
    ``keychain:``/``env:`` reference in ``~/.goalrail/config.yaml``).
    ...
    :returns: None. Side effects: may install the ``cursor`` extra, and may
        write the ``cursor:`` block of ``~/.goalrail/config.yaml`` and the
        secret store.
    """
    from goalrail.onboarding import secrets as secret_store
    from goalrail.onboarding.cursor_auth import (
...
def _set_cursor_api_key() -> str | None:
    ...
    goalrail secret store under ``keychain:cursor``. The ``crsr_`` prefix is
    ...
    from goalrail.onboarding.interactive import prompt_text
...
def _launch_goose_configure() -> str | None:
    ...
    """
    from goalrail.onboarding.goose_auth import goose_config_summary
    from goalrail.onboarding.harness_install import (
        GOOSE_KEY,
        harness_cli_installed,
        harness_install_spec,
...
def _manage_kimi_harness() -> None:
    ...
    :func:`~goalrail.onboarding.harness_install.harness_cli_logged_in` always
...
def _prompt_install_copilot() -> str | None:
    ...
    index-free — see
    :func:`goalrail.onboarding.copilot_auth.copilot_install_command`.
...
def _manage_copilot_harness() -> None:
    ...
    set / replace / remove a token stored in the goalrail secret store, mirroring
    ...
    :returns: None. Side effects: may install the ``copilot`` extra, and may
        write the ``copilot:`` block of ``~/.goalrail/config.yaml`` and the
        secret store.
    """
    from goalrail.onboarding import secrets as secret_store
    from goalrail.onboarding.copilot_auth import (
...
def _remove_subscription(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the level-2 status line, or ``None``
        ...
        harness logout command and writes ``~/.goalrail/config.yaml``.
    """
    from goalrail.onboarding.harness_install import harness_install_spec, harness_logout
    from goalrail.onboarding.interactive import select
...
def _set_harness_default(provider: str, family: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.goalrail/config.yaml``.
    """
    from goalrail.onboarding.configure_models import family_label
    from goalrail.onboarding.provider_config import load_providers, set_default_provider
...
def _remove_credential(provider: str) -> str | None:
    ...
    :returns: A confirmation message for the caller to show as a transient
        ...
        writes ``~/.goalrail/config.yaml`` (and, when the removed entry is
...
def _launch_opencode_auth_login() -> str | None:
    ...
    from goalrail.onboarding.interactive import console
    from goalrail.onboarding.opencode_auth import opencode_auth_summary
...
def _run_opencode_auth_list() -> None:
    ...
    from goalrail.onboarding.harness_install import OPENCODE_KEY, harness_install_spec
...
def _set_opencode_default_model(current: str | None) -> str | None:
    ...
    """
    from goalrail.onboarding.interactive import console, select
    from goalrail.onboarding.opencode_auth import reachable_provider_ids
...
def _run_configure_harnesses_interactive() -> None:
    ...
    Opening it adopts any ambient-detected credential — announcing the
    ...
    :returns: None. Side effect: may write ``~/.goalrail/config.yaml`` via
        the adopt step and any add/set-default/remove the user performs while
        navigating.
    """
    ...
    # one-time callout naming what was auto-configured and from where. No
    # progress spinner here: a transient spinner over the (fast) detection
    # left a cleared-region gap and a residual line directly above the menu
    # on first paint.
    _adopt_ambient_credentials()
    ...
    def _install_hint(command: str) -> str:
        ...
        # (e.g. ``pip install "goalrail[cursor]"``) renders literally instead of
    ...
    def build_harness_rows() -> list[tuple[str, str, str, str, str]]:
        ...
        from goalrail.onboarding.hermes_auth import hermes_config_summary
        from goalrail.onboarding.opencode_auth import opencode_auth_summary
...
# ─── debug group ──────────────────────────────────────────────────
#
# Operator-only maintenance commands, grouped under ``goalrail debug``
...
def debug_db_upgrade(url: str) -> None:
    ...
    from goalrail.db.utils import _run_migrations
...
def debug_migrate_accounts_to_oidc(
    ...
    against your live DB *before* flipping ``GOALRAIL_AUTH_PROVIDER``.
    ...
    Examples:
      ...
      goalrail debug migrate-accounts-to-oidc sqlite:///chat.db --domain example.com
      # Apply it
      goalrail debug migrate-accounts-to-oidc sqlite:///chat.db --domain example.com --commit
      # Explicit per-user mapping (add --commit to apply)
      goalrail debug migrate-accounts-to-oidc sqlite:///chat.db --map alice=alice@corp.com
...
def login(server_url: str) -> None:
    ...
    - accounts mode: prompts for username + password (no browser
      ...
      ``~/.goalrail/auth_tokens.json`` keyed by server URL.
    ...
    (the ``server`` key in ``~/.goalrail/config.yaml``), so a bare
    ...
    try:
        ...
        raise click.ClickException(
            ...
            f"Is the server running with GOALRAIL_AUTH_PROVIDER=oidc?"
        ) from exc
...
def _accounts_login(server: str) -> None:
    ...
    ``~/.goalrail/auth_tokens.json`` via the existing
    :func:`goalrail.cli_auth.store_token`. From there both
...
@click.option(
    ...
    "parent_pane",
    required=True,
    help="Tmux pane id of the parent goalrail pane (e.g. '%0'). "
    "Forwarded by the wrapped key-binding via #{pane_id}.",
)
def pane_split(direction: str | None, parent_pane: str) -> None:
    """
    Split the parent goalrail pane and run the chooser in the new pane.

    Internal subcommand invoked by the tmux key-binding wrappers
    installed by ``goalrail.repl._tmux_pane``. The wrapper fires
    ``run-shell 'goalrail pane-split -<v|h|w> -p #{pane_id}'`` when
    the user presses their split key while focused on an goalrail
    ...
    :param parent_pane: The goalrail pane id, e.g. ``%0``. Required.
    """
    import shlex
    ...
    # include the venv ``bin/`` where ``goalrail`` lives.
    # ``_resolve_goalrail_argv`` returns either an absolute
    # path to the binary (preferred) or ``[python, "-m",
    # "goalrail.cli"]`` as a fallback that always works.
    picker_argv = [
...
@click.option(
    ...
    "``goalrail.repl._tmux_pane.register_pane``.",
)
def pane_picker(parent_pane: str) -> None:
    ...
    1. Reads the parent goalrail pane's ``@goalrail-launch-argv``
       and friends.
    ...
    :param parent_pane: The parent goalrail pane id, e.g. ``%0``.
    """
    import json
    ...
    if not launch_argv_json:
        click.echo(
            f"error: parent pane {parent_pane} has no goalrail context "
...
def _strip_resume_flags(argv: list[str]) -> list[str]:
    ...
    :param argv: Parent's launch argv, e.g.
        ``["python", "-m", "goalrail.cli", "run", "agent.yaml",
