"""Host identity management for ``goalrail host``.

Reads or creates the ``host`` section in ``~/.goalrail/config.yaml``.
The host identity is auto-generated on first ``goalrail host``
...
CONFIG_PATH = Path.home() / ".goalrail" / "config.yaml"
...
HOST_TOKEN_ENV_VAR = "GOALRAIL_HOST_TOKEN"
HOST_ID_ENV_VAR = "GOALRAIL_HOST_ID"
HOST_NAME_ENV_VAR = "GOALRAIL_HOST_NAME"

# WebSocket upgrade header carrying a managed host's launch token.
# Mirrors the runner tunnel's X-Goalrail-Runner-Tunnel-Token pattern:
...
# auth provider.
MANAGED_HOST_TOKEN_HEADER = "X-Goalrail-Host-Token"
...
def default_config_path() -> Path:
    ...
    if (
        ...
        or os.environ.get("GOALRAIL_CONFIG_HOME")
