...
These helpers live under ``goalrail/host/`` (not ``cli.py``) so the
...
def _local_data_dir() -> Path:
    ...
    Honors ``GOALRAIL_DATA_DIR`` (the purpose-built data-isolation knob), else an existing
    Goalrail state home. This
    lets a checkout/worktree isolate its local runtime DB: two worktrees
    otherwise share ``~/.goalrail/chat.db``, and if their Alembic heads have
    diverged the shared DB can't migrate and the daemon-backed local server
    fails to boot ("schema is out of date").

    Must stay in lock-step with :func:`goalrail.chat._goalrail_persistent_dir`:
    the local server's DB lives here and ``goalrail run`` resolves the
    resume DB there, so the two MUST agree. ``GOALRAIL_CONFIG_HOME`` is
    ...
    inheriting a shared ``GOALRAIL_CONFIG_HOME``).
...
# config change (e.g. flipping GOALRAIL_AUTH_ENABLED) respawns the
...
# ``goalrail server`` (its logs stream to the terminal, not a file).
...
def server_config_signature() -> str:
    ...
    the user flips ``GOALRAIL_AUTH_ENABLED``, or upgrades the package) and
    ...
    * the installed package version — a running server holds its code in
      memory, so after ``goalrail upgrade`` (or a manual ``uv tool upgrade``)
      ...
      config-drift path in :func:`ensure_local_goalrail_server` — no
...
def local_server_url_if_healthy() -> str | None:
    ...
    :func:`ensure_local_goalrail_server`, the one place that decides reuse.
...
def _write_local_server_record(
    ...
    (``goalrail server``) registration paths stay symmetric: a server
    that advertises itself in the pidfile ALWAYS stamps a matching sig,
    so reuse-matching in :func:`ensure_local_goalrail_server` can't spuriously
    ...
    :param log_path: Absolute path of the spawned server's captured log file,
        e.g. ``Path("/Users/alice/.goalrail/logs/server/local-server-ab12cd.log")``.
...
def _read_local_server_log_path() -> Path | None:
    ...
    :returns: The absolute log path the background server writes to, e.g.
        ``Path("/Users/alice/.goalrail/logs/server/local-server-ab12cd.log")``, or
...
def _atomic_write(path: Path, text: str) -> None:
    ...
    :param path: Destination file, e.g. ``~/.goalrail/local_server.pid``.
...
def _terminate_pid(pid: int) -> None:
    ...
    Shared by :func:`stop_local_goalrail_server` (the pidfile-tracked server) and
...
def stop_local_goalrail_server() -> None:
    ...
    :func:`ensure_local_goalrail_server` spawns a fresh
    ...
    the off-switch (``goalrail stop`` / ``server stop``) calls both.
...
class LocalServerInfo:
    ...
    :param log_path: Absolute path of the background server's captured log
        file, e.g. ``Path("/Users/alice/.goalrail/logs/server/local-server-ab12cd.log")``.
...
def local_server_status() -> LocalServerInfo:
    ...
    Reads ``~/.goalrail/local_server.pid`` for the recorded pid/port and
...
class LocalServerStartup:
    """Outcome of :func:`ensure_local_goalrail_server`.
    ...
    :param spawned: ``True`` when this call started a NEW detached server
        ...
        (one started earlier by ``goalrail server`` or by a prior
    ...
    :param log_path: Absolute path of the background server's captured log
        ...
        ``None`` when the running server is a foreground ``goalrail server``
...
def ensure_local_goalrail_server() -> LocalServerStartup:
    ...
    detached ``goalrail server`` on a free loopback port, backed by the
    persistent ``~/.goalrail`` data store so conversations survive across
    invocations (designs/RUN_GOALRAIL_SESSION_RESUMPTION.md). The server runs
    ...
    # same URL as a manual `goalrail server` (and reuse via the pidfile
    ...
    while True:
        ...
        try:
            _wait_for_local_goalrail_server(spawned.base_url, spawned.proc, spawned.log_path)
...
class _SpawnedLocalServer:
    ...
    :param proc: The ``goalrail server`` subprocess handle.
    :param log_path: File capturing the child's stdout/stderr, e.g.
        ``Path("~/.goalrail/logs/server/local-server-ab12cd.log")``.
...
def _spawn_local_server(port: int) -> _SpawnedLocalServer:
    ...
    (:func:`ensure_local_goalrail_server`) records the server only after it
    ...
    :returns: The spawned subprocess plus its log path and base URL; the
        caller awaits readiness via :func:`_wait_for_local_goalrail_server`.
    """
    ...
    # An explicit GOALRAIL_DATABASE_URI wins over the per-data-dir sqlite
    # file (e.g. to point a worktree at its own Postgres). Defaults to the
    # isolated sqlite db under the runtime data dir.
    db_uri = os.environ.get("GOALRAIL_DATABASE_URI") or f"sqlite:///{db_path}"
    ...
    # POV, `goalrail run` (no --server) in accounts mode gets
    ...
    # the env-unset default; GOALRAIL_AUTH_ENABLED=1 opts into accounts (or
    # oidc when GOALRAIL_OIDC_* is set). In header/oidc mode we must NOT mint
    ...
    child_env["GOALRAIL_LOCAL_SINGLE_USER"] = "1"
    _accounts_mode = resolve_auth_source() == "accounts"
    if _accounts_mode:
        if "GOALRAIL_ACCOUNTS_COOKIE_SECRET" not in os.environ:
            from goalrail.server.accounts_secret import (
                load_or_generate_cookie_secret,
            )

            child_env["GOALRAIL_ACCOUNTS_COOKIE_SECRET"] = load_or_generate_cookie_secret(data_dir)
...
def pick_local_port(preferred: int = _DEFAULT_LOCAL_PORT) -> int:
    ...
    URL is the same across ``goalrail server`` and daemon spawns —
    ...
    goalrail server happens via the pidfile (:func:`register_local_server`
...
def stop_untracked_local_server(port: int = _DEFAULT_LOCAL_PORT) -> int | None:
    ...
    server then escapes :func:`stop_local_goalrail_server`, which only knows the
    pidfile PID — so ``goalrail stop`` / ``server stop`` would leave it
    ...
    it. Call it AFTER :func:`stop_local_goalrail_server` so a normally-tracked
...
def register_local_server(port: int) -> None:
    ...
    Lets a foreground ``goalrail server`` advertise itself in the same
    ``local_server.pid`` the daemon reads, so ``goalrail run`` /
    ...
    sig and reuse-matching in :func:`ensure_local_goalrail_server` always sees
...
def clear_local_server_record() -> None:
    ...
    Called on ``goalrail server`` shutdown so a clean exit doesn't
...
def _wait_for_local_goalrail_server(
    base_url: str,
    proc: subprocess.Popen[bytes],
    log_path: Path,
