"""Host process main loop for ``goalrail host``.
...
from goalrail.host.identity import HostIdentity, load_or_create_host_identity
from goalrail.onboarding.harness_install import harness_setup_hint
...
def _display_log_path(path: Path) -> str:
    ...
    :param path: Absolute path, typically under the user's state dir, e.g.
        ``Path("/Users/alice/.goalrail/logs/host-runner/runner-ab12.log")``.
...
def _read_log_tail(path: Path, max_bytes: int = _LOG_TAIL_MAX_BYTES) -> str:
    ...
    :param path: The runner's captured stdout/stderr log file, e.g.
        ``Path("~/.goalrail/logs/host-runner/runner-ab12.log")``.
...
_RUNNER_ENV_ALLOWLIST: frozenset[str] = frozenset(
    {
        ...
        #   - GOALRAIL_CONFIG_HOME: where config.yaml /
        #     provider config live, so the runner resolves the same providers
        #     the CLI configured.
        #   - GOALRAIL_DATA_DIR: where the sqlite db +
        ...
        #     timed out discovery).
        # GOALRAIL_DATABASE_URI is intentionally NOT here — it may embed a
        ...
        # GOALRAIL_AUTH_PROVIDER and the daemon-spawned local server
        # silently boots in accounts mode while the CLI thinks it's talking
        # to a header-mode server — every CLI request 401s (e.g. the
        # test_run_goalrail_resumption suite). Not a secret; safe to propagate to
        # any subprocess.
        "GOALRAIL_AUTH_PROVIDER",
        # Multi-user opt-in switch (create_auth_provider): GOALRAIL_AUTH_ENABLED
        # turns the env-unset header/local default into accounts (or oidc, when
        # GOALRAIL_OIDC_* is set); =0 opts back out. Must propagate down the
        # CLI → daemon → local-server chain or `goalrail run`/`connect` would
        # spawn the wrong auth mode while the operator set the switch on the CLI.
        # Not a secret. GOALRAIL_ACCOUNTS_ENABLED is the deprecated pre-rename
        ...
        # ``GOALRAIL_CLAUDE_SDK_NO_SANDBOX=1 goalrail run …`` had no
        ...
        "GOALRAIL_CLAUDE_SDK_NO_SANDBOX",
...
RUNNER_ENV_PASSTHROUGH_ENV_VAR: str = "GOALRAIL_RUNNER_ENV_PASSTHROUGH"
...
class HostProcess:
    ...
    def __init__(
        ...
        # Strong refs to background code-intel status replies. These
    ...
    def _fatal_upgrade_error(self, exc: InvalidURI | InvalidStatus) -> HostConnectError | None:
        ...
        if isinstance(exc, InvalidURI):
            ...
            if self._login_redirect_streak == 1:
                ...
                # `goalrail host` shows the auth problem and its fix instead
    ...
    async def _handle_code_intel_status(
        self, frame: HostCodeIntelStatusFrame
    ) -> HostCodeIntelStatusResultFrame:
        """Handle a ``host.code_intel_status`` request from the server.

        Resolves the workspace against the host filesystem (source of
        truth for ``~`` / symlinks, like ``host.stat``), runs the engine
        via the shared service, and returns the status envelope. The
        blocking engine call runs in a worker thread so the tunnel event
        loop stays responsive. Resolution failures collapse to a
        ``status: "failed"`` result; the server then falls back to the
        ``host_unsupported`` envelope.

        :param frame: The code-intel status request frame.
        :returns: Result frame carrying the envelope or a failure.
        """
        try:
            envelope = await asyncio.to_thread(
                _compute_code_intel_status_envelope, frame.workspace
            )
    ...
    async def _send_code_intel_status_result(
        self,
        ws: websockets.asyncio.client.ClientConnection,
        frame: HostCodeIntelStatusFrame,
    ) -> None:
        """Compute and send a code-intel status result without blocking dispatch.

        Code-intel can shell out to an engine and may be slow. Running this
        in its own task keeps the host receive loop free to process later
        launch/stop/stat frames on the same tunnel.

        :param ws: The open tunnel connection.
        :param frame: The status request frame.
        :returns: None.
        """
        try:
            result = await self._handle_code_intel_status(frame)
            await ws.send(encode_host_frame(result))
        except Exception:  # noqa: BLE001 - tunnel send failures are best-effort
            _logger.debug(
                "Failed to send code-intel status result for %s",
                frame.request_id,
                exc_info=True,
            )
    ...
    def _build_connect_headers(self) -> dict[str, str]:
        ...
        """
        from goalrail.host.identity import HOST_TOKEN_ENV_VAR, MANAGED_HOST_TOKEN_HEADER
        ...
        headers: dict[str, str] = {"Origin": GOALRAIL_INTERNAL_WS_ORIGIN}
    ...
    async def _dispatch_host_frame(
        ...
        elif isinstance(frame, HostCodeIntelStatusFrame):
            status_task = asyncio.create_task(
                self._send_code_intel_status_result(ws, frame),
                name=f"host-code-intel-status:{frame.request_id}",
            )
...
def run_host_process(
    ...
    """
    from goalrail.host.identity import default_config_path
    ...
    # Tell the user where logs land up front — `goalrail host` used to run
