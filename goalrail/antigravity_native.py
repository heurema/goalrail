...
``goalrail antigravity`` treats the Antigravity ``agy`` CLI as a
terminal-first program, mirroring ``goalrail codex`` / ``goalrail claude``.
...
* **RPC mirroring (read path) and RPC web-turn delivery (write path).** agy's
  ...
  (:mod:`goalrail.antigravity_native_reader`), which polls/streams agy's
  ...
  (:mod:`goalrail.inner.antigravity_native_executor`) over the connect-RPC
...
* **Per-session identity is minted at cold-start, not assigned at launch.** agy
  ...
  :func:`goalrail.antigravity_native_launch.build_agy_launch`).
...
from goalrail.antigravity_native_launch import (
    agy_binary_path,
    build_agy_launch,
...
class LaunchedAntigravityTerminal:
    ...
    :param tmux_socket: Local tmux socket path when the runner exposed
        one, e.g. ``"/tmp/goalrail-terminal-x/tmux.sock"``.
...
def run_antigravity_native(
    *,
    ...
    :param permission_mode: Optional Goalrail permission mode, e.g.
        ...
        hang (see
        :func:`goalrail.antigravity_native_launch.should_skip_permissions`).
    ...
    with TemporaryDirectory(prefix="goalrail-antigravity-native-") as tmpdir:
...
def _materialize_antigravity_agent_spec(tmpdir: Path) -> Path:
    """
    Write the terminal-first agent spec used by ``goalrail antigravity``.
    ...
    raw: dict[str, object] = {
        ...
        # ``_require_os_env`` in ``goalrail/runner/app.py``). agy already
...
def _run_with_remote_server(
    ...
    *,
    ...
    :returns: None.
    """
    from goalrail.chat import _bundle_agent, _remote_headers
...
async def _launch_and_record(
    ...
    *,
    ...
    :func:`goalrail.antigravity_native_rpc.resolve_language_server_port`). A pid
    ...
    conversation (:mod:`goalrail.antigravity_native_reader`) keeps an in-memory
    seen-set only.
...
async def _attach_terminal(
    ...
    **Read/write wiring on the CLI fallback only.** When the runner produced no
    ...
    driver (:func:`goalrail.antigravity_native_reader.run_reader_with_bridge`,
...
async def _agy_cold_start_poll_sleep(seconds: float) -> None:
    ...
    :func:`goalrail.runner.app._agy_cold_start_poll_sleep`.
...
async def _cold_start_agy_conversation(
    ...
    *,
    ...
    :func:`goalrail.runner.app._cold_start_agy_conversation`: once agy is live
    ...
    codex/pi) so a later ``goalrail antigravity --resume`` reads it back and passes
...
async def _attach_direct_tmux(socket_path: Path, tmux_target: str) -> _AttachOutcome:
    ...
    who runs ``goalrail antigravity`` from inside their own tmux can still
...
async def _launch_antigravity_terminal(
    ...
    *,
    ...
    body = {
        ...
        # ``goalrail/server/routes/sessions.py`` ``is_native_bootstrap``).
        #
...
def _launch_is_headless() -> bool:
    ...
    ``goalrail antigravity`` attaches the local TTY to the agy tmux terminal so
    ...
    (see :func:`goalrail.antigravity_native_launch.should_skip_permissions`).
    ...
    .. note:: This TTY signal governs ONLY the human-invoked CLI launch path
       ...
       (:func:`goalrail.runner.app._auto_create_antigravity_terminal`, the
