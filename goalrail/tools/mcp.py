...
The runner's :class:`goalrail.runner.mcp_manager.RunnerMcpManager`
...
class McpServerConnection:
    ...
    async def _invoke_tool(
        ...
        ``/mcp/execute`` → Goalrail server) can surface the elicitation
    ...
    async def call_tool_with_elicitation(
        ...
        Called by the Goalrail server after the user has responded to an
        ...
        # InputRequiredResult on the retry. Raise so the Goalrail server
    ...
    async def _open_stdio_transport(
        ...
        (``goalrail/inner/mcp_tools.py``); Goalrail now matches that
        baseline. Per-MCP sandboxing — if reintroduced — should
        flow through the ``goalrail/environments/`` primitive
...
async def _sleep(seconds: float) -> None:
    ...
    ``asyncio.sleep`` globally (patching ``goalrail.tools.mcp.asyncio.sleep``
...
async def _call_tool_with_reconnect(
    ...
    :param session_id: Goalrail session id forwarded to
