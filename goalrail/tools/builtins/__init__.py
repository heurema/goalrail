...
from goalrail.tools.builtins.read_skill_file import (
    ReadSkillFileTool,
)
...
from goalrail.tools.builtins.timer import (
    SysTimerCancelTool,
    SysTimerSetTool,
)
...
def _create_code_search(config: dict[str, str]) -> Tool:
    """
    Lazy factory for CodeSearchTool.

    :param config: Tool config (unused).
    :returns: A CodeSearchTool instance.
    """
    from goalrail.tools.builtins.code_intel import CodeSearchTool
...
# per ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §3a + §6.2. Their
...
_BUILTIN_REGISTRY: dict[str, _BuiltinFactory | None] = {
    ...
    "code_search": _create_code_search,
    ...
    # via the tool registry — see goalrail/runtime/policies/approval.py.)
    "web_fetch": None,
    "list_comments": None,
    "update_comment": None,
