...
``goalrail/runner/tool_dispatch.py::_execute_web_fetch_tool``) which
...
def build_researcher_spec(parent_spec: AgentSpec) -> AgentSpec:
    ...
    - An ``os_env`` block — registers ``sys_os_shell`` for one-shot
      ...
      per ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §3a in favor of
    ...
    - The parent's ``os_env.sandbox`` (filesystem grants, egress
      ...
      ``goalrail/inner/os_env.py::create_os_environment``). If the
