...
from goalrail.inner.os_env import OSEnvironment
from goalrail.spec import AgentSpec
from goalrail.spec.types import SharePolicy, ToolRuntime
from goalrail.tools._srt import is_srt_available
...
from goalrail.tools.client_specified import ClientSideTool, ClientSideToolSpec
from goalrail.tools.local import load_local_python_tools
...
class ToolManager:
    ...
    def _register_policy_tools(self) -> None:
        ...
        the Goalrail server's REST endpoints.
        """
    ...
    def _register_timer_tools(self) -> None:
        ...
        match ``goalrail/inner/datamodel.py::AgentDef.timers``).
    ...
    def _create_builtin(
        ...
        if name == "upload_file":
            from goalrail.tools.builtins.upload_file import UploadFileTool
    ...
    def _create_web_fetch(self) -> Tool:
        ...
        """
        from goalrail.tools.builtins.web_fetch import WebFetchTool
    ...
    def _register_task_lifecycle_tools(self) -> None:
        ...
        """
        from goalrail.tools.builtins.async_inbox import SysCancelTaskTool
    ...
    def _register_terminal_tools(self) -> None:
        ...
        Per ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` §4.3, the five
        ...
        :func:`goalrail.runtime.get_terminal_registry`) so state
    ...
    def _register_local_tools(self, workdir: Path | None) -> None:
        ...
        for client_info in client_local_tools:
            ...
            # The validator (see ``goalrail/spec/validator.py``)
        ...
        # ``tools/python/*.py`` files. Goalrail-style tools
        # (``language == "goalrail-python-callable"``) come
        ...
        if workdir is not None:
            for tool in load_local_python_tools(
                ...
                if not is_valid_tool_name(tool.name()):
                    ...
                    continue
                self._tools[tool.name()] = tool
        # Goalrail-style callable tools — sibling loader for the
        # ``goalrail-python-callable`` language entries the YAML
        # translator emits. See
        # :mod:`goalrail.tools.local_callable` for the reasoning;
        ...
        from goalrail.tools.local_callable import load_local_callable_tools

        for callable_tool in load_local_callable_tools(server_local_tools):
            if not is_valid_tool_name(callable_tool.name()):
                _logger.warning(
                    "Goalrail callable tool %r has invalid name — skipping",
                    callable_tool.name(),
                )
                continue
            if callable_tool.name() in self._tools:
                ...
                raise ValueError(
                    f"goalrail callable tool {callable_tool.name()!r} "
                    f"collides with an already-registered tool"
                )
    ...
    def _register_client_tools(
        ...
        Raises :class:`GoalrailError` if a tool name violates the
        ...
        :param specs: List of :class:`ClientSideToolSpec` objects to
            register, e.g.
            ``[ClientSideToolSpec(name="get_weather", ...)]``.
        :raises GoalrailError: If any tool name is invalid.
        """
        for spec in specs:
