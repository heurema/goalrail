"""
In-process callable tools for the goalrail-style spec adapter.

The goalrail YAML translator stores user-declared function
tools as :class:`goalrail.spec.types.LocalToolInfo` entries
with ``language == "goalrail-python-callable"`` and ``path``
holding a dotted-import string (e.g.
``"examples._shared.tool_functions.calculate"``). These are
distinct from native Goalrail local tools, which live as
``tools/python/*.py`` files in the agent bundle and execute via
:class:`goalrail.tools.local.LocalPythonTool` in a subprocess.
...
- :func:`load_local_callable_tools` — helper that
  :class:`goalrail.tools.manager.ToolManager` calls alongside
  :func:`goalrail.tools.local.load_local_python_tools` to
  pick up the goalrail-translated subset.
...
when the goalrail compat path retires (the legacy executor
...
# Match the language constant used by the goalrail YAML
# translator (goalrail/spec/goalrail.py:GOALRAIL_TOOL_LANGUAGE).
# Duplicated here because importing from spec/goalrail.py would
...
class LocalCallableTool(Tool):
    ...
    :attr:`goalrail.spec.types.LocalToolInfo.path` and is
    ...
    def dotted_path(self) -> str:
        ...
        Used by :func:`goalrail.runtime.workflow._dispatch_local_callable_tool_async`
    ...
    def description(cls) -> str:
        ...
        :returns: Generic class-level description. Per-instance
            ...
            :meth:`get_schema`.
        """
        return "User-declared function tool from an goalrail-style YAML."
    ...
    def get_schema(self) -> dict[str, Any]:
        ...
        used (matching what the goalrail harness advertises
...
def load_local_callable_tools(
    ...
    """
    Build :class:`LocalCallableTool` instances for goalrail tools.

    Filters *local_tools* to the ones the goalrail YAML
    translator produced — entries whose ``language`` is
    ``"goalrail-python-callable"``. Other entries (native AP
    file-based tools with ``language == "python"``) are
    handled by :func:`goalrail.tools.local.load_local_python_tools`.

    :param local_tools: All :class:`LocalToolInfo` entries from
        the agent spec.
    :returns: One :class:`LocalCallableTool` per goalrail-style
...
def _stringify(value: Any) -> str:
    ...
    The legacy spec path stringifies tool results the same
