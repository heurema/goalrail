...
``goalrail_client.tools`` — import them from there.

Imports are lazy so that loading ``goalrail.tools`` does not pull
...
if TYPE_CHECKING:
    ...
    from goalrail.tools.client_specified import (
        ClientSideTool,
        ClientSideToolSpec,
    )
    from goalrail.tools.local import LocalPythonTool
...
# `goalrail_client.tools` package (see sdks/python-client/). Import
# them from there — they used to be re-exported here but the
# duplicate surface was removed in the SDK carve-out.
_LAZY_EXPORTS: dict[str, tuple[str, str]] = {
    "Tool": ("goalrail.tools.base", "Tool"),
    "ToolContext": ("goalrail.tools.base", "ToolContext"),
    "ClientSideTool": ("goalrail.tools.client_specified", "ClientSideTool"),
    "ClientSideToolSpec": (
        "goalrail.tools.client_specified",
        "ClientSideToolSpec",
    ),
    "LocalPythonTool": ("goalrail.tools.local", "LocalPythonTool"),
    "ToolManager": ("goalrail.tools.manager", "ToolManager"),
}
...
def __getattr__(name: str) -> Any:
    ...
    ``tool`` decorator from ``goalrail_client`` (not here),
