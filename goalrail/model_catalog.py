...
precedence as :func:`goalrail.runtime.workflow._resolve_provider_for_build`
...
(:func:`goalrail.model_override.normalize_model_for_provider`).
...
from goalrail._platform import default_shell_argv
...
def resolve_model_provider(spec: Any, harness: str | None) -> ResolvedModelProvider:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    try:
        ...
        # GoalrailError text is this codebase's own (secret-free); anything
        # else is redacted to its type name — raw detail stays at DEBUG.
        reason = str(exc) if isinstance(exc, GoalrailError) else type(exc).__name__
...
def _resolve_model_provider_unsafe(spec: Any, harness: str | None) -> ResolvedModelProvider:  # type: ignore[explicit-any]  # structural spec stubs in tests
    ...
    from goalrail.runtime.workflow import _resolve_provider_for_build
...
def list_models_for_worker(
    ...
    *,
    ...
    family rule from :func:`~goalrail.model_override.model_family_mismatch`
