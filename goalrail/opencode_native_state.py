"""Persistent client-side state for ``goalrail opencode`` sessions.

The native OpenCode wrapper records the cwd used to create a session so a
later ``goalrail opencode --resume <conv_id>`` can launch OpenCode from
...
the shared Goalrail server. Mirrors :mod:`goalrail.codex_native_state`.
...
_STATE_ROOT_ENV_VAR = "GOALRAIL_OPENCODE_NATIVE_STATE_DIR"
