...
Goalrail elicitation, waits for the human's verdict, and delivers it back to agy
...
WAITING step at a HIGHER ``stepIndex``**. Goalrail elicitations wait on a human
...
* ``deliver`` — defaults to :func:`_deliver_via_rpc`, which offloads the blocking
  :func:`goalrail.antigravity_native_rpc.handle_user_interaction` to a worker
...
:func:`goalrail.codex_native_elicitation.codex_elicitation_id`), so a
...
from goalrail.antigravity_native_steps import PendingInteraction, pending_interaction
...
class Deliver(Protocol):
    ...
    Matches :func:`goalrail.antigravity_native_rpc.handle_user_interaction`
...
async def _inject_via_tui(keys: list[str]) -> None:
    ...
    :func:`goalrail.antigravity_native_bridge.send_interaction_keys_via_tui`) to a
    ...
    from goalrail.antigravity_native_bridge import send_interaction_keys_via_tui
    from goalrail.inner.antigravity_native_executor import _bridge_dir_from_env
...
def agy_elicitation_id(cascade_id: str, trajectory_id: str, step_index: int) -> str:
    """
    Build the Goalrail elicitation id for one agy WAITING interaction.
    ...
    :func:`goalrail.codex_native_elicitation.codex_elicitation_id`.
...
async def _deliver_via_rpc(
    ...
    *,
    ...
    :func:`goalrail.antigravity_native_rpc.handle_user_interaction` is synchronous
...
async def bridge_interaction(
    ...
    *,
    ...
    :param deliver: Delivers the built payload to agy; defaults to
        ...
        :func:`goalrail.antigravity_native_rpc.handle_user_interaction` to a
        worker thread.
