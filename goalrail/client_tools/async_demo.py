"""
Async-dispatch demo tool set for ``goalrail run --tools async_demo``.
...
Registered via :func:`goalrail.client_tools.get_tool_set`,
which ``goalrail run`` calls when ``--tools async_demo`` is
passed.
"""
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    """
    Execute a client-side tool call from ``goalrail run``.
