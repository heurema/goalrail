...
functions and surfaced through the ``goalrail_client``
...
(``examples/frontends/terminal.py``, ``goalrail chat``'s raw-schema
path) keep working without modification.

Used by ``goalrail chat --tools coding`` and the terminal TUI.
"""
...
# ``goalrail chat`` ``_load_tool_handler`` path reach into modules
...
def execute_tool(name: str, arguments: dict[str, Any]) -> str:
    ...
    Used by ``examples/frontends/terminal.py`` and ``goalrail chat``'s
    raw-schema path. New consumers should construct a
    :class:`~goalrail_client.tools.ToolHandler` via
    :func:`~goalrail_client.tools.build_tool_handler` against
