...
- `goalrail` — the server image (built from `../Dockerfile`)
...
docker compose up -d
docker compose logs -f goalrail       # ctrl-c when boot is clean
```
...
.env` and edit `POSTGRES_PASSWORD` (and `GOALRAIL_OIDC_COOKIE_SECRET`
...
GOALRAIL_ACCOUNTS_BASE_URL=https://goalrail.example.com

docker compose up -d
docker compose logs goalrail | grep -A4 "Created initial admin"
```
...
```bash
GOALRAIL_ACCOUNTS_INIT_ADMIN_PASSWORD=<your-strong-password>
```
...
2. **Mint a cookie secret.** `./bootstrap.sh` already did this on the
   quickstart path — `GOALRAIL_OIDC_COOKIE_SECRET` is set in your
...
3. **Edit `.env`:**
   ...
   GOALRAIL_OIDC_CLIENT_ID=Iv1.abc123…
   GOALRAIL_OIDC_CLIENT_SECRET=…
   GOALRAIL_OIDC_REDIRECT_URI=https://goalrail.example.com/auth/callback
   # GOALRAIL_OIDC_COOKIE_SECRET is already set by bootstrap.sh — leave it alone.
   ```
...
4. **Bring it up.**
   ...
   is missing — check `docker compose logs goalrail` if it doesn't
   come up.
...
GOALRAIL_OIDC_CLIENT_SECRET=…
GOALRAIL_OIDC_REDIRECT_URI=https://goalrail.example.com/auth/callback
GOALRAIL_OIDC_COOKIE_SECRET=<64-hex-chars>
GOALRAIL_OIDC_ALLOWED_DOMAINS=example.com,subsidiary.example.com
```
...
Set `GOALRAIL_OIDC_ISSUER` to the base URL; the server fetches
discovery at startup.

```bash
GOALRAIL_AUTH_PROVIDER=oidc
GOALRAIL_OIDC_ISSUER=https://your-tenant.okta.com
...
1. **Use the bundled Caddy overlay** (easiest — any VPS / EC2 / home
   ...
   ```bash
   # In .env:
   GOALRAIL_DOMAIN=goalrail.example.com
   GOALRAIL_ACME_EMAIL=you@example.com      # optional, for Let's Encrypt notices
   ...
   goalrail container stops being directly exposed and only :80 +
...
2. **Behind an existing reverse proxy** — point your proxy at
   `goalrail:8000` over the docker network (or `127.0.0.1:8000`
...
a different header name set `GOALRAIL_AUTH_HEADER` to point the
...
`accounts.google.com:`; set `GOALRAIL_AUTH_HEADER_STRIP_PREFIX` to drop
...
| `POSTGRES_USER` / `POSTGRES_DB` | `goalrail` | DB user + database name. |
| `GOALRAIL_PORT` | `8000` | Host port the server is published on. |
| `GOALRAIL_AUTH_ENABLED` | `1` (in compose) | Master auth switch. `1` → accounts (or oidc if `GOALRAIL_OIDC_ISSUER` is set); `0` → single-user local mode (every request is the shared `local` user — local dev only, never shared deploys). |
| `GOALRAIL_AUTH_PROVIDER` | unset | Escape hatch to pin a mode explicitly: `header` / `accounts` / `oidc`. Overrides the `AUTH_ENABLED` auto-selection. |
| `GOALRAIL_AUTH_HEADER` | `X-Forwarded-Email` | Header-mode only: name of the trusted identity header. Set for proxies that use another name, e.g. `Cf-Access-Authenticated-User-Email` (Cloudflare Access). |
| `GOALRAIL_AUTH_HEADER_STRIP_PREFIX` | unset (strip nothing) | Header-mode only: prefix removed from the identity header value. Set to `accounts.google.com:` for Google IAP's `X-Goog-Authenticated-User-Email`. |
| `GOALRAIL_OIDC_*` | unset | OIDC config — required in oidc mode (issuer set, or `AUTH_PROVIDER=oidc`). See `.env.example`. |
...
The same Dockerfile publishes a second image: the official Goalrail
...
the full goalrail install (all three packages + deps, `python` and
...
- `ghcr.io/heurema/goalrail-host:latest` — tracks main HEAD
  (the default for `goalrail sandbox create --provider modal`)
- `ghcr.io/heurema/goalrail-host:sha-<short>` — immutable
  per-commit pin
- `ghcr.io/heurema/goalrail-host:vX.Y.Z` — release tags

Build it locally from the repo root:

```bash
docker build -t goalrail-host:latest --target host \
...
`ghcr.io/heurema/goalrail-host:latest` by default. Your local
...
| Variable | Purpose |
|---|---|
| `GOALRAIL_MODAL_HOST_IMAGE` | Override the image ref, e.g. an org-internal copy (`ghcr.io/<your-org>/goalrail-host:latest`) or a `:sha-<short>` pin. |
| `GOALRAIL_MODAL_REGISTRY_SECRET` | Name of a [Modal secret](https://modal.com/secrets) holding registry credentials for private pulls. Create it with keys `REGISTRY_USERNAME` (your registry username) and `REGISTRY_PASSWORD` (for GHCR: a personal access token with `read:packages`). Unset = anonymous pull. |
...
`GOALRAIL_DAYTONA_HOST_IMAGE` or the server config's
