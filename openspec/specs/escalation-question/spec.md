# escalation-question Specification

## Purpose

Define the provider-neutral shape of the payload a run writes when the work item
cannot be completed as specified, and place its semantic validation outside
Goalrail — which treats the payload as opaque bytes with retention hygiene only,
consistent with Goalrail never running checks.
## Requirements
