...
- [x] 11.5b Obtain a separate explicit owner instruction before archiving this change and promoting the deltas, and re-read the promoted text immediately before promotion in case another change archived first. Granted 2026-07-31 after the correction merged. The promoted text was re-read immediately beforehand: `main` moved by three commits since this change began — its own two merges and one unrelated documentation change — and nothing else archived, so the three capabilities this change touches still carry exactly what the deltas were authored against. The `harness-update` requirement the `MODIFIED` delta replaces still holds its three scenarios, all three of which the delta preserves.
- [x] 11.6 Leave the owner's working scaffold configuration and the `codex/dogfood-run-v0` branch untouched throughout; every check, cache, and stand-in source this change exercises lives in a temporary directory or a fake state root. Held for the whole change and checked at its close: both `~/.claude/settings.json` and `~/.codex/config.toml` carry zero managed handlers, `codex/dogfood-run-v0` still points at `4b99eed`, and no update-check cache was written into the owner's real state root — every run used a scratch state directory, which is also why the diagnosis was exercised with `GOALRAIL_STATE_HOME` set throughout.
