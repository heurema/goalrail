...
checking attachment health, and disconnecting. It SHALL make plain that after
connection ordinary work needs no Goalrail command, so an operator cannot
mistake the wrapper lifecycle for something they must run per task.

The health command SHALL be discoverable from help, because the state it
reports — hooks registered but not yet trusted — is otherwise indistinguishable
...
- **WHEN** an operator invokes `gr help`
- **THEN** connection, initialization, health, and disconnection are presented
  as a separate surface, and the text states that ordinary work needs no
  per-task Goalrail command

#### Scenario: Operator cannot tell untrusted from broken
- **WHEN** an operator's attachment is registered but not yet trusted
- **THEN** help points at the health command that names that exact state
