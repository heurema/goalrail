...
| A user downloads through a browser | **observed**, recorded in `browser-download-2026-07-30.md`: a browser-quarantined archive carries the flag onto the extracted binary — through Finder and through `tar` alike — and the quarantined binary does not run, hanging on the Gatekeeper prompt rather than printing an error. Clearing the flag before the first run releases it; clearing it after a blocked run does not, and a fresh copy does. This corrected one clause of CTX-18 and three statements in the install documentation. |
