...
Where the marker records an adoption and the configuration's rules block still
matches the digest recorded at that moment, the diagnosis SHALL report it in one
line naming the replaced schema and when the adoption happened. Initialization
discloses the same fact once, to whoever ran it; that is routinely an agent, and
the person whose rules they are may never see it. The diagnosis is where a
standing condition belongs.
