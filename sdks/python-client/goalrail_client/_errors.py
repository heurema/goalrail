"""Typed exceptions for the goalrail client."""
...
class GoalrailError(Exception):
    """Base exception for all goalrail client errors."""
...
class AgentNotFoundError(GoalrailError):
    """Agent name or ID not found (HTTP 404)."""


class ResponseNotFoundError(GoalrailError):
    """Response ID not found (HTTP 404)."""


class FileNotFoundError(GoalrailError):
    """File ID not found (HTTP 404)."""


class ConversationNotFoundError(GoalrailError):
    """Conversation ID not found (HTTP 404)."""


class InvalidInputError(GoalrailError):
    """Bad request — invalid input, missing fields, etc. (HTTP 400)."""


class ConflictError(GoalrailError):
    """Resource conflict — duplicate name, stale state, etc. (HTTP 409)."""


class BundleInvalidError(GoalrailError):
    """Agent bundle is invalid — corrupt tarball, bad config, etc. (HTTP 400)."""


class ServerError(GoalrailError):
...
def raise_for_status(status_code: int, body: dict[str, object] | str) -> None:
    ...
    _CODE_MAP: dict[str, type[GoalrailError]] = {
        "not_found": GoalrailError,
    ...
    if status_code == 404:
        ...
        # GoalrailError or the specific subclass at the call site.
        raise GoalrailError(message, status_code, code)
...
def require_json_object(resp: httpx.Response, endpoint: str) -> dict[str, object]:
    ...
    protocol/base-URL/auth problem, so raise :class:`GoalrailError`
    ...
    :raises GoalrailError: If the body is not a JSON object.
    """
    try:
        data = resp.json()
