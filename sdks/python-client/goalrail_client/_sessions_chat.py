...
A higher-level wrapper over :class:`goalrail_client._sessions.SessionsNamespace`
that mirrors enough of :class:`goalrail_client._session.Session`'s
public surface for downstream consumers (terminal REPL, ``goalrail
...
:class:`goalrail_client._session.Session` synthesizes a multi-turn
...
``goalrail/server/API.md`` ("Sessions API"). Per the same spec
...
:data:`goalrail.server.schemas.ServerStreamEvent` envelopes
...
:class:`goalrail_client._session.Session` and renaming would break
...
from ._errors import GoalrailError
...
# :class:`goalrail.server.schemas.SessionEventInput`. Kept as a
...
class SessionToolCallInfo:
    ...
    :class:`goalrail_client._tool_handler.ToolCallInfo` — that
...
# terminal set listed in ``goalrail/server/schemas.py`` (and
# the ``_TERMINAL_STATUSES`` set in
# :mod:`goalrail_client._session`).
...
class SessionsChat:
    ...
    Usage mirrors :class:`goalrail_client._session.Session` for
    ...
    session is an HTTP call. Use :meth:`GoalrailClient.sessions_chat`
    ...
    :param namespace: The :class:`SessionsNamespace` this helper
        ...
        :class:`GoalrailClient`; the chat helper does not own
    ...
    async def create(
        ...
        *,
        ...
        :returns: A :class:`SessionsChat` bound to the newly created
            session.
        :raises GoalrailError: If session creation fails.
        """
    ...
    async def refresh(self) -> Session:
        ...
        :raises GoalrailError: If the session no longer exists
    ...
    async def tree_busy(self, *, max_depth: int = 3) -> bool:
        ...
        :raises GoalrailError: On non-2xx status.
        """
    ...
    async def send(
        ...
        *,
        ...
        :raises GoalrailError: If the underlying ``post_event`` or
        ...
        try:
            ...
            while True:
                ...
                if isinstance(event, SessionStatusEvent) and event.status == "failed":
                    ...
                    raise GoalrailError(message, code=code)
                try:
        ...
        * a key in ``tool_callables`` that does not name a
        ...
        if self._tool_callables and self._agent_tools_getter is None:
            raise RuntimeError(
                ...
                "GoalrailClient.sessions_chat which wires it "
    ...
    async def cancel(self) -> None:
        ...
        :raises GoalrailError: If the session does not exist
            (404).
        """
    ...
    async def query(
        ...
        *,
        ...
        :raises GoalrailError: If the underlying HTTP calls fail.
    ...
    async def await_turn(self, *, timeout: float | None = 1200.0) -> QueryResult:
        ...
        :raises GoalrailError: Propagated from :meth:`stream` if the
    ...
    async def _build_content(
        ...
        :meth:`goalrail_client._session.Session._build_input_with_files`
...
class _FilesUploader(Protocol):
    ...
    :meth:`goalrail_client._files.FilesNamespace.upload`. Defined
...
class _FilesGetter(Protocol):
    ...
    Matches the signature of
    :meth:`goalrail_client._files.FilesNamespace.get`.
    """
...
def _build_tool_call_info(item: dict[str, Any]) -> SessionToolCallInfo:
    ...
    ``goalrail/server/schemas.py:OutputItemDoneEvent``) —
