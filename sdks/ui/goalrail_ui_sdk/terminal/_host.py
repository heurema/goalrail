...
from goalrail_client import TERMINAL_TASK_STATUSES, child_session_busy
...
_OVERLAY_REQUEST_SENTINEL: str = "\x00__goalrail_ui_sdk.overlay_trigger__\x00"
...
_SUBAGENT_SELECT_SENTINEL: str = "\x00__goalrail_ui_sdk.subagent_select__\x00"
...
# that goalrail' cli.py uses so the two REPLs look identical
...
class _SubagentNode:
    ...
    # labels / title via :func:`goalrail.session_lifecycle.is_session_closed`.
...
class Overlay:
    ...
    * **Sidebar + content** (``targets_builder`` is supplied):
    ...
    :param builder: Async callable invoked each time the overlay
        ...
        See ``goalrail/repl/_repl.py::_build_debug_overview``
    ...
    :param sidebar_width: Column width (in characters) for the
        ...
        — matches the goalrail debug panel and fits the
...
def _install_csi_u_sequences() -> None:
    ...
    ``goalrail/inner/cli.py:1186-1239`` so behavior is
...
class TerminalHost:
    ...
    def __init__(
        ...
        *,
        ...
        history_file: str = "~/.goalrail_history",
    ...
    async def __aenter__(self) -> TerminalHost:
        ...
        try:
            from goalrail.cli_diagnostics import redirect_stderr_to_log
    ...
    async def __aexit__(self, *exc: object) -> None:
        ...
        try:
            from goalrail.cli_diagnostics import restore_stderr
    ...
    async def _show_overlay(self, overlay: Overlay) -> None:
        ...
        async def _refresh_loop() -> None:
            ...
            hammer the server. Matches goalrail' overview polling
            strategy (see ``goalrail/cli.py::_refresh_loop``),
            with the interval bumped from 50 ms → 500 ms because
            Goalrail' builder crosses a real HTTP boundary
            while goalrail' builder just reads in-process state.
            """
            try:
                while True:
                    await asyncio.sleep(0.5)
        ...
        def _get_sidebar() -> ANSI:
            ...
            even with variable-length labels. Matches the goalrail
    ...
    def upsert_subagent(
        ...
        *,
        ...
        # the ``goalrail`` server package on the path.
        if not node.closed:
            from goalrail.session_lifecycle import is_session_closed
    ...
    def build_toolbar(self) -> FormattedText:
        ...
        goalrail' main REPL shows at the bottom-right. A
