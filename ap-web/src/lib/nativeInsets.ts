...
// `--goalrail-native-top-bar` / `--goalrail-native-bottom-bar`; index.css folds
// them (together with the web-owned visibility flags and `env(safe-area-*)`)
// into `--goalrail-inset-top` / `--goalrail-inset-bottom`, which `<PageScroll>`
...
export function initNativeInsets(): () => void {
  return onNativeInsets(({ topBar, bottomBar }) => {
    ...
    root.setProperty("--goalrail-native-top-bar", `${topBar}px`);
    root.setProperty("--goalrail-native-bottom-bar", `${bottomBar}px`);
  });
}
