  ...
  var body: some View {
    GeometryReader { geometry in
      ...
      .overlay(alignment: .bottom) {
        ...
        // layer reserves a fixed footprint for it (`.goalrail-native-bottom-
  ...
  var body: some View {
    Menu {
      Button {
        ...
        Label(currentURL.goalrailHostLabel, systemImage: "checkmark")
      }
      .disabled(true)

      let otherServers = recents.filter {
        URL(string: $0)?.goalrailOrigin != currentURL.goalrailOrigin
      }
      if !otherServers.isEmpty {
        ...
        ForEach(otherServers, id: \.self) { recent in
          Button {
            ...
            Text(URL(string: recent)?.goalrailHostLabel ?? recent)
          }
        }
      }
      ...
      HStack(spacing: 6) {
        Text(currentURL.goalrailHostLabel)
          .fontWeight(.medium)
          .lineLimit(1)
