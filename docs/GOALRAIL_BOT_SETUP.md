...
> workflow identities below still use legacy `goalrail-*` names until the CI
...
| --- | --- | --- |
| `goalrail <noreply@goalrail.dev>` | Co-author trailer on commits authored by **polly's coding sub-agents** | These commits are produced by `git commit` in a worker's local worktree — they are **not** GitHub Actions runs, so a plain org co-author is the honest attribution. No GitHub App user is involved. |
| `goalrail-ci[bot]` (GitHub App) | **CI automation**: lockfile-regen commits/PRs **and** automated PR-review comments | These actions genuinely run inside GitHub Actions, where the App's private key lives and a short-lived installation token is minted per run. The App is an org-owned, least-privilege identity. |

> **Why two identities and not one?** An earlier draft of this work tried to use
> `goalrail-ci[bot]` everywhere, including the sub-agent commit trailer. That was
...
## The GitHub App: `goalrail-ci[bot]`

> **Naming note.** The App was registered as **`goalrail-ci`** (the bare
> `goalrail` name was unavailable), so GitHub renders the actor as
> **`goalrail-ci[bot]`**.

| Field | Value |
| --- | --- |
| App name | `goalrail-ci` |
| Bot actor | `goalrail-ci[bot]` |
| App ID | `4082516` |
| Bot numeric user ID | `294685417` |
| CI git author email | `294685417+goalrail-ci[bot]@users.noreply.github.com` |
...
Created at `https://github.com/organizations/heurema/settings/apps/new`:

- **GitHub App name:** `goalrail-ci` → actor `goalrail-ci[bot]`.
...
- **Where can this App be installed?** Only on `heurema`.
- Installed into `heurema`, scoped to the `goalrail` repo.
...
```bash
gh api users/goalrail-ci%5Bbot%5D --jq '.id'
# -> 294685417
```
...
In **`heurema/goalrail` → Settings → Secrets and variables → Actions**:

- **Variable** `GOALRAIL_BOT_APP_ID` = `4082516`
- **Secret** `GOALRAIL_BOT_APP_KEY` = the App's `.pem` private key

> The workflows gate on `vars.GOALRAIL_BOT_APP_ID != ''`. If the variable is
...
> **`goalrail-ci[bot]` replaced the old OSS regen bot.** The previous
...
```
Co-authored-by: goalrail <noreply@goalrail.dev>
```
...
- name: Mint App token
  id: app-token
  if: vars.GOALRAIL_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.GOALRAIL_BOT_APP_ID }}
    private-key: ${{ secrets.GOALRAIL_BOT_APP_KEY }}
```

```bash
git config user.name  "goalrail-ci[bot]"
git config user.email "294685417+goalrail-ci[bot]@users.noreply.github.com"
```
...
### Automated PR review posted as `goalrail-ci[bot]`
...
token and posts the review **as `goalrail-ci[bot]`**:

```yaml
- name: Mint App token
  id: app-token
  if: steps.trigger.outputs.skip != 'true' && steps.creds.outputs.available == 'true' && vars.GOALRAIL_BOT_APP_ID != ''
  uses: actions/create-github-app-token@bcd2ba49218906704ab6c1aa796996da409d3eb1 # v3.2.0
  with:
    app-id: ${{ vars.GOALRAIL_BOT_APP_ID }}
    private-key: ${{ secrets.GOALRAIL_BOT_APP_KEY }}
...
| --- | --- | --- |
| polly sub-agent commits | `goalrail <noreply@goalrail.dev>` (co-author trailer) | `examples/polly/agents/*/config.yaml` |
| Lockfile-regen commits/PRs | `goalrail-ci[bot]` | `oss-regenerate-and-smoke.yml`, `oss-regen-on-comment.yml` |
| Automated PR review comments | `goalrail-ci[bot]` (fallback `github-actions[bot]`) | `polly-review.yml` |

**Config:** variable `GOALRAIL_BOT_APP_ID` = `4082516`, secret
`GOALRAIL_BOT_APP_KEY` = App private key. The old `OSS_REGEN_APP_*` config and
App are retired.
