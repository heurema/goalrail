...
the current active OrganizationMembership boundary;
...
- `apps/runner` provides the first minimal API-only `goalrail-runner` checkout receipt loop. It uses `GOALRAIL_RUNNER_BEARER_TOKEN` for the current bearer-authenticated API boundary, polls `POST /v1/checkout-jobs/leases`, exits cleanly on no-work in `--once` mode, receives a bounded checkout instruction, and submits one workspace receipt through `POST /v1/checkout-jobs/{id}/receipts` with checkout lease proof and `raw_source_uploaded=false`. It uses local API DTOs only, does not import server internals or Postgres stores, and does not use command execution packages. It does not clone/fetch repositories in H1, assign or claim WorkItems, run commands, start `Run`, write `GateDecision`, create `Proof`, or add a queue/outbox/runtime registry.
