...
ADR-0029 now defines the H2 boundary, and H2.1 implements the first code slice:
`ExecutionJob(queued)` can be created or returned from a planned WorkItem plus
CheckoutReceipt, while `Run`, execution lease, command execution, execution
receipt, gate, and proof remain deferred;
