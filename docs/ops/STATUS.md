...
- `apps/web/console` consumes `GET /v1/contracts?limit=50` on authenticated
  ...
  `ready_for_approval` / `approved` / `seeded` state filter plus manual
  refresh, keeps existing visible rows on transient discovery errors, preserves
  selection across refresh when possible, and keeps manual Contract ID lookup as
  a secondary read-only fallback through `GET /v1/contracts/{id}`.
