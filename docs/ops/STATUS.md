...
- no production runtime CLI beyond the `apps/cli` command foundation, first browser-loopback login, normal server-backed `goalrail init`, low-level `goalrail init --project <project_id>`, and explicit auth-free `goalrail init --local-demo`
- no server integration for the CLI beyond `goalrail login <server_url>`, server-backed repository-context init, and low-level server-backed RepoBinding init
