...
- `GET /v1/contracts` is now an authenticated read-only Contract discovery
  ...
  gate, or proof state.
- `GET /v1/contracts/{id}/current-draft` is now an authenticated read-only
  current ContractDraft detail endpoint for future Console selected-draft
  rendering. It resolves the caller's active OrganizationMembership through
...
- `apps/web/console` consumes `GET /v1/contracts?limit=50` on authenticated
  ...
  a secondary read-only fallback through `GET /v1/contracts/{id}`. The backend
  now exposes `GET /v1/contracts/{id}/current-draft` for future selected draft
  body rendering, but this Console source does not render draft body fields yet.
