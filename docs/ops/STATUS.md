...
- Postgres-backed intake create, Goal promotion, Goal readiness, ClarificationAnswer recording, answer application, ContractSeed creation, ContractDraft creation/update, ContractDraft ready_for_approval, and ApprovedContract approval writes now share a transaction with their expected event appends, so the durable canonical write does not commit without its audit events
- ClarificationRequest creation stores an open request durably when DB is configured, falls back to in-memory storage without DB, generates deterministic questions from Goal readiness reason codes, guards one open request per Goal, and appends `clarification.requested` through the configured EventLog
