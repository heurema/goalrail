...
- `apps/web/console` consumes `GET /v1/contracts?limit=50` on authenticated
  ...
  a secondary read-only fallback through `GET /v1/contracts/{id}`. Selected
  Contract detail now renders the current draft body through read-only
