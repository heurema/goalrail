...
- an empty EN real console shell under `apps/web/console` for `console.goalrail.dev`, and a static RU console shell under `apps/web/console-ru` live at `https://console.goalrail.ru/` with login-only entry, three empty product surfaces, bottom-left Settings utility, Users add/edit UI in component state only, and no API, production auth, session, cookie, persistence, analytics, or product-loop behavior
...
- `apps/web/console-ru` is the separate Russian console shell for `console.goalrail.ru`, live as a static HTTPS surface with login-only entry, three empty product surfaces, bottom-left Settings utility, and Users add/edit UI in component state only
...
- ClarificationRequest creation stores an open request durably when DB is configured, falls back to in-memory storage without DB, generates deterministic questions from Goal readiness reason codes, guards one open request per Goal, and appends `clarification.requested` transactionally with the request write in the Postgres path
- ClarificationAnswer recording stores canonical answer evidence durably when DB is configured, falls back to in-memory storage without DB, requires all questions answered, transitions the request from `open` to `answered`, and appends `clarification.answer_recorded` and `clarification.request_answered` through the configured EventLog
...
- `apps/web/console` provides a verified empty EN console shell only, and `apps/web/console-ru` provides the verified static RU console shell now live at `https://console.goalrail.ru/`; neither claims backend, server integration, production auth, durable data, or product-loop implementation
