...
with lease proof; command execution, execution receipt, gate, and proof remain
deferred;
...
- H2.1 implements the preparation-only `ExecutionJob(queued)` bridge from
  ...
  acquisition does not create `Run`; run start does not execute commands,
  create execution receipt, decide gates, or create proof.
