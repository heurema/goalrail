...
through `Run(started)` while command execution, execution receipt, gate, and
...
- H2.1 implements the preparation-only `ExecutionJob(queued)` bridge from
  ...
  `ExecutionJob(queued)` -> execution lease -> `Run(started)` path. Run start
  does not execute commands, create execution receipt, decide gates, or create
  proof.
...
- ADR-0029 defines the Run / execution receipt boundary, and H2.1 implements
  ...
  H2.2+ smoke coverage pinning that runtime transition. Execution receipts
  remain separate from Gate / Proof verdicts
