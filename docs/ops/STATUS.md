...
submission with lease/run proof and no command execution, while gate and proof
remain deferred;
...
- H2.1 implements the preparation-only `ExecutionJob(queued)` bridge from
  ...
  `run_started` jobs without receipts. It does not execute commands, decide
  gates, or create proof.
...
- ADR-0029 defines the Run / execution receipt boundary, and H2.1 implements
  ...
  metadata-only `ExecutionReceipt` submission for started Runs; receipts remain
...
- no WorkItem assignment/claiming, command execution, GateDecision, or Proof yet
- no worker controller, runner-backed planning implementation,
  assignment/claiming, command execution, generic queue, outbox, broker,
  runtime registry, `GateDecision`, or `Proof` yet
