...
only; it is not the product web console login UI. `apps/web/console-ru` source
now has a React auth flow for the existing server endpoints, but live console
deployment still needs API routing before it can talk to a backend. SaaS
...
- an EN real console shell under `apps/web/console` for `console.goalrail.dev`, and an RU console shell under `apps/web/console-ru` for `console.goalrail.ru`; RU source now has a repo-side auth client for existing server auth endpoints with login, optional first-login password change, `/v1/me`, logout, in-memory tokens only, and no cookies or token browser-storage persistence; the live RU console deployment remains static-only until a separate API routing slice; both shells keep three structured empty product surfaces, bottom-left Settings utility, Appearance theme picker with local-only browser preference under `goalrail.console.theme`, Users add/edit UI in component state only, and no user-settings persistence, analytics, or product-loop behavior
...
- ADR-0023 documents the user bootstrap, auth, and CLI login boundary:
  ...
  loopback login. `apps/web/console-ru` source now implements a bounded React
...
- `apps/web/console` is the EN real console shell with login-only entry, three structured empty product surfaces, bottom-left Settings utility, Appearance theme picker, local-only theme preference under `goalrail.console.theme`, and Users add/edit UI in component state only
...
- `apps/web/console-ru` is the separate Russian console shell for `console.goalrail.ru`; repo source now has a typed auth client and bounded login -> optional password change -> `/v1/me` -> console -> logout flow against existing server auth endpoints, with access/refresh tokens held in React memory only, `goalrail.console.theme` as the only localStorage key, no cookies, no token `localStorage` / `sessionStorage`, and Users add/edit UI still component-state only; live `https://console.goalrail.ru/` remains a static HTTPS surface until a later API routing/proxy/CORS deployment slice
...
- Postgres-backed intake create, Goal promotion, Goal readiness, ClarificationRequest creation, ClarificationAnswer recording, answer application, ContractSeed creation, ContractDraft creation/update, ContractDraft ready_for_approval, and ApprovedContract approval writes now share a transaction with their expected event appends, so the durable canonical write does not commit without its audit events
...
- `apps/web/console` provides a verified EN console shell matching the RU login/settings/users structure, and `apps/web/console-ru` source now provides a verified repo-side auth flow for existing server login / first-login password change / `/v1/me` / logout before the same structured empty product surfaces; RU tokens remain in React memory only, `goalrail.console.theme` remains the only browser storage key, Users remains component-state only, and live `https://console.goalrail.ru/` still needs a separate API routing slice before deployed auth can work; neither console claims durable product data, user settings API, analytics, runner, gate, proof, repo integration, or product-loop implementation
