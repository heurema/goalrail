...
`WorkItem(planned)`; H1 checkout preparation now adds a server-owned checkout
job / instruction, a minimal API-only `goalrail-runner` checkout receipt loop
under `apps/runner`, and bounded runner-submitted workspace receipts without
assignment, claiming, actual clone/fetch, execution, `Run`, gate, or proof;
