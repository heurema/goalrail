...
- local/demo CLI commands now exist for `version`, Git-aware local/demo `init`, `readiness scan`, `contract validate`, `proof show`, and the first `goalrail login <server_url>` browser-loopback auth path; explicit `goalrail init --project <project_id>` uses the stored login profile to call the server RepoBinding init endpoint and writes a non-secret Git-root `.goalrail/project.yml` marker only after server success
- `apps/server` is the first Go HTTP server bootstrap with canonical binary entrypoint `cmd/goalrail-server`
- server endpoints include `GET /livez`, `GET /readyz`, `GET /version`, `POST /v1/auth/login`, `GET /cli/login`, `POST /cli/login`, `POST /v1/auth/cli/exchange`, `POST /v1/auth/refresh`, `POST /v1/auth/change-password`, `POST /v1/auth/logout`, `GET /v1/me`, `POST /v1/projects/{project_id}/repo-bindings/init`, `POST /v1/intakes`, `GET /v1/intakes/{id}`, `POST /v1/intakes/{id}/goals`, `POST /v1/goals/{id}/readiness`, `POST /v1/goals/{id}/clarifications`, `POST /v1/clarifications/{id}/answers`, `POST /v1/answers/{id}/applications`, `POST /v1/contracts`, `GET /v1/contracts/{id}`, `PATCH /v1/contracts/{id}`, `POST /v1/contracts/{id}/submissions`, `POST /v1/contracts/{id}/approvals`, `POST /v1/contracts/{id}/plans`, `GET /v1/plans/{id}`, `POST /v1/plans/{id}/proposals`, `GET /v1/proposals/{id}`, `POST /v1/proposals/{id}/acceptance`, and `GET /v1/tasks/{id}`; there is no full RepoBinding CRUD endpoint, `GET /v1/plans`, `GET /v1/proposals`, or `GET /v1/tasks` list endpoint, and the previous public `/v1/goals/{id}/contract-seeds`, `/v1/contract-seeds/{id}/contract-drafts`, `/v1/contract-drafts/{id}`, and direct `POST /v1/contracts/{id}/tasks` lifecycle/planning routes are no longer registered
...
- `apps/server` now has a Postgres persistence foundation for the Organization / Project / RepoBinding context plus metadata-only RepoBinding init, IntakeRecord, Goal, public Contract aggregate, ContractSeed, ContractDraft, ApprovedContract, and EventLog state
...
- Postgres-backed RepoBinding init create, intake create, Goal promotion, Goal readiness, ClarificationRequest creation, ClarificationAnswer recording, answer application, ContractSeed creation, ContractDraft creation/update, ContractDraft ready_for_approval, and ApprovedContract approval writes now share a transaction with their expected event appends, so the durable canonical write does not commit without its audit events
...
- no production runtime CLI beyond the local/demo `apps/cli` command foundation, first browser-loopback login, and explicit server-backed `goalrail init --project <project_id>`
- no server integration for the CLI beyond `goalrail login <server_url>` and explicit server-backed RepoBinding init
...
- no broad RepoBinding state sync beyond explicit metadata-only init
...
- no full manual-declared repository registration flow beyond explicit server-backed RepoBinding init
