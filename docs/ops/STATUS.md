...
- ADR-0024 documents the accepted provider-neutral VCS connection boundary:
  `VcsConnection` is a provider connection / account authorization /
  metadata-discovery boundary, not a checkout credential, raw provider token,
  repository catalog, or permission to clone. The server now has the smallest
  provider-neutral, credentialless `pending_setup` `VcsConnection` skeleton for
  setup intent only. `RepoBinding` remains the current Project-to-repository
  metadata reference, `RepositoryRecord` remains deferred, provider-specific
  namespace/account/project concepts stay in adapters, and GitLab may be a
  first provider candidate without becoming the source of core Goalrail
  terminology. No provider client, OAuth, token storage, checkout, runner, gate,
  proof, provider sync, repository catalog, or queue implementation exists from
  this boundary.
...
- ADR-0025 documents the accepted provider credential / token storage boundary:
  ...
  repository metadata listing exists. The current backend skeleton remains
  credentialless and non-live: `pending_setup` cannot list metadata, refresh
  metadata, authorize checkout, create RepoBinding, mutate RepoBinding, run
  GitLab/provider clients, start runners, write gate decisions, or generate
  proof.
...
- server endpoints include `GET /livez`, `GET /readyz`, `GET /version`, `POST /v1/auth/login`, `GET /cli/login`, `POST /cli/login`, `POST /v1/auth/cli/exchange`, `POST /v1/auth/refresh`, `POST /v1/auth/change-password`, `POST /v1/auth/logout`, `GET /v1/me`, `POST /v1/init/repository-context`, `POST /v1/repo-bindings/{repo_binding_id}/context-snapshots`, `POST /v1/projects/{project_id}/repo-bindings/init`, `POST /v1/vcs-connections`, `GET /v1/vcs-connections/{id}`, `POST /v1/intakes`, `GET /v1/intakes/{id}`, `POST /v1/intakes/{id}/goals`, `POST /v1/goals/{id}/readiness`, `POST /v1/goals/{id}/clarifications`, `POST /v1/clarifications/{id}/answers`, `POST /v1/answers/{id}/applications`, `POST /v1/contracts`, `GET /v1/contracts/{id}`, `PATCH /v1/contracts/{id}`, `POST /v1/contracts/{id}/submissions`, `POST /v1/contracts/{id}/approvals`, `POST /v1/contracts/{id}/plans`, `GET /v1/plans/{id}`, `POST /v1/plans/{id}/proposals`, `GET /v1/proposals/{id}`, `POST /v1/proposals/{id}/acceptance`, and `GET /v1/tasks/{id}`; there is no full RepoBinding CRUD endpoint, `GET /v1/vcs-connections`, `GET /v1/plans`, `GET /v1/proposals`, or `GET /v1/tasks` list endpoint, and the previous public `/v1/goals/{id}/contract-seeds`, `/v1/contract-seeds/{id}/contract-drafts`, `/v1/contract-drafts/{id}`, and direct `POST /v1/contracts/{id}/tasks` lifecycle/planning routes are no longer registered
...
- no production organization/user/VCS connection/repository catalog implementation beyond the dev-seeded Installation / Organization / Project / RepoBinding Postgres foundation, metadata-only RepoBinding init, and credentialless non-live `pending_setup` VcsConnection setup-intent skeleton yet
...
- no active/live `VcsConnection` implementation yet; the backend has only
  credentialless `pending_setup` setup-intent schema/API state, not OAuth,
  provider credentials, provider-client behavior, metadata listing, checkout,
...
- next sales-pack and runner-boundary slices remain explicit and bounded; the
  provider-neutral VCS setup-intent skeleton is credentialless/non-live while
  GitLab/provider OAuth, token storage, metadata listing, checkout, runner,
  gate, and proof implementation remain unimplemented
