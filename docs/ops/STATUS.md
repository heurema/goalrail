...
- D-0046 documents WorkItem planning as a non-executable boundary; the server
  ...
  claiming, execution, runner checkout, receipt, gate, or proof; this direct
  endpoint is simple v0 behavior only and not the final repo-aware planning
  architecture
...
- server endpoints include `GET /livez`, `GET /readyz`, `GET /version`, `POST /v1/intakes`, `GET /v1/intakes/{id}`, `POST /v1/intakes/{id}/goals`, `POST /v1/goals/{id}/readiness`, `POST /v1/goals/{id}/clarifications`, `POST /v1/clarifications/{id}/answers`, `POST /v1/answers/{id}/applications`, `POST /v1/contracts`, `GET /v1/contracts/{id}`, `PATCH /v1/contracts/{id}`, `POST /v1/contracts/{id}/submissions`, `POST /v1/contracts/{id}/approvals`, and `POST /v1/contracts/{id}/tasks`; the previous public `/v1/goals/{id}/contract-seeds`, `/v1/contract-seeds/{id}/contract-drafts`, and `/v1/contract-drafts/{id}` lifecycle routes are no longer registered
...
- WorkItem planning creates exactly one in-memory `WorkItem(planned)` per `ApprovedContract(approved)` in v0, requires approved contract scope / acceptance criteria / proof expectations, guards repeated planning with `409 already_planned`, appends `work_item.created`, and does not assign, claim, create `Run`, start execution, checkout a repository, submit a receipt, write `GateDecision`, or create `Proof`; ADR-0019 now treats this direct endpoint as simple v0 behavior only and directs richer repo-aware planning through future worker/controller/runner planning request, proposal, and acceptance boundaries
...
- no server-owned canonical domain implementation beyond the persisted `IntakeRecord` / `Goal` / public Contract lifecycle façade / internal `ContractSeed` / `ContractDraft creation/update/ready_for_approval` / `ApprovedContract` slice and in-memory `ClarificationRequest` / `ClarificationAnswer` / `WorkItem planning` prototypes yet
