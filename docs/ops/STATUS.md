...
Status: planning / product canon and pilot frame active; first local Go CLI and Go server intent-plane / ContractSeed / ContractDraft / ApprovedContract persistence plus WorkItem planning prototype exists; pilot-intake-ru approved as candidate public RU landing demo per D-0048 with deployment-prep complete (READY WITH WARNINGS) per `docs/ops/PILOT_INTAKE_RU_DEPLOYMENT_PREP.md`, active target domain is `pilot.goalrail.ru` per D-0053 (which supersedes D-0049 for target domain and canonical public URL; public path `/` and public status `candidate-public` are preserved; the `.dev` domain is reserved for a later global-market rollout), pre-publish canonical metadata in `apps/web/pilot-intake-ru/index.html` matches D-0053 (`https://pilot.goalrail.ru/`), the static hosting path is operator-managed SSH static server per D-0051 (D-0050 Cloudflare Pages Direct Upload superseded for hosting provider and deployment mode), and SSH static deployment wiring is `READY FOR SSH DEPLOY — RUNTIME VALUES REQUIRED` per `docs/ops/PILOT_INTAKE_RU_DEPLOYMENT_WIRING.md` (Phase 8H ran local preflight, boundary audit, and local preview smoke — all PASS; Phase 8I re-attempted the slice and re-ran the same local half end-to-end with identical PASS results — but in both phases no remote SSH/rsync/scp was performed because the operator did not provide the required runtime env vars `GR_PILOT_REMOTE_DEPLOY=yes`, `GR_PILOT_SSH_TARGET`, `GR_PILOT_RELEASE_ROOT`, `GR_PILOT_CURRENT_LINK`, `GR_PILOT_DOMAIN`; no server identifiers, hostnames, IPs, ports, usernames, keys, tokens, or credentials are committed to the repo); not deployed; D-0047 local-only boundary intact
Owner: Vitaly
...
- eighteen kernel/CLI/server/domain boundary ADRs
- ops rails
...
- a complete local RU pilot-first interactive landing demo under `apps/web/pilot-intake-ru` covering the full 5-step Goal Intake → Clarification → Contract Draft → Review → Honest Outcome walkthrough on deterministic local helpers (`detectScenario`, `buildContractDraft`, `buildReviewReport`, `deriveOutcomeTone`, `buildOutcomeReport`), with no backend, no persistence, no LLM/API calls, no repo connection, and no execution; Phase 6A/6B/6C polish covers responsive demo rail (horizontal scroll-snap with right-edge mask, very-narrow <420px breakpoint), outcome CTA email-input focus + temporary local highlight, skip-link with focus handler targeting `<main id="main-content" tabIndex={-1} aria-label="Демо GoalRail">`, textarea `aria-describedby` safety microcopy, `prefers-reduced-motion`-safe step transitions, and back-navigation animation suppression via `data-skip-animation`; documented and governed in `docs/product/GOALRAIL_LANDING_COPY_PILOT_FIRST.md` and `docs/ops/DECISIONS.md` D-0047; internal review evidence is captured in `docs/ops/PILOT_INTAKE_RU_INTERNAL_REVIEW_NOTES.md`, and the surface is approved as the candidate public RU landing demo per `docs/ops/DECISIONS.md` D-0048 pending a separate deployment-prep slice (D-0047 still applies; deployment, backend, analytics, and email submission remain disallowed without their own explicit decisions; one non-blocking warning — narrow hero-title tightness around 380px — remains)
- an open-source community baseline (`LICENSE`, `NOTICE`, contributor docs, issue forms, `CODEOWNERS`)
- a Go server bootstrap under `apps/server` with Postgres-backed source-neutral intake, Goal promotion, Goal readiness state, ContractSeed creation, ContractDraft creation/update/ready_for_approval, ApprovedContract approval, EventLog persistence, and transactional canonical write + event append hardening when DB is configured, plus in-memory ClarificationRequest, ClarificationAnswer recording, answer application, explicit re-check-after-applied-answers, and WorkItem planning prototypes
...
- D-0046 documents WorkItem planning as a non-executable boundary; the server
  now implements a one-WorkItem in-memory planning prototype without assignment,
  claiming, execution, runner checkout, receipt, gate, or proof
...
- kernel schema note and eighteen boundary ADRs exist
...
- the init migration creates `users`, `organizations`, `organization_memberships`, `projects`, `repo_bindings`, `intake_records`, `goals`, `contract_seeds`, `contract_drafts`, and `events` with UUID persisted ID columns for canonical entities
...
- ContractSeed creation stores a `ContractSeed(created)` snapshot in Postgres when DB is configured and uses the in-memory store otherwise, guards repeated creation with `409 already_seeded`, appends `contract_seed.created`, and does not mutate Goal state
...
- ApprovedContract approval creates an immutable `ApprovedContract(approved)` snapshot from `ContractDraft(ready_for_approval)` in Postgres when DB is configured and uses the in-memory store otherwise, requires `approved_by` as approval actor, guards repeated approval with `409 already_approved`, appends `contract.approved`, does not mutate `ContractDraft`, and does not start execution, write `GateDecision`, or create `Proof`
- WorkItem planning creates exactly one in-memory `WorkItem(planned)` per `ApprovedContract(approved)` in v0, requires approved contract scope / acceptance criteria / proof expectations, guards repeated planning with `409 already_planned`, appends `work_item.created`, and does not assign, claim, create `Run`, start execution, checkout a repository, submit a receipt, write `GateDecision`, or create `Proof`
...
- the WorkItem planning boundary is documented in ADR-0018 and implemented as an in-memory `ApprovedContract(approved) -> WorkItem(planned)` prototype; WorkItem planning is not assignment, claiming, execution, `Run`, runner checkout, receipt, `GateDecision`, or `Proof`
...
- `apps/server` provides a verified Go server bootstrap plus Postgres-backed source-neutral intake with Project / RepoBinding context validation, Goal promotion, deterministic Goal readiness state, ContractSeed creation, ContractDraft creation/update/ready_for_approval, ApprovedContract approval, EventLog persistence, transactional canonical write + event append hardening, explicit re-check-after-applied-answers, and in-memory clarification request/answer and WorkItem planning prototypes when DB is configured; it creates `IntakeRecord`, non-executable `Goal`, open in-memory `ClarificationRequest`, recorded in-memory `ClarificationAnswer`, `ContractSeed(created)`, `ContractDraft(draft/ready_for_approval)`, `ApprovedContract(approved)`, and `WorkItem(planned)` only, updates Goal readiness state, request answered state, Goal intent-plane hints, ContractDraft proposed fields, and ContractDraft readiness state only, and does not claim durable clarification storage, automatic readiness re-check, WorkItem assignment/claiming, execution, `Run`, receipt, gate, proof, repo readiness, auth, workers, or repository checkout
- `apps/web/pilot-intake-ru` provides a verified local RU pilot-first interactive landing demo with the full 5-step deterministic walkthrough (intake → clarification → contract draft → review → honest outcome); it does not call backends, LLMs, or repo providers, does not persist user input, does not execute code, and does not send analytics; the final CTA only focuses the existing `mailto:` email input; canonical copy and governance live in `docs/product/GOALRAIL_LANDING_COPY_PILOT_FIRST.md`, the local-only boundary is recorded in `docs/ops/DECISIONS.md` D-0047, the surface is the approved candidate public RU landing demo per `docs/ops/DECISIONS.md` D-0048, deployment-prep readiness is captured in `docs/ops/PILOT_INTAKE_RU_DEPLOYMENT_PREP.md` (recommendation: READY WITH WARNINGS — build/test/typecheck/boundary/copy-parity checks all pass; the single very-narrow hero-title warning was resolved by a CSS-only `<420px` polish), the active target domain is `pilot.goalrail.ru` per `docs/ops/DECISIONS.md` D-0053 (which supersedes D-0049 for target domain and canonical public URL; D-0049's `pilot.goalrail.dev` body is preserved as historical record and the `.dev` domain is reserved for a later global-market rollout) with public path `/` and public status `candidate-public`, the canonical-link metadata mismatch identified in earlier wiring was first resolved in Phase 8E (against D-0049's `.dev` value) and was realigned to D-0053's `https://pilot.goalrail.ru/` in Phase 8K (also reflected in built `dist/index.html`), the static hosting path has been changed from Cloudflare Pages Direct Upload (`docs/ops/DECISIONS.md` D-0050, now `superseded by D-0051 for hosting provider and deployment mode`; Cloudflare Pages, Workers, Functions, KV / R2 / D1 / Durable Objects / Queues, proxy / CDN, and Web Analytics remain disallowed for this surface) to an operator-managed SSH static server per `docs/ops/DECISIONS.md` D-0051 (manual `rsync` / `scp` upload of `apps/web/pilot-intake-ru/dist/` to a timestamped release directory with atomic `current` symlink, no automatic redeploys, no CI deploy workflow, no SSH credentials/hostnames/IPs/keys/tokens committed to the repository, server-managed HTTPS via existing reverse proxy or Let's Encrypt, DNS handled externally by the operator with the record kept DNS-only / non-proxied if the zone is in Cloudflare), and SSH static deployment wiring is captured in `docs/ops/PILOT_INTAKE_RU_DEPLOYMENT_WIRING.md` with status `READY FOR SSH DEPLOY — RUNTIME VALUES REQUIRED` (Phase 8H ran local preflight — typecheck 0 errors, tests 67/67, build OK, source boundary scan PASS — and a local production-build preview smoke through a full RG walkthrough — `data-outcome-tone="ready"`, primary outcome CTA focused the email input, canonical link (then) `https://pilot.goalrail.dev/`, 0 console errors, 0 non-static network requests; Phase 8I re-attempted the same slice and re-ran the local half end-to-end with identical PASS outcomes — typecheck 0 errors, tests 67/67 (~18.91s), build ~198ms, dist canonical line PASS, source boundary scan PASS, local preview smoke PASS — but in both phases no remote SSH connection was opened because the operator did not provide `GR_PILOT_REMOTE_DEPLOY=yes`, `GR_PILOT_SSH_TARGET`, `GR_PILOT_RELEASE_ROOT`, `GR_PILOT_CURRENT_LINK`, or `GR_PILOT_DOMAIN`; Phase 8J was stopped at the env gate immediately on the same condition; Phase 8K then realigned target-domain metadata to D-0053 (`pilot.goalrail.ru`) ahead of any remote attempt — the future SSH deployment runs will use `GR_PILOT_DOMAIN=pilot.goalrail.ru`, DNS / TLS / public smoke targets `https://pilot.goalrail.ru/`); the surface is not deployed, no backend exists, and email capture remains `mailto:` / focus-only / manual handoff
