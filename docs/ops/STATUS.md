...
- H2.1 implements the preparation-only `ExecutionJob(queued)` bridge from
  ...
  accepts one metadata-only `ExecutionReceipt` for a started Run with lease
  proof. It does not execute commands, decide gates, or create proof.
