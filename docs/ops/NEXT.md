...
- **5-7 day stabilization freeze:** first target is the artifact-led Proof Gap
  ...
  expansion.
- Proof Gap Report schema is now the artifact spec for manual examples and
  future renderer/parser work. Future implementation still requires a bounded
  implementation slice and must not be introduced in this docs task.
- Guided Demo North Star is documented as the future user-facing CLI/demo
  experience; implementation remains deferred and requires a bounded brief.
