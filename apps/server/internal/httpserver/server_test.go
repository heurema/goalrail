...
func TestPublicV1RouteInventoryUsesResourcePaths(t *testing.T) {
	router := httpserver.NewRouter(httpserver.RouteHandlers{
		...
		PlanStatus:                    probeRoute("plan_status"),
		...
		CheckoutJobGet:                probeRoute("checkout_job_get"),
	...
	tests := []struct {
		...
		{name: "qualification_feed", method: http.MethodGet, path: "/v1/qualification-feed", wantRoute: "qualification_feed"},
		...
		{name: "contract_list", method: http.MethodGet, path: "/v1/contracts", wantRoute: "contract_list"},
		{name: "contract_get", method: http.MethodGet, path: "/v1/contracts/contract-1", wantRoute: "contract_get"},
		{name: "contract_current_draft", method: http.MethodGet, path: "/v1/contracts/contract-1/current-draft", wantRoute: "contract_current_draft"},
		...
		{name: "plan_status", method: http.MethodPost, path: "/v1/plans/plan-1/status", wantRoute: "plan_status"},
		...
		{name: "checkout_job_get", method: http.MethodGet, path: "/v1/checkout-jobs/job-1", wantRoute: "checkout_job_get"},
...
func TestPublicV1OldVerbStyleRoutesAreNotRegistered(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "public_contract_draft_get", method: http.MethodGet, path: "/v1/contract-drafts/draft-1"},
...
func newRouter(
	...
	return httpserver.NewRouter(httpserver.RouteHandlers{
		...
		ContractList:                  http.HandlerFunc(contractHandler.List),
		ContractGet:                   http.HandlerFunc(contractHandler.Get),
		ContractCurrentDraft:          http.HandlerFunc(contractHandler.CurrentDraft),
		...
		PlanStatus:                    http.HandlerFunc(workItemPlanHandler.GetPlanStatus),
		...
		CheckoutJobGet:                http.HandlerFunc(checkoutHandler.InspectJob),
