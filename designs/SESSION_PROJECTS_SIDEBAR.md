...
User-facing term: **`project`**. Internal reserved label key: **`goalrail.project`**.

The label key is namespaced (`goalrail.*`) to keep the internal storage key distinct from
...
> Migration note from #869: rename the reserved key `"collection"` → `"goalrail.project"`,
...
| key             | `"goalrail.project"` (reserved)    |
| value           | the project name               |
| updated_at      | last write (epoch seconds)     |

- A session is **in a project** iff it has a `(key="goalrail.project")` row; the project
  name is that row's `value`.
- A session is **unfiled** iff it has no `goalrail.project` row.
- "Removing from a project" = deleting the row (not upserting an empty string).
- Implicit lifecycle falls out for free: distinct `value`s (where `key="goalrail.project"`)
  = the set of projects;
  when the last member is moved/deleted, no rows remain and the project vanishes.

### Label invisibility
`goalrail.project` is a reserved key and must be excluded from any surface that renders
generic session labels (the `labels` dict flows into `SessionListItem` and is used for
guardrail/sensitivity display). Audit and filter `goalrail.project` out of those surfaces so
...
- `list_projects(accessible_by) -> list[str]` — distinct `value` where
  `key="goalrail.project"`, ordered alphabetically, ACL-scoped to sessions the user has a
...
- `delete_label(conversation_id, key)` — no-op if absent; used for "remove from
  project" (`key="goalrail.project"`).
- `list_conversations(..., project: str | None)`:
  - `None` → filter disabled.
  - `""` → only sessions with **no** `goalrail.project` label (unfiled).
  - non-empty → only sessions whose `goalrail.project` label equals it.
...
- `PATCH /v1/sessions/{id}` with `{labels:{goalrail.project:"X"}}` to set;
  `{labels:{goalrail.project:""}}` is special-cased to `delete_label(id, "goalrail.project")`
...
`{goalrail.project: "X"}`) so the start-time picker sets membership atomically at creation
...
- `useMoveToProject()` → `PATCH /v1/sessions/{id}` with `{labels:{goalrail.project}}`; on
...
- Collapsible, persisted in the existing `goalrail:collapsed-sidebar-sections`
