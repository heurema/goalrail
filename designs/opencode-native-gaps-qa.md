...
with the running Goalrail web UI.

## 0. Setup (once)

1. `goalrail setup` → OpenCode section: add a provider, pick a default model
   (confirm the model actually used matches the selection, not `big-pickle`).
2. Have a workspace with the Goalrail web UI reachable.
3. Keep two terminals handy: the Goalrail server logs and (optionally) an
...
## 2a. MCP — Goalrail builtin relay  [needs web: model must call a sys_* tool]

This is the real "connects to Goalrail MCP" — opencode's model calling Goalrail
...
- Steps: in an opencode-native session, ask the model to do something that needs
  ...
  X skill" (`load_skill`).
- Expected:
  - opencode's `opencode.json` `mcp` block has an `goalrail` `{type:"local"}`
    entry whose command is `… -m goalrail.claude_native_bridge serve-mcp
  ...
  - The model can call the builtin and gets a real result (proxied through the
    Goalrail server, so policy applies — a builtin call shows up at the TOOL_CALL
...
- Expected:
  - opencode's per-session `opencode.json` contains the agent servers in the
    ...
    the `goalrail` relay entry, **and** `permission:{"*":"ask"}`.
  - The MCP tools are visible/callable by the model.
  - Because `permission:ask` is set, the tool call routes through the Goalrail
...
- Expected:
  - The Goalrail transcript is rehydrated as a **`noReply` context message**
...
**Goalrail → opencode (model switch):**
- Steps: change the model from the Goalrail web UI (model pill) on an
...
**Goalrail → opencode (clear):**
- Steps: trigger `/clear` from Goalrail on an opencode-native session.
...
The `goalrail-policy.js` plugin (loaded via `opencode.json` `plugin:[…]`) bridges
...
- **TOOL_RESULT phase** (`tool.execute.after` → `PHASE_TOOL_RESULT`):
  ...
  - Expected: the model receives `[Goalrail policy: tool result withheld]`
    instead of the real output (the tool already ran; its result is withheld).
- Fail-open: with the Goalrail server unreachable, prompts/tools still flow
