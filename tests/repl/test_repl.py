...
def test_startup_header_shows_server_version_on_url_line() -> None:
    ...
    remote = "https://goalrail.example.com"
    plain = re.sub(
        r"\x1b\[[0-9;]*m",
        "",
        _render_startup_banner_ansi(
...
def _fake_version_client(by_path: dict[str, dict]) -> tuple[object, list[str]]:
    """Build a fake ``GoalrailClient`` whose ``_http.get`` serves per-path JSON.
    ...
    class _FakeClient:
        _base_url = "https://goalrail.example.com"
        _http = _FakeHttp()
...
def test_fetch_server_version_parses_info(payload, expected) -> None:
    ...
    assert targets[0] == "https://goalrail.example.com/v1/info"
...
def test_fetch_server_version_falls_back_to_api_version() -> None:
    ...
    assert targets == [
        "https://goalrail.example.com/v1/info",
        "https://goalrail.example.com/api/version",
    ]
...
def test_build_startup_header_creds_line_hints_first_available(tmp_path, monkeypatch) -> None:
    ...
    The vendor-key GPT-head scenario: a multi-family agent (anthropic +
    openai) where the ``openai`` surface has NO default, but an API-key
    provider that serves openai is configured. The creds line must not read a
    bare "not configured" — the head WILL launch through that key (the
    ...
    for var in ("ANTHROPIC_API_KEY", "OPENAI_API_KEY", "OPENROUTER_API_KEY"):
        monkeypatch.delenv(var, raising=False)
    # A real Ollama server on the test host would otherwise ambient-default
    # the openai family before the no-default hint is ever computed.
    ...
    # first-available credential the launch falls back to (the vendor key).
    assert "Claude → Subscription" in header.creds_line
    assert "Codex → no default → will use 🔑 OpenAI API Key" in header.creds_line
