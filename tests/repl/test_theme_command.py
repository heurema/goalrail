...
from goalrail.repl._repl import COMMANDS, _load_startup_theme, handle_slash_command
...
def test_startup_theme_falls_back_to_light_for_corrupt_config(
    ...
    config_path = tmp_path / ".goalrail" / "config.yaml"
...
def test_startup_theme_shows_picker_on_first_launch(
    ...
    monkeypatch.setattr(
        "goalrail.repl._theme_picker.startup_theme_picker",
...
async def test_theme_light_updates_host_and_formatter(
    ...
    assert (tmp_path / ".goalrail" / "config.yaml").read_text(encoding="utf-8") == (
