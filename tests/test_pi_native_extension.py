...
fs.writeFileSync(
  configPath,
  JSON.stringify({ serverUrl: "http://goalrail.test", sessionId: "session-1" }),
);
process.env.GOALRAIL_PI_NATIVE_CONFIG = configPath;
...
def test_registers_goalrail_tools_and_execute_round_trips(tmp_path: Path) -> None:
    ...
    tool's ``execute`` POSTs a JSON-RPC ``tools/call`` to the Goalrail server's
...
global.fetch = async (url, request) => {
  ...
  // Mimic the Goalrail /mcp proxy success envelope.
  return {
    ok: true,
    async json() {
      return {
...
(async () => {
  ...
  assert.equal(call.url, "http://goalrail.test/v1/sessions/conv_abc/mcp");
...
def test_bridged_tool_call_skips_hook_policy_eval(tmp_path: Path) -> None:
    """The tool_call hook must NOT re-evaluate policy for bridged Goalrail tools.
...
def test_mcp_unreachable_fails_closed_without_throwing(tmp_path: Path) -> None:
    """An unreachable Goalrail MCP server resolves to an error, never a throw.
...
fs.writeFileSync(
  configPath,
  JSON.stringify({ serverUrl: "http://goalrail.test", sessionId: "session-1", inboxDir }),
);
...
(async () => {
  ...
  assert.match(evalBodies[0]._goalrail_elicitation_id, /^elicit_evaluate_[0-9a-f]{32}$/);
...
(async () => {
  ...
  assert.equal(
    evalBodies[0]._goalrail_elicitation_id,
    evalBodies[1]._goalrail_elicitation_id,
  );
})().catch((error) => {
  console.error(error && error.stack ? error.stack : error);
  process.exit(1);
...
def test_policy_transport_error_fails_closed(tmp_path: Path) -> None:
    ...
    null, matching goalrail.policies.types.FAIL_CLOSED_PHASES and the Python
...
(async () => {
  ...
  const ids = new Set(evalBodies.map((b) => b._goalrail_elicitation_id));
