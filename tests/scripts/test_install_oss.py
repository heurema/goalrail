...
def test_install_codebase_memory_failure_warns_without_failing(lib: Path) -> None:
    ...
    r = run(
        lib,
        (
            "run_with_spinner() { return 42; }\n"
            "install_codebase_memory /tmp/does-not-matter\n"
            'echo "continued"\n'
        ),
    )
    assert r.returncode == 0, r.stderr
...
def test_main_respects_skip_codebase_memory(lib: Path) -> None:
    """The skip flag keeps main from invoking the companion install."""
    r = run(
        lib,
        "\n".join(
            [
                ...
                "print_next_steps() { :; }",
                "main --skip-codebase-memory",
            ]
        ),
    )
    assert r.returncode == 0, r.stderr
    assert "called" not in r.stdout
