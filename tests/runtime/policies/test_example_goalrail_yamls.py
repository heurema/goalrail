...
goalrail-format example YAMLs under ``examples/*.yaml``.

Complements :mod:`test_enforcement_integration`, which loads
pre-translated goalrail-native fixtures. The fixtures there
are hand-maintained ports; any bug in the goalrail → goalrail
...
:func:`goalrail.spec.load` — the same path ``goalrail run``
...
from goalrail.spec.types import Phase, PolicyAction
...
def _load_engine_from_yaml(
    ...
    """
    Parse an goalrail-format example YAML and build a real
    :class:`PolicyEngine` bound to a fresh conversation.

    Goes through :func:`goalrail.spec.load`, so the
    ``_goalrail_compat`` adapter runs on every call. A bug
...
async def test_agent_with_policies_allows_short_sleep(
    ...
    goalrail YAML ends at ALLOW for an in-bounds duration.
    """
...
async def test_agent_with_policies_denies_long_sleep(
    ...
    Goalrail' engine: it DOES. The engine calls 2-arg
    ...
    Claim: the goalrail YAML + its legacy-style example
    callable actually works through Goalrail' engine. If
...
# composition is already covered at the goalrail-native
...
async def test_secure_research_clean_shell_allows(
    ...
    goalrail ``labels:`` block and the enforcement chain
...
# which collides with an goalrail reserved builtin name. The
