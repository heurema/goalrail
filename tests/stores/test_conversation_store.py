...
def test_get_conversations_bulk(
    ...
    conversation_store.set_labels(a.id, {"goalrail.ui": "terminal"})
    ...
    assert result[a.id].labels == {"goalrail.ui": "terminal"}
...
def test_unique_position_constraint(
    ...
    from goalrail.db.utils import generate_item_id
...
def test_list_items_type_filter_returns_only_matching_type(
    ...
    """
    from goalrail.entities import CompactionData
...
def test_append_bumps_updated_at(
    ...
    to the current time.
    """
    import goalrail.stores.conversation_store.sqlalchemy_store as store_mod
...
def test_create_duplicate_title_under_same_parent_raises(
    ...
    from goalrail.stores.conversation_store import NameAlreadyExistsError
...
def test_list_conversations_filtered_by_agent_id_returns_matching_only(
    ...
    """
    Powers Goalrail mode ``--continue`` (resume the most-recent
...
def test_set_host_id_missing_conversation_raises(
    ...
    """
    from goalrail.stores.conversation_store import ConversationNotFoundError
...
def test_create_session_with_agent_top_level_unchanged(
    ...
    contract (CLI ``goalrail run`` sessions): kind stays "default",
...
def test_fork_conversation_drops_instance_scoped_labels(
    ...
    conversation_store.set_labels(
        source.id,
        {
            "goalrail.claude_native.bridge_id": source.id,
            "goalrail.codex_native.bridge_id": source.id,
            "goalrail.last_context_tokens": "39903",
            "goalrail.last_context_window": "1000000",
            # The dangerous bypass opt-in must NOT ride into the fork.
            "goalrail.codex_native.bypass_sandbox": "1",
    ...
    # usage.
    assert fork.labels == {"goalrail.wrapper": "claude-code-native-ui"}, (
...
def test_fork_conversation_stamps_source_external_session_id(
    ...
    ``goalrail.fork.source_external_session_id`` to resume + branch the
    ...
    assert fork.labels.get("goalrail.fork.source_external_session_id") == "claude-uuid-abc", (
    ...
    # yet. Copying external_session_id would make two Goalrail sessions claim
...
def test_fork_conversation_no_external_session_id_no_directive(
    ...
    assert "goalrail.fork.source_external_session_id" not in fork.labels, (
...
def test_fork_conversation_truncated_drops_external_session_directive(
    ...
    If ``goalrail.fork.source_external_session_id`` were stamped, the
...
def test_fork_conversation_cross_family_drops_external_session_directive(
    ...
    must not stamp ``goalrail.fork.source_external_session_id``: the
    ...
    the copied Goalrail items) instead.
    """
    from goalrail.stores.conversation_store import (
        FORK_CARRY_HISTORY_LABEL_KEY,
        FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY,
...
def test_fork_conversation_up_to_last_response_keeps_external_directive(
    ...
    """
    from goalrail.stores.conversation_store import FORK_SOURCE_EXTERNAL_SESSION_LABEL_KEY
...
def test_instance_scoped_label_keys_match_harness_constants() -> None:
    ...
    from goalrail.codex_native_bridge import CODEX_NATIVE_BRIDGE_ID_LABEL_KEY
    from goalrail.stores.conversation_store import _INSTANCE_SCOPED_LABEL_KEYS
...
def test_fork_conversation_carry_history_into_native_stamps_label(
    ...
    The runner reads ``goalrail.fork.carry_history`` to decide whether a
...
def test_switch_conversation_agent_cross_family_resets_and_relabels(
    ...
    # goalrail.stopped marker was retired upstream.
    instance_label = "goalrail.last_context_tokens"
    ...
    assert "goalrail.codex_native.bypass_sandbox" not in updated.labels, (
...
def test_get_session_connectivity_batches_runner_and_host(
    ...
    fresh" — the retired ``goalrail.stopped`` marker is no longer a
...
def test_get_session_connectivity_reports_needs_workspace_for_fork(
    ...
    ``goalrail.fork.source_id`` label (set by ``fork_conversation``).
    ...
    conversation_store.set_labels(fork.id, {"goalrail.fork.source_id": "conv_src"})
...
def test_get_session_owner_excludes_public_sentinel(
    ...
    from goalrail.server.auth import RESERVED_USER_PUBLIC
    from goalrail.stores.permission_store.sqlalchemy_store import (
        SqlAlchemyPermissionStore,
    )
...
def _stored_next_position(
    ...
    from goalrail.db.db_models import SqlConversation
...
# ── Projects (conversation_labels key="goalrail.project") ───────
...
def test_list_projects_returns_distinct_names_sorted(
    ...
    conversation_store.set_labels(a1.id, {"goalrail.project": "Sprint 42"})
    conversation_store.set_labels(a2.id, {"goalrail.project": "Sprint 42"})
    conversation_store.set_labels(b1.id, {"goalrail.project": "Customer X"})
...
def test_list_projects_excludes_all_archived_projects(
    ...
    conversation_store.set_labels(solo.id, {"goalrail.project": "Gone"})
    conversation_store.set_labels(mix_archived.id, {"goalrail.project": "Mixed"})
    conversation_store.set_labels(mix_active.id, {"goalrail.project": "Mixed"})
...
def test_list_projects_scoped_by_accessible_by(
    ...
    conversation_store.set_labels(mine.id, {"goalrail.project": "Mine"})
    conversation_store.set_labels(theirs.id, {"goalrail.project": "Theirs"})
...
def test_delete_label_removes_only_target_key(
    ...
    conv = conversation_store.create_conversation()
    conversation_store.set_labels(conv.id, {"goalrail.project": "X", "integrity": "1"})

    conversation_store.delete_label(conv.id, "goalrail.project")
...
def test_delete_label_is_noop_when_absent(
    ...
    conv = conversation_store.create_conversation()
    conversation_store.delete_label(conv.id, "goalrail.project")  # must not raise
    got = conversation_store.get_conversation(conv.id)
    assert got is not None
    assert got.labels == {}
...
def test_list_conversations_filters_by_project(
    ...
    conversation_store.set_labels(filed.id, {"goalrail.project": "X"})
    conversation_store.set_labels(other.id, {"goalrail.project": "Y"})
