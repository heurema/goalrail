"""
Unit tests for the Goalrail YAML spec adapter.
...
- Dispatch detection in :func:`goalrail.spec.load`:
  goalrail YAMLs route to the adapter; goalrail YAMLs
  (identified by ``spec_version``) use the existing parser.

These are the phase 2 translation unit tests + fail-loud tests
called out in ``designs/GOALRAIL_INTEGRATION.md`` under the
phase 2 test scope.

Round-trip tests live in ``test_goalrail_roundtrip.py`` since
...
from goalrail.spec.goalrail import (
    GOALRAIL_EXECUTOR_TYPE,
    GOALRAIL_TOOL_LANGUAGE,
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal goalrail YAML: ``name`` + ``prompt`` only.
...
def executor_block_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML with an ``executor:`` block declaring
...
def function_tools_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML with one function-type tool whose
...
def policies_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML declaring a ``policies:`` block. The adapter
    lifts this into ``AgentSpec.guardrails.policies`` so the
    goalrail workflow enforces it at the configured phases.
...
def os_env_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML declaring a top-level ``os_env:`` block. The
...
def mcp_tool_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML with a stdio MCP-type tool.
    ...
    :class:`~goalrail.tools.mcp.McpServerConnection`.

    Includes ``executor.harness`` so the synthesized AgentSpec
    passes :mod:`goalrail.spec.validator` — the adapter runs
...
def mcp_http_tool_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML with an HTTP MCP-type tool (``url`` + headers).
...
def cancellable_tool_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML declaring a legacy ``cancellable_function``
...
@pytest.fixture()
def goalrail_spec_dir(tmp_path: Path) -> Path:
    """
    An goalrail spec directory (``spec_version: 1`` in
    ``config.yaml``). Routes through the existing parser, not
    the goalrail adapter. Used by the detection tests.
    """
    config = {
        "spec_version": 1,
        "name": "ap-agent",
...
def test_agent_def_to_agent_spec_hello_world(
    ...
    ``goalrail chat hello_world.yaml`` can't produce a valid spec
    ...
    # spec_version is synthesized to the current goalrail
    # schema version (no spec_version in goalrail YAMLs).
    assert spec.spec_version == 1
    # executor.type drives the runtime to pick GoalrailExecutor.
...
def test_agent_def_to_agent_spec_accepts_claude_harness_alias(tmp_path: Path) -> None:
    """Goalrail YAML may use ``harness: claude`` as a spec-level alias."""
...
def test_agent_def_to_agent_spec_executor_block(
    ...
    What breaks if this fails: the GoalrailExecutor cannot
    pick a harness or workspace profile at instantiation time,
    so every non-trivial goalrail YAML routes to the wrong
    harness (or fails).
    """
    from goalrail.inner.loader import load_agent_def
...
def test_agent_def_to_agent_spec_function_tool(
    ...
    What breaks if this fails: GoalrailExecutor cannot resolve
    ...
    # from an goalrail YAML and must be re-resolved via
...
def test_function_tool_parameters_derived_from_callable_signature(
    ...
    the Goalrail adapter introspects the resolved Python callable's
    signature and exposes that as the LLM-facing JSON-Schema
    ``parameters`` block. Without this fallback, goalrail
...
def test_load_goalrail_yaml_missing_package_raises_with_install_hint(
    hello_world_yaml: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    When the ``goalrail`` package is not importable (e.g. agent-
    plane pip-installed standalone without the sibling goalrail
    source on PYTHONPATH), loading an goalrail YAML must surface
    a friendly :class:`GoalrailError` with an install hint —
    not a bare ``ModuleNotFoundError`` from deep in the import
    machinery.

    What breaks if this fails: a user running ``goalrail chat foo.yaml``
    from an env that only has goalrail gets a cryptic
    ``ModuleNotFoundError: No module named 'goalrail'`` from
    ``_goalrail_compat.py``'s import line, with no clue what
    to install. The rewritten error says what's missing and how
    to fix it.
    """
    # Simulate ``goalrail`` not being importable by wiping it
    ...
    for mod_name in list(sys.modules):
        if mod_name == "goalrail" or mod_name.startswith("goalrail."):
    ...
    class _GoalrailBlocker:
        """Meta-path finder that refuses to resolve goalrail."""
        ...
        def find_spec(
            ...
            del path, target
            if fullname == "goalrail" or fullname.startswith("goalrail."):
...
def test_load_policies_yaml_lifts_into_guardrails(policies_yaml: Path) -> None:
    """
    Goalrail YAMLs with a ``policies:`` block produce an
    AgentSpec whose ``guardrails.policies`` carries the
    translated policy, preserving ``name``, the dotted callable
    path, and the phase. The goalrail workflow then enforces
    at runtime via the standard :class:`PolicyEngine`.

    What breaks if this fails: policies in an goalrail YAML
    ...
    assert policy.function.path == "goalrail.spec._goalrail_legacy_shim.build"
...
def test_load_os_env_yaml_carries_through_top_level_field(
    ...
    """
    A top-level ``os_env:`` block on an goalrail YAML
...
def test_load_mcp_stdio_yaml_translates_to_mcp_server(mcp_tool_yaml: Path) -> None:
    """
    Goalrail YAMLs declaring a subprocess MCP tool translate to
    a native ``MCPServerConfig(transport="stdio", ...)`` entry on
    ``AgentSpec.mcp_servers``. At runtime
    :class:`~goalrail.tools.mcp.McpServerConnection` spawns the
...
def test_load_mcp_http_yaml_translates_to_mcp_server(mcp_http_tool_yaml: Path) -> None:
    """
    Goalrail YAMLs with an HTTP MCP (``url`` + headers)
    ...
    goalrail-legacy to Goalrail mode get either a translator crash
...
def test_mcp_stdio_yaml_reverse_trip_recovers_mcp_tool(mcp_tool_yaml: Path) -> None:
    ...
    path is what :meth:`GoalrailExecutor.from_spec` calls
    when wrapping an goalrail spec for an goalrail
    harness; a missing reverse translation drops every MCP
    tool from the AgentDef the harness sees.

    What breaks if this fails: a live Goalrail mode run with a
    ...
    the live E2E test under tests/e2e/goalrail/ guards
    against.
    """
    from goalrail.inner.tools import MCPTool
    from goalrail.spec.goalrail import agent_spec_to_agent_def
...
def test_load_cancellable_function_yaml_rejected_post_step_c(
    ...
    """
    Goalrail YAMLs declaring ``type: cancellable_function``
    are rejected by the Goalrail adapter with a clear migration hint.
    ...
    inner-stack YAML to Goalrail mode gets pointed at the new shape.
    ...
    ``TypeError`` instead.
    """
    with pytest.raises(GoalrailError, match="cancellable_function"):
        load(cancellable_tool_yaml)


# ── Dispatch detection in goalrail.spec.load ──────────────


def test_load_goalrail_yaml_routes_to_adapter(
    executor_block_yaml: Path,
) -> None:
    """
    A ``.yaml`` file with ``name`` + ``prompt`` and no
    ``spec_version`` routes through the goalrail adapter.
    The produced spec has ``executor.type='goalrail'``, which
    no goalrail native spec ever emits.
    ...
    itself doesn't depend on which valid goalrail YAML we feed.

    What breaks if this fails: ``goalrail chat foo.yaml`` against an
    goalrail YAML would either crash (no config.yaml in a
    file-shaped source) or silently parse as an goalrail
...
def test_load_goalrail_directory_uses_existing_parser(
    goalrail_spec_dir: Path,
) -> None:
    """
    An goalrail spec directory (``spec_version`` declared)
    routes through the existing parser unchanged. The resulting
    spec has ``executor.type='goalrail'`` (the default)
    and no goalrail extras.

    What breaks if this fails: all existing goalrail specs
    would stop loading (regression surfaces in every goalrail
    test suite, so this is a smoke check).
    """
    spec = load(goalrail_spec_dir)
...
def test_load_yaml_with_spec_version_not_routed_to_adapter(
    ...
    an goalrail spec. The detection rule's negative check on
    ``spec_version`` prevents misrouting.

    This case currently still fails (goalrail YAML specs are
    directories, not files), but the dispatch MUST pick the
    non-goalrail branch so the resulting error is an
    goalrail "dest is required" / parser error — NOT an
    goalrail adapter error. The assertion below verifies the
    error shape is NOT an goalrail-adapter error.

    What breaks if this fails: a future change that starts
    supporting goalrail YAML-file specs would silently route
    to the wrong adapter.
    """
    config = {
        "spec_version": 1,
        "name": "hybrid",
        "prompt": "Looks goalrail-y but is goalrail.",
    }
    path = tmp_path / "hybrid.yaml"
    path.write_text(yaml.dump(config))

    # ``spec_version`` marks this file as an goalrail spec, but
    # goalrail specs must live in a directory with a
    # ``config.yaml``, not a single YAML file. ``load()`` rejects with
    # an actionable diagnostic. If detection were wrong, we'd get an
    # goalrail-adapter error (e.g. "missing system-prompt key")
    # instead — the assertion below pins the goalrail shape.
    with pytest.raises(GoalrailError, match="spec_version"):
        load(path)
...
def test_cancellable_function_parameters_forward_trip_preserves_input_schema() -> None:
    ...
    **What breaks if this fails**: the adapter silently translates
    ...
    from goalrail.inner.tools import CancellableFunctionTool
...
def test_function_tool_parameters_round_trip_preserves_input_schema(
    ...
    explicit ``input_schema`` round-trips through goalrail
    translation and back without losing the schema.

    Step (c) made plain callables the only supported function-tool
    shape on the Goalrail path. Schema preservation matters because the
    ...
    from goalrail.inner.datamodel import AgentDef
    from goalrail.inner.datamodel import ExecutorSpec as GoalrailExecutorSpec
    from goalrail.inner.tools import FunctionTool
    from goalrail.spec import goalrail as spec_omni
    from goalrail.spec.goalrail import agent_def_to_agent_spec
...
def test_os_env_round_trips_through_translator() -> None:
    ...
    from goalrail.inner.datamodel import (
        ExecutorSpec as GoalrailExecutorSpec,
    )
    from goalrail.spec.goalrail import (
        agent_def_to_agent_spec,
        agent_spec_to_agent_def,
...
def test_inline_agent_tool_inherit_resolves_to_parent_os_env() -> None:
    ...
    :class:`OSEnvSpec` at translation time — goalrail spawns
    ...
    legacy spec semantics — silently breaks.
    """
    from goalrail.inner.datamodel import (
        AgentDef,
        OSEnvSpec,
    ...
    from goalrail.inner.tools import AgentTool
    from goalrail.spec.goalrail import agent_def_to_agent_spec
    ...
    parent = AgentDef(
        ...
        tools={
            "worker": AgentTool(
                name="worker",
                prompt="",
                executor=GoalrailExecutorSpec(harness="claude-sdk"),
                os_env=child_os_env,
            ),
        },
    )
...
def test_inline_agent_tool_inherit_with_no_parent_os_env_yields_none() -> None:
    ...
    ``None`` — matches legacy spec behavior when the
...
def test_instructions_field_falls_back_to_prompt_when_unset() -> None:
    ...
    goalrail YAML written before the field existed.
    """
    from goalrail.inner.datamodel import AgentDef
    from goalrail.spec.goalrail import agent_def_to_agent_spec
...
def test_instructions_yaml_loads_through_full_pipeline(tmp_path: Path) -> None:
    """
    End-to-end through ``load_goalrail_yaml`` (the integration
    path the Goalrail server hits when registering an goalrail
...
# ── Terminals threading (GOALRAIL_TERMINAL_BRIDGE §6.1) ────────────
...
def test_terminals_thread_through_translator() -> None:
    ...
    makes ``terminals:`` declarations in goalrail YAML reach the
    AP-side ``sys_terminal_*`` tools — the whole feature from
    ``designs/GOALRAIL_TERMINAL_BRIDGE.md`` collapses if this breaks.

    What breaks if this fails: goalrail YAMLs that declare
    ``terminals:`` boot under Goalrail mode with
...
def test_inline_agent_tool_inherits_parent_terminals() -> None:
    ...
    ``goalrail/tools/manager.py:426``) and the sub-agent has
    no way to launch a terminal even though the parent has one
    configured. The supervisor pattern stops working again.
    """
    from goalrail.inner.datamodel import AgentDef, TerminalEnvSpec
    from goalrail.inner.datamodel import (
        ExecutorSpec as GoalrailExecutorSpec,
    )
    from goalrail.inner.tools import AgentTool
...
def test_harness_auto_picks_from_model_prefix(
    ...
    """
    When an goalrail YAML declares a model but no harness,
    the adapter fills in the right harness by matching the
    model prefix against
    :data:`~goalrail.spec.goalrail._HARNESS_FOR_MODEL_PREFIX`.

    Mirrors the auto-pick pure goalrail' CLI does at
    ...
    blocking the entire Goalrail path.
    """
    from goalrail.inner.datamodel import AgentDef
    from goalrail.inner.datamodel import ExecutorSpec as GoalrailExecutorSpec
    from goalrail.spec.goalrail import agent_def_to_agent_spec
    ...
    agent_def = AgentDef(
        ...
        prompt="",
        tools={},
        executor=GoalrailExecutorSpec(model=model),
    )
    spec = agent_def_to_agent_spec(agent_def)
...
def test_harness_auto_pick_unknown_model_raises() -> None:
    ...
    :raises GoalrailError: With a message explaining that the
    ...
    agent_def = AgentDef(
        ...
        prompt="",
        tools={},
        executor=GoalrailExecutorSpec(model="exotic/some-new-model-v1"),
    )
    with pytest.raises(GoalrailError, match=r"[Hh]arness"):
...
# :func:`goalrail.loader.load_agent_def`) because the loader
...
def _build_agent_def_with_raw_yaml(
    ...
    goalrail loader (here stubbed with an empty ``policies``
    ...
    :param policies: Raw goalrail ``policies:`` dict.
    :param labels: Raw goalrail top-level ``labels:`` dict
        (initial values).
    :param label_schema: Raw goalrail top-level
        ``label_schema:`` dict.
    :param ask_timeout: Raw goalrail top-level
        ``ask_timeout:`` value.
...
def test_function_policy_routes_callable_through_legacy_shim() -> None:
    ...
    legacy spec ``(content, phase)`` convention keep
    working under Goalrail' ``(ctx, context)`` convention —
    see ``goalrail.spec._goalrail_legacy_shim``. The shim
    is a runtime no-op for goalrail-native callables, so
...
def test_function_policy_callable_alias_resolves_identically_to_handler() -> None:
    ...
    Old goalrail YAMLs used ``callable:`` as the key name; current
...
def test_labels_and_schema_merge_and_monotonic_maps() -> None:
    ...
    (values + monotonic) merge into Goalrail'
...
def test_ask_timeout_top_level_propagates_to_guardrails() -> None:
    """
    The goalrail top-level ``ask_timeout:`` lands on
    :attr:`GuardrailsSpec.ask_timeout`.

    What breaks if this fails: ASK-action policies park forever
    (no timeout) or fall back to the goalrail default even
...
def test_no_policies_no_guardrails_block() -> None:
    """
    An goalrail YAML without any policies/labels/ask_timeout
    produces a spec with ``guardrails=None``, matching the
    zero-policy case documented in POLICIES.md §10.

    What breaks if this fails: goalrail would instantiate a
...
def test_executor_extra_field_propagates_to_llm_config() -> None:
    """
    An goalrail YAML declaring ``executor.extra: {max_turns: 3}``
    produces an :class:`AgentSpec` whose ``llm.extra`` carries
    those kwargs byte-for-byte.

    The downstream chain (``GoalrailExecutor.run_turn`` →
    ``GoalrailExecutorConfig.extra`` → ``cfg.extra.get("max_turns")``
    in the per-harness executor) then reads these kwargs at
    runtime. This field is not part of the goalrail
    ...
    ``parallel_tool_calls``, etc.) through the Goalrail path, even
    though those knobs work fine via legacy spec. Makes
    Goalrail mode a downgrade rather than a compatible integration.
    """
    agent_def, raw_yaml = _build_agent_def_with_raw_yaml()
    raw_yaml["executor"] = {
        "model": "openai/gpt-5-mini",
...
def test_executor_extra_absent_yields_empty_llm_extra() -> None:
    """
    When the goalrail YAML omits ``executor.extra``, the
...
def test_use_responses_false_propagates_to_executor_config() -> None:
    """
    ``use_responses: false`` in an goalrail YAML executor block lands on
    ``spec.executor.config["use_responses"]`` as ``False`` after
    ``agent_def_to_agent_spec``.

    The inner ``ExecutorSpec`` dataclass (``goalrail.inner.datamodel``)
    has no ``use_responses`` field, so the goalrail YAML loader silently
...
def test_use_responses_absent_omits_key_from_executor_config() -> None:
    """
    When the goalrail YAML omits ``use_responses``, the key is absent
...
def test_unknown_policy_type_rejected_with_clear_message() -> None:
    ...
    with pytest.raises(GoalrailError, match="weird") as exc_info:
...
def test_self_clone_string_shorthand_loader_produces_selfagent_tool(
    ...
    """
    from goalrail.inner.loader import load_agent_def
    from goalrail.inner.tools import SelfAgentTool
...
def test_agent_def_to_agent_spec_self_clone_propagates_parent_config(
    ...
    """
    Translating an goalrail YAML with ``tools.subtask: self``
...
def test_compat_yaml_executor_api_key_auth_is_not_dropped(tmp_path: Path) -> None:
    """
    ``executor.auth: {type: api_key, …}`` in an goalrail-compat YAML is
