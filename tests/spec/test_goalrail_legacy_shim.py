"""
Tests for :mod:`goalrail.spec._goalrail_legacy_shim` — the
compatibility layer that lets legacy spec
``(content, phase)`` function-policy callables run under
Goalrail' ``(ctx, context)`` convention.
...
``tests/e2e/goalrail/test_run_goalrail_policy_enforcement.py``.
"""
...
from goalrail.spec._goalrail_legacy_shim import (
    _convert_args,
...
def test_legacy_content_input_passes_string_through() -> None:
    """REQUEST: goalrail puts the raw text in ``ctx.content``; legacy expects the same."""
...
def test_legacy_content_tool_call_passes_dict_through() -> None:
    """
    TOOL_CALL: goalrail already normalizes to
    ``{"tool": name, "args": parsed_args}`` at the enforcement
    site (see ``_enforce_tool_call_policy``). The legacy spec
...
def test_legacy_content_tool_result_plain_string_passes_through() -> None:
    """
    TOOL_RESULT: goalrail passes the raw tool output string
...
def test_legacy_content_tool_result_json_text_parses_to_dict() -> None:
    """
    TOOL_RESULT: when Goalrail' raw string IS a JSON-encoded
    object, the shim parses it into the corresponding Python
    dict. Mirrors goalrail-native's
...
def test_legacy_context_does_not_mutate_engine_context() -> None:
    ...
    goalrail adds ``tool_name`` on (see
...
def test_legacy_context_threads_configured_phases_when_provided() -> None:
    ...
    silently denies under Goalrail mode.
    """
    ...
    # native goalrail — see ``_legacy_context``'s docstring),
...
def test_legacy_context_omits_tool_name_on_tool_call() -> None:
    """
    Native goalrail adds ``tool_name`` to the legacy context
    ...
    identically across native and Goalrail mode.
...
def test_legacy_context_omits_tool_name_on_input_and_output() -> None:
    ...
    on REQUEST/RESPONSE in goalrail today, and any future change
...
def test_build_wraps_legacy_callable_and_converts_args(
    ...
    """
    Legacy callables get wrapped. The wrapper takes Goalrail'
    ...
    # {"decision": {"result": ...}} format so the Goalrail'
...
def test_build_returns_modern_callable_unchanged(ephemeral_module: Any) -> None:
    ...
    goalrail-native policies and lets mixed YAMLs (one
...
def test_shim_builds_usable_FunctionPolicy_through_spec_factory(
    ...
    :func:`goalrail.policies.function.resolve_function_policy`
...
def test_build_forwards_reset_turn_attribute_from_legacy_callable() -> None:
    ...
    Goalrail' :class:`FunctionPolicy.reset_turn` can find
...
def test_rate_limit_factory_reset_turn_propagates_through_shim_and_policy() -> None:
    ...
    Goalrail mode. The agent runs further than the YAML author
