"""Unit tests for ``goalrail.spec.goalrail.agent_spec_to_agent_def``.

Phase 1 ships the forward direction only. These tests hand-craft
:class:`AgentSpec` objects and assert the resulting
:class:`goalrail.datamodel.AgentDef` has the expected shape. The
...
from goalrail.inner.tools import AgentTool, FunctionTool
from goalrail.spec import (
    AgentSpec,
    ExecutorSpec,
    FunctionPolicySpec,
    FunctionRef,
...
# SandboxConfig is not re-exported from goalrail.spec's public
...
from goalrail.spec.types import SandboxConfig
...
# via ``tests.spec.test_goalrail_translator.sample_tool_callable``.
...
def basic_spec() -> AgentSpec:
    """
    Minimal ``AgentSpec`` targeting the goalrail executor.

    :returns: A spec with name, instructions, one ``llm.model``, and
        ``executor.type == "goalrail"`` carrying harness+profile.
    """
    return AgentSpec(
        spec_version=1,
        name="hello-agent",
...
def test_basic_spec_produces_agent_def_with_name_and_prompt(
    ...
    **What breaks if this fails**: the goalrail harness can
...
def test_basic_spec_maps_llm_and_executor_config(
    ...
    **What breaks if this fails**: the goalrail
...
def test_missing_profile_maps_to_none(
    ...
    surfaces ``None`` so the goalrail
    :class:`~goalrail.datamodel.ExecutorSpec.profile` field
...
def test_spec_with_local_tool_resolves_dotted_path(
    ...
    **What breaks if this fails**: the harness receives a tool
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="sample",
            path="tests.spec.test_goalrail_translator.sample_tool_callable",
            language="python",
        ),
    ]
    agent_def = agent_spec_to_agent_def(basic_spec)
...
def test_policies_dropped_from_forward_translation(
    ...
    because goalrail enforces them upstream of the executor.

    **What breaks if this fails**: two regressions to guard
    against:
    1. The translator starts rejecting again (``GoalrailError``
       with ``"policies"``) — the goalrail executor would then
    ...
    2. The translator starts round-tripping policies INTO the
       AgentDef — meaning both the goalrail workflow AND the
       goalrail runtime would enforce them, double-counting
       every DENY.
    """
...
def test_sandbox_block_rejected_with_clear_message(
    ...
    **What breaks if this fails**: the harness runs tools outside
    ...
    with pytest.raises(GoalrailError, match=r"sandbox"):
...
def test_tool_with_filesystem_path_rejected(
    ...
    **What breaks if this fails**: the translator silently drops
    ...
    with pytest.raises(GoalrailError, match=r"dotted"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_with_unimportable_module_rejected(
    ...
    **What breaks if this fails**: a typo in the tool path
    ...
    with pytest.raises(GoalrailError, match=r"no_such_module_xyzzy"):
        agent_spec_to_agent_def(basic_spec)
...
def test_tool_pointing_at_non_callable_rejected(
    ...
    **What breaks if this fails**: the goalrail harness
    ...
    basic_spec.local_tools = [
        LocalToolInfo(
            name="stringy",
            path="tests.spec.test_goalrail_translator.NOT_A_FUNCTION",
            language="python",
        ),
    ]
...
def test_missing_llm_rejected(basic_spec: AgentSpec) -> None:
    """
    A spec with ``executor.type='goalrail'`` but no ``llm``
    block is rejected — the goalrail harness needs a model
    name. We fail loud at translation time, not deep inside the
    harness constructor.
    """
    basic_spec.executor.model = None
    with pytest.raises(GoalrailError, match=r"executor\.model"):
        agent_spec_to_agent_def(basic_spec)


# ── Harness inference for native Goalrail v1 specs ────────────────────────────────
...
def test_native_goalrail_spec_infers_harness_from_model(
    ...
    spec = AgentSpec(
        ...
        executor=ExecutorSpec(type="goalrail", model=model, config={}),
    )
...
def test_sub_agent_infers_harness_and_forwards_os_env() -> None:
    """
    When a parent spec's sub-agent uses a native Goalrail v1 executor (no
...
def test_server_runtime_tool_with_no_path_rejected(
    ...
    :func:`goalrail.spec.validator.validate`, but the translator
    ...
    with pytest.raises(GoalrailError, match=r"server-runtime tool has no"):
        agent_spec_to_agent_def(basic_spec)
