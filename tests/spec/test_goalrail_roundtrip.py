"""
Round-trip invariant tests for the Goalrail ↔ AgentSpec
adapter.
...
:func:`goalrail.spec.goalrail.agent_spec_to_agent_def`,
...
goalrail-testing skill, skipped tests rot invisibly. The
...
from goalrail.spec.goalrail import (
    ...
    # ``goalrail/spec/goalrail.py`` per the design. Phase 1
...
def hello_world_yaml(tmp_path: Path) -> Path:
    """
    Minimal goalrail YAML — name + prompt only. Round-trip
...
def function_tool_yaml(tmp_path: Path) -> Path:
    """
    Goalrail YAML with one function-type tool pointing at a
...
def _roundtrip(yaml_path: Path) -> None:
    """
    Load the YAML via goalrail' loader, translate to an
    ...
    **Lossy by design.** :class:`AgentSpec` does not model every
    ...
    # goalrail' CLI auto-pick) — the round-trip contract is
...
def test_roundtrip_hello_world_is_incomplete_for_goalrail(
    ...
    model, which Goalrail' strict spec rejects on the way
    back. This is intentional: the goalrail validator requires
    a harness when ``executor.type == "goalrail"``, and that
...
def test_roundtrip_executor_block(executor_block_yaml: Path) -> None:
    ...
    differs between the two directions — GoalrailExecutor
