...
def test_parse_skill_non_utf8_raises_goalrail_error(agent_dir: Path) -> None:
    """
    A non-UTF-8 SKILL.md must funnel through GoalrailError (not escape as a
    ...
    with pytest.raises(GoalrailError, match=r"could not be read"):
        parse(agent_dir)
