...
from goalrail.codex_native_forwarder import _persist_codex_compaction_item
...
def test_terminal_turn_status_edge_empty_turn_idle_and_warns(
    ...
    with caplog.at_level("WARNING", logger="goalrail.codex_native_forwarder"):
        edge = fwd._terminal_turn_status_edge(tmp_path, "turn/completed", params)
...
def test_goalrail_status_from_resume_turn_error_parity() -> None:
    ...
    assert fwd._goalrail_status_from_resume_turn(turn_with_error) == "failed"
    # Parity check: the clean turn still resolves to idle.
    assert fwd._goalrail_status_from_resume_turn(turn_clean) == "idle"
