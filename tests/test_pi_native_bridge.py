...
def test_write_extension_files_embeds_tools(tmp_path: Path) -> None:
    ...
    The runner builds the session's Goalrail tool surface (sys_* tools) and
    ...
    _ext, cfg = pi_native_bridge.write_extension_files(
        bridge_dir,
        session_id="conv_abc",
        server_url="http://goalrail.test/",
        conversation_url="http://goalrail.test/c/conv_abc",
    ...
    assert payload["serverUrl"] == "http://goalrail.test"
