...
class TestModelFlagHelpers:
    ...
    def test_model_flag_disabled_when_env_absent(self) -> None:
        ...
        from goalrail.codex_native_app_server import _model_flag_enabled
    ...
    async def test_supports_model_flag_true_when_help_lists_it(
        ...
        from goalrail.codex_native_app_server import _codex_supports_model_flag

        async def _fake_exec(*_args: Any, **_kwargs: Any) -> Any:
            return _HelpProc(b"Options:\n  -m, --model <MODEL>\n      Model to use\n")

        monkeypatch.setattr("goalrail.codex_native_app_server._create_subprocess_exec", _fake_exec)
...
def _patch_start_spawn(monkeypatch: pytest.MonkeyPatch, recorder: _SpawnRecorder) -> None:
    ...
    monkeypatch.setattr(
        "goalrail.codex_native_app_server._create_subprocess_exec", recorder._record
    )
    ...
    monkeypatch.setattr(
        "goalrail.codex_native_app_server.acquire_codex_native_process_owner_lock",
        lambda: None,
    )
...
class TestModelFlagPlumbing:
    ...
    async def test_flag_off_omits_model_flag_and_env(
        ...
        """
        from goalrail.codex_native_app_server import _MODEL_FLAG_ENV_VAR
        ...
        server = await _model_flag_app_server(
            tmp_path, codex_path="/usr/bin/codex", model="openai/gpt-5-4-mini", env={}
        )
        await server.start()
        ...
        assert 'model = "openai/gpt-5-4-mini"' in (server.codex_home / "config.toml").read_text(
            encoding="utf-8"
        )
    ...
    async def test_flag_on_with_cli_support_passes_global_model_flag(
        ...
        monkeypatch.setattr(
            "goalrail.codex_native_app_server._codex_supports_model_flag", _supports
        )
        # The opt-in lives in the server's process env, NOT the cleaned codex
        # spawn env (env={}): _clean_codex_env strips GOALRAIL_* keys, so a
        ...
        assert argv[model_idx + 1] == "openai/gpt-5-4-mini"
    ...
    async def test_flag_on_without_cli_support_skips_model_flag(
        ...
        monkeypatch.setattr(
            "goalrail.codex_native_app_server._codex_supports_model_flag", _unsupported
        )
    ...
    async def test_flag_in_spawn_env_alone_does_not_enable(
        ...
        whose prefix allowlist strips ``GOALRAIL_*`` keys, so the opt-in can
