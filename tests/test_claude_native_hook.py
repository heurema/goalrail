...
from goalrail import claude_native_hook, native_policy_hook
...
def test_session_start_hook_records_transcript_state_without_output(
    ...
    This fails if the ``goalrail claude`` hook reintroduces
...
def test_session_start_hook_emits_conversation_url_system_message(
    ...
    This fails if ``goalrail claude`` stops routing the web URL
    through Claude's hook output path, leaving users with no startup
    pointer back to the Goalrail conversation.
    """
    monkeypatch.setattr("goalrail.claude_native_bridge._TRUSTED_PARENT", tmp_path)
...
def test_clear_session_start_hook_rotates_before_printing_conversation_url(
    ...
    """
    ``/clear`` SessionStart prints the URL for the replacement Goalrail session.
        ...
        def get(self, url: str) -> object:
            ...
            return httpx.Response(
                ...
                json={
                    ...
                    "labels": {"goalrail.claude_native.bridge_id": "bridge_shared"},
                },
    ...
    assert requests == [
        ...
        (
            ...
            {
                "runner_id": "",
                "labels": {"goalrail.claude_native.bridge_id": "conv_old-cleared"},
            },
        ),
    ]
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert '"goalrail_clear_rotated_to":"conv_new"' in recorded
    ...
    # _PERMISSION_TIMEOUT_S (86400) an unresponsive Goalrail server would hang
...
def test_fork_session_start_hook_forks_before_printing_conversation_url(
    ...
    """
    Claude ``/fork`` SessionStart prints the URL for the forked Goalrail session.
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Fork the Goalrail session or transfer the terminal.
    ...
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert '"goalrail_previous_claude_session_id":"claude_old"' in recorded
    assert '"goalrail_claude_session_was_seen":false' in recorded
    assert '"goalrail_fork_detected":true' in recorded
    assert '"goalrail_fork_rotated_to":"conv_fork"' in recorded
...
def test_resume_session_start_without_branch_marker_does_not_fork(
    ...
    class _FailingHttpxClient:
        """
        HTTP client stub that fails if fork detection makes Goalrail calls.
    ...
    recorded = (bridge_dir / "hooks.jsonl").read_text(encoding="utf-8")
    assert "goalrail_fork_detected" not in recorded
    assert "goalrail_fork_rotated_to" not in recorded
...
def test_permission_request_hook_posts_to_active_session_from_bridge_config(
    ...
    """
    Permission command hook routes to the current active Goalrail session.

    This fails if the hook bakes in the launch conversation id: after
    Claude ``/clear`` rotates the bridge to a new Goalrail session, approval
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the outgoing Goalrail request.
    ...
    assert {k: v for k, v in sent.items() if k != "_goalrail_elicitation_id"} == payload
    assert re.fullmatch(r"elicit_claude_[0-9a-f]{32}", sent["_goalrail_elicitation_id"]), (
        f"re-attach id outside the claude-hook namespace: {sent.get('_goalrail_elicitation_id')!r}"
    )
...
def _prepare_permission_bridge(tmp_path: Path, session_id: str) -> Path:
    ...
    :param session_id: Active Goalrail session id, e.g. ``"conv_x"``.
...
def test_permission_request_hook_retries_transport_cut_with_same_id(
    ...
    ``_goalrail_elicitation_id`` — a fresh id per attempt would park a
    ...
    first_id = attempts[0]["_goalrail_elicitation_id"]
    ...
    assert attempts[1]["_goalrail_elicitation_id"] == first_id
...
def test_permission_request_hook_does_not_retry_rejections(
    ...
    """
    A 4xx from the Goalrail server is a deliberate answer — no retry.
...
def test_build_hook_settings_registers_policy_hooks_when_goalrail_server_url_set(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
...
def test_build_hook_settings_registers_message_display_hook(
    ...
    assert "goalrail.claude_native_message_display_hook" in command
...
def test_build_hook_settings_omits_policy_hooks_without_goalrail_server_url(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    ``build_hook_settings`` omits policy hooks when no Goalrail URL is set.

    Without a Goalrail server there are no policies to evaluate; registering
    ...
    for entry in hooks.get("PostToolUse", []):
        ...
        assert "evaluate-policy" not in cmd, (
            "Policy evaluation hook should not be registered without Goalrail URL"
        )
...
def test_evaluate_policy_pre_tool_use_converts_and_returns_deny(
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the outgoing Goalrail request and return a DENY verdict.
...
def test_ask_user_question_hook_noop_in_non_bypass_mode(
    ...
    class _RaisesIfCalled:
        ...
        def post(self, *_args: object, **_kwargs: object) -> object:
            """
            Fail if Goalrail is called — must not happen in non-bypass mode.
    ...
    for mode in ("default", "acceptEdits", "plan", None):
        ...
        captured = capsys.readouterr()
        # No Goalrail call, no output — "no opinion" so PermissionRequest takes over.
...
def test_ask_user_question_hook_posts_and_returns_pre_tool_use_output_in_bypass_mode(
    ...
    must POST the payload to the Goalrail session's permission-request endpoint, then
    ...
    Fails if: Goalrail is not called in bypass mode, the URL targets the wrong session,
    ...
    class _FakeHttpxClient:
        ...
        def post(self, url: str, *, json: dict[str, object]) -> object:
            """
            Record the Goalrail request and return a canned PermissionRequest response.
    ...
    captured = capsys.readouterr()
    assert exit_code == 0
    # Goalrail must be called with the active session's URL.
    assert posted["url"] == (
...
def test_build_reauth_remints_and_preserves_routing_header(
    ...
    The fresh token must be merged OVER the existing headers so any other
    routing header the caller sends alongside the bearer is preserved, not
    dropped.
    """
    monkeypatch.setattr(
        "goalrail.runner._entry._make_auth_token_factory",
        lambda server_url=None: lambda: "fresh-token",
    )
    reauth = claude_native_hook._build_reauth(
        "https://ap.example.com",
        {"Authorization": "Bearer stale", "X-Org-Id": "o9"},
    )
    assert reauth() == {"Authorization": "Bearer fresh-token", "X-Org-Id": "o9"}
...
def test_evaluate_policy_reauths_on_expired_token_instead_of_failing_closed(
    ...
    past its lifetime) hits a login-flow ``302 → /oidc`` on every tool call
    and used to fail CLOSED ("policy evaluation unavailable"). The hook must
    now re-mint through the token factory and retry, returning the real
    ALLOW verdict (no deny output).
    """
    ...
    build_hook_settings(
        bridge_dir,
        ap_server_url="http://127.0.0.1:8787",
        ap_auth_headers={"Authorization": "Bearer stale-token", "X-Org-Id": "o1"},
    )
    payload = {
        "hook_event_name": "PreToolUse",
        "tool_name": "Bash",
