"""Tests for goalrail.chat — goalrail chat CLI logic."""
...
import goalrail.chat as chat_module
...
def test_validate_agent_spec_unresolved_env_var(
    ...
    # proves the underlying GoalrailError message reached the user
...
def test_validate_agent_spec_missing_config(tmp_path: Path) -> None:
    ...
    # (not the GoalrailError branch) — both must convert.
...
def test_wait_for_server_uses_fast_poll_before_backoff(
    ...
    monkeypatch.setattr("goalrail.chat.time.monotonic", _fake_monotonic)
    monkeypatch.setattr("goalrail.chat.time.sleep", _fake_sleep)
...
def test_raise_server_failed_truncates_log_to_tail(tmp_path: Path) -> None:
    ...
    must be preserved.
    """
    from goalrail.chat import _SERVER_LOG_TAIL_LINES
    ...
    tail_lines = [
        ...
        "  File goalrail/server/app.py, line 42, in create_app",
        "RuntimeError: missing required field 'agent'",
    ]
    log.write_text("\n".join(head_lines + tail_lines) + "\n")
    server = SimpleNamespace(
        proc=SimpleNamespace(args=["python", "-m", "goalrail", "server"]),
        log_path=log,
    )
    ...
    assert "python -m goalrail server" in msg
...
def test_start_local_server_spawns_runner_as_sibling(
    ...
    via ``GOALRAIL_RUNNER_TUNNEL_TOKEN`` so it accepts exactly the
    sibling runner's tunnel. The runner is NOT a child of the server.
    """
    from goalrail.cli import _CliRunnerProcess
    ...
    import goalrail.runner.identity  # noqa: F401

    monkeypatch.setattr("goalrail.chat.subprocess.Popen", _fake_popen)
    monkeypatch.setattr("goalrail.chat._goalrail_log_dir", lambda: tmp_path / "logs")
    monkeypatch.setattr(
        "goalrail.chat.load_spec",
        lambda _path: SimpleNamespace(executor=SimpleNamespace(profile=None)),
    )
    monkeypatch.setattr(
        "goalrail.cli._start_cli_runner_process",
        _fake_start_runner,
    )
    ...
    assert "GOALRAIL_RUNNER_TUNNEL_TOKEN" in server_popen_calls[0].env
    ...
    assert (
        ...
        == server_popen_calls[0].env["GOALRAIL_RUNNER_TUNNEL_TOKEN"]
    )
...
def test_wait_for_remote_runner_timeout_surfaces_log_path(
    ...
    monkeypatch.setattr("goalrail.chat.time.monotonic", _fake_monotonic)
    monkeypatch.setattr("goalrail.chat.time.sleep", lambda _s: None)
    monkeypatch.setattr(
        "goalrail.chat.httpx.get",
        lambda *_a, **_k: _Resp(),
    )
...
def test_wait_for_remote_runner_early_exit_surfaces_log_path(
    ...
    monkeypatch.setattr("goalrail.chat.time.monotonic", lambda: 0.0)
    monkeypatch.setattr("goalrail.chat.time.sleep", lambda _s: None)
...
def test_run_chat_with_server_url_routes_through_daemon(
    ...
    monkeypatch.setattr("goalrail.cli._ensure_backend", _fake_ensure_backend)
...
def test_run_local_headless_prompt_uses_directory_bundle_for_root_config_yaml(
    ...
    This covers ``goalrail run bundle/config.yaml -p ...``. Without the
...
class _FakeSdkClient:
    """Async-context-manager stand-in for ``GoalrailClient`` in prep tests.
...
def _patch_daemon_launch(monkeypatch: pytest.MonkeyPatch, captured: dict[str, object]) -> None:
    ...
    """
    monkeypatch.setattr(
        "goalrail_client.GoalrailClient",
    ...
    monkeypatch.setattr("goalrail.host.daemon_launch.wait_for_host_online", _no_host_wait)
    monkeypatch.setattr("goalrail.host.daemon_launch.launch_or_reuse_daemon_runner", _fake_launch)
    monkeypatch.setattr("goalrail.host.daemon_launch.wait_for_runner_online", _no_runner_wait)
    monkeypatch.setattr("goalrail.native_terminal.bind_session_runner", _fake_bind)
...
def test_prepare_chat_session_via_daemon_binds_runner_to_clear_stopped_marker(
    ...
    paths, neither of which clears the server-side ``goalrail.stopped`` marker
    ...
    # PATCH chokepoint that clears goalrail.stopped — same pattern as
    # ``goalrail claude`` (claude_native.py's bind_session_runner call).
...
# ── GOALRAIL_MODEL env-var fallback ───────────────────
#
# These tests pin the env-var contract on the
# ``goalrail/cli.py`` → ``run_chat`` direct path. Without
# them, ``GOALRAIL_MODEL=foo`` was silently dropped on the
# ``goalrail`` console-script default Goalrail path because
# ``_apply_overrides_to_raw`` used the hardcoded
# ``_DEFAULT_AD_HOC_MODEL`` instead of the env-var-aware
# helper. See ``designs/RUN_GOALRAIL_REPL_PARITY.md``.
...
def test_default_cli_model_returns_hardcoded_default_when_env_unset(
    ...
    """
    With ``GOALRAIL_MODEL`` unset, the helper returns the
    ...
    ``goalrail run hello.yaml`` without setting the env var
    would suddenly land on a different model than they did
    before — silently breaking their workflows.
    """
    monkeypatch.delenv("GOALRAIL_MODEL", raising=False)
    assert _default_cli_model() == _DEFAULT_AD_HOC_MODEL


def test_default_cli_model_honors_goalrail_model_env_var(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """
    With ``GOALRAIL_MODEL=foo`` set, the helper returns
    ``"foo"``.
...
def test_apply_overrides_uses_env_var_when_yaml_has_no_model_or_harness(
    ...
    ``GOALRAIL_MODEL=foo`` set, lands with ``executor.model =
    "foo"``.
...
def test_apply_overrides_explicit_model_wins_over_env_var(
    ...
    assert executor.get("model") == "from-flag", (
        f"--model override must win over GOALRAIL_MODEL. Got "
...
def test_apply_overrides_writes_nested_config_harness_for_spec_version_bundle() -> None:
    ...
    Regression guard for the polly no-op: ``goalrail run
...
def test_apply_overrides_flat_harness_creates_no_config_for_single_file_yaml() -> None:
    """
    Single-file goalrail YAMLs (no ``spec_version``) keep the flat
...
def test_apply_overrides_canonicalizes_alias_into_spec_version_config(
    ...
    assert executor["config"]["harness"] == canonical, (
        ...
        f"A raw alias here would fail GOALRAIL_HARNESSES validation or "
...
def test_apply_overrides_harness_and_model_together_for_spec_version_bundle() -> None:
    ...
    This is the polly-on-GPT invocation shape: ``goalrail run
...
def test_apply_overrides_rejects_harness_for_non_goalrail_executor_type() -> None:
    """
    A spec_version bundle with a non-goalrail ``executor.type`` fails
...
def test_materialize_override_bundle_bakes_env_var_into_yaml(
    ...
    real YAML file, set ``GOALRAIL_MODEL=foo``, materialize a
...
def test_materialize_directory_bundle_with_override_keeps_nested_harness_unpinned(
    ...
    monkeypatch.setenv("GOALRAIL_CONFIG_HOME", str(tmp_path / "config"))
...
def test_materialize_bundle_overrides_brain_harness(
    ...
    ``_apply_overrides_to_raw`` → ``yaml.safe_dump`` → ``goalrail.spec.load``
    → ``validate``. This is the exact path ``goalrail run examples/polly
    ...
    still validates (the harness is in GOALRAIL_HARNESSES); (3) the
    ...
    :param bundle_name: Packaged example bundle under
        ``goalrail.resources.examples``, e.g. ``"polly"``.
    ...
    # Isolate from the developer's goalrail config and ambient creds so
    ...
    bundle_dir = Path(
        str(importlib.resources.files("goalrail.resources.examples").joinpath(bundle_name))
    )
...
def test_materialize_override_bundle_cleans_tempdir_when_directory_invalid(
    ...
    def fake_mkdtemp(*, prefix: str) -> str:
        ...
        """
        assert prefix == "goalrail-override-"
        tempdir.mkdir()
        return str(tempdir)
...
def test_run_repl_auto_opens_conversation_when_session_starts(
    ...
    class _Client:
        """Async context manager stub for :class:`GoalrailClient`."""
    ...
    monkeypatch.setattr(chat_module, "GoalrailClient", _Client)
    monkeypatch.setattr("goalrail.repl.run_repl", _fake_run_repl)
    monkeypatch.setattr(chat_module, "open_conversation_link_if_enabled", _fake_open)
    monkeypatch.setattr("goalrail.repl._tmux_pane.register_pane", lambda **kwargs: None)
...
def _make_run_context(**params: object) -> click.Context:
    ...
    ctx = click.Context(run_cmd, info_name="run", parent=click.Context(cli, info_name="goalrail"))
    ctx.params = merged
    return ctx
...
def test_build_resume_parts_omits_defaults() -> None:
    ...
    assert parts == ["goalrail", "run", "agent.yaml"]
...
def _stub_run_repl_deps(
    ...
    *,
    ...
    Replaces ``run_repl`` (the async REPL), ``GoalrailClient``
    ...
    # ``from goalrail.repl import run_repl``, which reads the
    ...
    import goalrail.repl as _repl_pkg

    monkeypatch.setattr(_repl_pkg, "run_repl", _fake_run_repl)
    monkeypatch.setattr("goalrail.repl._repl.run_repl", _fake_run_repl)
    monkeypatch.setattr("goalrail.chat.GoalrailClient", _FakeClientCtx)
...
def test_run_repl_passes_resume_parts_to_run_repl(
    ...
    parts = ["goalrail", "run", "agent.yaml", "--server", "https://example.com"]
...
class _FakeResumeClient:
    """Minimal GoalrailClient-like object exposing sessions."""
...
class _FakeClientCtx:
    """Minimal GoalrailClient stand-in that works as an async context manager.
...
def test_spec_used_families_multi_vendor_directory_agent(tmp_path) -> None:
    ...
    # (the latter is what `goalrail run <dir>` threads through).
...
def test_await_accounts_setup_noop_when_token_present(
    ...
    monkeypatch.setattr("goalrail.cli_auth.load_token", lambda _url: "existing-token")

    def _must_not_probe(*_a: object, **_k: object) -> object:
        raise AssertionError("must not call /v1/info when a token already exists")

    monkeypatch.setattr("goalrail.chat.httpx.get", _must_not_probe)
...
def test_await_accounts_setup_noop_for_header_mode(
    ...
    monkeypatch.setattr("goalrail.chat.time.sleep", _must_not_sleep)
...
def test_await_accounts_setup_waits_then_continues(
    ...
    monkeypatch.setattr("goalrail.cli_auth.load_token", _load)
    monkeypatch.setattr(
        "goalrail.chat.httpx.get",
...
async def _run_one_shot(
    ...
    :param client: The fake Goalrail client supplying the transcript.
    :param query_impl: The async ``query`` behavior to install.
    :param monkeypatch: pytest monkeypatch fixture.
    :returns: Whatever ``_query_sessions_once`` returns.
    """
    # chat.py does ``from goalrail_client import SessionsChat`` inside
    # the function, so patch the attribute on the package (resolved at
    # call time), not a chat-module-local alias.
    monkeypatch.setattr("goalrail_client.SessionsChat", _fake_sessions_chat_cls(query_impl))
...
async def test_query_sessions_once_reconciles_persisted_text_on_failed_status(
    ...
    This is the exact reported bug: ``goalrail run -p`` printed
...
async def test_query_sessions_once_reraises_when_no_persisted_text(
    ...
    with pytest.raises(ClientGoalrailError, match="auth misconfigured"):
...
async def test_query_sessions_once_surfaces_persisted_error_when_no_text(
    ...
    with pytest.raises(ClientGoalrailError, match="Failed to start cursor-sdk agent"):
...
async def test_query_sessions_once_reraises_on_failed_with_only_partial_item(
    ...
    with pytest.raises(ClientGoalrailError, match="turn failed"):
...
def test_env_auth_injection_skipped_when_global_auth_configured(
    ...
    # runner resolves it via GOALRAIL_CONFIG_HOME. A baked api_key here
...
def test_redirect_native_resume_handles_cursor(monkeypatch: pytest.MonkeyPatch) -> None:
    """A cursor-native resume hands off to ``goalrail cursor`` (direct attach).

    Regression: without a cursor branch in ``_redirect_native_resume_if_needed``
    the resume fell through to the Goalrail REPL, which drove an Goalrail turn
    ...
    """
    from goalrail._wrapper_labels import CURSOR_NATIVE_WRAPPER_VALUE
    ...
    monkeypatch.setattr("goalrail.cursor_native.run_cursor_native", _fake_run_cursor_native)
...
def test_cursor_native_resume_never_drives_an_goalrail_turn(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    duplicate had two sources: (1) the Goalrail turn the REPL drives, which
    ...
    def _fail_repl(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("_run_repl drove an Goalrail turn for a cursor-native resume")

    def _fail_one_shot(*_args: object, **_kwargs: object) -> None:
        raise AssertionError("_run_one_shot drove an Goalrail turn for a cursor-native resume")
