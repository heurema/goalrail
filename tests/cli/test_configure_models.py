...
def _capture_setup_overview(
    ...
    monkeypatch.setattr("goalrail.onboarding.interactive.select", _capture_select)
...
def test_overview_hermes_row_reflects_configured_model(isolated_config, monkeypatch) -> None:
    ...
    * a finished ``hermes model`` run (a concrete ``provider`` + ``default``
    ...
    monkeypatch.setattr("goalrail.onboarding.hermes_auth.hermes_cli_installed", lambda: True)
...
def test_overview_truncates_long_status_for_narrow_terminal(isolated_config, monkeypatch) -> None:
    ...
    from goalrail.onboarding import interactive
    from goalrail.onboarding.opencode_auth import OpenCodeAuthSummary

    monkeypatch.setattr(
        "goalrail.cli.shutil.get_terminal_size", lambda fallback: os.terminal_size((40, 24))
    )
    monkeypatch.setattr(
        "goalrail.onboarding.opencode_auth.opencode_auth_summary",
...
def test_overview_dispatches_to_correct_manager(
    ...
    called: list[str] = []
    monkeypatch.setattr(
        f"goalrail.cli.{manager_attr}", lambda *a, **k: called.append(manager_attr)
    )
...
def test_overview_descriptions_map_to_their_rows(isolated_config, monkeypatch) -> None:
    ...
    from goalrail.onboarding.goose_auth import GooseConfigSummary
    from goalrail.onboarding.hermes_auth import HermesConfigSummary
    from goalrail.onboarding.opencode_auth import OpenCodeAuthSummary

    monkeypatch.setattr("goalrail.onboarding.cursor_auth.cursor_sdk_installed", lambda: True)
    monkeypatch.setattr(
        "goalrail.onboarding.antigravity_auth.antigravity_sdk_installed", lambda: True
    )
    monkeypatch.setattr("goalrail.onboarding.copilot_auth.copilot_sdk_installed", lambda: True)
    monkeypatch.setattr(
        "goalrail.onboarding.opencode_auth.opencode_auth_summary",
        lambda: OpenCodeAuthSummary(installed=True, stored_providers=(), env_providers=()),
    )
    monkeypatch.setattr(
        "goalrail.onboarding.goose_auth.goose_config_summary",
    ...
    monkeypatch.setattr(
        "goalrail.onboarding.hermes_auth.hermes_config_summary",
...
def test_cursor_overview_install_command_is_selection_only(
    ...
    The exact ``pip install "goalrail[cursor]"`` (brackets included) is the
    ...
    assert 'pip install "goalrail[cursor]"' in Text.from_markup(descriptions[cursor]).plain
...
def test_antigravity_overview_install_command_is_selection_only(
    ...
    The exact ``pip install "goalrail[antigravity]"`` (brackets included) is the
    ...
    assert (
        'pip install "goalrail[antigravity]"' in Text.from_markup(descriptions[antigravity]).plain
    )
...
def _copilot_sdk_absent(monkeypatch):
    ...
    for var in ("COPILOT_GITHUB_TOKEN", "GH_TOKEN", "GITHUB_TOKEN"):
        monkeypatch.delenv(var, raising=False)
    monkeypatch.setattr("goalrail.onboarding.copilot_auth.copilot_sdk_installed", lambda: False)
...
def test_copilot_overview_install_command_is_selection_only(
    ...
    soft-SDK-extra harness: the ``goalrail[copilot]`` install command is the
    ...
    assert "goalrail[copilot]" in Text.from_markup(descriptions[copilot]).plain
...
@pytest.mark.parametrize(
    ...
    [
        ("3", "goalrail.onboarding.cursor_auth.cursor_sdk_installed", "Cursor — no API key yet"),
        (
            "7",
            "goalrail.onboarding.antigravity_auth.antigravity_sdk_installed",
        ...
        (
            "10",
            "goalrail.onboarding.copilot_auth.copilot_sdk_installed",
