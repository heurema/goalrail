...
async def test_events_compact_on_pi_native_enqueues_compact_payload(
    ...
    compaction must run there (the Goalrail server's AP-side compaction would
    ...
    Pins:
    1. 200 returned (not 204) so the Goalrail server skips its own AP-side
       compaction.
...
async def test_events_compact_on_pi_native_returns_503_when_inbox_unwritable(
    ...
    request; the Goalrail server then treats it as not-handled.
    """
    import goalrail.pi_native_bridge as pi_native_bridge
    from goalrail.spec.types import ExecutorSpec
...
async def test_events_compact_on_qwen_native_submits_compress_and_raises_spinner(
    ...
    qwen_native_spec = AgentSpec(
        spec_version=1,
        name="t",
        executor=ExecutorSpec(type="goalrail", config={"harness": "qwen-native"}),
    )
