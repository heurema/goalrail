...
def test_pi_native_model_passthrough() -> None:
    ...
    assert _pi_native_model_from_spec(_spec("claude-opus-4-7")) == "claude-opus-4-7"


def test_pi_native_model_keeps_provider_qualified_id() -> None:
    """A provider-qualified id is usable here (Pi routes by the id verbatim)."""
...
def test_pi_native_model_from_resolved_spec_wrapper() -> None:
    ...
    wrapped = ResolvedSpec(spec=_spec("claude-opus-4-7"), workdir=Path("/tmp"))
    assert _pi_native_model_from_spec(wrapped) == "claude-opus-4-7"
...
async def test_auto_create_pi_terminal_threads_spec_model_into_models_json(
    ...
    :returns: None.
    """
    import goalrail.pi_native_bridge as pi_bridge
    import goalrail.pi_native_credentials as creds
    ...
    monkeypatch.setattr("goalrail.pi_native.resolve_pi_executable", lambda: "/usr/bin/pi")
    ...
    assert models["providers"]["goalrail"]["models"] == [{"id": "claude-opus-4-7"}]
...
async def test_auto_create_pi_terminal_no_spec_model_uses_provider_default(
    ...
    agent_dir = Path(launched["env"]["PI_CODING_AGENT_DIR"])
    models = json.loads((agent_dir / "models.json").read_text(encoding="utf-8"))
    assert models["providers"]["goalrail"]["models"] == [{"id": "claude-sonnet-4-6"}]
