...
_GPT_POOL = ("openai/gpt-5-4", "openai/gpt-5-5")
_CLAUDE_POOL = ("anthropic/claude-sonnet-4-6", "anthropic/claude-opus-4-6")
...
def test_unpooled_model_passes_through(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        _model_pools.resolve_model("custom-gpt-bogus-xyz", key="k", attempt=2)
        == "custom-gpt-bogus-xyz"
    )
...
def test_spread_off_returns_input(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _model_pools.resolve_model("openai/gpt-5-4", key="anything") == "openai/gpt-5-4"


def test_spread_is_deterministic_and_in_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(_SPREAD_ENV, "1")
    first = _model_pools.resolve_model("openai/gpt-5-4", key="tests/foo.py::test_bar")
    second = _model_pools.resolve_model("openai/gpt-5-4", key="tests/foo.py::test_bar")
...
def test_spread_distributes_across_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(_SPREAD_ENV, "1")
    resolved = {
        _model_pools.resolve_model("anthropic/claude-sonnet-4-6", key=f"tests/t{i}.py::test")
        for i in range(64)
    }
...
def test_env_override_replaces_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv(_SPREAD_ENV, "1")
    monkeypatch.setenv("GOALRAIL_TEST_MODEL_POOL_GPT", "custom-gpt-9-test")
    # Single-member env override captures every gpt-family resolution.
    assert _model_pools.resolve_model("openai/gpt-5-4", key="any") == "custom-gpt-9-test"
...
def test_spread_false_skips_spread_but_rotates(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert (
        _model_pools.resolve_model("openai/gpt-5-5", key="any", attempt=0, spread=False)
        == "openai/gpt-5-5"
    )
    # ...but a rerun must still move to a DIFFERENT model.
    rotated = _model_pools.resolve_model("openai/gpt-5-5", key="any", attempt=1, spread=False)
    assert rotated != "openai/gpt-5-5"


def test_rotation_changes_model_each_attempt(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(_SPREAD_ENV, raising=False)
    base = _model_pools.resolve_model("anthropic/claude-sonnet-4-6", key="k", attempt=0)
    second = _model_pools.resolve_model("anthropic/claude-sonnet-4-6", key="k", attempt=1)
    # 2-member anthropic chain: attempts 0 and 1 cover both members.
    assert base == "anthropic/claude-sonnet-4-6"
    assert second == "anthropic/claude-opus-4-6"
    # Chain wraps: attempt 2 is back at the base.
    assert _model_pools.resolve_model("anthropic/claude-sonnet-4-6", key="k", attempt=2) == base


def test_rotation_walks_full_provider_chain(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv(_SPREAD_ENV, raising=False)
    seen = {_model_pools.resolve_model("openai/gpt-5-4", key="k", attempt=a) for a in range(4)}
...
def test_pinned_context_disables_everything(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _model_pools.resolve_model("openai/gpt-5-4") == "openai/gpt-5-4"
...
def test_context_supplies_default_key_and_attempt(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    via_context = _model_pools.resolve_model("openai/gpt-5-4")
    # Implicit context key == passing the nodeid explicitly.
    assert via_context == _model_pools.resolve_model("openai/gpt-5-4", key=nodeid)

    _model_pools.set_current_test(nodeid, attempt=1, pinned=False)
    rotated = _model_pools.resolve_model("openai/gpt-5-4")
...
def test_env_pool_member_outside_static_chain_still_rotates(
    ...
    resolved = {
        _model_pools.resolve_model("anthropic/claude-sonnet-4-6", key="k", attempt=a)
        for a in range(3)
    }
    # Rotation must reach env-added pool members too.
    assert "anthropic/claude-haiku-4-5" in resolved
...
def test_drained_model_excluded_from_spread_and_retry_rotation(
    ...
    resolved = {
        _model_pools.resolve_model("openai/gpt-5-4", key=f"k{i}", attempt=a)
        for i in range(8)
        for a in range(6)
    }
    # Neither spreading nor retry rotation may route to the drained model.
    assert "openai/gpt-5-4" not in resolved
    assert "openai/gpt-5-5" in resolved
...
def test_drained_model_kept_when_it_is_the_pinned_base(
    ...
    monkeypatch.setenv(_SPREAD_ENV, "1")
    monkeypatch.setenv("GOALRAIL_TEST_MODEL_POOL_GPT", "openai/gpt-5-5")
    ...
    assert (
        _model_pools.resolve_model("openai/gpt-5-4", key="k", attempt=0, spread=False)
        == "openai/gpt-5-4"
    )
    rotated = _model_pools.resolve_model("openai/gpt-5-4", key="k", attempt=1, spread=False)
    assert rotated != "openai/gpt-5-4"
