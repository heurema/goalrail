"""End-to-end test: ``goalrail codex`` runs the agent in the launch cwd.

The CLI sibling of the host worktree/workspace cwd tests in
``test_host_codex_native_e2e.py``. The ``goalrail codex`` wrapper launches
...
``GOALRAIL_RUNNER_WORKSPACE`` (the wrapper's launch cwd), never the
...
def test_codex_native_cli_runs_in_launch_cwd(
    ...
    Spawns a backgrounded ``goalrail codex`` session whose process cwd is a
    ...
    i.e. ``GOALRAIL_RUNNER_WORKSPACE`` / the session workspace, not the
    bundle dir.
        ...
        with httpx.Client(base_url=resume_test_server, timeout=30) as client:
            ...
            try:
                ...
                raise AssertionError(
                    f"`goalrail codex` did not return marker {marker!r} from "
