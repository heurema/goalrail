...
Runs ``goalrail run hello_world.yaml --harness pi --model
...
- Goalrail' ``PiExecutor`` regresses (the ``pi --mode rpc``
  ...
  Goalrail tools with ``pi.registerTool()``).
...
def test_per_harness_pi_one_shot(
    ...
    """
    ``goalrail run hello_world.yaml --harness pi -p <prompt>``
