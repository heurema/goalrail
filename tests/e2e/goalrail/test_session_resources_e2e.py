"""E2E test -- Session Resources API against ``goalrail server`` (mock LLM).

Boots ``goalrail server --agent <yaml>`` as a subprocess and
...
def _goalrail_server(
    *,
    ...
    """Spawn ``goalrail server --agent <agent_path>`` and a sibling runner.

    :param python: Interpreter with goalrail installed.
    ...
    runner_proc = subprocess.Popen(
        [str(python), "-m", "goalrail.runner._entry"],
        env={
            **env,
...
def test_session_resources_e2e(
    ...
    with _goalrail_server(
        ...
        with httpx.Client(
            ...
            # Goalrail REPL terminal (``terminal_tui_main``) on bind;
