"""Fixtures for Goalrail e2e tests (mock LLM).
...
# Root of the Goalrail checkout that ships the ``goalrail``
# package, the example YAMLs, and (in the main checkout) the
# ``.venv`` with goalrail + pexpect + openai-agents installed.
#
# Derived from the conftest's own location so git worktrees work
# naturally: this file lives at
# ``<root>/tests/e2e/goalrail/conftest.py`` (post-unification), so
...
# exec the main-checkout ``goalrail`` (via the editable install),
# missing any per-worktree edits.
_GOALRAIL_REPO = Path(__file__).resolve().parents[3]
    ...
    while True:
        ...
        if current.parent == current:
            ...
            raise RuntimeError(
                ...
                f"{_GOALRAIL_REPO} — worktrees share the main "
...
_GOALRAIL_VENV_PYTHON = _resolve_venv_python()


@pytest.fixture(scope="session")
def goalrail_python() -> Path:
    """
    Path to the Python interpreter that has the ``goalrail``
    package + its harness dependencies installed.

    The Goalrail repo ships its own ``.venv`` with
    ``goalrail``, ``pexpect``, ``openai-agents``,
    ``claude-agent-sdk``, etc. pre-installed. Agent-plane's e2e
    tests use that interpreter directly rather than adding
    goalrail as an goalrail dep (goalrail is not
    distributed as a package yet).

    :returns: Absolute path to the Goalrail ``.venv`` Python
        interpreter, e.g.
        ``"/path/to/goalrail/.venv/bin/python"``.
    :raises RuntimeError: If the interpreter is not present at
        the expected path — indicates the Goalrail checkout is
        missing or its .venv hasn't been created.
    """
    if not _GOALRAIL_VENV_PYTHON.is_file():
        raise RuntimeError(
            f"Goalrail venv python not found at {_GOALRAIL_VENV_PYTHON}. "
            f"These e2e tests require the sibling checkout at "
            f"{_GOALRAIL_REPO} with .venv set up."
        )
    return _GOALRAIL_VENV_PYTHON


@pytest.fixture(scope="session")
def goalrail_repo_root() -> Path:
    """
    Root of the Goalrail checkout used as the subprocess cwd.

    Goalrail YAMLs reference example tool modules via dotted
    ...
    :returns: Absolute path to the Goalrail repo root, e.g.
        ``"/path/to/goalrail"``.
    """
    return _GOALRAIL_REPO
...
def mock_credentials_env(
    ...
    config_home = tmp_path_factory.mktemp("goalrail-mock-e2e-config")
    (config_home / "config.yaml").write_text(
        "auth:\n  type: api_key\n",
        encoding="utf-8",
    )
    env["GOALRAIL_CONFIG_HOME"] = str(config_home)
    repo = str(_GOALRAIL_REPO)
    goalrail_path = str(_GOALRAIL_REPO / "goalrail")
    existing_pp = env.get("PYTHONPATH", "")
    env["PYTHONPATH"] = os.pathsep.join(p for p in (repo, goalrail_path, existing_pp) if p)
    return env
...
def mock_llm_server_url(
    ...
    proc = subprocess.Popen(
        [
            sys.executable,
            str(_GOALRAIL_REPO / "tests" / "server" / "integration" / "mock_llm_server.py"),
            str(mock_port),
        ],
        env={**os.environ, "PYTHONPATH": str(_GOALRAIL_REPO)},
        stdout=log_handle,
