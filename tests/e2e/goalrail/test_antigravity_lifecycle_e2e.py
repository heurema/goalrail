...
``test_per_harness_cursor.py``): it drives real ``goalrail run`` subprocesses
...
1. **Parallel turns / no state bleed** — two concurrent ``goalrail run``
   ...
   :class:`~goalrail.inner.antigravity_executor.AntigravityExecutor`) does not
...
2. **No orphaned ``localharness``** — snapshot the live ``localharness`` PIDs
   ...
   native binary).
3. **Session cleanup reaps the runner/harness** — after a single ``goalrail
...
- ``google-antigravity`` importable in the *goalrail* venv (the test shells
...
- The ``goalrail run`` one-shot path stops tearing down its local server /
...
# After a clean ``goalrail run`` exit, the local server/runner shutdown that
...
def _antigravity_prereqs_missing(goalrail_python: Path) -> str | None:
    """Return a skip reason if the antigravity harness can't run, else ``None``.

    Probes the *goalrail* venv (the interpreter the subprocess uses), not the
    test's own interpreter: the SDK import and the key resolution both have to
    hold for the spawned ``goalrail run`` to do anything. Booleans only — the
    key is never printed.

    :param goalrail_python: Interpreter with goalrail installed, from the
    ...
    probe = subprocess.run(
        ...
        cwd=str(goalrail_python.parent),
    )
    out = probe.stdout.strip().split()
    if probe.returncode != 0 or len(out) != 2:
        return (
            "could not probe antigravity prerequisites in the goalrail venv "
...
def _write_antigravity_bundle(bundle_dir: Path, *, name: str) -> Path:
    ...
    :returns: The bundle directory path (the argument ``goalrail run`` takes).
    """
...
def _antigravity_env() -> dict[str, str]:
    """Build the subprocess env for an antigravity ``goalrail run``.
...
def _run_antigravity_turn(
    *,
    ...
    """Run one ``goalrail run <bundle> --harness antigravity -p <prompt>``.
    ...
    :param goalrail_repo_root: Cwd for the subprocess (puts example tool modules
        on sys.path, matching the other run_goalrail tests).
...
@pytest.fixture
def antigravity_runnable(goalrail_python: Path) -> None:
    """Skip the whole module's tests when antigravity can't run.

    :param goalrail_python: Interpreter probed for the SDK + key.
    :raises pytest.skip.Exception: When a prerequisite is missing.
    """
    reason = _antigravity_prereqs_missing(goalrail_python)
...
def test_antigravity_parallel_turns_no_state_bleed(
    ...
    :param goalrail_python: Interpreter with goalrail + google-antigravity.
    :param goalrail_repo_root: Cwd for the subprocesses.
...
def test_antigravity_session_cleanup_reaps_runner(
    ...
    A single ``goalrail run`` turn owns a self-contained process tree (local
