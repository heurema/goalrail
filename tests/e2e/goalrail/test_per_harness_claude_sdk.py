...
Runs ``goalrail run hello_world.yaml --harness claude-sdk -p
...
- Goalrail' ``ClaudeSDKExecutor`` regresses (auth, MCP tool
...
- The Claude Agent SDK dependency or the ``claude`` CLI binary
  goes missing from the Goalrail venv.
...
def claude_sdk_available(goalrail_python: Path) -> bool:
    ...
    *goalrail* venv — the test's own venv is irrelevant because
    ...
    :param goalrail_python: The interpreter the subprocess
...
def test_per_harness_claude_sdk_one_shot(
    ...
    """
    ``goalrail run hello_world.yaml --harness claude-sdk -p
