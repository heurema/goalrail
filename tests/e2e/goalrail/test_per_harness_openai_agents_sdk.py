...
Runs ``goalrail run hello_world.yaml --harness openai-agents
...
- Goalrail' ``OpenAIAgentsSDKExecutor`` regresses (the Runner
  lifecycle, the Responses-API adapter in
  ``goalrail.open_responses_sdk``, the MCP tool bridging, or
  the event stream translation to ``ExecutorEvent`` types).
- The ``openai-agents`` Python package (``agents`` module) is
  missing from the goalrail venv or its public API changes
  incompatibly.
...
@pytest.fixture
def openai_agents_available(goalrail_python: Path) -> bool:
    """
    ...
    *goalrail* venv (the subprocess interpreter) — the current
    pytest interpreter is irrelevant because the test shells out.

    :param goalrail_python: Interpreter the subprocess will
...
def test_per_harness_openai_agents_sdk_one_shot(
    ...
    """
    ``goalrail run hello_world.yaml --harness openai-agents -p
    ...
    :param openai_agents_available: True when the ``agents``
        package is importable in the goalrail venv. On False
    ...
    if not openai_agents_available:
        pytest.skip(
            ...
            "installed in the Goalrail venv. Skipping — package absent."
        )
