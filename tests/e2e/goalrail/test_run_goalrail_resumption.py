"""End-to-end tests for ``goalrail run`` conversation resumption.
...
LLM in run #2, proving that the persistent goalrail store at
``$HOME/.goalrail/chat.db`` carries history between invocations.

**What breaks if this fails:**

- The persistent store path regresses in
  ``goalrail.chat._goalrail_persistent_dir`` or
  ``goalrail.inner.cli._build_goalrail_stores`` — e.g. someone
  flips back to ``mkdtemp`` and ``--continue`` silently
  starts a fresh conversation.
- Idempotent agent registration regresses
  (``_goalrail_register_yaml_bundle``) — the second subprocess
...
``designs/RUN_GOALRAIL_SESSION_RESUMPTION.md`` feature. If it
...
# Subprocess timeout per ``goalrail run`` invocation.
# 180s matches the existing run_goalrail tests' headroom for DBOS
...
def _argv_run_goalrail(
    *,
    ...
    """
    Build the ``goalrail run`` argv for a one-shot ``-p`` invocation.

    Use for plant steps (no resumption flags). Resume steps need
    :func:`_argv_run_goalrail_interactive` instead, since the CLI rejects
    ``--continue`` / ``--resume`` combined with ``-p``.

    :param goalrail_python: Interpreter from the
        ``goalrail_python`` fixture.
    :param goalrail_repo_root: Repo root from the
        ``goalrail_repo_root`` fixture.
...
def _argv_run_goalrail_interactive(
    *,
    goalrail_python: Path,
    goalrail_repo_root: Path,
    extra_flags: list[str],
    ...
    """
    Build the ``goalrail run`` argv for the interactive REPL.
    ...
    :param goalrail_python: Interpreter from the fixture.
    :param goalrail_repo_root: Repo root from the fixture.
...
def _daemon_log_tails(home: Path, *, tail_chars: int = 3000) -> str:
    ...
    Each ``goalrail run`` subprocess spawns its own local server, host
    daemon, and runner whose logs land under ``$HOME/.goalrail/logs/``
...
def _isolated_env(
    ...
    """
    Override ``HOME`` and the explicit Goalrail state/config roots so
    ...
    developer's real ``~/.goalrail/chat.db`` and could
    ...
    env = dict(base_env)
    env["HOME"] = str(home)
    env["GOALRAIL_CONFIG_HOME"] = str(home / ".goalrail")
    env["GOALRAIL_DATA_DIR"] = str(home / ".goalrail")
    return env


def test_run_goalrail_continue_carries_history_across_invocations(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    ...
    persistent_db = fake_home / ".goalrail" / "chat.db"
    assert persistent_db.is_file(), (
        f"Persistent store was not created at {persistent_db}. "
        f"Run #1 didn't write to ``~/.goalrail/chat.db`` — "
        f"either ``--no-session`` slipped in, or "
        f"``_goalrail_persistent_dir`` regressed."
    )
    ...
    result2 = subprocess.run(
        _argv_run_goalrail_interactive(
            goalrail_python=goalrail_python,
            goalrail_repo_root=goalrail_repo_root,
...
def test_run_goalrail_continue_with_no_prior_conversation_exits_nonzero(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
...
def test_run_goalrail_continue_works_across_oneshot_and_interactive_paths(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    (``test_run_goalrail_continue_carries_history_across_invocations``)
    couldn't catch it because both subprocesses there go
    through ``_goalrail_register_yaml_bundle``.
    ...
    interactive_argv = [
        ...
        "goalrail",
        "run",
        str(goalrail_repo_root / "tests" / "resources" / "examples" / "hello_world.yaml"),
        "--model",
        _MODEL,
        "--harness",
        _HARNESS,
...
def test_run_goalrail_session_id_pins_the_specific_conversation(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
...
def test_run_goalrail_session_id_unknown_exits_nonzero(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
...
def test_run_goalrail_no_session_does_not_pollute_persistent_store(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    tmp_path: Path,
    ...
    the persistent ``$HOME/.goalrail/chat.db`` must NOT
    ...
    # ``_goalrail_persistent_dir`` regardless of
    ...
    assert not persistent_db.exists(), (
        ...
        f"Either the ephemeral branch in _build_goalrail_stores "
        f"is no longer being honored, or _goalrail_persistent_dir "
