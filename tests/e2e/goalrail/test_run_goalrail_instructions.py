...
- ``goalrail/inner/loader.py::_resolve_instructions`` regresses.
- ``goalrail/spec/goalrail.py::agent_def_to_agent_spec`` stops
  preferring ``AgentDef.instructions`` over ``AgentDef.prompt``.
- ``goalrail/spec/_goalrail_compat.py::is_goalrail_yaml``
...
_MARKER_PATH_CASE = "GOALRAIL_INSTR_PATH_QXZP"
_MARKER_INLINE_CASE = "GOALRAIL_INSTR_INLINE_QXZP"
...
def test_instructions_path_field_loaded_via_goalrail_run_goalrail(
    goalrail_python: Path,
    ...
    ``goalrail run`` and the agent starts successfully.
