...
Runs ``goalrail run <spec> --harness antigravity ...`` as a real subprocess and
...
- ``google.antigravity`` importable in the Goalrail venv (the ``antigravity``
  extra — ``pip install 'goalrail[antigravity]'``).
- A Gemini / Antigravity API key configured (a stored ``antigravity:`` config
  block resolvable via
  :func:`goalrail.onboarding.antigravity_auth.antigravity_api_key_configured`,
...
- ``goalrail.cli``'s ``run`` one-shot / interactive paths stop persisting the
...
# Subprocess timeout per ``goalrail run`` invocation. The antigravity SDK boots
...
# ``prompt`` / ``executor``) — the form ``goalrail run <file>`` accepts without a
...
def _antigravity_skip_reason(goalrail_python: Path) -> str | None:
    ...
    Both prerequisites are probed in the *Goalrail venv* interpreter (the one the
    ...
    :param goalrail_python: Interpreter the ``goalrail`` subprocess will use.
    ...
    if probe.returncode != 0:
        ...
        return (
            "antigravity prerequisite probe failed in the Goalrail venv "
    ...
    if not have_sdk:
        return (
            ...
            "not importable in the Goalrail venv (install the 'antigravity' "
            "extra: pip install 'goalrail[antigravity]')."
        )
    if not have_key:
        return (
...
def _antigravity_env(base_env: dict[str, str], home: Path) -> dict[str, str]:
    ...
    Starts from the shared ``goalrail_credentials_env`` (so PATH, the onboarding
    suppression knobs, and the worktree ``PYTHONPATH`` propagate) but isolates
    ``$HOME`` and the Goalrail state/config roots into the test's temp dir, so
    the persistent conversation store this test reads (``$HOME/.goalrail/chat.db``)
    ...
    :param base_env: The ``goalrail_credentials_env`` fixture dict.
...
def _assistant_transcript_texts(db_path: Path) -> list[str]:
    ...
    :mod:`tests.e2e.goalrail.test_server_remote_goalrail_autonomous_flows`, but
    filtered to assistant-authored messages so the assertions can't be satisfied
    by the echoed user prompt.

    :param db_path: Path to ``$HOME/.goalrail/chat.db``.
...
def _run_one_shot(
    *,
    ...
    """Run a one-shot ``goalrail run <spec> --harness antigravity -p <prompt>``.

    Session-backed (no ``--no-session``) so the turn is persisted to
    ``$HOME/.goalrail/chat.db`` for transcript inspection and so a later
    ``--continue`` can thread onto it.

    :param goalrail_python: Interpreter from the ``goalrail_python`` fixture.
    :param goalrail_repo_root: Cwd for the subprocess (repo root on sys.path).
...
def _assert_clean_assistant_reply(
    ...
    *,
    ...
    :param result: The completed ``goalrail run`` process.
...
def test_per_harness_antigravity_smoke(
    ...
    The end-to-end smoke gate: one ``goalrail run --harness antigravity -p``
    ...
    :param goalrail_python: Interpreter with goalrail + the antigravity SDK.
    ...
    """
    skip_reason = _antigravity_skip_reason(goalrail_python)
    ...
    _assert_clean_assistant_reply(fake_home / ".goalrail" / "chat.db", result, label="smoke")
...
def test_per_harness_antigravity_model_selection(
    ...
    _assert_clean_assistant_reply(
        fake_home / ".goalrail" / "chat.db", result, label=f"model={model}"
    )
...
def test_per_harness_antigravity_multi_turn_history_retention(
    ...
    fake_home.mkdir()
    env = _antigravity_env(mock_credentials_env, fake_home)
    db_path = fake_home / ".goalrail" / "chat.db"
