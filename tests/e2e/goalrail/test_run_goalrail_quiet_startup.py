"""End-to-end: ``goalrail run`` starts the REPL without leaking
...
- ``_quiet_goalrail_server_logging`` stops suppressing DBOS /
...
from tests.e2e.goalrail._repl_test_helpers import drain_for
...
def test_run_goalrail_startup_does_not_leak_server_logs(
    ...
    """
    yaml_path = goalrail_repo_root / _YAML_REL
    ...
    assert not leaked, (
        f"``goalrail run`` leaked server boot output onto "
