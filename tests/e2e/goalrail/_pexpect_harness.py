"""Reusable pexpect helpers for driving the Goalrail REPL.

The Goalrail REPL is a prompt-toolkit app that renders a full
...
# every pexpect boot.
_TEST_THEME_CONFIG = "# Goalrail user configuration\ntui:\n  theme: light\n"
...
def ensure_repl_test_theme_env(env: Mapping[str, str]) -> dict[str, str]:
    ...
    ``$HOME/.goalrail/config.yaml`` has no ``tui.theme`` entry.
    ...
    :returns: A copy of *env* with ``HOME`` pointing at a directory
        that has ``.goalrail/config.yaml`` seeded.
    """
    prepared = dict(env)
    real_home = Path.home()
...
def spawn_goalrail_run(
    ...
    *,
    ...
    """
    Spawn ``goalrail run`` under a PTY for REPL tests.

    :param goalrail_python: Python interpreter with goalrail
        installed, e.g.
        ``Path("/Users/.../goalrail/.venv/bin/python")``.
    ...
    :param env: Environment dict for the subprocess — supplied
        by the ``goalrail_credentials_env`` fixture so PAT
        and base URL propagate.
    :param cwd: Working directory for the subprocess. Must be
        the Goalrail repo root so YAML ``callable:`` entries
    ...
    if yaml_path is None:
        ...
        # ``python -m goalrail``. The package ``__main__`` module
        # intentionally routes through the legacy argparse quick-chat
        # CLI, while the installed ``goalrail`` script invokes the
        ...
        command = str(goalrail_python.parent / "goalrail")
    else:
        args = [
            "-m",
            "goalrail",
