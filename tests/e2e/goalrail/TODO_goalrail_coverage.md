# `--goalrail` example-YAML coverage: known gaps

Companion to `test_run_goalrail_example_agents.py` and
`test_run_goalrail_adapter_rejections.py`. Tracks gaps the e2e audit
...
### Fixed — Gap 7: Stock goalrail policy callables don't work under Goalrail

`resolve_function_policy` now detects legacy `(content, phase)` callables at
load time and wraps them in `_goalrail_legacy_shim` — a one-argument adapter
...
exists to protect any user-written goalrail-style policy callables that have
...
executor.type is 'goalrail' — must be one of [...]
```
...
installed. Orthogonal to `--goalrail`. Needs either an
...
If your goalrail YAML declares a sub-agent tool named
`worker` and you want a policy that denies calls to it, the
natural thing to write is `match_tools: [worker]`. That doesn't
work under Goalrail. Sub-agents are invoked through a generic
...
works but it's non-obvious and authors following the goalrail
...
### Fixed — Gap 6: `--goalrail` hung when FunctionTools were registered on the top-level agent

**Root cause**: the adapter synthesizes `LocalToolInfo.language =
"goalrail-python-callable"` for every user FunctionTool /
...
`_build_goalrail_tool_schemas` but never **registered** on
Goalrail's `ToolManager`. When the harness's LLM invoked one,
...
dispatching, so the same guardrails Goalrail's native tool
...
- `examples/agent_with_tools.yaml` (xfail removed, passes
  end-to-end through `--goalrail` with the claude-sdk harness).
- `tests/e2e/goalrail/test_run_goalrail_policy_enforcement.py::test_policy_denies_tool_call_by_name`
  (real `goalrail run --goalrail -p "what is 6 + 6?"` subprocess
