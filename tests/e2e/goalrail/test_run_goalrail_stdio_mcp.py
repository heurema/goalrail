"""Live E2E: ``goalrail run`` calls a stdio MCP tool.

Proves the full path landed by the stdio-MCP work:

    YAML  →  Goalrail translator
          ...
          stdio, through the harness, through the Goalrail mode reply
...
- The Goalrail translator silently drops the
  :class:`~goalrail.inner.tools.MCPTool`: the spec loads, but
  the goalrail runtime never registers the tool, the LLM
  never calls it, and the ``echo: <probe>`` fingerprint is
  absent. The unit test
  (``test_goalrail_adapter.py::test_load_mcp_stdio_yaml_
...
# existing Goalrail mode example-agent tests use 240s; shorter here
...
def _write_stdio_mcp_yaml(tmp_path: Path, repo_root: Path) -> Path:
    """
    Materialize an goalrail agent YAML that declares the
    ...
    :param repo_root: Goalrail repo root — used to resolve the
    ...
    agent = {
        "name": "echo_goalrail_stdio_mcp_test",
        "prompt": (
    ...
    yaml_path = tmp_path / "echo_goalrail_stdio.yaml"
    yaml_path.write_text(yaml.dump(agent))
    return yaml_path


def test_goalrail_stdio_mcp_tool_roundtrip(tmp_path: Path) -> None:
    """
    Run ``goalrail run <yaml>`` with a stdio MCP; the
    ...
    # ``goalrail_repo_root`` session fixture in conftest.py
    ...
    # same ``openai-agents`` / ``goalrail`` install the test is
    ...
    # mistake the ask for one of Goalrail' auto-registered
    ...
    # Goalrail path has historically swallowed a few of these
    # (see designs/GOALRAIL_INTEGRATION.md gap log), so assert
    # up front.
    forbidden = (
    ...
    for marker in forbidden:
        assert marker not in combined, (
            f"Forbidden marker {marker!r} in output — a --goalrail "
            f"failure mode fired. stderr tail:\n{result.stderr[-2000:]}"
        )

    assert result.returncode == 0, (
        f"--goalrail exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-2000:]}"
    )
