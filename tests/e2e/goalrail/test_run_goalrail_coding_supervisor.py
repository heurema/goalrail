"""End-to-end test: ``goalrail run examples/coding_supervisor.yaml
--goalrail`` works.

Exercises the full pipeline that was failing in user-reported bugs:

1. Goalrail YAML with ``async: true``, ``cancellable: true``, and
...
2. Non-interactive ``run -p <prompt>`` path — POSTs to
   ``/v1/responses`` on an in-process goalrail server.
3. The bidirectional translator: YAML → AgentDef →
   :func:`agent_def_to_agent_spec` → AgentSpec → (registered with
   goalrail server) → :func:`agent_spec_to_agent_def` (inside
   :class:`GoalrailExecutor.from_spec`) → AgentDef → goalrail
...
- Someone reintroduces a fail-loud for ``AgentTool`` in the
  ...
  sub-agent-as-tool declarations stop loading under Goalrail mode.
- ``_run_agent_via_goalrail`` regresses to the "requires a prompt"
  hard-error path — interactive ``goalrail run <yaml>``
...
- The
  :func:`goalrail.spec.goalrail._sub_spec_to_agent_tool`
  reverse translation stops emitting ``AgentTool`` entries —
  :class:`GoalrailExecutor.from_spec` reconstructs an
...
from tests.e2e.goalrail.conftest import configure_mock_llm, reset_mock_llm
...
# Goalrail mode boot (FastAPI + uvicorn + DBOS + alembic) completes
...
# Cold-boot of ``coding_supervisor.yaml`` under Goalrail mode —
# spawns the in-process Goalrail server and registers supervisor +
# two sub-agents. Without Goalrail mode boot is <10s; with Goalrail mode
# the in-process FastAPI + uvicorn + DBOS + alembic stack adds
# ~30-60s on a cold DBOS db. 120s keeps the test from flaking
# on cold starts. (Genuine regressions in Goalrail mode boot would
...
_EXPECTED_ROOT_ENTRIES = ("openapi.json", "goalrail", "pyproject.toml")


def test_run_goalrail_coding_supervisor_oneshot(
    ...
    """
    ``goalrail run examples/coding_supervisor.yaml -p ...``
    ...
    :param goalrail_python: Interpreter with both goalrail and
        goalrail installed (from the shared conftest).
    :param goalrail_repo_root: Goalrail repo root. Used as cwd
    ...
    yaml_path = goalrail_repo_root / _YAML_PATH_REL
    ...
    result = subprocess.run(
        [
            ...
            # test_run_goalrail_example_agents.py for the
    ...
    assert result.returncode == 0, (
        f"`goalrail run --goalrail` exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1000:]}"
    )
...
def test_run_goalrail_coding_supervisor_exposes_subagent_tools(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    ``codex_worker``) AND at least one goalrail task-lifecycle
    builtin (``check_task``) — proves :class:`GoalrailExecutor`
    advertises both surfaces to the inner goalrail harness.
    ...
    :param goalrail_python: Interpreter with goalrail +
        goalrail installed.
    :param goalrail_repo_root: Goalrail repo root.
    :param mock_credentials_env: Mock-LLM env vars pointing at the
        mock server.
    :param mock_llm_server_url: Mock server URL for configuring
        response queues.
...
def test_run_goalrail_coding_supervisor_spawns_codex_worker_to_list_files(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: ``goalrail run`` on
    coding_supervisor.yaml boots the Goalrail stack, the mock
    ...
    is that the Goalrail boot path, sub-agent tool registration,
    ...
    :param goalrail_python: Shared session fixture pointing at
        the repo's ``.venv`` Python.
    :param goalrail_repo_root: Goalrail repo root — used as
    ...
    configure_mock_llm(
        mock_llm_server_url,
        [{"text": "openapi.json goalrail pyproject.toml"}],
    )
    ...
    for entry in _EXPECTED_ROOT_ENTRIES:
        assert (goalrail_repo_root / entry).exists(), (
            f"Expected filesystem anchor {entry!r} missing from "
            f"{goalrail_repo_root}. Update _EXPECTED_ROOT_ENTRIES "
...
def test_run_goalrail_coding_supervisor_interactive_enters_repl(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
) -> None:
    """
    ``goalrail run examples/coding_supervisor.yaml`` (no
    ...
    :func:`_run_chat_via_goalrail` so interactive semantics match the
    legacy ``goalrail run`` no-prompt behavior.
    ...
    # Use the shared ``spawn_goalrail_run`` harness so PTY
    ...
    try:
        ...
        assert index == 0, (
            f"`goalrail run --goalrail` (no prompt) did not reach the "
...
@pytest.mark.flaky(reruns=2, reruns_delay=5)
def test_run_goalrail_coding_supervisor_codex_shell_not_disabled(
    goalrail_python: Path,
    goalrail_repo_root: Path,
    mock_credentials_env: dict[str, str],
    mock_llm_server_url: str,
    ...
    """
    Infrastructure smoke test: under Goalrail mode, the codex
    ...
    validates is that the infrastructure plumbing (Goalrail mode
    ...
    ``shell_tool`` whenever any tools were passed. Under Goalrail
    mode, :class:`GoalrailExecutor` always injects goalrail
    ...
    :param goalrail_python: Interpreter with goalrail +
        goalrail installed.
    :param goalrail_repo_root: Goalrail repo root — also the
    ...
    fixture_path = goalrail_repo_root / _CODEX_REGRESSION_TODO_NAME
    ...
    assert _CODEX_NONEXISTENT_ERROR not in combined, (
        ...
        f"goalrail/codex_executor.py.\n"
        f"stdout tail:\n{result.stdout[-2500:]}\n"
        f"stderr tail:\n{result.stderr[-1500:]}"
    )
    assert result.returncode == 0, (
        f"--goalrail exited {result.returncode}. stderr tail:\n"
