"""Snapshot loader + comparator for Goalrail Phase 0 characterization tests.

Snapshots are JSON files under ``tests/e2e/goalrail/snapshots/``
and capture the *structural* observations a test makes about a
live Goalrail run. They are the golden-master contract the
Phase 0 design requires: written against current Goalrail,
...
def load_snapshot(test_name: str) -> dict[str, SnapshotField]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            ...
            f"their snapshots captured against current Goalrail."
        )
...
def compare_snapshot(
    ...
    :param observed: Dict of observed values the test gathered
        from the live Goalrail run, e.g.
