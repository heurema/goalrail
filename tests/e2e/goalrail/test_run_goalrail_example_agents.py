"""
End-to-end: every example YAML the Goalrail adapter is
*expected* to accept can actually boot and execute under
``goalrail run -p <prompt>``.
...
The companion file
``test_run_goalrail_adapter_rejections.py`` covers YAMLs the adapter
...
- The Goalrail mode CLI shim loses a dispatch for a harness.
...
LLM nondeterminism) are tracked in ``TODO_goalrail_coverage.md`` in
...
_CASES = [
    ...
    pytest.param(
        ...
        ("pyproject.toml", "README.md", "goalrail", "examples"),
...
def test_run_goalrail_example_yaml(
    ...
    """
    Drive one example YAML under ``goalrail run -p <prompt>``
    and assert the agent exercised the declared capability.

    :param goalrail_python: Shared interpreter fixture.
    :param goalrail_repo_root: Repo root — subprocess cwd so
    ...
    :param success_markers: Any-one-of substrings that MUST
        ...
        reply traversed the full Goalrail mode stack. The mock
    ...
    args = [
        ...
        # ``~/.goalrail/chat.db`` and DBOS may attempt to
        ...
        # ``HarnessProcessManager not initialized — Goalrail lifespan
    ...
    assert result.returncode == 0, (
        f"{yaml_rel}: --goalrail exited {result.returncode}. "
        f"stderr tail:\n{result.stderr[-2000:]}\n"
        f"stdout tail:\n{result.stdout[-1500:]}"
    )
