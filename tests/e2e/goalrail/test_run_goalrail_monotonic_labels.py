...
Drives a real LLM-backed agent through the full goalrail
...
- ``goalrail.runtime.policies.engine.PolicyEngine.apply_label_writes``
...
- The goalrail → goalrail label-schema translation
...
# Same harness as the other run_goalrail e2e tests — the
...
def _isolated_home_env(base_env: dict[str, str], home: Path) -> dict[str, str]:
    ...
    ``~/.goalrail/chat.db`` and the post-run label query
...
def test_monotonic_decreasing_constraint_enforced_end_to_end(
    ...
    # The agent YAML. Goalrail-flavored format (no
    # ``spec_version``); the goalrail-compat adapter
    # translates labels/label_schema/policies into the native
    # Goalrail guardrails shape at load time.
...
label_schema:
  integrity:
    values: ["0", "1"]
    # ``min`` translates to ``decreasing`` on the Goalrail side —
...
"""
    ...
    assert result.returncode == 0, (
        f"goalrail run exited {result.returncode}. "
        f"stdout={result.stdout!r} stderr={result.stderr!r}"
    )

    # The persistent store the subprocess writes lives at
    # ``$HOME/.goalrail/chat.db`` — see
    # ``goalrail.chat._goalrail_persistent_dir``. With HOME pointed
...
label_schema:
  integrity:
    values: ["0", "1", "2"]
    # ``max`` translates to ``increasing`` on the Goalrail side —
