...
Runs ``goalrail run hello_world.yaml --harness codex --model
...
- Goalrail' ``CodexExecutor`` regresses (``codex app-server``
...
def test_per_harness_codex_one_shot(
    ...
    """
    ``goalrail run hello_world.yaml --harness codex -p <prompt>``
