...
Runs ``goalrail run hello_world.yaml --harness cursor -p "..."`` as a real
...
- The ``cursor-sdk`` API contract changes (``AsyncAgent`` / ``AsyncClient`` /
  ``run.messages()`` shape).
- ``goalrail.cli`` for the ``-p`` one-shot path stops printing assistant text
...
def test_per_harness_cursor_one_shot(
    ...
    """``goalrail run hello_world.yaml --harness cursor -p <prompt>`` works.

    :param goalrail_python: Interpreter with goalrail installed and importable.
    :param goalrail_repo_root: Cwd for the subprocess so the YAML spec and
