"""Live REPL e2e for ``goalrail run --harness`` without AGENT.

Migrated to use the mock LLM server. This test drives the user-facing
launcher shape::

    goalrail run --harness <harness> -p <prompt>
...
def test_run_harness_without_agent_live_repl_round_trip(
    ...
    """``goalrail run --harness`` boots and answers via each wrapped harness.
...
def test_run_harness_live_matrix_covers_registered_coding_harnesses() -> None:
    ...
    ``GOALRAIL_HARNESSES`` also contains ``open-responses`` for the
    ...
    ``goalrail run --harness <native>``. (``opencode-native`` is a
    ...
    ``cursor-native``: it is a terminal-first TUI launched via ``goalrail goose``
    (tmux pane + bridge dir), not ``goalrail run --harness goose-native``.
    ...
    not ``goalrail run --harness qwen-native``. Its coverage is the dedicated
    ...
    ``goalrail kiro`` (tmux pane + bridge dir), not ``goalrail run --harness
    ...
    ``goalrail kimi`` (tmux pane + bridge dir), not ``goalrail run --harness
    ...
    terminal-first TUI launched via ``goalrail hermes`` (tmux pane + bridge dir), not
    ``goalrail run --harness hermes-native``, AND it wraps the ``hermes`` CLI
    binary. Its coverage is the dedicated hermes-native bridge/executor/forwarder/
    approval-mirror unit tests.
    """
    expected_live_harnesses = set(GOALRAIL_HARNESSES).intersection(_HARNESS_MODULES) - {
        "claude-native",
        "codex-native",
        "pi-native",
        "opencode-native",
