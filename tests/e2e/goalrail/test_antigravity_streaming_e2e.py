...
``/v1/sessions`` flow — register an inline Goalrail agent
...
:mod:`goalrail.inner.antigravity_executor`: ``conversation.send`` →
...
from goalrail.onboarding.antigravity_auth import antigravity_api_key_configured
...
def _antigravity_skip_reason() -> str | None:
    ...
    if importlib.util.find_spec("google.antigravity") is None:
        return (
            ...
            "is not importable. Install it with: pip install 'goalrail[antigravity]'."
        )
    ...
    if not key_present:
        return (
            ...
            "via 'goalrail setup' (Antigravity) or export GEMINI_API_KEY / "
...
def _assistant_text_from_items(http_client: httpx.Client, session_id: str) -> str:
    ...
    (see ``goalrail/server/API.md`` § List Conversation Items).
