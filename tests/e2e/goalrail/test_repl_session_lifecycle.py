...
import pexpect
import pytest
from goalrail_client import GoalrailClient, SessionsChat
...
def _stop_host_daemon(home: Path) -> None:
    ...
    ``goalrail run --server`` leaves the daemon alive after the REPL exits
    ...
    """
    pid_path = home / ".goalrail" / "host.pid"
...
class _ServerHandle:
    """
    Live standalone Goalrail server used by ``--server`` e2e tests.
...
def _spawn_run(
    ...
    *,
    ...
    """
    Spawn ``goalrail run`` under a real PTY.
    ...
    :param server_url: Optional Goalrail server URL for ``--server`` mode.
...
def _session_runner_id(base_url: str, session_id: str) -> str:
    ...
    :param base_url: Goalrail server URL.
...
def _drive_turn(
    ...
    * **``--server``/daemon** (``base_url`` set): the session is
    ...
    :param base_url: Goalrail server URL for the ``--server`` flow;
...
def _runner_pid_from_daemon_log(home: Path, runner_id: str) -> int:
    ...
    when it spawns a runner (goalrail/host/connect.py). Reading the pid
    ...
    """
    log_dir = home / ".goalrail" / "logs" / "host-daemon"
...
def _server_entrypoint() -> str:
    """
    Return a Python entrypoint for a remote-style Goalrail server.
...
from goalrail.cli import _create_artifact_store
...
db_uri = os.environ["GOALRAIL_E2E_DB_URI"]
artifact_location = Path(os.environ["GOALRAIL_E2E_ARTIFACT_LOCATION"])
port = int(os.environ["GOALRAIL_E2E_PORT"])
...
def _running_server(
    ...
    """
    Run a remote-style Goalrail server for ``--server`` tests.
    ...
    server_env = {
        **env,
        "GOALRAIL_E2E_DB_URI": f"sqlite:///{db_path}",
        "GOALRAIL_E2E_ARTIFACT_LOCATION": str(artifacts),
        "GOALRAIL_E2E_PORT": str(port),
    }
...
def test_repl_full_session_lifecycle(
    ...
    :param goalrail_repo_root: Repository root fixture.
...
def test_repl_recover_after_runner_death(
    ...
    try:
        with _running_server(goalrail_python, goalrail_repo_root, env, tmp_path) as server:
            child = _spawn_run(
                goalrail_python,
                goalrail_repo_root,
                yaml_path,
...
async def test_repl_reasoning_effort_threads_through(
    ...
    with _running_server(goalrail_python, goalrail_repo_root, env, tmp_path) as server:
        ...
        with _registered_runner(
            ...
            async with GoalrailClient(base_url=server.base_url) as client:
