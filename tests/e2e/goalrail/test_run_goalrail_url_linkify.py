...
What can break this without breaking the lower layers:
  ...
  - The Goalrail mode SSE bridge translates tool-result events into
...
_TEST_URL = "https://goalrail-linkify-e2e-test.example.com/path"
...
def test_run_goalrail_url_linkify_emits_osc_8_in_pty(
    ...
    1. Spawn ``goalrail run --goalrail`` via pexpect.
    ...
    :param goalrail_python: Path to the worktree's
        ``.venv/bin/python``.
    :param goalrail_repo_root: Repo root used as cwd so the
        subprocess imports this worktree's ``goalrail``
    ...
    # Short top-level tmpdir so the Goalrail server's per-run
    # subdirs (incl. tmux socket paths if the agent ever
    # touched one) stay under macOS's 104-char Unix-socket
    # limit. Same shape as test_run_goalrail_terminal_idle.py.
    ...
    try:
        ...
        # On failure, leave the tmpdir intact so the Goalrail server
        # log is inspectable.
...
def _assert_url_was_linkified(captured: str) -> None:
    ...
    assert bare_count == 0, (
        ...
        f"``sdks/ui/goalrail_ui_sdk/terminal/_host.py`` for "
