...
This test drives a real ``goalrail run --harness pi`` subprocess with:

- an isolated ``HOME`` carrying ``~/.pi/agent/settings.json`` + a marker
  extension;
- a mock OpenAI provider in ``GOALRAIL_CONFIG_HOME`` so pi enters gateway mode
...
``tests/e2e/goalrail/`` pi rows — do not run under xdist against a shared mock.
"""
...
_MARKER_NAME = "goalrail-pi-ext-marker"
...
def test_pi_gateway_run_loads_global_extensions(
    ...
    """
    ``goalrail run --harness pi`` in gateway mode loads extensions from the
    user's global Pi agent settings.

    The marker extension writes ``~/goalrail-pi-ext-marker`` on
