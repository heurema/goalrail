"""
End-to-end proof that policies declared in an goalrail YAML
are enforced by the goalrail workflow under Goalrail mode.

The adapter in :mod:`goalrail.spec.goalrail` lifts the
YAML's ``policies:`` block into
:attr:`AgentSpec.guardrails.policies`; the goalrail runtime
...
the legacy spec ``(content, phase)`` callable signature
(``examples.tool_functions.block_long_sleep`` et al.), which
Goalrail' :class:`FunctionPolicy` dispatcher can't invoke
(it passes ``(ctx, context)`` where ``ctx`` is an
:class:`EvaluationContext` dataclass, not a dict). This test
uses the goalrail-shaped
``goalrail._e2e_policy_callables.block_on_sentinel``
callable — an arity-1 callable matching Goalrail'
convention — so the test proves the translator + engine
integration works and isn't muddied by a separate callable-
portability gap. That gap is tracked in ``TODO_goalrail_coverage.md``.
...
- The runtime stops reading ``guardrails`` from specs
  synthesized via the goalrail adapter.
...
# in ``goalrail/_e2e_policy_callables.py`` DENYs on. The token
...
# The standard DENY sentinel text the goalrail workflow
# stamps into the response when a policy returns DENY. See
# :func:`goalrail.runtime.workflow._build_deny_sentinel` —
...
def policy_enforcement_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an goalrail-shaped YAML registering one
    function policy on the ``input`` phase pointing at the
    goalrail e2e callable.
    ...
    (``block_on_sentinel``) is already on ``goalrail`` and
    matches the goalrail FunctionPolicy calling convention.
...
def test_policy_denies_input_containing_sentinel(
    ...
    :param goalrail_repo_root: Subprocess cwd — the YAML's
    ...
    :param policy_enforcement_yaml_factory: Builder for the
        harness-specific goalrail YAML.
    ...
    assert result.returncode == 0, (
        f"--goalrail exited {result.returncode} before reaching "
...
def tool_ban_yaml_factory(tmp_path: Path) -> Callable[[str, str], Path]:
    """
    Factory that writes an goalrail-shaped YAML declaring one
    ...
    - ``GoalrailExecutor._make_tool_executor_bridge``
...
def test_policy_denies_tool_call_by_name(
    ...
    """
    ``goalrail run <yaml> -p "<arithmetic prompt>"``
    ...
    1. The translator expanded ``on: [tool_call] + match_tools:
       ...
       name.
    2. ``GoalrailExecutor._make_tool_executor_bridge``
    ...
    - The GoalrailExecutor bridge stopped calling
    ...
    :param goalrail_python: Shared interpreter fixture.
    :param goalrail_repo_root: Subprocess cwd — conftest's
