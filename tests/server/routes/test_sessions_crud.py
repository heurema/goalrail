...
async def test_list_projects_returns_names_sorted(
    ...
    conv_store.set_labels(a.id, {"goalrail.project": "Sprint 42"})
    conv_store.set_labels(b.id, {"goalrail.project": "Customer X"})
...
async def test_list_sessions_filtered_by_project(
    ...
    conv_store.set_labels(filed.id, {"goalrail.project": "X"})
...
async def test_patch_session_sets_project_label(
    ...
    resp = await client.patch(
        f"/v1/sessions/{session_id}",
        json={"labels": {"goalrail.project": "Sprint 42"}},
        headers={"Content-Type": "application/json"},
    )
    assert resp.status_code == 200
    ...
    assert conv.labels.get("goalrail.project") == "Sprint 42"
...
async def test_patch_session_empty_project_removes_label(
    ...
    conv_store = SqlAlchemyConversationStore(db_uri)
    conv_store.set_labels(session_id, {"goalrail.project": "Sprint 42"})

    resp = await client.patch(
        f"/v1/sessions/{session_id}",
        json={"labels": {"goalrail.project": ""}},
        headers={"Content-Type": "application/json"},
    )
    assert resp.status_code == 200
    ...
    conv = conv_store.get_conversation(session_id)
    assert conv is not None
    assert "goalrail.project" not in conv.labels
