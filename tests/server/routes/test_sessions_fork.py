...
@pytest.mark.parametrize(
    ...
    [
        ...
        # session file from the copied Goalrail items. Cross-family from a
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("cursor-native", True),
        # pi rebuilds its JSONL session file from the copied Goalrail items
...
async def test_fork_cursor_pi_native_carry_gating(
    ...
    JSONL session file from the copied Goalrail items. Both therefore mark
...
async def test_fork_model_override_rejected_when_harness_unresolvable(
    ...
    monkeypatch.setattr(
        "goalrail.server.routes.sessions._agent_harness_id",
        lambda _agent: None,
    )
    client = TestClient(_build_app(conv_store))
