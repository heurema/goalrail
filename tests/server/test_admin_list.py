"""Tests for the file-backed admin roster (:mod:`goalrail.server.admin_list`).
...
from goalrail.server.admin_list import (
    AdminList,
...
from goalrail.server.routes.accounts_auth import create_accounts_auth_router
...
def test_resolve_admin_list_path_env_override(monkeypatch: pytest.MonkeyPatch) -> None:
    """``GOALRAIL_ADMIN_LIST_PATH`` wins over the default."""
    monkeypatch.setenv("GOALRAIL_ADMIN_LIST_PATH", "/etc/goalrail/admins")
    assert resolve_admin_list_path() == Path("/etc/goalrail/admins")
...
def test_resolve_data_dir_uses_credentials_parent(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("GOALRAIL_ADMIN_LIST_PATH", raising=False)
...
def test_resolve_data_dir_uses_goalrail_data_dir(
    ...
    monkeypatch.delenv("GOALRAIL_ADMIN_CREDENTIALS_PATH", raising=False)
    monkeypatch.delenv("GOALRAIL_ADMIN_LIST_PATH", raising=False)
...
def test_resolve_data_dir_defaults_to_home(monkeypatch: pytest.MonkeyPatch) -> None:
    """With no env, the data dir is ``~/.goalrail``."""
    monkeypatch.delenv("GOALRAIL_DATA_DIR", raising=False)
    monkeypatch.delenv("GOALRAIL_DATA_DIR", raising=False)
    monkeypatch.delenv("GOALRAIL_ADMIN_CREDENTIALS_PATH", raising=False)
    monkeypatch.delenv("GOALRAIL_ADMIN_LIST_PATH", raising=False)
    assert resolve_data_dir() == Path.home() / ".goalrail"
    assert resolve_admin_list_path() == Path.home() / ".goalrail" / "admins"
...
def test_load_admin_list_binds_resolved_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("GOALRAIL_ADMIN_LIST_PATH", "/tmp/goalrail-admins-test")
    al = load_admin_list()
    assert al.path == Path("/tmp/goalrail-admins-test")
...
def test_load_admin_list_passes_extra(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("GOALRAIL_ADMIN_LIST_PATH", "/tmp/goalrail-admins-absent")
