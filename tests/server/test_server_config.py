"""Tests for the shared server-config loader (:mod:`goalrail.server.server_config`).
...
from goalrail.server.server_config import (
    config_str_list,
    load_server_config,
    resolve_config_path,
)
...
def _pin_data_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("GOALRAIL_CONFIG", raising=False)
...
def test_resolve_config_path_env_override(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    """``GOALRAIL_CONFIG`` wins over the data-dir default."""
    p = tmp_path / "custom.yaml"
    p.write_text("{}")
    monkeypatch.setenv("GOALRAIL_CONFIG", str(p))
