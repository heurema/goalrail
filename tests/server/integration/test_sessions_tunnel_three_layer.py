"""
Tunnel three-layer integration test: Goalrail → WS tunnel → runner → harness.
...
Layers:
    Goalrail server ──HTTP──> RunnerRouter ──WSTunnelTransport──>
...
from goalrail_client._events import ResponseCompleted, ResponseFailed
from goalrail_client._files import FilesNamespace
from goalrail_client._sessions import SessionsNamespace

from goalrail.repl._repl import _SessionsChatReplAdapter
...
def _build_harness_agent_bundle() -> bytes:
    ...
    Uses ``executor.type='goalrail'`` with
...
# ── Fixture: Goalrail app + tunnel WS + runner app + EchoHarness ────
...
async def tunnel_three_layer_stack(tmp_path: Path) -> AsyncIterator[_TunnelStack]:
    """Wire Goalrail server + WS-tunneled runner + EchoHarness in-process.

    Lifecycle: build stores, init runtime, override the test harness
    module entry, build Goalrail app + runner app, open the WS tunnel via
    ...
    # Build the Goalrail server. ``create_app`` constructs the
...
def _fake_client(ap_client: httpx.AsyncClient) -> object:
    ...
    :param ap_client: ASGI-backed client pointed at the Goalrail app.
    :returns: Duck-typed client with the namespaces the adapter uses.
    """
    base = str(ap_client.base_url).rstrip("/")

    class _FakeClient:
        """Duck-typed Goalrail client backed by the tunnel-stack httpx client."""
...
def _new_repl_adapter(
    ...
    *,
    ...
    """Create a sessions REPL adapter over the tunneled Goalrail stack.
...
async def test_on_runner_connect_restarts_relay_via_router(
    ...
    from goalrail.server.routes.sessions import _runner_relay_tasks
    ...
    from goalrail.runtime import get_conversation_store
