...
def test_validate_agent_bundle_rejects_escaping_os_env_cwd(bad_cwd: str) -> None:
    ...
    On a runner without ``GOALRAIL_RUNNER_WORKSPACE`` such a cwd becomes the
    ...
    """
    with pytest.raises(GoalrailError, match=r"os_env\.cwd must be a relative path"):
...
# A goalrail function tool whose ``callable:`` is a dotted import path the
...
def test_validate_bundle_rejects_server_callable_tool() -> None:
    ...
    with pytest.raises(GoalrailError, match=r"may not declare a server-side Python callable tool"):
        validate_agent_bundle(bundle)
...
def test_reject_uploaded_callable_tools_recurses_into_sub_agents() -> None:
    ...
    """
    from goalrail.server.bundles import _reject_uploaded_callable_tools
