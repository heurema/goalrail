...
from goalrail import native_policy_hook
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (_resp(302, "https://w.example.com/.auth/callback"), True),
...
def test_is_login_redirect_or_unauthorized_classifies_reauth_signals(
    ...
    """
    401 and a login-flow 302 are re-auth signals; nothing else is.

    A fronting proxy/SSO front door can bounce an *expired* bearer with a
...
def test_post_evaluate_with_retry_reauths_on_login_redirect(
    ...
    (token past its lifetime) failed CLOSED on every tool call. The first
    attempt carries the lapsed token (302), the retry carries the fresh token
    and gets the ALLOW verdict — exactly as the runner's refresh-capable
    ``_RunnerBearerAuth`` does for its own callbacks.
    """
    seen_headers: list[dict[str, str]] = []
    redirect = httpx.Response(
        302,
    ...
    def _reauth() -> dict[str, str]:
        ...
        return {"Authorization": "Bearer fresh", "X-Org-Id": "o1"}
