...
How should Goalrail integrate codebase-memory-mcp so local agents get code
...
2. Vendored binary/source
   - Bundle `../codebase-memory-mcp` or built artifacts into Goalrail.
...
3. Companion tool from installer/setup
   ...
   - `goalrail setup` checks/repairs configuration explicitly.
...
4. Remote sandbox host image layer
   - Bake codebase-memory-mcp into Goalrail host image.
   - Risk: separate from local installer; wheel overlays use `--no-deps`.
   - Status: separate follow-up.

## Subquestions

- Where can Goalrail install the companion binary without breaking base install?
...
- Gate C: Can failure leave Goalrail unusable?
