...
Use codebase-memory-mcp as an explicit companion tool, not as an Goalrail wheel
...
- Call it after `verify_goalrail "$bin_dir"` and before PATH/next-step output.
...
Add a small Goalrail-side helper, e.g. `goalrail/onboarding/codebase_memory.py`,
with:
...
- It avoids bundling a platform binary into Goalrail's wheel lifecycle.
- It lets Goalrail remain usable when CBM download/config fails.
