...
codebase memory integration for goalrail


## Project context

Goalrail needs codebase-memory-mcp available to local coding agents without
making the Goalrail Python wheel vendor or depend on the codebase-memory-mcp
binary/runtime side effects.

## Decision context

The proposed path is to install codebase-memory-mcp as an explicit companion
tool from the Goalrail installer/setup flow. The decision needs to separate:
...
Goalrail maintainers implementing the first integration PR.
...
- Inspect current Goalrail installer/setup/remote host image surfaces.
...
- No Goalrail source edits in this run.
...
- Identify exact Goalrail files/functions for first PR.
...
- Goalrail install should remain successful if codebase-memory-mcp install or
  download fails.
...
- The Goalrail OSS installer may run in interactive and non-interactive modes.
...
- Should codebase-memory-mcp add a non-destructive repair mode before Goalrail
  enables full agent-config repair by default?
- What minimum CBM version should Goalrail require once the exact safe command
  shape is chosen?
