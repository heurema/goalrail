...
should be a companion tool installed/configured explicitly by Goalrail
installer/setup, not vendored into the Goalrail wheel and not hidden as a
...
Goalrail wants codebase-memory available for local coding agents. CBM install
mutates external agent configs, instructions, skills, and hooks. That is a
setup action, not a Python dependency side effect.

## What is known

- Goalrail installer has a clean insertion point after `verify_goalrail`.
- Installer tests already source `install_oss.sh` and can cover new shell
  functions without network.
- `goalrail setup` is an explicit user-facing configuration flow and can host
...
- Goalrail remote sandboxes use a prebaked host image and wheel overlay with
...
- Exact minimum CBM version Goalrail should require after command shape is
  finalized.
...
- Vendoring `../codebase-memory-mcp` into Goalrail.
- Adding CBM as an Goalrail runtime dependency and expecting package install to
...
- `goalrail setup` can become the durable repair/check point.
...
- Narrow: CBM download fails; Goalrail should still install.
- Narrow: `auto_index` config fails; Goalrail should warn only.
...
- Should Goalrail pin `codebase-memory-mcp>=0.8.1` or wait for a safer command?
- What should the prompt copy say when existing indexes may be rebuilt?

## Evidence notes

Primary evidence is local code in Goalrail and codebase-memory-mcp. Provider
