...
- Server-side admin/allowed-domains/config data root now honors
  `GOALRAIL_DATA_DIR` unless `OMNIGENT_ADMIN_CREDENTIALS_PATH` pins a Docker
  volume parent.
- Host runner env propagation now allows both Goalrail and Omnigent path envs.
- Legacy state migration skips when `GOALRAIL_DATA_DIR` is set, matching the
  existing `OMNIGENT_DATA_DIR` safety behavior.

## Intentionally Left For Later

Remaining `~/.omnigent` roots need separate compatibility cuts:
...
- Frontend/e2e fixtures and docs that still describe `~/.omnigent`.
