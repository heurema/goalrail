...
- Public docs, e2e comments, and user-facing `~/.omnigent` text were not broadly rewritten.
...
```sh
uv run pre-commit run --files omnigent/cli.py omnigent/runner/identity.py tests/host/test_cli_host.py tests/runner/test_identity.py .heurema/rdlab/runs/2026-06-26-omnigent-to-goalrail-rename/milestone_11_runtime_data_paths_report.md
```
...
The main remaining rename risk is broad state not covered by this slice: update-check cache, auth token storage, local-server user-facing path text, and documentation/e2e comments that still mention `~/.omnigent`.
