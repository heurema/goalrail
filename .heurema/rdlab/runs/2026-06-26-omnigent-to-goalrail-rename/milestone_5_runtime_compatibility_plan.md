...
Do not rename `omnigent/`, SDK packages, Docker images, storage paths, workflow
...
`omnigent` surfaces working, and only remove legacy names after a release and
...
- `pyproject.toml`: package name is still `omnigent`; console scripts are
  `omnigent` and `omni`; installed package discovery is `omnigent*`.
- `omnigent/_env_compat.py`: current env compatibility mirrors
  `OMNIAGENTS_*` / `OMNIGENTS_*` into `OMNIGENT_*`.
- codebase-memory `search_code` for `GOALRAIL_`: no matches.
- codebase-memory `search_code` for `OMNIGENT_`: broad runtime, deploy, CI,
  web, and test usage.
- SDK surfaces: `omnigent-client`, `omnigent-ui-sdk`, `omnigent_client`,
  `OmnigentClient`.
- Deploy surfaces: `ghcr.io/omnigent-ai/omnigent-server`,
  `ghcr.io/omnigent-ai/omnigent-host`, Kubernetes service/database examples,
  Fly/Railway/Render/Modal/Cloudflare references.
- Storage/native surfaces: `~/.omnigent`, `.omnigents`, `.omniagents`,
  `ap-web/ios/Omnigent.xcodeproj`, `OmnigentLogo`, Electron/iOS app metadata,
  and web `localStorage` keys such as `omnigent:panel-size-preferences`.
...
- Keep `omnigent` CLI, Python import root, PyPI package, SDK package names,
...
- Keep `omni` as a short CLI alias unless a separate product decision removes
  it.

New names:

- Introduce `goalrail` CLI as an additive alias to `omnigent.cli:main`.
- Introduce `GOALRAIL_*` env variables as the new preferred names while keeping
  `OMNIGENT_*`, `OMNIGENTS_*`, and `OMNIAGENTS_*` as compatibility inputs.
...
- Add `goalrail = "omnigent.cli:main"` in package metadata.
- Keep `omnigent` and `omni`.
- Do not rename modules, tests, or internal command references in the same PR.

Required tests:

- Installed script metadata includes `goalrail`, `omnigent`, and `omni`.
- `goalrail --help`, `omnigent --help`, and `omni --help` reach the same CLI.
...
- Conflict precedence should be:
  ...
  2. explicit `OMNIGENT_*`
  3. legacy `OMNIGENTS_*`
  4. legacy `OMNIAGENTS_*`

Implementation shape:

- Extend `omnigent/_env_compat.py`; do not scatter one-off fallback checks.
- Keep direct readers safe by calling the shim before modules read env.
- For deploy entrypoints that read env before importing `omnigent`, keep direct
  calls to the shim.

Required tests:

- `GOALRAIL_*` wins over `OMNIGENT_*`.
- `OMNIGENT_*` still works when `GOALRAIL_*` is absent.
- `OMNIGENTS_*` and `OMNIAGENTS_*` still mirror correctly.
- Existing tests using `OMNIGENT_CONFIG_HOME`, `OMNIGENT_SKIP_WEB_UI`,
  `OMNIGENT_DISABLE_KEYRING`, auth, runner, and deploy env continue to pass.
...
- Read existing `~/.omnigent`, `~/.omnigents`, and `~/.omniagents` as legacy
  inputs.
...
- Existing `~/.omnigent/config.yaml` is read when `~/.goalrail/config.yaml` is
  absent.
...
- Keep existing `omnigent:*` `localStorage` keys until migration code exists.
...
- Do not rename `omnigent/` to `goalrail/` in the next implementation PR.
...
- If a `goalrail` import package is introduced, it should be a thin compatibility
  re-export first, with tests proving existing `omnigent` imports still work.

Required tests before any package move:

- `import omnigent` still works.
...
- Do not rename `omnigent-client`, `omnigent-ui-sdk`, `omnigent_client`, or
  `OmnigentClient` until package distribution names are reserved and tested.
...
- Keep `ghcr.io/omnigent-ai/omnigent-*` deploy docs as legacy runtime truth
...
- Do not rename `Omnigent.xcodeproj`, targets, schemes, bundle ids, app group
...
1. Add CLI alias `goalrail`.
   ...
   - Gate: `goalrail --help` and `omnigent --help` both work.

2. Add env alias support.
   - Files likely touched: `omnigent/_env_compat.py`, targeted env tests,
...
- removes `omnigent` CLI support;
- breaks `OMNIGENT_*` env variables;
...
- keeping all existing `omnigent` files, commands, packages, env names, storage
...
codebase-memory-mcp cli search_code '{"project":"Users-vi-personal-heurema-goalrail","pattern":"OMNIGENT_","limit":120}'
rg -n "localStorage|omnigent:|\\.omnigent|\\.omnigents|\\.omniagents|Omnigent\\.xcodeproj|OmnigentLogo|ghcr\\.io/omnigent-ai|omnigent_client|OmnigentClient|omnigent-ui-sdk|omnigent-client|release-omnigent" ap-web omnigent sdks deploy .github pyproject.toml setup.py tests
git diff --check
```
