...
- `goalrail/cli.py`
- `goalrail/host/daemon_launch.py`
...
- `GOALRAIL_DATA_DIR` are honored through
  `data_home_path()`.
...
- Fresh default remains `~/.goalrail` through `data_home_path()` until a later
...
```sh
uv run pre-commit run --files goalrail/cli.py goalrail/host/daemon_launch.py tests/cli/test_backend.py tests/host/test_daemon_launch.py
```
