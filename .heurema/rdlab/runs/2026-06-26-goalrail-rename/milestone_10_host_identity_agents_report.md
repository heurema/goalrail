...
This milestone moved host identity and user-level agent registry defaults onto the effective Goalrail config home, while preserving legacy spec compatibility.

Covered paths:

- `goalrail/host/identity.py`
- `goalrail/host/connect.py`
- `goalrail/cli.py`
- `goalrail/onboarding/wizard.py`
...
- `GOALRAIL_CONFIG_HOME` remains supported.
- Existing `~/.goalrail` and `~/.goalrail` config homes are still detected by the shared env compatibility layer.
...
- Project-local `.goalrail/config.yaml` behavior.
...
```sh
uv run pre-commit run --files goalrail/cli.py goalrail/host/connect.py goalrail/host/identity.py goalrail/onboarding/wizard.py tests/cli/test_cli.py tests/host/test_identity.py tests/onboarding/test_wizard.py .heurema/rdlab/runs/2026-06-26-goalrail-rename/milestone_10_host_identity_agents_report.md
```
