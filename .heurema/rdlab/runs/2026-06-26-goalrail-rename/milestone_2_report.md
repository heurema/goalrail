...
Updated user-visible `ap-web` labels from Goalrail where the change is branding-only and does not alter runtime identifiers, API contracts, storage keys, packages, or build paths.
...
- `goalrail:*` localStorage keys.
- `goalrail/server/static/web-ui` build output path.
- `GoalrailApp`, `GoalrailError`, SDK/REPL comments, package names, and module identifiers.
...
git diff --check
git grep -InE 'goalrail\.ai|heurema' -- ap-web/src ap-web/index.html ap-web/electron/setup ap-web/electron/find
```
...
- No `goalrail.dev` / `heurema` URL remains in the scoped `ap-web` search.
