...
- Initial desire: Omnigent install should immediately make codebase-memory-mcp available.
...
- Resolution: do not run full `install -y` blindly in the first Omnigent PR. Prefer:
...
- Resolution: no Omnigent `pyproject.toml` dependency for this integration.

## C4: Local installer vs. remote sandbox parity

- Local path: install codebase-memory-mcp via `uv tool`.
- Remote path: Omnigent sandbox host image overlays wheels with `pip --no-deps`.
...
- Product constraint: Omnigent must install even if CBM GitHub release/download is unavailable.
